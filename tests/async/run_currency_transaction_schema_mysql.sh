...
[[ "${environment_name,,}" =~ (dev|local|test) ]] || { echo 'refusing currency test: environment is not development/local/test' >&2; exit 1; }
[[ "${DB_NAME,,}" =~ (dev|local|test) ]] || { echo 'refusing currency test: configured database name is not development/local/test' >&2; exit 1; }
