...
class ContractSource(str):
    """Keep source contracts stable across clang-format whitespace changes."""

    def __contains__(self, needle: object) -> bool:
        return isinstance(needle, str) and contains(str(self), needle)

    def count(self, sub: str, start: int = 0, end: int | None = None) -> int:
        segment = self[start:] if end is None else self[start:end]
        return count(str(segment), sub)

    def index(self, sub: str, start: int = 0, end: int | None = None) -> int:
        result = index(str(self), sub, start, end)
        if result < 0:
...
def function_body(source: str, signature: str, next_signature: str) -> str:
    ...
    return ContractSource(source[start:end])
...
if __name__ == "__main__":
    tests = [
        ...
        test_runtime_harness_covers_compression_reset_and_rejection,
        ...
        test_legacy_binary_output_cannot_bypass_websocket_framing,
