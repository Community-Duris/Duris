...
zone_story_quest_feature::completion_event completion(const char *txid, const char *quest,
								int zone, uint32_t direct, int64_t at,
								std::vector<uint32_t> credited)
{
	zone_story_quest_feature::completion_event event;
	event.transaction = { .schema_version = zone_story_quest_tracking::ZONE_STORY_QUEST_TRACKING_SCHEMA_VERSION,
					 .transaction_id = txid,
...
int main()
{
	...
	std::string error;
	completion_event conflicting = completion("tx-conflict", "zone-story:900:001", 900, 42,
							 172800050, { 42 });
	...
	completion_event first = completion("tx-1", "zone-story:900:001", 900, 42, 172800100,
						 { 42 });
	...
	completion_event group = completion("tx-2", "zone-story:900:002", 900, 42, 172800200,
						 { 42, 77 });
	...
	completion_event third = completion("tx-3", "zone-story:901:001", 901, 77, 172800300,
						 { 77 });
...
require(tracker.summary_for(7, 42).renown == 1,
	...
	require(recovered_after_delete.summary_for(7, 42).completed == 0 &&
			...
			recovered_after_delete.evidence_for("zone-story:900:001", 7).observed_attempts == 0,
...
service disabled(catalog);
	...
	require(disabled.daily_for(7, 42, service::period_for(daily_now)).status == daily_status::none,
	...
	for (uint32_t pid : { 100U, 101U })
	{
		...
		observation.outcome = telemetry_outcome::success;
		observation.accessible = true;
		require(disabled_after_assignment.record_telemetry(observation, &error) == result::applied,
	...
	completion_event disabled_completion = completion(
		"tx-disabled", disabled_fixture.quest_definition_id.c_str(), 901, 42,
	...
	zone_story_quest_catalog::catalog empty_catalog = { .content_revision = 7, .definitions = {} };
