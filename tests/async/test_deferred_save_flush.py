...
flush_char_guard = text.find('if (do_save_silent(ch, pending.type ? pending.type : 1))', flush_char)
flush_char_clear = text.find('memset(slot, 0, sizeof(*slot));', flush_char_guard)
flush_char_log = text.find('Failed to flush deferred save for %s', flush_char_guard)
