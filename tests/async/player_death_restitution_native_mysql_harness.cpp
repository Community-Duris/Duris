...
void query_or_fail(MYSQL *connection, const std::string &sql)
{
    if (mysql_query(connection, sql.c_str()) != 0)
        fail("SQL error " + std::to_string(mysql_errno(connection)) + ": " + mysql_error(connection));
}
...
fixture make_fixture(uint8_t operation_seed)
{
    ...
    SHA256(value.death_payload.data(), value.death_payload.size(), value.plan.evidence_digest.data());
    ...
    SHA256(item.metadata_payload.data(), item.metadata_payload.size(), item.metadata_digest.data());
...
void seed_database(MYSQL *connection, const fixture &value)
{
    ...
    query_or_fail(connection, "INSERT INTO artifact_bind(vnum,owner_pid,timer) VALUES(104,42,654321)");
    query_or_fail(connection,
...
critical_command build_command(const fixture &value)
{
    ...
    require(player_death_restitution_command_build(value.plan, &command), "build native command");
    return command;
}
                                      ...
                                      uint64_t expected_domain_revision)
{
    ...
    require(scalar(connection, "SELECT item_revision FROM item_current_owner WHERE item_uid=1004") == "11",
...
{
    const critical_apply_result applied = critical_command_repository_apply(connection, command);
    ...
    require(applied.result_size > 0 && player_death_restitution_command_decode_result(
    ...
    require(scalar(connection, "SELECT HEX(name) FROM player_items WHERE obj_uid=1004") == "7370656C6C626F6F6B",
            "spellbook metadata was not preserved");
    require(scalar(connection, "SELECT HEX(short_descr) FROM player_items WHERE obj_uid=1004") == "6172746966616374",
    ...
    require(scalar(connection, "SELECT COUNT(*) FROM player_item_affects WHERE item_id=(SELECT id FROM player_items WHERE obj_uid=1004)") == "1",
            "item affects were not preserved");
    require(scalar(connection, "SELECT COUNT(*) FROM player_item_extra_descr WHERE item_id=(SELECT id FROM player_items WHERE obj_uid=1004)") == "1",
            "item extra descriptions were not preserved");
    require(scalar(connection, "SELECT owner_id FROM item_current_owner WHERE item_uid=1004") == "43",
            "current owner was not updated");
    require(scalar(connection, "SELECT item_revision FROM item_current_owner WHERE item_uid=1004") == "12",
            "delivered item revision is wrong");
    require(scalar(connection, "SELECT revision FROM artifact_domain_state WHERE vnum=104") == "5",
            "artifact domain revision was not fenced and advanced");
    require(scalar(connection, "SELECT location FROM artifact_domain_state WHERE vnum=104") == "43",
            "artifact domain location is wrong");
    require(scalar(connection, "SELECT loc_type FROM artifact_domain_state WHERE vnum=104") == "3",
            "artifact domain location type is wrong");
    const uint64_t delivered_timer = std::stoull(scalar(
        connection, "SELECT artifact_delivered_timer_epoch FROM player_death_restitution_item WHERE item_uid=1004"));
    const uint64_t delivered_domain_timer = std::stoull(scalar(
        connection, "SELECT timer_epoch FROM artifact_domain_state WHERE vnum=104"));
    require(decoded.delivery_epoch + 456 == delivered_timer && delivered_timer == delivered_domain_timer,
    ...
    require(scalar(connection, "SELECT UNIX_TIMESTAMP(timer) FROM artifacts_mortal WHERE vnum=104") ==
                std::to_string(delivered_domain_timer),
            "legacy artifact timer does not match domain timer");
    require(scalar(connection, "SELECT artifact_timing_basis FROM player_death_restitution_item WHERE item_uid=1004") ==
                "historical_loss_remainder",
            "historical timing basis was not audited");
    require(scalar(connection, "SELECT artifact_usable_lifetime_seconds FROM player_death_restitution_item WHERE item_uid=1004") ==
                "456",
    ...
    require(scalar(connection, "SELECT COUNT(*) FROM critical_outbox WHERE operation_id=UNHEX('" +
                         operation_hex(command.operation_id) + "')") == "1",
            "restitution outbox event missing");
    require(scalar(connection, "SELECT status FROM critical_operation_inbox WHERE operation_id=UNHEX('" +
                         operation_hex(command.operation_id) + "')") == "1",
...
void run_positive_and_replay(MYSQL *connection)
{
    ...
    require(replay.outcome == critical_apply_outcome::already_applied, "replay was not idempotent");
    require(scalar(connection, "SELECT COUNT(*) FROM player_items WHERE obj_uid=1004") == "1",
            "replay duplicated original UID");
    require(scalar(connection, "SELECT COUNT(*) FROM player_death_restitution_delivery WHERE item_uid=1004") == "1",
...
void run_stale_owner(MYSQL *connection)
{
    ...
    const critical_apply_result result = critical_command_repository_apply(connection, build_command(value));
    require(result.outcome == critical_apply_outcome::terminal_failure && result.error_code == ESTALE,
...
void run_stale_domain_revision(MYSQL *connection)
{
    ...
    require(scalar(connection, "SELECT COUNT(*) FROM critical_operation_inbox WHERE operation_id=UNHEX('" +
...
void run_original_uid_guard(MYSQL *connection)
{
    ...
    const critical_apply_result result = critical_command_repository_apply(connection, build_command(value));
    require(result.outcome == critical_apply_outcome::terminal_failure && result.error_code == EEXIST,
            "original UID collision was accepted");
    require(scalar(connection, "SELECT COUNT(*) FROM player_items WHERE obj_uid=1004") == "1",
            "original UID guard changed the pre-existing row");
    require(scalar(connection, "SELECT COUNT(*) FROM player_death_restitution_delivery WHERE item_uid=1004") == "0",
...
void run_atomic_rollback(MYSQL *connection)
{
    ...
    require(scalar(connection, "SELECT owner_id FROM item_current_owner WHERE item_uid=1004") == "42",
            "rollback left owner mutation");
    require(scalar(connection, "SELECT location FROM artifact_domain_state WHERE vnum=104") == "42",
...
int main()
{
    ...
    const unsigned int port = port_text ? static_cast<unsigned int>(std::strtoul(port_text, nullptr, 10)) : 3306;
    require(mysql_real_connect(connection, host ? host : "127.0.0.1", user ? user : "root",
                               password ? password : std::getenv("MYSQL_PWD"),
                               database ? database : "duris_issue_331_test", port, nullptr, 0) != nullptr,
