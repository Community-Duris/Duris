...
racial_guard = text.count('logit(LOG_WIZ, "Failed to save %s after clearing racial skills.", GET_NAME(ch));')
wrapped_tch = text.count('if (!do_save_silent(t_ch, 1))')
wrapped_ch = text.count('if (!do_save_silent(ch, 1))')
print(f'reset_guard={reset_guard} racial_guard={racial_guard} wrapped_tch={wrapped_tch} wrapped_ch={wrapped_ch}')

ok = True
if reset_guard < 2 or racial_guard < 1 or wrapped_tch < 2 or wrapped_ch < 1:
