...
static int make_compressed_fragment_frames(const char *text, unsigned char *first,
					   size_t first_capacity, size_t *first_len,
					   unsigned char *second, size_t second_capacity,
					   size_t *second_len)
{
...
static int outbound_frame_matches(const unsigned char *frame, size_t frame_len,
				  const char *expected)
{
...
static int expect_frame_error(descriptor_data *d, const unsigned char *frame, size_t len,
			      const char *label)
{
	...
	int opcode = -1;
	int fin = 0;
	if (websocket_parse_frame(d, (const char *)frame, len, &payload, &payload_len, &opcode,
				  &fin) >= 0)
	{
		free(payload);
...
int main()
{
	...
	if (write(pair[1], input, handshake_len + sizeof(ping)) !=
	...
	const char *untrusted_xff_handshake = "GET / HTTP/1.1\r\n"
					      "Host: localhost\r\n"
					      "Upgrade: websocket\r\n"
					      "Connection: Upgrade\r\n"
					      "Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\n"
	...
	if (!make_compressed_text_frame("compressed-one", compressed_frame,
					sizeof(compressed_frame), &compressed_frame_len))
	...
	if (websocket_parse_frame(&d, (const char *)compressed_frame, compressed_frame_len,
				  &compressed_payload, &compressed_payload_len, &compressed_opcode,
				  &compressed_fin) < 0 ||
	    compressed_opcode != WS_OPCODE_TEXT || !compressed_fin ||
	    compressed_payload_len != strlen("compressed-one") ||
	...
	if (!make_compressed_text_frame("compressed-two", compressed_frame,
					sizeof(compressed_frame), &compressed_frame_len))
		return fail("build second compressed frame");
	if (websocket_parse_frame(&d, (const char *)compressed_frame, compressed_frame_len,
				  &compressed_payload, &compressed_payload_len, &compressed_opcode,
				  &compressed_fin) < 0 ||
	    compressed_payload_len != strlen("compressed-two") ||
	    memcmp(compressed_payload, "compressed-two", compressed_payload_len) != 0)
	{
		free(compressed_payload);
		inflateEnd(&inbound_inflater);
	...
	if (!make_compressed_fragment_frames("fragmented-compressed", compressed_first,
					     sizeof(compressed_first), &compressed_first_len,
					     compressed_second, sizeof(compressed_second),
					     &compressed_second_len))
		return fail("build fragmented compressed frames");
	char *fragment_payload = nullptr;
	size_t fragment_payload_len = 0;
	if (websocket_parse_frame(&d, (const char *)compressed_first, compressed_first_len,
				  &fragment_payload, &fragment_payload_len, &compressed_opcode,
				  &compressed_fin) < 0 ||
	    compressed_fin)
	{
	...
	if (websocket_parse_frame(&d, (const char *)compressed_second, compressed_second_len,
				  &fragment_payload, &fragment_payload_len, &compressed_opcode,
				  &compressed_fin) < 0 ||
	    !compressed_fin ||
	...
	if (write(pair[1], compressed_first, compressed_first_len) !=
	...
	if (!expect_frame_error(&d, compressed_frame, compressed_frame_len,
	...
	if (deflateInit2(&outbound_deflater, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -15, 8,
	...
	ssize_t generated_close_len =
		read(generated_close_pair[1], generated_close_frame, sizeof(generated_close_frame));
	if (generated_close_desc.ws_state != WS_STATE_CLOSING || generated_close_len < 2 ||
	    generated_close_frame[0] != 0x88 || generated_close_frame[1] > 125)
		return fail("generated close exceeded control payload limit");
	generated_close_desc.ws_state = WS_STATE_OPEN;
	unsigned char oversized_pong[WS_LEN_7BIT_MAX + 1] = { 0 };
	if (websocket_send_pong(&generated_close_desc, (const char *)oversized_pong,
				sizeof(oversized_pong)) >= 0 ||
	    websocket_send_pong(&generated_close_desc, nullptr, 1) >= 0 ||
	    generated_close_desc.ws_state != WS_STATE_OPEN)
		return fail("invalid generated pong changed state");
	if (websocket_send_close(&generated_close_desc, 1005, "bad") >= 0 ||
	    generated_close_desc.ws_state != WS_STATE_OPEN)
	...
	generated_close_len =
		read(generated_close_pair[1], generated_close_frame, sizeof(generated_close_frame));
	if (generated_close_len < 4 || generated_close_frame[0] != 0x88 ||
	    generated_close_frame[2] != 0x03 || generated_close_frame[3] != 0xF4)
	...
	ssize_t server_close_len =
		read(server_close_pair[1], server_close_frame, sizeof(server_close_frame));
	if (server_close_desc.ws_state != WS_STATE_CLOSING || server_close_len < 2 ||
	    server_close_frame[0] != 0x88)
	...
	if (saturated_flags < 0 ||
	    fcntl(saturated_close_pair[0], F_SETFL, saturated_flags | O_NONBLOCK) < 0)
		return fail("set saturated close nonblocking");
	saturated_close_desc.ws_output_buffer =
		(unsigned char *)malloc(WS_MAX_OUTPUT_BYTES - WS_CONTROL_OUTPUT_RESERVE);
	...
	ssize_t saturated_close_len =
		read(saturated_close_pair[1], saturated_close_frame, sizeof(saturated_close_frame));
	if (saturated_close_desc.ws_state != WS_STATE_CLOSING || saturated_close_len < 2 ||
	    saturated_close_frame[0] != 0x88)
	{
	...
	const unsigned char inbound_close[] = { 0x88, 0x85, 0x01, 0x02, 0x03, 0x04,
						0x02, 0xEA, 0x61, 0x7D, 0x64 };
	...
	if (close_reply_len != (ssize_t)sizeof(expected_close_reply) ||
	    memcmp(close_reply, expected_close_reply, sizeof(expected_close_reply)) != 0)
		return fail("inbound close reply was not delivered");
	int close_peer_flags = fcntl(close_pair[1], F_GETFL, 0);
	if (close_peer_flags < 0 ||
	    fcntl(close_pair[1], F_SETFL, close_peer_flags | O_NONBLOCK) < 0)
	...
	if (!expect_frame_error(&d, unmasked_ping, sizeof(unmasked_ping),
				"unmasked client frame") ||
	    !expect_frame_error(&d, reserved_bits, sizeof(reserved_bits), "reserved bits") ||
	    !expect_frame_error(&d, fragmented_ping, sizeof(fragmented_ping),
				"fragmented control frame") ||
	    !expect_frame_error(&d, oversized_ping, sizeof(oversized_ping),
				"oversized control frame"))
		return 1;

	const unsigned char invalid_close_length[] = { 0x88, 0x81, 0x00, 0x00, 0x00, 0x00, 0x00 };
	const unsigned char invalid_close_code[] = {
		0x88, 0x82, 0x00, 0x00, 0x00, 0x00, 0x03, 0xED
	};
	const unsigned char invalid_close_utf8[] = { 0x88, 0x83, 0x00, 0x00, 0x00,
						     0x00, 0x03, 0xE8, 0xFF };
	const unsigned char compressed_close[] = { 0xC8, 0x80, 0x00, 0x00, 0x00, 0x00 };
	if (!expect_frame_error(&d, invalid_close_length, sizeof(invalid_close_length),
				"one-byte close payload") ||
	    !expect_frame_error(&d, invalid_close_code, sizeof(invalid_close_code),
				"reserved close code") ||
	    !expect_frame_error(&d, invalid_close_utf8, sizeof(invalid_close_utf8),
				"invalid close reason UTF-8") ||
	    !expect_frame_error(&d, compressed_close, sizeof(compressed_close),
	...
	setsockopt(pressure_pair[0], SOL_SOCKET, SO_SNDBUF, &small_send_buffer,
		   sizeof(small_send_buffer));
	...
	if (websocket_send_text(&pressure, "one") < 0 ||
	    websocket_send_text(&pressure, "two") < 0 || pressure.write_failed ||
	    pressure.ws_output_len == 0)
	...
	if (ordered_len != (ssize_t)sizeof(ordered_frames) ||
	    memcmp(drained, ordered_frames, sizeof(ordered_frames)) != 0)
	...
	pressure.ws_state = WS_STATE_OPEN;
	if (websocket_close(&pressure, 1000, "bye"), pressure.ws_state != WS_STATE_CLOSING ||
							     pressure.write_failed ||
							     pressure.ws_control_output_len == 0)
	...
	if (close_len != (ssize_t)sizeof(close_frame) ||
	    memcmp(drained, close_frame, sizeof(close_frame)) != 0)
	...
	if (websocket_send_pong(&pressure, "p", 1) < 0 || pressure.write_failed ||
	    pressure.ws_control_output_len == 0)
	...
	if (priority_len < (ssize_t)sizeof(priority_pong) ||
	    memcmp(drained, priority_pong, sizeof(priority_pong)) != 0)
	...
	if (pressure.ws_output_buffer || pressure.ws_output_len != 0 ||
	    pressure.ws_output_offset != 0 || pressure.ws_control_output_buffer ||
	    pressure.ws_control_output_len != 0 || pressure.ws_control_output_offset != 0 ||
	    pressure.ws_last_ping != 0 || pressure.ws_pong_received != 0 ||
	    pressure.ws_ping_queued != 0 || pressure.ws_ping_outstanding != 0)
	...
	if (websocket_parse_handshake(&d, invalid_compression_handshake,
				      sizeof(invalid_compression_handshake) - 1) != 0 ||
	    d.ws_deflate_requested)
	...
	const char embedded_nul_handshake[] =
		"GET / HTTP/1.1\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n\0"
		"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\n\r\n";
	if (websocket_parse_handshake(&d, embedded_nul_handshake,
				      sizeof(embedded_nul_handshake) - 1) != 0)
		return fail("embedded-NUL handshake was accepted");
	const char malformed_header_handshake[] =
		"GET / HTTP/1.1\r\nUpgrade websocket\r\nConnection: Upgrade\r\n"
		"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nSec-WebSocket-Version: 13\r\n\r\n";
	if (websocket_parse_handshake(&d, malformed_header_handshake,
				      sizeof(malformed_header_handshake) - 1) != 0)
		return fail("header without colon was accepted");
	const char folded_header_handshake[] =
		"GET / HTTP/1.1\r\n Upgrade: websocket\r\nConnection: Upgrade\r\n"
		"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nSec-WebSocket-Version: 13\r\n\r\n";
	if (websocket_parse_handshake(&d, folded_header_handshake,
				      sizeof(folded_header_handshake) - 1) != 0)
		return fail("leading-whitespace header was accepted");
	const char noncanonical_version_handshake[] =
		"GET / HTTP/1.1\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n"
		"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nSec-WebSocket-Version: +13\r\n\r\n";
	if (websocket_parse_handshake(&d, noncanonical_version_handshake,
				      sizeof(noncanonical_version_handshake) - 1) != 0)
		return fail("noncanonical websocket version was accepted");
	const char control_value_handshake[] =
		"GET / HTTP/1.1\r\nUpgrade: websocket\r\nConnection: Upgrade\x0b\r\n"
		"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nSec-WebSocket-Version: 13\r\n\r\n";
	if (websocket_parse_handshake(&d, control_value_handshake,
				      sizeof(control_value_handshake) - 1) != 0)
	...
	if (websocket_parse_handshake(&d, noncanonical_key_handshake,
				      sizeof(noncanonical_key_handshake) - 1) != 0)
		return fail("noncanonical websocket key was accepted");
	const char token_list_handshake[] =
		"GET / HTTP/1.1\r\nUpgrade: websocket, h2\r\nConnection: keep-alive, Upgrade\r\n"
		"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nSec-WebSocket-Version: 13\r\n\r\n";
	if (websocket_parse_handshake(&d, token_list_handshake, sizeof(token_list_handshake) - 1) !=
	    1)
		return fail("valid upgrade token list was rejected");
	const char lookalike_connection_handshake[] =
		"GET / HTTP/1.1\r\nUpgrade: websocket\r\nConnection: NotUpgrade\r\n"
		"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nSec-WebSocket-Version: 13\r\n\r\n";
	if (websocket_parse_handshake(&d, lookalike_connection_handshake,
				      sizeof(lookalike_connection_handshake) - 1) != 0)
	...
	if (websocket_parse_handshake(&d, invalid_extension_handshake,
				      sizeof(invalid_extension_handshake) - 1) != 1 ||
	    d.ws_deflate_requested)
	...
	const char duplicate_key_handshake[] =
		...
		"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nSec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\n"
		"Sec-WebSocket-Version: 13\r\n\r\n";
	if (websocket_parse_handshake(&d, duplicate_key_handshake,
				      sizeof(duplicate_key_handshake) - 1) != 0)
