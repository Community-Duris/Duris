...
child_all_ok = text.find('bool all_ok = true;', child_block)
child_save_check = text.find('if (!sql_save_player(ch, RENT_CRASH, get_room_vnum(ch)))', child_block)
child_exit = text.find('_exit(all_ok ? 0 : 1);', child_block)
