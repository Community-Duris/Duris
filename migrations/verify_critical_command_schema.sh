...
failure_stage_type=$(read_scalar "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='critical_operation_inbox' AND column_name='failure_stage' AND data_type='smallint' AND column_type LIKE '%unsigned' AND is_nullable='NO' AND column_default='0';")
...
[[ "$columns" == 35 ]] || { echo "FAILED: expected 35 named columns; found $columns" >&2; exit 1; }
...
[[ "$failure_stage_type" == 1 ]] || { echo 'FAILED: failure_stage must be a non-null SMALLINT UNSIGNED defaulting to zero' >&2; exit 1; }
...
printf 'critical command schema verified: 4 tables, 35 columns, 8 indexes, 2 foreign keys\n'
