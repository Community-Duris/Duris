...
static int count_corpse_files(void)
{
	...
	while ((de = readdir(dir)))
	{
		if (de->d_name[0] == '.')
			continue;
		if (strstr(de->d_name, ".bak"))
			continue;
		if (strstr(de->d_name, ".backup"))
			continue;
		if (strstr(de->d_name, ".old"))
			continue;
		total++;
	}
	closedir(dir);
...
static int save_saved_item(const char *item_key, int room_vnum, struct mig_obj *obj,
			   int container_id)
{
	if (!obj)
		return 0;
	...
	snprintf(query, sizeof(query),
		 ...
		 "%s, %s, %s, %s"
		 ")",
		 esc_key, room_vnum, obj->vnum, container_str, obj->weight, obj->cost, obj->timer,
		 obj->extra_flags, obj->value[0], obj->value[1], obj->value[2], obj->value[3],
		 obj->value[4], obj->value[5], obj->value[6], obj->value[7], name_str, short_str,
		 desc_str, action_str);
