...
static void free_account(struct acct_entry *acct)
{
	...
	if (acct->acct_name)
		free(acct->acct_name);
	if (acct->acct_email)
		free(acct->acct_email);
	if (acct->acct_password)
		free(acct->acct_password);
	if (acct->acct_confirmation)
		free(acct->acct_confirmation);
...
int migrate_accounts_from_files(void)
{
	...
	for (char letter = 'a'; letter <= 'z'; letter++)
	{
		...
		while ((entry = readdir(dir)) != NULL)
		{
			...
			struct acct_entry *acct =
				(struct acct_entry *)malloc(sizeof(struct acct_entry));
