...
static int parse_player_status(char **bufptr, struct mig_player *p, int stat_vers)
{
	...
	else
	{
		...
		if (p->frags < 0 || p->frags > 100000)
			p->frags = 0;
		if (p->oldfrags < 0 || p->oldfrags > 100000)
			p->oldfrags = 0;
	}
...
static int parse_player_witness(char **bufptr)
{
	...
	for (int i = 0; i < count; i++)
	{
		mig_getInt(bufptr);
		mig_getInt(bufptr);
		char *s1 = mig_getString(bufptr);
		if (s1)
			free(s1);
		char *s2 = mig_getString(bufptr);
		if (s2)
			free(s2);
		mig_getLong(bufptr);
	}
...
static int save_player_to_db(struct mig_player *p)
{
	...
	if (pid > 0)
	{
		...
		snprintf(
			...
			"WHERE pid=%d",
			short_str, long_str, desc_str, title_str, p->m_class, p->secondary_class,
			p->spec, p->race, p->racewar, p->level, p->sex, p->weight, p->height,
			p->size, p->hometown, p->birthplace, p->orig_birthplace, p->last_room,
			p->birth_time, p->played_time, p->last_save, p->perm_aging, p->str, p->dex,
			p->agi, p->con, p->pow, p->intel, p->wis, p->cha, p->kar, p->luk, p->mana,
			p->base_mana, p->hp - p->base_hp, p->base_hp, p->vitality, p->base_vitality,
			p->spells_memmed, p->copper, p->silver, p->gold, p->platinum,
			p->bank_copper, p->bank_silver, p->bank_gold, p->bank_platinum, p->exp,
			p->epics, p->epic_skill_points, p->skillpoints, p->spell_bind_used, p->act,
			p->act2, p->act3, p->vote, p->alignment, p->prestige, p->assoc_id,
			p->guild_status, p->time_left_guild, p->nb_left_guild, p->time_unspecced,
			p->frags, p->oldfrags, p->numb_deaths, p->conditions[0], p->conditions[1],
			p->conditions[2], p->conditions[3], p->conditions[4], poof_in_str,
			poof_out_str, poof_in_snd_str, poof_out_snd_str, p->echo_toggle, p->prompt,
			p->wiz_invis, p->law_flags, p->wimpy, (short)p->aggressive,
			p->highest_level, p->screen_length, p->quest_active, p->quest_mob_vnum,
			p->quest_type, p->quest_accomplished, p->quest_started,
			p->quest_zone_number, p->quest_giver, p->quest_level, p->quest_receiver,
			p->quest_shares_left, p->quest_kill_how_many, p->quest_kill_original,
			p->quest_map_room, p->quest_map_bought, pid);
	...
	else
	{
		...
		snprintf(
			...
			")",
			esc_name, short_str, long_str, desc_str, title_str, p->m_class,
			p->secondary_class, p->spec, p->race, p->racewar, p->level, p->sex,
			p->weight, p->height, p->size, p->hometown, p->birthplace,
			p->orig_birthplace, p->last_room, p->birth_time, p->played_time,
			p->last_save, p->perm_aging, p->str, p->dex, p->agi, p->con, p->pow,
			p->intel, p->wis, p->cha, p->kar, p->luk, p->mana, p->base_mana,
			p->hp - p->base_hp, p->base_hp, p->vitality, p->base_vitality,
			p->spells_memmed, p->copper, p->silver, p->gold, p->platinum,
			p->bank_copper, p->bank_silver, p->bank_gold, p->bank_platinum, p->exp,
			p->epics, p->epic_skill_points, p->skillpoints, p->spell_bind_used, p->act,
			p->act2, p->act3, p->vote, p->alignment, p->prestige, p->assoc_id,
			...
			p->quest_zone_number, p->quest_giver, p->quest_level, p->quest_receiver,
			p->quest_shares_left, p->quest_kill_how_many, p->quest_kill_original,
			p->quest_map_room, p->quest_map_bought);
	}
	...
	if (esc_poof_out)
		free(esc_poof_out);
	if (esc_poof_in_snd)
		free(esc_poof_in_snd);
	if (esc_poof_out_snd)
		free(esc_poof_out_snd);
	...
	if (len > 0)
	{
		...
		my_qry("INSERT INTO player_skills (pid,skill_id,learned,taught) VALUES %s "
		       "ON DUPLICATE KEY UPDATE learned=VALUES(learned),taught=VALUES(taught)",
		       values);
	}
	...
	if (len > 0)
	{
		...
		my_qry("INSERT INTO player_timers (pid,timer_id,timer_value) VALUES %s "
		       "ON DUPLICATE KEY UPDATE timer_value=VALUES(timer_value)",
		       values);
	}
	...
	if (len > 0)
	{
		...
		my_qry("INSERT INTO player_forged_items (pid,forge_index,item_vnum) VALUES %s "
		       "ON DUPLICATE KEY UPDATE item_vnum=VALUES(item_vnum)",
		       values);
	}
	...
	if (len > 0)
	{
		...
		my_qry("INSERT INTO player_intros (pid,intro_index,intro_pid,intro_time) VALUES %s "
		       "ON DUPLICATE KEY UPDATE intro_pid=VALUES(intro_pid)",
		       values);
	}
	...
	for (struct mig_affect *af = p->affects; af; af = af->next)
	{
		...
		if (esc_woc)
		{
			snprintf(woc_str, sizeof(woc_str), "'%s'", esc_woc);
			free(esc_woc);
		}
		else
			strcpy(woc_str, "NULL");
		if (esc_wor)
		{
			snprintf(wor_str, sizeof(wor_str), "'%s'", esc_wor);
			free(esc_wor);
		}
		else
			strcpy(wor_str, "NULL");
...
static struct mig_player *parse_player_pfile(const char *filepath)
{
	...
	// validate offsets
	if (skill_off >= size || witness_off >= size || affect_off >= size || item_off >= size ||
	    size_off > size)
	{
	...
	for (char letter = 'a'; letter <= 'z'; letter++)
	{
		...
		while ((entry = readdir(dir)) != NULL)
		{
			...
			if (strstr(entry->d_name, ".bak"))
				continue;
			if (strstr(entry->d_name, ".locker"))
				continue;
			if (strstr(entry->d_name, ".spellbook"))
				continue;
			if (strstr(entry->d_name, ".preconvert"))
				continue;
