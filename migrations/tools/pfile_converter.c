...
static int convert_pfile(unsigned char *input_buf, size_t input_size, unsigned char **output_buf,
			 size_t *output_size)
{
...
static int convert_item(unsigned char **in, unsigned char **out, const pfile_header_t *header,
			int depth);
	...
	*(*out)++ = o_f_flag;
	...
	if (o_f_flag & O_F_CONTAINS)
	{
		...
		while (1)
		{
			...
			{
				fprintf(stderr,
					"ERROR: Ran out of data while processing container at depth %d\n",
					depth);
				return 0;
			}
			...
			if (next_flag == O_F_EOL)
			{
				*(*out)++ = *(*in)++; /* Copy EOL marker */
				...
				{
					fprintf(stderr,
						"DEBUG: depth=%d found EOL in container after %d items\n",
			...
			{
				fprintf(stderr,
					"ERROR: Too many items in container (possible infinite loop) at depth %d\n",
					depth);
				return 0;
			}
