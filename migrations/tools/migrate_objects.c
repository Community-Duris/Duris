...
static void parse_unique_fields(char **buf, struct mig_obj *obj, unsigned long o_u_flag)
{
	...
	{
		obj->item_type = MIG_GET_BYTE(*buf);
		obj->item_type_set = 1;
	}
	if (o_u_flag & O_U_WEAR)
	{
		obj->wear_flags = mig_getInt(buf);
		obj->wear_flags_set = 1;
	}
	if (o_u_flag & O_U_EXTRA)
	{
		obj->extra_flags = mig_getInt(buf);
		obj->extra_flags_set = 1;
	}
	...
	if (o_u_flag & O_U_BV1)
	{
		obj->bitvector1 = mig_getLong(buf);
		obj->bitvector_set |= 1;
	}
	if (o_u_flag & O_U_BV2)
	{
		obj->bitvector2 = mig_getLong(buf);
		obj->bitvector_set |= 2;
	}
	if (o_u_flag & O_U_BV3)
	{
		obj->bitvector3 = mig_getLong(buf);
		obj->bitvector_set |= 4;
	}
	if (o_u_flag & O_U_BV4)
	{
		obj->bitvector4 = mig_getLong(buf);
		obj->bitvector_set |= 8;
	}
	if (o_u_flag & O_U_BV5)
	{
		obj->bitvector5 = mig_getLong(buf);
		obj->bitvector_set |= 16;
	}
