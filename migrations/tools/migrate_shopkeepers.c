...
static int save_shopkeeper_item(int shopkeeper_id, struct mig_obj *obj, int equip_slot,
				int container_id)
{
	if (!obj)
		return 0;
	return save_item_to_db(obj, "shopkeeper_items", "shopkeeper_id", shopkeeper_id,
			       container_id, equip_slot);
}
...
int migrate_shopkeepers_from_files(void)
{
	...
	while ((de = readdir(dir)))
	{
		...
		if (diff > 50000)
		{
			...
			int corr_diff = corrected > room_vnum ? corrected - room_vnum :
								room_vnum - corrected;
			if (corr_diff < 1000)
			{
				mob_vnum = corrected;
