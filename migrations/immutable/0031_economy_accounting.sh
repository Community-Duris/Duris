...
if [[ "$version" == 10.11.*MariaDB* ]]; then
    engine=mariadb
    expected=53a2a5a828352ea57072329086b013c38731b85e799a2f7dc083f6850a19da7d
elif [[ "$version" == 8.0.* && "$version" != *MariaDB* ]]; then
    engine=mysql
    expected=f79cff9303ff365c38e419636a09d7f02e85f84ca948ba61dfbcc14c3dabbc2c
else
    echo 'unsupported database engine for accounting schema' >&2; exit 1
fi
query=$(cat <<'DURIS_METADATA_SQL'

SELECT CONCAT('T',CHAR(9),table_name,CHAR(9),engine,CHAR(9),table_collation)
FROM information_schema.tables WHERE table_schema=DATABASE() AND table_type='BASE TABLE' AND table_name IN ('economic_epoch','economic_lineage_state','economic_account_mapping','economic_accounting_operation','economic_accounting_account_effect','economic_accounting_coin_posting','economic_accounting_child','economic_accounting_item_reference','economic_accounting_source_claim')
UNION ALL
...
FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name IN ('economic_epoch','economic_lineage_state','economic_account_mapping','economic_accounting_operation','economic_accounting_account_effect','economic_accounting_coin_posting','economic_accounting_child','economic_accounting_item_reference','economic_accounting_source_claim')
UNION ALL
...
FROM information_schema.statistics WHERE table_schema=DATABASE() AND (table_name IN ('economic_epoch','economic_lineage_state','economic_account_mapping','economic_accounting_operation','economic_accounting_account_effect','economic_accounting_coin_posting','economic_accounting_child','economic_accounting_item_reference','economic_accounting_source_claim') OR (table_name='item_ownership_ledger' AND index_name='uq_item_ledger_accounting_reference'))
UNION ALL
...
WHERE k.constraint_schema=DATABASE() AND k.table_name IN ('economic_epoch','economic_lineage_state','economic_account_mapping','economic_accounting_operation','economic_accounting_account_effect','economic_accounting_coin_posting','economic_accounting_child','economic_accounting_item_reference','economic_accounting_source_claim') AND k.referenced_table_name IS NOT NULL
UNION ALL
...
WHERE t.constraint_schema=DATABASE() AND t.table_name IN ('economic_epoch','economic_lineage_state','economic_account_mapping','economic_accounting_operation','economic_accounting_account_effect','economic_accounting_coin_posting','economic_accounting_child','economic_accounting_item_reference','economic_accounting_source_claim') AND t.constraint_type='CHECK'
ORDER BY 1;

DURIS_METADATA_SQL
)
enforced=$(cat <<'DURIS_ENFORCED_SQL'
SELECT CONCAT('E',CHAR(9),table_name,CHAR(9),constraint_name,CHAR(9),enforced) FROM information_schema.table_constraints WHERE constraint_schema=DATABASE() AND table_name IN ('economic_epoch','economic_lineage_state','economic_account_mapping','economic_accounting_operation','economic_accounting_account_effect','economic_accounting_coin_posting','economic_accounting_child','economic_accounting_item_reference','economic_accounting_source_claim') AND constraint_type='CHECK' ORDER BY 1;
DURIS_ENFORCED_SQL
)
actual=$({ "${MYSQL[@]}" -e "$query"; if [[ "$engine" == mysql ]]; then "${MYSQL[@]}" -e "$enforced"; fi; } | sha256sum | cut -d' ' -f1)
[[ "$actual" == "$expected" ]] || { echo 'accounting schema metadata fingerprint mismatch' >&2; exit 1; }
echo 'accounting schema verified: 9 InnoDB tables, exact columns, indexes, foreign keys and checks'
