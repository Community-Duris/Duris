...
foreign_keys=$(scalar "SELECT COUNT(*) FROM information_schema.referential_constraints WHERE constraint_schema=DATABASE() AND constraint_name IN ('collector_death_operation_fk','collector_listing_death_fk','collector_ledger_operation_fk');")
[[ "$foreign_keys" == 3 ]] || { echo "FAILED: collector foreign keys differ; found $foreign_keys of 3" >&2; exit 1; }

checks=$(scalar "SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_schema=DATABASE() AND constraint_type='CHECK' AND constraint_name IN ('chk_collector_catalog_singleton','chk_collector_catalog_next_listing','chk_collector_death_beneficiary','chk_collector_death_time','chk_collector_hint_state','chk_collector_listing_id','chk_collector_listing_beneficiary','chk_collector_listing_item','chk_collector_listing_status','chk_collector_listing_paused','chk_collector_listing_revision','chk_collector_listing_item_revision','chk_collector_record_size','chk_collector_item_blob_size','chk_collector_ledger_listing','chk_collector_ledger_action','chk_collector_ledger_catalog_revision','chk_collector_ledger_listing_revision','chk_collector_ledger_item','chk_collector_ledger_reason','chk_collector_quarantine_listing','chk_collector_quarantine_conflict','chk_collector_quarantine_evidence');")
[[ "$checks" == 23 ]] || { echo "FAILED: collector checks differ; found $checks of 23" >&2; exit 1; }
