...
| Shop and auction publication | Their composite commands now keep the coordinator's entity fences through wallet, custody, and shop revision publication, then acknowledge. Retry state records completed phases so a failed later phase does not apply an earlier phase twice. | In-memory phase state still needs a restart recovery/reconciliation strategy; the existing live callbacks and outbox remain separate. |
...
4. **Economic restart recovery:** shop and auction now retain failed live publication in process, but their phase state is not reconstructed from the command journal after a server restart. Add an owner/payload reconciliation on startup and verify an interrupted publication with a process restart journey.
