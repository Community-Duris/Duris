...
Counters are process-local saturating integers. `selected` means runtime-owned selection; `started` means admission/payment succeeded; `completed` means every captured effect was invoked. Rejections include busy, invalid, scheduling, consumption, insufficient mana and unavailable storage. Cancellation reasons distinguish departures, definition/configuration changes, reload, abort, scheduler rejection and cleanup. `partial` and `effect_failures` mean dispatch stopped after at least one effect ran. Native spell calls return no success status: saves, immunity and zero damage are not engine effect failures or completed-hit statistics.
