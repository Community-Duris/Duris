...
Its owned implementation is [DurisWebApp #43](https://github.com/Community-Duris/DurisWebApp/issues/43), paired with
[server #288](https://github.com/Community-Duris/Duris/issues/288). Both implementations and integration evidence
are required before declaring structured chat complete.
...
breaks and visible content remain. `policy` is `preserve`, `static` or `animated`; even an animated packet is
...
Snapshots allow at most 16,384 code points and 4,096 runs. A raw ESC sequence, an oversized frame or too many
runs omits the optional snapshot and retains the compatible packet. The ordinary command input limit is
...
channels, offsets, colors, control characters and oversize input fall back safely. Release issue #289 owns
actual-client screenshots and measured end-to-end overhead in addition to these runtime assertions.
