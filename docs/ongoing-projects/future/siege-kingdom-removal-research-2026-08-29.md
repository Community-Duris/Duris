...
The feature is disabled at compile time, but it is not detached:
...
No later commit re-enabled the macro. Recent changes hardened and migrated dormant
persistence code, but did not reconnect the feature.
...
Read-only inspection of the allow-listed local database on 2026-08-29 found:
...
No dedicated siege object VNUM was present in `player_items`, `corpse_items`,
`locker_items`, `account_locker_items`, `saved_items`, `player_pet_items`,
`shopkeeper_items`, `siege_items`, or `item_current_owner`.

The local `pages` table still contains one `ADD` page and one `kingdoms` page. One local
player record still has persisted `PLR2_KINGDOMVIEW` bit `BIT_4` set. These counts are
local evidence only; production must be audited independently through an approved
read-only path or a recent restored clone.
...
Remove both the `__NO_MYSQL__` stubs and real implementations for:
...
- change the three siege table entries in
  `migrations/data_lifecycle_manifest.json` from `reset_delete` to `retain` and describe
  them as retired compatibility schema.
...
Do not edit or delete siege/kingdom definitions from:
...
- the immutable migration manifest or already checksummed migrations
...
The five empty tables should remain as schema tombstones during this cleanup. A future
explicit baseline-rollover project may remove them after retention approval, production
archive verification, MySQL 8 and MariaDB 10.11 fingerprint regeneration, bootstrap and
legacy adoption redesign, lifecycle inventory removal, and rollback planning.
...
5. Delete the eight prototypes only after the production custody audit is zero or an
   explicit compensation/conversion policy has been completed.
...
7. Run `make world` to regenerate ignored `areas/world.*` and `lib/misc/lookup*` outputs.
   Do not hand-edit or commit generated outputs.

VNUMs 178 and 179 create the only meaningful two-release risk because a live shop still
sells them. If the production audit is zero immediately before deployment, the acquisition
freeze and prototype deletion may be combined. Otherwise:
...
- confirm there are no operator-maintained town seed files outside the repository;
...
Do not remove object prototypes when any dedicated VNUM remains in authoritative custody
unless an explicit product decision defines conversion, compensation, or deletion.
...
- no dedicated siege area files or hometown quartermaster resets remain;
...
6. If custody preflight is zero, delete the eight dedicated object prototypes. Otherwise,
   leave only the prototypes until Release B.

This step prevents new data while preserving load compatibility if production contains
legacy ammo.

### Step 3 - Remove runtime persistence and town types

1. Delete siege/town/kingdom-land functions and helpers from `src/sql_player.c` and
   `src/sql_player.h`.
2. Correct the unrelated private-chest log label.
3. Delete `P_town`, `struct town`, troop constants/arrays/structs, and town globals after
   their SQL and siege consumers are gone.
4. Remove stale commented kingdom room/stat code.
...
3. Remove `event_move_engine` from `src/prototypes.h`.
...
1. Delete the kingdom proposal help file and import mapping.
...
Complete all checks below. Use a full cold development boot, not only minimal mode, because
the dedicated zone and hometown reset changes exist only in the full world dataset.
...
| Minimal-mode-only validation | Misses full-world area/reset faults | Require full-world cold boot |
...
3. Restore help sources and re-import exact pages.
4. Restart from a cold full-world boot.
...
- Dedicated quartermasters, siege zone/room, shop sales, and gated object prototypes are
  absent from tracked area inputs and regenerated full-world data.
...
- Full build, area generation, focused persistence/lifecycle tests, complete regressions,
  and a cold full-world smoke test pass.
