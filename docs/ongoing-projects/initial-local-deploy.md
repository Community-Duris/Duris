...
### F016 - Persistence backend switches reuse incompatible build objects

- Status: Fixed and verified
- Evidence: running the documented MariaDB and flat-file build selectors in the same
  checkout reused `bin/objects/server/` across incompatible compile definitions. A
  later link could combine client-free and MariaDB objects or leave `dms_new` built
  for the previously selected backend.
- Impact: changing a local checkout to the supported flat-file build could fail at
  link time or silently retain a server binary for the wrong persistence backend.
- Correction: backend builds now use isolated object directories beneath
  `bin/objects/server/<backend>/`. A backend stamp invalidates the stable
  `bin/server/dms_new` target whenever the selected backend changes.
- Verification: the binary-layout regression exercises both selectors, the strict
  client-free build passed from its isolated cache, and
  `make test-all PERSISTENCE_BACKEND=flatfile` passed all 338 tests.
...
- During merge review, reproduced cross-backend object-cache contamination, isolated
  backend objects, and added binary invalidation plus regression coverage.
- Kept the broad all-classes creation override disabled by default in `.env.example`
  and updated its source contract to enforce that safe default.
