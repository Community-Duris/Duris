...
- Replaced root-plus-immediate-VNUM recovery with schema 8 item records containing a
  ...
  binary record is used by world generations and `WRF2` floor deltas, so descendants are
...
Severity: Critical
Confidence: Confirmed
Remediation status: Completed on branch; schema 8 and `WRF2` carry complete bounded item
...
3. Stage recovery and validate every item UID, VNUM, parent, root, state, and owner before
   ...
   by schema 8 transactional recovery.
