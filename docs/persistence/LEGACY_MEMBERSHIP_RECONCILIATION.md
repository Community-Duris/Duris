...
reference. Administrative policy blocks, including an administrative requested
rank or `administrative_permissions: restore`, are classified as
`insufficient_evidence` before disposition validation and therefore require a
disposition. A disposition attached to a row that passes every semantic,
current-authority, effect, and policy gate and remains `uniquely_mappable` is
invalid and aborts the entire run. Operators must not alter valid semantic
evidence to withhold that restoration; stop and obtain a reviewed policy or
change-record decision before rerunning. A disposition is the durable support
route for a returning player; operators must not improvise a numeric
reassignment.
