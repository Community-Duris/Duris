			  ...
			  P_obj /*tar_obj*/)
{
	...
	case SPELL_TYPE_SPELL:
		...
		case 1: /* warm */
			...
			break;
		case 2: /* wet */
			if (zone->climate.season_precip[(int)get_season(
				    in_weather_sector(ch->in_room))] == SEASON_NO_PRECIP_EVER)
			{
...
void cast_gate(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj /*tar_obj*/)
{
	...
	struct portal_create_messages msg = {
		...
		/*vic r*/ 0,
		/*ch   */
		"&+cA sudden breeze blows by, creating a &+ycloud of dust&+c that quickly condenses into $p&n&+c.",
		/*ch r */
		"&+cA sudden breeze blows by, creating a &+ycloud of dust&+c that quickly condenses into $p&n&+c.",
		/*vic  */
		"&+cA sudden breeze blows by, creating a &+ycloud of dust&+c that quickly condenses into $p&n&+c.",
	...
	case SPELL_TYPE_SPELL:
		...
		else
		{
			...
			do
			{
				to_room = number(real_room(planes_room_num[6]), top_of_world);
				 ...
				 (world[to_room].sector_type ==
				  SECT_MOUNTAIN) || /* mountains are no-walk on maps nowdays*/
				 (world[to_room].sector_type ==
				  SECT_UNDRWLD_MOUNTAIN) || /* underworld mountains double so */
		...
		if ((to_room == NOWHERE) || (to_room == from_room) ||
		    IS_ROOM(to_room, ROOM_NO_MAGIC) || IS_ROOM(from_room, ROOM_NO_GATE) ||
		    IS_ROOM(to_room, ROOM_NO_GATE) || IS_HOMETOWN(to_room) ||
		...
		if (spell_general_portal(level, ch, 0, &set, &msg))
		{
			snprintf(Gbuf4, MAX_STRING_LENGTH,
				 "A gateway is opened to the %s plane!\r\n", planes_name[plane_id]);
...
void cast_nether_gate(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj /*tar_obj*/)
{
	...
	struct portal_create_messages msg = {
		...
		/*vic r*/ 0,
		/*ch   */
		"&+LA dark &n&+brift&+L peels through reality, accompanied by the &n&+gstench&+L of &+Rhell.",
		/*ch r */
		"&+LA dark &n&+brift&+L peels through reality, accompanied by the &n&+gstench&+L of &+Rhell.",
		/*vic  */
		"&+LA dark &n&+brift&+L peels through reality, accompanied by the &n&+gstench&+L of &+Rhell.",
	...
	case SPELL_TYPE_SPELL:
		...
		if ((to_room == NOWHERE) || (to_room == from_room) ||
		    IS_ROOM(from_room, ROOM_NO_GATE) || IS_ROOM(to_room, ROOM_NO_GATE) ||
		    IS_HOMETOWN(to_room) || IS_HOMETOWN(from_room))
		{
			send_to_char(msg.fail_to_caster, ch);
			return;
		}
	...
	case SPELL_TYPE_SPELL:
		...
		else
		{
			    ...
			    (world[ch->in_room].sector_type != SECT_AIR_PLANE))
			{
				...
				send_to_char(
					"&+WPlane shift is used for interplanar travel, try walking!&n\n",
					ch);
				return;
			}
			do
		...
		if ((to_room == NOWHERE) || (to_room == ch->in_room) ||
		    IS_ROOM(to_room, ROOM_NO_MAGIC) || IS_ROOM(ch->in_room, ROOM_NO_GATE) ||
		    IS_ROOM(to_room, ROOM_NO_GATE) || IS_HOMETOWN(to_room) ||
			 ...
			 P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	send_to_char(
		"&+rYou silently prepare for the incantation, as it will surely draw you close to &+Ldeath&+r.\r\n",
		ch);
	...
	{
		send_to_char(
			"&+rThe strain on your body is too much, and the &+Ccold &+rembrace of &+Ldeath&+r welcomes you.&n\r\n",
			ch);
		die(ch, ch);
	}
	else
		       ...
		       P_obj /*tar_obj*/)
{
	...
	if (create_walls(ch->in_room, var, ch, level, WALL_OF_ICE, 40, 1800, "&+Wa wall of ice&N",
		    ...
		    P_obj /*tar_obj*/)
{
	...
	{
		snprintf(buf1, MAX_STRING_LENGTH,
			 "&+LA wall of blackness begins to spread to the %s!&n\r\n", dirs[var]);
		snprintf(buf2, MAX_STRING_LENGTH,
			 "&+LA wall of blackness begins to spread to the %s!&n\r\n",
			 dirs[rev_dir[var]]);
			 ...
			 P_obj /*tar_obj*/)
{
	...
	if (create_walls(ch->in_room, var, ch, level, WALL_OF_FORCE, level, 1800,
			 "&+Wa wall of force&n", "&+WThe outline of some wall is here to the %s.&n",
			 ITEM_INVISIBLE))
	{
		SET_BIT(EXIT(ch, var)->exit_info, EX_BREAKABLE);
		SET_BIT(VIRTUAL_EXIT((world[ch->in_room].dir_option[var])->to_room, rev_dir[var])
			    ...
			    P_obj /*tar_obj*/)
{
	...
	if (create_walls(ch->in_room, var, ch, level, LIGHTNING_CURTAIN, dice(level / 4, 10), 1800,
			 "&+Ya crackling curtain of lightning&n",
			 ...
			 P_obj /*tar_obj*/)
{
	...
	for (dir = 0; dir < NUM_EXITS; dir++)
	{
		...
		if (create_walls(
			    room, dir, ch, level, PRISMATIC_WALL, level, 1800, "a prismatic wall",
...
void event_earthen_tomb(P_char /*ch*/, P_char /*victim*/, P_obj /*obj*/, void *data)
{
	...
	if (available_exits == 0)
	{
		send_to_room(
			"&+yThe ground &+Lr&+yu&+Lm&+ybl&+ye&+Ls &+yand quivers under your feet.&n\r\n",
			room);
		add_event(event_earthen_tomb, (int)(PULSE_VIOLENCE * 1.5), 0, 0, 0, 0, &room,
			  sizeof(room));
		return;
	}
			    ...
			    event_func_type trans_func, event_func_type func_bye)
{
	...
	{
		send_to_char(
			"Sparks flow from your hands, but CHAOS in room disperse your magic.\r\n",
			ch);
		return FALSE;
	}
...
void finish_room_transmute(P_char ch, struct grow_data *data)
{
	...
	for (e = get_scheduled(ch, event_transmute_bye); e;
	     e = get_next_scheduled_char(e, event_transmute_bye))
	{
...
void event_ethereal_grounds_bye(P_char /*ch*/, P_char /*victim*/, P_obj /*obj*/, void *data)
{
	...
	send_to_room(
		"&+LAs mysterious &+wh&+Waz&+we &+Wf&+wad&+Les the surroundings start to look familiar again.\n",
		g_data->room);
...
void event_ethereal_grounds(P_char ch, P_char /*victim*/, P_obj /*obj*/, void *data)
{
	...
	send_to_room(
		"&+LThe surroundings begin to &+cb&+Cl&+cu&+Cr &+Land &+ct&+Cw&+ci&+Cs&+ct &+Las mysterious &+wh&+Waz&+we &+Lfills the area.\n",
		g_data->room);
			    ...
			    P_char /*victim*/, P_obj /*obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_ETHEREAL_GROUNDS, 4 * 60, 4 * seconds,
			     ...
			     P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_MUD_ROCK, 4 * 60, 4 * seconds,
			     ...
			     P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_ROCK_MUD, 4 * 60, 4 * seconds,
			      ...
			      P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_MUD_WATER, 4 * 60, 4 * seconds,
			      ...
			      P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_WATER_MUD, 4 * 60, 4 * seconds,
			      ...
			      P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_WATER_AIR, 4 * 60, 4 * seconds,
			      ...
			      P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_AIR_WATER, 4 * 60, 4 * seconds,
			      ...
			      P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_ROCK_LAVA, 4 * 60, 4 * seconds,
			      ...
			      P_char /*tar_ch*/, P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_LAVA_ROCK, 4 * 60, 4 * seconds,
			  ...
			  P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_DEPRESSED_EARTH, 4 * 60, 4 * seconds,
	       ...
	       P_obj /*tar_obj*/)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_GROW, 4 * 60, 4 * seconds, event_grow,
				    event_grow_bye))
		return;
		...
		P_obj /*tar_obj*/)
{
	...
	*/
	...
	act("&+GGreen&n vines sprout up around you forming a protective shield.", FALSE, ch, 0, 0,
	    TO_CHAR);
	act("&+GVines&n sprout up around $n forming a protective shield.", FALSE, ch, 0, 0,
	    TO_NOTVICT);
...
void event_spike_growth(P_char ch, P_char /*victim*/, P_obj obj, void *data)
{
	...
	{
		send_to_room(
			"The ground fissures and &+ytiny spikes&n can be seen in the broken ground.\r\n",
			sgd->room);
	...
	for (tch = world[sgd->room].people; tch; tch = tch->next_in_room)
	{
		...
		case 2:
			...
			spell_damage(ch, tch, GET_LEVEL(ch) * number(1, 3), SPLDAM_GENERIC, 0,
				     &message);
...
void event_awaken_forest(P_char ch, P_char /*victim*/, P_obj obj, void *data)
{
	...
	{
		send_to_room(
			"The &+Gtrees&n stretch their old gnarly &+ybranches&n and you feel their roots slowly moving beneath the ground.\r\n",
			awd->room);
	...
	for (tch = world[awd->room].people; tch; tch = tch->next_in_room)
	{
		...
		case 2:
			...
			{
				act("Thick &+groots&n burst from the &+yground&n knocking you off balance.",
				    FALSE, tch, 0, 0, TO_CHAR);
				act("Thick &+groots&n burst from the &+yground&n knocking&n $n &+yoff balance.&n",
				    FALSE, tch, 0, 0, TO_NOTVICT);
				SET_POS(tch, POS_SITTING + GET_STAT(tch));
				if (!NewSaves(tch, SAVING_SPELL, 3))
				{
					act(" .. the &+groots&n hit you really hard!", FALSE, tch,
					    0, 0, TO_CHAR);
					act(" .. the &+groots&n hit $n really hard!", FALSE, tch, 0,
					    0, TO_NOTVICT);
			...
			{
				act("Thick &+groots&n burst from the &+yground&n missing you.",
				    FALSE, tch, 0, 0, TO_CHAR);
				act("Thick &+groots&n burst from the &+yground&n missing&n $n.",
				    FALSE, tch, 0, 0, TO_NOTVICT);
			}
			break;
		case 3:
		     ...
		     P_obj /*tar_obj*/)
{
	...
	act("You feel as if though your blood starts to flow slower in your veins.", FALSE, victim,
	    0, 0, TO_CHAR);
...
bool exit_wallable(int room, int dir, P_char ch)
{
	...
	for (obj = world[room].contents; obj; obj = next_obj)
	{
		...
		{
			send_to_char("The magic attempts to take hold, but disperses suddenly...",
				     ch);
			return FALSE;
		}
	}
		  ...
		  P_char /*victim*/, P_obj /*obj*/)
{
	...
	for (fm = world[ch->in_room].people; fm; fm = fm->next_in_room)
	{
		...
		{
			send_to_char(
				"Your mirage swirls briefly before dissapearing from a disturence in the area.\r\n",
				ch);
			return;
		}
	}
	...
	act("&+LAs $n spreads $s hands out wide, bright &+rp&+Rr&+Yi&+Gs&+ym&+Ca&+ct&+Bi&+bc &+wm&+Wote&+ws&+L of &+ma&+Mr&+Lca&+Mn&+me\n"
	    ...
	    TRUE, ch, 0, 0, TO_ROOM);
	act("&+LAs a &+rp&+Rr&+Yi&+Gs&+ym&+Ca&+ct&+Bi&+bc &+wm&+Wot&+we&+L settles before you $n speaks an &+ma&+Mr&+Lca&+Mn&+me&+L word of &+Wpower&+L!&n",
	    TRUE, ch, 0, 0, TO_ROOM);
	...
	act("&+LAs you spread your hands out wide, bright &+rp&+Rr&+Yi&+Gs&+ym&+Ca&+ct&+Bi&+bc &+wm&+Wote&+ws&+L of &+ma&+Mr&+Lca&+Mn&+me\n"
	    ...
	    TRUE, ch, 0, 0, TO_CHAR);
	act("&+LAs a &+rp&+Rr&+Yi&+Gs&+ym&+Ca&+ct&+Bi&+bc &+wm&+Wot&+we&+L settles before you, you speak an &+ma&+Mr&+Lca&+Mn&+me&+L word of &+Wpower&+L!&n",
	    TRUE, ch, 0, 0, TO_CHAR);
	...
	for (gl = ch->group; gl; gl = gl->next)
	{
		if (ch->in_room == gl->ch->in_room)
		{
			...
			if (IS_RACEWAR_GOOD(gm))
			{
				...
				} while (race != RACE_HUMAN && race != RACE_GREY &&
					 race != RACE_MOUNTAIN && race != RACE_BARBARIAN &&
					 race != RACE_GNOME && race != RACE_HALFLING &&
					 race != RACE_HALFELF && race != RACE_CENTAUR &&
					 race != RACE_GITHZERAI && race != RACE_AGATHINON &&
					 race != RACE_THRIKREEN && race != RACE_MINOTAUR &&
					 race != RACE_FIRBOLG && race != RACE_WOODELF);
			}
			else if (IS_RACEWAR_EVIL(gm))
			{
				...
				} while (race != RACE_DROW && race != RACE_DUERGAR &&
					 race != RACE_GITHYANKI && race != RACE_OGRE &&
					 race != RACE_GOBLIN && race != RACE_ORC &&
					 race != RACE_OROG && race != RACE_TROLL &&
					 race != RACE_THRIKREEN && race != RACE_MINOTAUR &&
					 race != RACE_DRIDER && race != RACE_PILLITHID &&
					 race != RACE_KUOTOA);
			}
			else if (IS_RACEWAR_UNDEAD(gm))
			{
				...
				{
					race = number(0, LAST_RACE);
				} while (race != RACE_LICH && race != RACE_PVAMPIRE &&
					 race != RACE_PDKNIGHT && race != RACE_SHADE &&
					 race != RACE_REVENANT && race != RACE_PSBEAST &&
					 race != RACE_WIGHT && race != RACE_GARGOYLE &&
					 race != RACE_PHANTOM);
			}
			else
			{
...
void event_change_yzar_race(P_char ch, P_char victim, P_obj /*obj*/, void * /*data*/)
{
	...
	int yzar_races[NUM_YZARS] = { RACE_SHADE,    RACE_REVENANT, RACE_LICH,	RACE_PVAMPIRE,
				      RACE_PDKNIGHT, RACE_PSBEAST,  RACE_WIGHT, RACE_PHANTOM,
				      RACE_PARASITE, RACE_SLIME };
	if (!IS_ALIVE(ch))
	{
		return;
	}
	...
	{
		send_to_char(
			"You feel &+Yho&+yrr&+Yible&n as &+Wbones&n shift and &+wsnap&n, and your &+rf&+yles&+rh&n falls to the ground.\n",
			ch);
	...
	{
		send_to_char(
			"The &+Rpain&n is almost unbearable as your &+Wbones&n dissolve into a pile of &+ggoo&n.\n",
			ch);
	}
	else if (IS_UNDEADRACE(ch))
	{
		send_to_char(
			"The sense of &+Ldeath&n and &+ydecay&n fills your nostrils, your throat, and your lungs.\n",
			ch);
	}
	...
	{
		send_to_char(
			"As your &+Wbones&n collapse into &+Lashe&n, you feel &+gregrowth&n and somehow &+Ysticky&n.\n",
			ch);
	}
	else
	{
		send_to_char(
			"Your &+Wbones&n &+wcrack&n even more, twisting, &+Rtormenting&n you, as your body reforms.\n",
			ch);
		send_to_char(
			"Your &+rblood&n congeals quickly forming a mass of &+Rmuscle&n and &+yskin&n.\n",
			ch);
	}

	// Amount of mud-hours until 3am.
	time_to_witching_hour = (time_info.hour >= 3) ? (27 - time_info.hour) :
							(3 - time_info.hour);
	// Convert to seconds.
			 ...
			 P_char victim, P_obj /*obj*/)
{
	...
	{
		act("Your spell is ineffective against $N, since $E is not Yzar.", FALSE, ch, 0,
		    victim, TO_CHAR);
		return;
	}
		...
		P_obj /*obj*/)
{
	...
	if ((afp = get_spell_from_char(victim, TAG_RESTED)) != NULL)
	{
		...
		act("You refresh $N's rested bonus and upgrade it to well-rested.", FALSE, ch, NULL,
		    victim, TO_CHAR);
		debug("%s refreshed %s's rested bonus and upgrades it to well-rested!", J_NAME(ch),
		      J_NAME(victim));
