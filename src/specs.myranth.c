...
{
	char Cbuf1[] = "aligator", Cbuf2[] = "mist", Cbuf3[] = "muck", Cbuf4[] = "skeleton", Cbuf5[] = "swamp";
...
int myr_gowell(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	switch (number(1, 100))
	{
		...
		case 2:
		{
			mobsay(ch, "Someday, I will kill Sir Lagarias, and avenge my brothers!");
			return TRUE;
		}
		case 3:
		{
			mobsay(ch, "I killed King Ventor, and I will do the same to Sir Lagarias ! ");
			return TRUE;
		}
		case 4:
		{
...
int myr_gareth(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	switch (number(1, 100))
	{
		case 1:
		{
			mobsay(ch, "If only my brother Sir Gowell and Sir Lagarias could get along.");
			return TRUE;
		}
		case 2:
		{
			mobsay(ch, "Someday, I will bring peace to my best friend and brother.");
			return TRUE;
		}
		case 3:
		{
...
int myr_lagarias(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	switch (number(1, 100))
	{
		...
		case 2:
		{
			mobsay(ch, "I shall impale all who stand against me in the jousting ring ! ");
			return TRUE;
		}
		case 3:
		{
			mobsay(ch, "I have killed 40 knights with my pole, and am willing to add to that sum ! ");
			return TRUE;
		}
		case 4:
		{
...
int myr_golahy(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	switch (number(1, 100))
	{
		...
		case 4:
		{
			mobsay(ch, "I shall not be stopped in any of my endeavors for the King!");
			return TRUE;
		}
		case 5:
		{
...
int myr_bors(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	switch (number(1, 100))
	{
		...
		case 2:
		{
			mobsay(ch, "So easily has Sir Golahy forgotten my aid in the Test of Knights ! ");
			return TRUE;
		}
		case 3:
		{
...
int myr_bantan(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	switch (number(1, 100))
	{
		...
		case 3:
		{
			mobsay(ch, "My brother and I spar all of the time to improve our skills.");
			return TRUE;
		}
		case 4:
		{
...
int myr_gracian(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	switch (number(1, 100))
	{
		...
		case 3:
		{
			mobsay(ch, "I love to run my sword through weakling humans that defy the King ! ");
			return TRUE;
		}
		case 4:
		{
...
int myr_hagatha(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if (!pl)
	{
		for (dummy = world[ch->in_room].people; dummy; dummy = dummy->next_in_room)
		{
			if ((dummy->equipment[WEAR_WRIST_R] && (obj_index[dummy->equipment[WEAR_WRIST_R]->R_num].virtual == bracelet_vr)) ||
			    (dummy->equipment[WEAR_WRIST_L] && (obj_index[dummy->equipment[WEAR_WRIST_L]->R_num].virtual == bracelet_vr)))
			{
				i = TRUE;
			}
			if (i)
	...
	if (got_it && (pl->in_room == ch->in_room))
	{
		...
		act("\nHagatha's eyes open wide as she sees the triangular bracelet on $n's\n"
		    ...
		    TO_NOTVICT); /*
		                  ...
		                  */
		unequip_char(pl, OBJ_WORN_POS(bracelet, WEAR_WRIST_R) ? WEAR_WRIST_R : WEAR_WRIST_L);
		/*
...
int breale_townsfolk(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	for (TmpCh = world[ch->in_room].people; TmpCh && !IS_FIGHTING(ch); TmpCh = TmpCh->next_in_room)
	{
		if (!IS_SET(TmpCh->specials.act, PLR_ANONYMOUS) &&
		    (GET_CLASS(TmpCh) == CLASS_SHAMAN || GET_CLASS(TmpCh) == CLASS_SORCERER || GET_CLASS(TmpCh) == CLASS_NECROMANCER || GET_CLASS(TmpCh) == CLASS_SUMMONER ||
 ...
 */
const int tree_keys[] = {70013, 70014, 70015, 70039};
...
int myranth_key(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd != CMD_GIVE && cmd != CMD_GET && cmd != CMD_REMOVE && cmd != CMD_TAKE)
		return FALSE;
...
int blackness_sword(P_obj obj, P_char ch, int cmd, char *arg)
{
	 ...
	 */
	unequip_char(ch, OBJ_WORN_POS(ankh, WEAR_NECK_1) ? WEAR_NECK_1 : WEAR_NECK_2);
	extract_obj(ankh);
...
const int volheru[] = {70995, 70990, 70985, 70980, 70975, 70970, 70965, 70960, 70955, 70950, 70945, 70940, 70935, 70930,
                       70925, 70920, 70915, 70910, 70905, 70900, 70895, 70890, 70885, 70880, 70875, 70870, 70867, 70866};

#define NUM_MORLANTHRA 8
const int morlanthra[] = {71430, 71425, 71420, 71415, 71410, 71405, 71400, 71270};

#define NUM_QUESTING 8
const int questing[] = {71685, 71680, 71675, 71670, 71665, 71660, 71655, 71650};
