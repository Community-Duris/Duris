...
void do_list_innates(P_char ch, char *args)
{
  ...
  for (h = 0; h <= LAST_INNATE; h++)
  {
    ...
    for (j = 1; j <= (playeronly ? RACE_PLAYER_MAX : LAST_RACE); j++)
    {
      ...
      if (playeronly && (j == RACE_ILLITHID || j == RACE_SHADE || j == RACE_LICH || j == RACE_PVAMPIRE || j == RACE_PDKNIGHT || j == RACE_PSBEAST || j == RACE_SGIANT || j == RACE_WIGHT || j == RACE_PHANTOM || j == RACE_HARPY || j == RACE_OROG || j == RACE_PILLITHID || j == RACE_KUOTOA || j == RACE_WOODELF))
      {
        continue;
      }
    ...
    if (class_innates_at_all[i])
    {
      ...
      for (j = 1; j <= CLASS_COUNT; j++)
      {
        ...
        if (playeronly && (j == flag2idx(CLASS_ASSASSIN) || j == flag2idx(CLASS_THIEF) || j == flag2idx(CLASS_MINDFLAYER) || j == flag2idx(CLASS_ALCHEMIST) || j == flag2idx(CLASS_DREADLORD) || j == flag2idx(CLASS_AVENGER) || j == flag2idx(CLASS_THEURGIST) || j == flag2idx(CLASS_WARLOCK)))
        {
          continue;
        }
