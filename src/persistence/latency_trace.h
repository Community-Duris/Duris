...
static inline void latency_trace_dump(FILE *output)
{
	...
	int n = _latency_count < LATENCY_TRACE_MAX_SAMPLES ? _latency_count :
							     LATENCY_TRACE_MAX_SAMPLES;
	for (int i = 0; i < n; i++)
	{
		...
		if (top_count < 10)
		{
			...
			{
				if (_latency_buf[top_idx[k]].duration_us >
				    _latency_buf[top_idx[k - 1]].duration_us)
				{
					int t = top_idx[k];
	...
	for (int i = 0; i < top_count && i < 10; i++)
	{
		...
		fprintf(output, "%-30s %8ld %8ld\n", _latency_buf[idx].name,
			_latency_buf[idx].duration_us, _latency_buf[idx].tick);
	}
