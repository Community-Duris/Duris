    ...
    player_death_restitution_runtime_submission *submission_out)
{
    ...
    return player_death_restitution_runtime_submit_live_approved(
        command, actor, actor_level, submission_out);
}
    ...
    const char *actor, int actor_level, const char *hex_chunk, size_t hex_chunk_size)
{
    ...
    if (staged->hex.size() > PLAYER_DEATH_RESTITUTION_STAFF_MAX_COMMAND_BYTES * 2 ||
        hex_chunk_size > PLAYER_DEATH_RESTITUTION_STAFF_MAX_COMMAND_BYTES * 2 -
                              staged->hex.size())
        return player_death_restitution_runtime_result::invalid_plan;
    try
    {
    ...
    player_death_restitution_runtime_submission *submission_out)
{
    ...
    staged->hex.clear();
    staged->active = false;
    return player_death_restitution_staff_submit_hex(
        actor, actor_level, canonical_hex.data(), canonical_hex.size(), submission_out);
}
