...
bool callbacks_complete(const player_death_restitution_runtime_callbacks &callbacks)
{
    return callbacks.recipient_is_offline && callbacks.pending_save_is_empty &&
           callbacks.acquire_target_save_login_fence && callbacks.release_target_save_login_fence;
}
...
namespace
{
player_death_restitution_runtime_result submit_command_internal(
    const critical_command &command, const player_death_restitution_plan &plan,
    const player_death_restitution_runtime_callbacks &callbacks, void *context,
