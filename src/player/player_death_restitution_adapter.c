...
std::array<player_death_restitution_runtime_submission, MAX_LIVE_SUBMISSIONS>
    live_submissions = {};
...
bool acquire_target_save_login_fence(uint32_t pid, uint64_t expected_revision, void *)
{
    ...
    if (!recipient_is_offline(pid, nullptr) ||
        !player_save_pipeline_acquire_target_save_login_fence(
            static_cast<int>(pid), expected_revision))
        return false;
    if (!recipient_is_offline(pid, nullptr))
    {
        player_save_pipeline_release_target_save_login_fence(
            static_cast<int>(pid), expected_revision);
        return false;
    }
    return true;
...
player_death_restitution_runtime_submission *find_submission(
...
player_death_restitution_runtime_submit_live(
...
player_death_restitution_runtime_result
player_death_restitution_runtime_submit_live_approved(
    ...
    player_death_restitution_runtime_submission *submission_out)
{
    ...
    player_death_restitution_runtime_submission submission = {};
    const player_death_restitution_runtime_result result =
        player_death_restitution_runtime_submit_command(
            approved_command, live_callbacks, nullptr, &submission);
    if (result == player_death_restitution_runtime_result::accepted ||
        result == player_death_restitution_runtime_result::attached ||
        result == player_death_restitution_runtime_result::journal_uncertain)
    {
...
void player_death_restitution_runtime_handle_completions(
    const critical_completion *completions, size_t count)
{
    if (!completions)
        return;
...
void player_death_restitution_runtime_shutdown(void)
{
    for (auto &submission : live_submissions)
        if (submission.target_save_login_fence_held)
            player_death_restitution_runtime_abort(
                &submission, live_callbacks, nullptr);
}
