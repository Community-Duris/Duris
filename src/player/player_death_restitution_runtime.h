...
using player_death_restitution_recipient_is_offline_fn = bool (*)(
    uint32_t recipient_pid, void *context);
using player_death_restitution_pending_save_empty_fn = bool (*)(
    uint32_t recipient_pid, uint64_t expected_save_revision, void *context);
using player_death_restitution_acquire_save_login_fence_fn = bool (*)(
    uint32_t recipient_pid, uint64_t expected_save_revision, void *context);
using player_death_restitution_release_save_login_fence_fn = void (*)(
    uint32_t recipient_pid, uint64_t expected_save_revision, void *context);
...
player_death_restitution_runtime_result player_death_restitution_runtime_submit(
    const player_death_restitution_plan &plan,
    const player_death_restitution_runtime_callbacks &callbacks, void *context,
    player_death_restitution_runtime_submission *submission);
