...
bool ground_casting_check(P_char ch, int spl)
{
  ...
  if (IS_SET(ch->specials.affected_by2, AFF2_CASTING) && !IS_SET(skills[spl].targets, TAR_NOCOMBAT) && ((number(0, 100) < (int)(GET_CHAR_SKILL(ch, SKILL_GROUND_CASTING) / 2)) || (number(0, 120) < (int)(GET_CHAR_SKILL(ch, SKILL_CONCENTRATION) / 2))))
  {
...
void do_cast(P_char ch, char *argument, int cmd)
{
  ...
  else if ((GET_CLASS(ch, CLASS_DRUID) && !IS_MULTICLASS_PC(ch)) || (GET_CLASS(ch, CLASS_BLIGHTER) && !IS_MULTICLASS_PC(ch)) || ((!is_tank || number(0, 1)) && (IS_NPC(ch) || IS_SET(ch->specials.act2, PLR2_QUICKCHANT)) && (notch_skill(ch, SKILL_QUICK_CHANT, get_property("skill.notch.quickChant", 2.5)) || (GET_CHAR_SKILL(ch, SKILL_QUICK_CHANT) > number(1, 100)))))
  {
    dura >>= 1;
  }
...
void event_spellcast(P_char ch, P_char victim, P_obj obj, void *data)
{
  ...
  if (!IS_SET(skills[arg->spell].targets, TAR_IGNORE) && !IS_SET(skills[arg->spell].targets, TAR_AREA) && !IS_SET(skills[arg->spell].targets, TAR_OFFAREA) && !IS_SET(skills[arg->spell].targets, (TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_OBJ_WORLD | TAR_OBJ_EQUIP)))
  {
    if ((!tar_char && !IS_SET(skills[arg->spell].targets, TAR_CHAR_WORLD)) || (tar_char && !IS_SET(skills[arg->spell].targets, TAR_CHAR_WORLD) && !IS_SET(skills[arg->spell].targets, TAR_CHAR_RANGE) && (tar_char->in_room != ch->in_room)))
    {
  ...
  if (GET_LEVEL(ch) > MAXLVLMORTAL && IS_PC(ch))
  {
    snprintf(buf, MAX_STRING_LENGTH, "%s cast '%s' at %s in room %d", GET_NAME(ch), skills[arg->spell].name,
             tar_char ? GET_NAME(tar_char) : tar_obj ? tar_obj->short_description
                                                     : "(no target ch|obj)",
  ...
  if (tar_char)
  {
    if ((has_innate(ch, INNATE_WILDMAGIC) || IS_AFFECTED4(ch, AFF4_WILDMAGIC) || (IS_NPC(ch) && IS_SET(ch->specials.act, ACT_WILDMAGIC))) && !number(0, 5) && IS_AGG_SPELL(arg->spell) && !(IS_SET(skills[arg->spell].targets, TAR_IGNORE) || IS_SET(skills[arg->spell].targets, TAR_AREA)))
    {
