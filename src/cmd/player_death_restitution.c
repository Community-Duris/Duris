                             ...
                             const player_death_restitution_runtime_submission &submission)
{
    ...
    case player_death_restitution_runtime_result::attached:
        if (have_operation)
        {
            ...
            send_to_char("Restitution submission accepted; the recipient remains fenced until completion.\r\n",
                         ch);
        return;
    case player_death_restitution_runtime_result::journal_uncertain:
        if (have_operation)
        {
            ...
            send_to_char("Restitution submission is journal-uncertain; the recipient remains fenced and do not retry it.\r\n",
                         ch);
        return;
    ...
    case player_death_restitution_runtime_result::fence_unavailable:
        send_to_char("The recipient save/login fence is unavailable; nothing was submitted.\r\n", ch);
        return;
    case player_death_restitution_runtime_result::coordinator_unavailable:
        send_to_char("The critical command coordinator is unavailable; nothing was submitted.\r\n", ch);
        return;
    ...
    case player_death_restitution_runtime_result::journal_failure:
        send_to_char("The restitution journal rejected the submission; nothing was submitted.\r\n", ch);
        return;
...
ACMD(do_restitution)
{
    ...
    if (!str_cmp(subcommand, "begin"))
    {
        ...
        if (result == player_death_restitution_runtime_result::accepted)
            send_to_char("Restitution payload staging started; append canonical chunks, then commit.\r\n",
                         ch);
        else
    ...
    const player_death_restitution_runtime_result result =
        player_death_restitution_staff_submit_hex(
            GET_NAME(ch), GET_LEVEL(ch), canonical_hex, std::strlen(canonical_hex), &submission);
    send_restitution_result(ch, result, submission);
}
