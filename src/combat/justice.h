...
#define NPC_INVADER(a, b)                                                    \
	(IS_NPC(a) && !TRUSTED_NPC(a) && (!BORNHERE(a, b) || IS_MORPH(a)) && \
	 (IS_SET(hometowns[b - 1].flags, EVILRACE(a) ? JUSTICE_GOODHOME :    \
					 GOODRACE(a) ? JUSTICE_EVILHOME :    \
						       0)))
...
#define IS_ELFIE(a)                                                     \
	((GET_RACE(a) == RACE_GREY) || (GET_RACE(a) == RACE_CENTAUR) || \
...
#define PC_NOTWELCOME(a, b)                                                                                                              \
	(IS_PC(a) && !TRUSTED_NPC(a) &&                                                                                                  \
	 ((IS_SET(hometowns[b - 1].flags, JUSTICE_GOODHOME) && !IS_RACEWAR_GOOD(a)) ||                                                   \
	  ...
	  (IS_SET(hometowns[b - 1].flags, JUSTICE_UNDEADHOME) &&                                                                         \
	   !IS_RACEWAR_UNDEAD(                                                                                                           \
		   a)) /*                            || (IS_SET(hometowns[b - 1].flags, JUSTICE_NEUTRALHOME) && !RACE_NEUTRAL(a)) */) && \
...
#define EVILRACE(a)                                                                           \
	(IS_RACEWAR_EVIL(a) || (GET_RACE(a) == RACE_DEMON) || (GET_RACE(a) == RACE_DEVIL) ||  \
	 (IS_UNDEAD(a)) || (GET_RACE(a) == RACE_GOBLIN) ||                                    \
	 (GET_RACE(a) == RACE_DRAGON && !IS_PC_PET(a)) ||                                     \
	 (GET_RACE(a) == RACE_DRAGON && IS_PC_PET(a) && get_linked_char(a, LNK_PET) != NULL ? \
		  GET_RACEWAR(get_linked_char(a, LNK_PET)) == RACEWAR_EVIL :                  \
		  FALSE) ||                                                                   \
	 (GET_RACE(a) == RACE_HALFORC) || (GET_RACE(a) == RACE_DEMON))
...
/* invaders and outcasts can't hide behind shapechange, so use the
   GET_PLYR() macros. */
#define IS_NOTWELCOME(a) \
	(CHAR_IN_TOWN(a) && IS_TOWN_NOTWELCOME(a, CHAR_IN_TOWN(a)) && !IS_AFFECTED(a, AFF_BOUND))

#define IS_INVADER(a) \
	(CHAR_IN_TOWN(a) && IS_TOWN_INVADER(a, CHAR_IN_TOWN(a)) && !IS_AFFECTED(a, AFF_BOUND))
