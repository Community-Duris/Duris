...
void random_encounters(P_char ch)
{
	...
	if (world[ch->in_room].sector_type == SECT_FIREPLANE || world[ch->in_room].sector_type == SECT_LAVA || world[ch->in_room].sector_type == SECT_UNDRWLD_LIQMITH ||
	 ...
	 */
	if (ch->group && (ch->group->ch != ch) && (ch->group->ch->in_room == ch->in_room))
		return;

	if (world[ch->in_room].sector_type == SECT_UNDRWLD_WILD || world[ch->in_room].sector_type == SECT_UNDRWLD_CITY || world[ch->in_room].sector_type == SECT_UNDRWLD_INSIDE ||
	    world[ch->in_room].sector_type == SECT_UNDRWLD_MOUNTAIN || world[ch->in_room].sector_type == SECT_UNDRWLD_MUSHROOM)
	    ...
	    world[ch->in_room].sector_type == SECT_UNDRWLD_MOUNTAIN || world[ch->in_room].sector_type == SECT_UNDRWLD_MUSHROOM)
	{
		...
		if (mobs_vnum != 0)
		{
			...
			else if (IS_SET(mob->specials.act, ACT_CANSWIM) && ch->specials.z_cord < 0)
				mob->specials.z_cord = ch->specials.z_cord;
			else
...
P_obj ran_magical(P_char mob)
{
	...
	const char *lights[]  = {"&+ytorch", "&+Llantern", "&+Wstrange fungus", "&+Wglowing crystal"};
	int         nlights   = 3;
	const char *scrolls[] = {"tattered", "small", "&+Ldusty", "vellum", "&+yparchment", "frail", "&+Wglowing"};
	int         nscrolls  = 6;
	const char *potions[] = {"&+Ldark", "light", "&+ggreen", "&+Yyellow", "&+Bbubbly", "&+Lsmoking", "clear", "cloudy", "&+mpurple"};
	int         npotions  = 8;
	const char *bags[]    = {"&+yleather", "&+ybrown", "&+Lblack", "&+gsnakeskin", "large", "&+Wruffled", "&+mvelvet", "torn", "&+Rdragonscale", "&+yfurred", "silverish"};
	int         nbags     = 10;
	const char *drinks[]  = {"&+ybottle", "flagon", "jar", "&+Yvase", "&+Lwineskin", "&+Wdrinking horn", "&+ymug"};
	int         ndrinks   = 6;
	const char *foods[]   = {"pastry",
	                         "&+yloaf of bread",
	                         "&+Ltravel ration",
	                         "&+Rleg of beef",
	                         "&+Ljuicy steak",
...
P_obj ran_obj(P_char mob, ulong which)
{
	...
	const char *earrings[] = {"&+Wdiamond", "&+gemerald", "&+ytopaz", "&+ywooden", "&+gjade", "&+Wwhite gold", "&+Lonyx", "tin", "glass", "&+Wmarble", "&+Lblack", "&+Lgranite"};
	int         nearrings  = 11;
	const char *rings[]    = {"tarnished", "jeweled", "silver", "&+Ygolden", "&+Wshiny", "dull", "sparkling", "attractive", "&+Lblack", "glass"};
	int         nrings     = 9;
	const char *generics[] = {                                                      /* These MUST have blank at end */
	                          "",           "",         "",       "",           "", /* not all needs describing :) */
	                          "splendid ",  "ancient ", "dusty ", "scratched ", "antique ", "old ", "flawed ", "sooty ", "plain ", "ornate ", "dull ", "shiny ", "odd-looking ",
	                          "tarnished ", "unusual ", "spiffy "};
	int         ngenerics  = 20;
	const char *necks[]    = {"necklace", "collar", "cape", "amulet", "pendant"};
	int         nnecks     = 4;
	const char *bodys[]    = {"breastplate", "coat", "jacket", "chain shirt", "splint mail"};
	int         nbodys     = 4;
	const char *heads[]    = {"hat", "headdress", "cap", "helm", "coif", "derby"};
	int         nheads     = 5;
	const char *feets[]    = {"sandals", "boots", "shoes", "riding boots", "leathers"};
	int         nfeets     = 4;
	const char *shields[]  = {"dented", "round", "oval", "center-grip", "heater", "door", "shiny", "embossed", "coffin-shaped", "curved"};
	int         nshields   = 9;
	const char *weps[]     = {"axe", "sword", "mace", "club", "bardiche", "pike", "whip", "maul", "bone", "staff", "dagger", "spear"};
	int         nweps      = 11;
	const char *color[]    = /* better colors are duped often */
		{"&+L", "&+g", "&+y", "&+m", "&+b", "&+c", "&+w", "&+r", "&+G", "&+Y", "&+M", "&+B", "&+C", "&+W", "&+R", "&+L", "&+L", "&+L", "&+y", "&+y", "&+y", "&+y", "&+W", "&+c"};
	int ncolors = 23;
	...
	switch (which)
	{
		case ITEM_WEAR_FINGER: /* 1293 */
			...
			else if (GET_LEVEL(mob) > 24)
			{
				...
				if ((obj->affected[1].location == 7) || (obj->affected[1].location == 8) || /* can't use these */
				    (obj->affected[1].location == 6) || (obj->affected[1].location == 15) || (obj->affected[1].location == 16))
		...
		case ITEM_WEAR_NECK: /* 1285 */
			...
			obj->short_description = str_dup(buf);
			snprintf(buf, MAX_STRING_LENGTH, "A %s%s%s &nlies here.", color[temp3], generics[temp2], necks[temp]);
			obj->description = str_dup(buf);
		...
		case ITEM_WEAR_BODY: /* 1286 */
			...
			if (!(obj = read_object(1286, VIRTUAL)))
				return NULL;
			obj->str_mask = (STRUNG_KEYS | STRUNG_DESC1 | STRUNG_DESC2);
			snprintf(buf, MAX_STRING_LENGTH, "a %s%s%s&n", color[temp3], generics[number(0, nbodys)], bodys[temp]);
			obj->short_description = str_dup(buf);
			snprintf(buf, MAX_STRING_LENGTH, "A %s%s%s &nlies here.", color[temp3], generics[temp2], bodys[temp]);
			obj->description = str_dup(buf);
		...
		case ITEM_WEAR_HEAD: /* 1281 */
			...
			obj->short_description = str_dup(buf);
			snprintf(buf, MAX_STRING_LENGTH, "A %s%s%s &nlies here.", color[temp3], generics[temp2], heads[temp]);
			obj->description = str_dup(buf);
		...
		case ITEM_WEAR_LEGS: /* 1296 */
			...
			obj->short_description = str_dup(buf);
			snprintf(buf, MAX_STRING_LENGTH, "Some %s%sleggings &nlie here.", color[temp3], generics[temp]);
			obj->description = str_dup(buf);
		...
		case ITEM_WEAR_FEET: /* 1297 */
			...
			if (!(obj = read_object(1297, VIRTUAL)))
				return NULL;
			obj->str_mask = (STRUNG_KEYS | STRUNG_DESC1 | STRUNG_DESC2);
			snprintf(buf, MAX_STRING_LENGTH, "a pair of %s%s%s&n", color[temp3], generics[temp2], feets[temp]);
			obj->short_description = str_dup(buf);
			snprintf(buf, MAX_STRING_LENGTH, "A pair of %s%s%s &nlies here.", color[temp3], generics[temp2], feets[temp]);
			obj->description = str_dup(buf);
		...
		case ITEM_WEAR_ARMS: /* 1289 */
			...
			obj->short_description = str_dup(buf);
			snprintf(buf, MAX_STRING_LENGTH, "A pair of %s%ssleeves &nlie here.", color[temp3], generics[temp]);
			obj->description = str_dup(buf);
	...
	obj->extra_flags = obj->extra_flags & !ITEM_NOSELL & !ITEM_NOLOCATE & !ITEM_NOIDENTIFY & !ITEM_TRANSIENT;
...
void random_gems(P_obj gem, int base_value)
{
   ...
   "&+bazurite&n", "&+yba&n&+Bn&n&+yded a&N&+Rg&N&+yate&n",
         "&+Bblue quartz&n", "&+Weye a&n&+Lg&n&+Wate&n",
         "&+Lhematite&n", "&+bla&+Bp&n&+bis la&n&+Bz&n&+buli&n",
         "&+gm&+Ga&n&+gl&+Ga&n&+gc&+Gh&n&+gi&+Gt&N&+ge&n", "&+Wmo&n&+gs&+Ws ag&n&+ga&+Wte&n",
         "&+Lobsidian&n",	            "&+Rrhodochrosite&n",
         "&+ytiger &+Yeye&n",	              "&+Bturquiose&n",
         "",	                         ""						},

        {     /* Semi-precious Stones, base value 5 GP */
     "&+Yamber&n",	                    "&+Rcarnelian&n",
         "&+Wchalcedony&n",         "&+gchrysoprase&n",
         "&+ycitrine&n",	                     "&+bjasper&n",
         "&+Wmoon&n&+Bstone&n",	                   "&+Lonyx&n",
         "rock crystal", "&+Rs&n&+War&n&+Rdo&n&+Wny&n&+Rx&n",
         "smoky quartz", "&+Rstar &n&+Wrose&n&+R quartz&n",
         "&+Bzircon&n",	                         ""             },

        {     /* Fancy Stones, base value 10 GP */
     "&+yamber&n",	                  "&+Yalexandrite&n",
         "&+mamethyst&n",	                     "",
         "&+gchrysoberyl&n",	                      "&+Rcoral&n",
         "&+rgarnet&n",	                   "&+gjade&n",
         "&+Ljet&n",	                    "&+Wpearl&n",
         "",	                 "&+gspinel&n",
         "",	          "&+Btourmaline&n"                        },
...
"&+Lblack opal&n",	                                  "",
         "",             "&+gemerald&n",
         "&+Rfire opal",	                                "",
         "&+Wopal&n",          "&+Moriental amethyst&n",
         "",	           "&+Yoriental topaz&n",
         "",	               "&+Bsapphire&n",
         "&+Rstar &n&+Wr&n&+Ruby&n", "&+Bstar &n&+Ws&n&+Bapphire&n"},

        {/* Gem Stones, base value 500 GP */
"&+Lblack opal&n",	               "&+Lblack sapphire&n",
         "&+Wdiamond&n",             "&+gemerald&n",
         "&+rfire opal&n",	                    "&+Rjacinth&n",
         "&+Wopal&n",          "&+Boriental amethyst&n",
         "&+goriental emerald&n",	           "&+Yoriental topaz&n",
         "&+Rruby&n",	               "&+Bsapphire&n",
         "&+Rstar &n&+Wr&n&+Ruby&n", "&+Bstar &n&+Ws&n&+Bapphire&n"}
    };
	...
	if (base_value)
	{
		...
		while ((orig_base_value_place < 12) && (base_value > bv_table[orig_base_value_place][1]))
	...
	else
	{
		bv_die                = number(1, 100);
		orig_base_value_place = 0;
		while ((orig_base_value_place < 5) && (bv_die >= bv_table[orig_base_value_place][0]))
			orig_base_value_place++;
	...
	while (!get_out)
	{
		...
		{
			case 1:
				if ((current_base_value_place <= 12) && (current_base_value_place <= orig_base_value_place + 6) && !ignore_results_below_2)
...
void random_jewelry(P_obj obj, int base_value)
{
	...
	const char *descriptions[7] = {"wrought silver",
	...
	const char *kind[51]        = {"anklet",    "arm band", "belt",    "box(small)", "bracelet", "brooch",  "buckle",     "chain",  "chalice",          "choker", "clasp",   "collar",
	                               "comb",      "coronet",  "crown",   "decanter",   "diadem",   "earring", "fob",        "goblet", "headband(fillet)", "idol",   "locket",  "medal",
	                               "medallion", "necklace", "pendant", "pin",        "orb",      "ring",    "sceptre",    "seal",   "statuette",        "tiara",  "bracers", "cup",
	                               "dagger",    "flask",    "helm",    "horn",       "mirror",   "periapt", "phylactery", "rod",    "scarab",           "skull",  "shield",  "staff",
	                               "sword",     "talisman", "trident"};
	...
	if (base_value)
	{
		 ...
		 * increase more than 7 places beyond its original base value  */
		base_value_place = 0;
		while ((base_value_place < 12) && (base_value > bv_table[base_value_place][1]))
			++base_value_place;
	...
	else
	{
		bv_die           = number(0, 100);
		base_value_place = 0;
		while ((base_value_place < 6) && (bv_die >= bv_table[base_value_place][0]))
			base_value_place++;
	}
	...
	else
	{
		...
		for (bv_die = 1; bv_die <= bv_table[base_value_place][1]; bv_die++)
			bv += number(0, bv_table[base_value_place][2]) * bv_table[base_value_place][3];
	}
	...
	if (number(0, 10) == 1)
	{
		...
		if (bv == bv_table[base_value_place][1] * bv_table[base_value_place][2] * bv_table[base_value_place][3])
			if (base_value_place < 6)
			{
				++base_value_place;
				/*      goto roll_bv_dice;*/
		...
		else
		{
			...
			bv = bv_table[base_value_place][1] * bv_table[base_value_place][2] * bv_table[base_value_place][3];
	...
	obj->name = str_dup(buf);
	snprintf(buf, MAX_STRING_LENGTH, "a %s %s &n%s%s", descriptions[base_value_place], kind[temp], nice_gems, nice_work);
	obj->short_description = str_dup(buf);
	snprintf(buf, MAX_STRING_LENGTH, "A %s %s &nlies here.", descriptions[base_value_place], kind[temp]);
	obj->description = str_dup(buf);
	/* randomize some more */
	if (!number(0, 5))
	{
