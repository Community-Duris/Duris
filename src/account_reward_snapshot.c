...
{
    cJSON *item = cJSON_GetObjectItemCaseSensitive(root, name);
    if (!cJSON_IsNumber(item) || item->valuedouble < low || item->valuedouble > high || item->valuedouble != item->valueint)
        return false;
    *value = item->valueint;
    return true;
}
...
{
    if (value) cJSON_AddStringToObject(root, name, value);
...
static void clear_extra_descriptions(P_obj obj)
{
    ...
    while (ed)
    {
        ...
        if (ed->keyword) str_free(ed->keyword);
        if (ed->description) str_free(ed->description);
        FREE(ed);
        ed = next;
    }
...
char *account_reward_snapshot_serialize(P_obj obj)
{
    ...
    for (int i=0; i<NUMB_OBJ_VALS; ++i) cJSON_AddItemToArray(values, cJSON_CreateNumber(obj->value[i]));
...
bool account_reward_snapshot_apply(P_obj obj, const char *json, int template_version)
{
    ...
    if (!root || !cJSON_IsObject(root)) goto fail;
    if (!read_int(root, "template_version", template_version, template_version, &number)) goto fail;
    if (!read_int(root, "vnum", 1, INT_MAX, &number) || number != OBJ_VNUM(obj)) goto fail;
    if (!read_int(root, "type", ITEM_LOWEST, ITEM_LAST, &number)) goto fail;
    obj->type = (::byte)number;
    if (!replace_string(obj, root, "name", &obj->name, STRUNG_KEYS)) goto fail;
    if (!replace_string(obj, root, "description", &obj->description, STRUNG_DESC1)) goto fail;
    if (!replace_string(obj, root, "short_description", &obj->short_description, STRUNG_DESC2)) goto fail;
    if (!replace_string(obj, root, "action_description", &obj->action_description, STRUNG_DESC3)) goto fail;

    array = cJSON_GetObjectItemCaseSensitive(root, "values");
    if (!cJSON_IsArray(array) || cJSON_GetArraySize(array) != NUMB_OBJ_VALS) goto fail;
    for (int i=0; i<NUMB_OBJ_VALS; ++i) { entry=cJSON_GetArrayItem(array,i); if (!cJSON_IsNumber(entry) || entry->valuedouble < INT_MIN || entry->valuedouble > INT_MAX || entry->valuedouble != entry->valueint) goto fail; obj->value[i]=entry->valueint; }
    array = cJSON_GetObjectItemCaseSensitive(root, "timers");
    if (!cJSON_IsArray(array) || cJSON_GetArraySize(array) != 6) goto fail;
    for (int i=0; i<6; ++i) { char *end=NULL; long long parsed; entry=cJSON_GetArrayItem(array,i); if (!cJSON_IsString(entry) || !entry->valuestring) goto fail; errno=0; parsed=strtoll(entry->valuestring,&end,10); if (errno || !end || *end!='\0') goto fail; obj->timer[i]=(time_t)parsed; }

#define APPLY_U64(name, field) do { if (!read_u64(root,name,&value)) goto fail; field=(decltype(field))value; } while(0)
    APPLY_U64("wear_flags", obj->wear_flags); APPLY_U64("extra_flags", obj->extra_flags);
    APPLY_U64("anti_flags", obj->anti_flags); APPLY_U64("anti2_flags", obj->anti2_flags);
    APPLY_U64("extra2_flags", obj->extra2_flags);
    APPLY_U64("bitvector1", obj->bitvector); APPLY_U64("bitvector2", obj->bitvector2);
    APPLY_U64("bitvector3", obj->bitvector3); APPLY_U64("bitvector4", obj->bitvector4); APPLY_U64("bitvector5", obj->bitvector5);
#undef APPLY_U64
#define APPLY_REWARD_INT(name, low, high, field, cast_type) do { if (!read_int(root,name,low,high,&number)) goto fail; field=(cast_type)number; } while(0)
...
#undef APPLY_REWARD_INT
    ...
    if (!cJSON_IsArray(array) || cJSON_GetArraySize(array)!=MAX_OBJ_AFFECT) goto fail;
    for (int i=0;i<MAX_OBJ_AFFECT;++i) { entry=cJSON_GetArrayItem(array,i); if (!cJSON_IsObject(entry) || !read_int(entry,"location",APPLY_NONE,APPLY_LAST,&number)) goto fail; obj->affected[i].location=(::byte)number; if (!read_int(entry,"modifier",SCHAR_MIN,SCHAR_MAX,&number)) goto fail; obj->affected[i].modifier=(sbyte)number; }

    while (obj->affects) affect_from_obj(obj, obj->affects->type);
    array=cJSON_GetObjectItemCaseSensitive(root,"linked_affects");
    if (!cJSON_IsArray(array)) goto fail;
    for (int pass=0; pass<2; ++pass)
    {
        cJSON_ArrayForEach(entry,array)
        {
            int type,data,time;
            if (!cJSON_IsObject(entry) || !read_int(entry,"type",SHRT_MIN,SHRT_MAX,&type) || !read_int(entry,"data",SHRT_MIN,SHRT_MAX,&data) || !read_int(entry,"time",-1,INT_MAX,&time) || !read_u64(entry,"extra2",&value)) goto fail;
            bool altered_extra2 = type == TAG_ALTERED_EXTRA2;
            if ((pass == 0 && altered_extra2) || (pass == 1 && !altered_extra2))
                set_obj_affected_extra(obj,time,(sh_int)type,(sh_int)data,(ulong)value);
        }
    }
    ...
    if (!cJSON_IsArray(array)) goto fail;
    {
        ...
        cJSON_ArrayForEach(entry,array) {
            cJSON *keyword=cJSON_GetObjectItemCaseSensitive(entry,"keyword"), *description=cJSON_GetObjectItemCaseSensitive(entry,"description");
            struct extra_descr_data *ed;
            if (!cJSON_IsObject(entry) || (!cJSON_IsString(keyword) && !cJSON_IsNull(keyword)) || (!cJSON_IsString(description) && !cJSON_IsNull(description))) goto fail;
            CREATE(ed, struct extra_descr_data, 1, MEM_TAG_EXDESCD);
            ed->keyword=cJSON_IsString(keyword)?str_dup(keyword->valuestring):NULL;
            ed->description=cJSON_IsString(description)?str_dup(description->valuestring):NULL;
            ed->next=NULL; *tail=ed; tail=&ed->next;
        }
    }
    cJSON_Delete(root);
    return true;
