...
{
	if (IS_HUMANOID(ch)) /* wow, a genuine PC race. let's ronk. :) */
...
void language_show(P_char ch)
{
	...
	for (a = 1; a < TONGUE_LASTHEARD; a++)
		...
		{
			if (lang < 40)
				snprintf(buf, 512, "You have a basic understanding of %s.", language_names[a - 1]);
			else if (lang < 60)
				snprintf(buf, 512, "You comprehend %s%s.", language_names[a - 1], lang < 55 ? " somewhat" : "");
			else if (lang < 90)
	...
	if (GET_LANGUAGE(ch, 0) >= 1 && GET_LANGUAGE(ch, 0) <= TONGUE_GOD)
		snprintf(buf, 512, "Currently speaking: %s", language_names[GET_LANGUAGE(ch, 0) - 1]);
	else
		snprintf(buf, 512, "Currently speaking impossible language. Please contact someone about this.");
	act(buf, TRUE, ch, 0, 0, TO_CHAR);
}
...
void init_defaultlanguages(P_char ch)
{
	...
	if (!GET_LANGUAGE(ch, 0) || (!IS_TRUSTED(ch) && GET_LANGUAGE(ch, 0) >= TONGUE_GOD))
	...
	if (EVIL_RACE(ch))
		GET_LANGUAGE(ch, TONGUE_ORC) = MAX(GET_LANGUAGE(ch, TONGUE_ORC), number(30, 50) + GET_LEVEL(ch));
	else
		GET_LANGUAGE(ch, TONGUE_COMMON) = MAX(GET_LANGUAGE(ch, TONGUE_COMMON), number(30, 50) + GET_LEVEL(ch));
}
...
char *language_known(P_char ch, P_char vict)
{
	...
	kala[0] = '\0';
	if (IS_TRUSTED(ch) || (IS_PC(ch) && ((GET_LANGUAGE(ch, 0) < 1) || (GET_LANGUAGE(ch, 0) > TONGUE_GOD))))
		return kala;
	if (IS_NPC(GET_PLYR(ch)))
		...
		a = GET_LANGUAGE(ch, 0);
	if ((a > 0) && (a <= TONGUE_GOD))
		if (affected_by_spell(ch, SPELL_COMPREHEND_LANGUAGES) || ((IS_PC(vict) && (GET_LANGUAGE(vict, a) >= (20 + number(1, 20))))))
...
void language_gain(P_char ch, P_char vict, int tongue)
{
	...
	if (!number(0, 19) && (number(50, 150) < (GET_C_INT(vict) - 15)) && GET_LANGUAGE(vict, tongue) && (GET_LANGUAGE(vict, tongue) < 98) && (GET_LANGUAGE(ch, tongue) > 75))
		GET_LANGUAGE(vict, tongue)
	++;

	if (((tongue == TONGUE_ORC) && EVIL_RACE(vict)) || ((tongue == TONGUE_COMMON) && !EVIL_RACE(vict)))
		GET_LANGUAGE(vict, tongue) = MAX(GET_LEVEL(vict) + 20, GET_LANGUAGE(vict, tongue));
...
char *language_CRYPT(P_char ch, P_char victim, char *message)
{
	...
	//    return string;
	if (IS_TRUSTED(ch) || IS_TRUSTED(victim) || IS_NPC(ch) || IS_NPC(victim) || affected_by_spell(victim, SPELL_COMPREHEND_LANGUAGES))
		return string;
	if (((IS_TRUSTED(ch) && GET_LANGUAGE(ch, 0) != TONGUE_GOD)) || IS_TRUSTED(victim))
		return string;
	if (IS_PC(ch) && (GET_LANGUAGE(ch, 0) < 1 || GET_LANGUAGE(ch, 0) > TONGUE_GOD))
		return string;
