...
void do_raid(P_char ch, char *argument, int /*cmd*/)
{
	...
	if (IS_TRUSTED(ch))
	{
		...
		{
			send_to_char(
				"raid [room|zone|good|evil|all|(character name)] [bad] <-Show only bad players option.\n",
				ch);
			return;
		}
		...
		for (i = descriptor_list; i; i = i->next)
		{
			if (i->character && IS_PC(i->character) && (i->connected == CON_PLAYING))
			{
				...
				if ((!str_cmp(arg1, "room") &&
				    ...
				    (!str_cmp(arg1, "zone") && (world[i->character->in_room].zone ==
				...
				if (tch && IS_PC(tch) && !IS_TRUSTED(tch))
				{
					if (hasRequiredSlots(tch))
					{
						if (!str_cmp(arg2, "bad"))
							continue;
						snprintf(Gbuf, MAX_INPUT_LENGTH,
							 "%s is &+Wgood&n.\n", GET_NAME(tch));
		...
		if (!found &&
		    !(!str_cmp(arg1, "room") || !str_cmp(arg1, "zone") || !str_cmp(arg1, "evil") ||
		      !str_cmp(arg1, "good") || !str_cmp(arg1, "all")))
		{
...
bool hasRequiredSlots(P_char ch)
{
	...
	int wear_order[] = { 41, 24, 40, 6,  19, 21, 22, 20, 39, 3,  4,	 5,  35,
			     37, 12, 27, 23, 13, 10, 31, 11, 14, 15, 33, 34, 9,
	...
	for (int j = 0; wear_order[j] != -1; j++)
	{
		if ((t_obj = ch->equipment[wear_order[j]]) != NULL)
		{
			...
			{
				if (obj_index[t_obj->R_num].virtual_number ==
				    minor_create_name_list[i].obj_number)
				{
					minor_created = TRUE;
					break;
				}
