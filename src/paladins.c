...
void apply_aura(P_char ch, P_char victim, int aura)
{
	...
	switch (aura)
	{
		case AURA_PROTECTION:
			...
			af.modifier = (int)(-1 * get_property("innate.paladin_aura.protection_ac_mod", 1.0) * GET_LEVEL(ch));
			break;
		case AURA_PRECISION:
			af.location = APPLY_HITROLL;
			af.modifier = (int)(get_property("innate.paladin_aura.precision_hitroll_mod", 0.2) * GET_LEVEL(ch));
			break;
		...
		case AURA_ENDURANCE:
			...
			af.modifier = (int)(get_property("innate.paladin_aura.endurance_move_mod", 1.0) * GET_LEVEL(ch));
			break;
		case AURA_VIGOR:
			af.location = APPLY_HIT;
			af.modifier = (int)(get_property("innate.paladin_aura.vigor_hp_mod", 1.0) * GET_LEVEL(ch));
			break;
		case AURA_SPELL_PROTECTION:
			af.modifier = (int)(get_property("innate.paladin_aura.protection_spell_mod", 0.5) * GET_LEVEL(ch)) - 3;
			break;
		default:
...
bool cleave(P_char ch, P_char victim)
{
	if (is_wielding_paladin_sword(ch) && !affected_by_spell(victim, SKILL_CLEAVE) &&
	    (notch_skill(ch, SKILL_CLEAVE, get_property("skill.notch.cleave", 10)) || number(5, 100) < GET_CHAR_SKILL(ch, SKILL_CLEAVE)))
	{
		act("You bring down $p in a mighty slash, cleaving $N from $S target!", TRUE, ch, ch->equipment[WIELD], victim, TO_CHAR);
		act("$n brings down $p in a mighty slash, cleaving $N from $S target!", TRUE, ch, ch->equipment[WIELD], victim, TO_NOTVICT);
		act("$n brings down $p in a mighty slash, cleaving you from your target!", TRUE, ch, ch->equipment[WIELD], victim, TO_VICT);
...
void event_smite_evil(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	if (is_wielding_paladin_sword(ch))
	{
		...
		{
			if (!grouped(tch, ch) && GET_HIT(tch) < GET_MAX_HIT(tch) / 10 && IS_ALIVE(tch) && GET_HIT(tch) < 15 && IS_EVIL(tch) && (GET_RACEWAR(tch) != 1) &&
			    (notch_skill(ch, SKILL_SMITE_EVIL, get_property("skill.notch.smiteEvil", 10)) || GET_CHAR_SKILL(ch, SKILL_SMITE_EVIL) / 2 > number(0, 100)))
			{
...
void event_righteous_aura_check(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	if (IS_AWAKE(ch) && !IS_IMMOBILE(ch))
	{
		...
		{
			if (victim != ch && affected_by_spell(tch, SPELL_BLEAK_FOEMAN) && GET_OPPONENT(ch) != tch && !IS_TRUSTED(ch) && !IS_TRUSTED(tch))
			{ // Soulshield up!
				if (!IS_AFFECTED2(ch, AFF2_SOULSHIELD) && GET_ALIGNMENT(ch) >= 950 && !IS_AFFECTED4(ch, AFF4_NEG_SHIELD))
				{
					bzero(&af, sizeof(af));
					...
					act("&+WA holy aura forms around $n!&n", TRUE, ch, 0, 0, TO_ROOM);
					act("&+WA holy aura forms around you!&n", TRUE, ch, 0, 0, TO_CHAR);
				}
				// Dharma up!

				if (!IS_AFFECTED5(ch, AFF5_HOLY_DHARMA) && IS_AFFECTED2(ch, AFF2_SOULSHIELD))
				{
					bzero(&af, sizeof(af));
	...
	if (IS_FIGHTING(ch))
	{
		...
		if (GET_OPPONENT(opponent) != ch && CAN_ACT(opponent) && (GET_ALIGNMENT(opponent) <= -500))
		{
...
bool dread_blade_proc(P_char ch, P_char victim)
{
	...
	spell_func_type spells[5] = {spell_disease, spell_blindness, spell_wither, spell_harm, spell_unholy_word};
	spell_func_type spell_func;

	if (!IS_FIGHTING(ch) || !(victim = GET_OPPONENT(ch)) || !IS_ALIVE(victim) || !(room) || number(0, 32)) // 3%
		return false;
	...
	{
		if ((wpn = ch->equipment[pos]) && wpn->type == ITEM_WEAPON && wpn->type != ITEM_FIREWEAPON && CAN_SEE_OBJ(ch, wpn))
			break;
	}
	...
	act("&+LYour&n $p releases a torrent of pure foul &+revil!&n", TRUE, ch, wpn, vict, TO_CHAR);
	act("$n's $p &+Lturns black and spews a torrent of pure foul &+revil&n &+Lat $N!", TRUE, ch, wpn, vict, TO_NOTVICT);
	act("$n's $p &+Lturns midnight black and spews a torrent of foul &+revil&n &+Lat you.", TRUE, ch, wpn, vict, TO_VICT);
...
bool holy_weapon_proc(P_char ch, P_char victim)
{
	...
	if (!IS_ALIVE(ch) || !IS_FIGHTING(ch) || !(victim = GET_OPPONENT(ch)) || !IS_ALIVE(victim) || !(room = ch->in_room) || number(0, 12)) // 7.69%
	{
		return FALSE;
	}
	...
	lvl = GET_LEVEL(ch) == 56
	          ...
	          : (GET_LEVEL(ch) >= 53
	                 ? 7
	                 : (GET_LEVEL(ch) >= 50 ? 6 : (GET_LEVEL(ch) >= 40 ? 5 : (GET_LEVEL(ch) >= 30 ? 4 : (GET_LEVEL(ch) >= 20 ? 3 : (GET_LEVEL(ch) >= 15 ? 2 : (GET_LEVEL(ch) >= 10 ? 1 : 0)))))));
	...
	act("&+WThe holy power of your&n $p &+Wreplenishes your &+ysoul&+W with holy &+Yenergy&+W!&n", TRUE, ch, wpn, NULL, TO_CHAR);
	act("$n's $p &+Weminates a soft &+yg&+Yl&+Wow and $n &+Wlooks &+Yrefreshed&+W!", TRUE, ch, wpn, NULL, TO_ROOM);
