...
{
	if (aura < FIRST_AURA || aura > LAST_AURA)
		return 0;
...
void do_aura(P_char ch, int aura)
{
	...
	snprintf(_buff, MAX_STRING_LENGTH,
		 "You recite a chant and a %s radiates from your finger tips.",
		 auras[aura - FIRST_AURA].glow_name);
	act(_buff, FALSE, ch, 0, ch, TO_CHAR);

	snprintf(_buff, MAX_STRING_LENGTH,
		 "$n recites a chant and a %s radiates from $s finger tips.",
		 auras[aura - FIRST_AURA].glow_name);
...
void apply_aura(P_char ch, P_char victim, int aura)
{
	...
	case AURA_PROTECTION:
		...
		af.modifier =
			(int)(-1 * get_property("innate.paladin_aura.protection_ac_mod", 1.0) *
			      GET_LEVEL(ch));
		break;
	case AURA_PRECISION:
		af.location = APPLY_HITROLL;
		af.modifier = (int)(get_property("innate.paladin_aura.precision_hitroll_mod", 0.2) *
				    GET_LEVEL(ch));
		break;
	...
	case AURA_ENDURANCE:
		...
		af.modifier = (int)(get_property("innate.paladin_aura.endurance_move_mod", 1.0) *
				    GET_LEVEL(ch));
		break;
	case AURA_VIGOR:
		af.location = APPLY_HIT;
		af.modifier = (int)(get_property("innate.paladin_aura.vigor_hp_mod", 1.0) *
				    GET_LEVEL(ch));
		break;
	case AURA_SPELL_PROTECTION:
		af.modifier = (int)(get_property("innate.paladin_aura.protection_spell_mod", 0.5) *
				    GET_LEVEL(ch)) -
			      3;
		break;
	default:
	...
	snprintf(_buff, MAX_STRING_LENGTH, "You are surrounded by a %s.",
		 auras[aura - FIRST_AURA].glow_name);
...
void send_paladin_auras(P_char ch, P_char tar_ch)
{
	for (struct affected_type *aff = tar_ch->affected; aff; aff = aff->next)
	{
		...
		{
			snprintf(_buff, MAX_STRING_LENGTH, "$E is surrounded by a %s.",
				 auras[aff->type - FIRST_AURA].glow_name);
			  ...
			  P_obj /*obj*/)
{
	...
	act("&+LA swirling vortex of blackness billows from $n's outstretched hands.", FALSE, ch, 0,
	    0, TO_ROOM);
	act("&+LA swirling vortex of blackness billows from your outstretched hands.", FALSE, ch, 0,
	    0, TO_CHAR);
	...
	for (P_char vict = world[ch->in_room].people; vict; vict = vict->next_in_room)
	{
		if (vict == ch)
			continue;
		if (IS_DRAGON(vict) || IS_UNDEAD(vict) || (GET_RACE(vict) == RACE_GOLEM) ||
		    IS_TRUSTED(vict))
			continue;
		if (should_area_hit(ch, vict) &&
		    !NewSaves(vict, SAVING_FEAR, (int)level / 11 - 3) && !fear_check(vict))
		{
			do_flee(vict, 0, 1);
			num_hit++;
		}
		...
		for (struct group_list *gl = ch->group; gl; gl = gl->next)
		{
			if (gl->ch->in_room == ch->in_room)
			{
				if (GET_HIT(gl->ch) < GET_MAX_HIT(gl->ch))
				{
					...
					if (num_hit > 0)
						send_to_char(
							"&+LYou feel a grim satisfaction as the power of your enemy's fear flows into your soul!",
							gl->ch);
					else
...
{
	if (is_wielding_paladin_sword(ch) && !affected_by_spell(victim, SKILL_CLEAVE) &&
	    (notch_skill(ch, SKILL_CLEAVE, get_property("skill.notch.cleave", 10)) ||
	     number(5, 100) < GET_CHAR_SKILL(ch, SKILL_CLEAVE)))
	{
		act("You bring down $p in a mighty slash, cleaving $N from $S target!", TRUE, ch,
		    ch->equipment[WIELD], victim, TO_CHAR);
		act("$n brings down $p in a mighty slash, cleaving $N from $S target!", TRUE, ch,
		    ch->equipment[WIELD], victim, TO_NOTVICT);
		act("$n brings down $p in a mighty slash, cleaving you from your target!", TRUE, ch,
...
void event_smite_evil(P_char ch, P_char /*victim*/, P_obj /*obj*/, void * /*data*/)
{
	...
	if (is_wielding_paladin_sword(ch))
	{
		for (tch = world[ch->in_room].people; tch; tch = tch->next_in_room)
		{
			if (!grouped(tch, ch) && GET_HIT(tch) < GET_MAX_HIT(tch) / 10 &&
			    IS_ALIVE(tch) && GET_HIT(tch) < 15 && IS_EVIL(tch) &&
			    (GET_RACEWAR(tch) != 1) &&
			    (notch_skill(ch, SKILL_SMITE_EVIL,
					 get_property("skill.notch.smiteEvil", 10)) ||
			     GET_CHAR_SKILL(ch, SKILL_SMITE_EVIL) / 2 > number(0, 100)))
			{
				act("A stern look appears on $n's face, and $e rushes to extinguish the evil being!",
				    FALSE, ch, 0, tch, TO_NOTVICT);
				act("You exploit the evil creature's momentary weakness, and dive in for the killing blow!",
				    FALSE, ch, 0, 0, TO_CHAR);
				act("$n looks at you sternly, and then dives in, $s blade rushing for your heart!",
	...
	add_event(event_smite_evil, get_property("skill.timer.secs.smiteEvil", 5) * WAIT_SEC, ch, 0,
		  0, 0, 0, 0);
}
			  ...
			  P_char /*victim*/, P_obj /*obj*/)
{
	...
	else
	{
		...
		act("&+WYou are bathed in bright light as a powerful barrier of holiness surrounds you!",
		    FALSE, ch, 0, 0, TO_CHAR);
		act("$n &+Wis suddenly surrounded by blinding, peaceful light.", FALSE, ch, 0, 0,
		    TO_ROOM);
		return;
	}
}
...
void event_righteous_aura_check(P_char ch, P_char victim, P_obj /*obj*/, void * /*data*/)
{
	...
	if (IS_AWAKE(ch) && !IS_IMMOBILE(ch))
	{
		P_char tch;
		LOOP_THRU_PEOPLE(tch, ch)
		{
			if (victim != ch && affected_by_spell(tch, SPELL_BLEAK_FOEMAN) &&
			    GET_OPPONENT(ch) != tch && !IS_TRUSTED(ch) && !IS_TRUSTED(tch))
			{ // Soulshield up!
				if (!IS_AFFECTED2(ch, AFF2_SOULSHIELD) &&
				    GET_ALIGNMENT(ch) >= 950 && !IS_AFFECTED4(ch, AFF4_NEG_SHIELD))
				{
					bzero(&af, sizeof(af));
					...
					if (IS_PC(ch))
					{
						af.duration =
							(int)(5 +
							      GET_CHAR_SKILL(ch, SKILL_DEVOTION) /
								      5);
					}
					else
					{
					...
					act("&+WA holy aura forms around $n!&n", TRUE, ch, 0, 0,
					    TO_ROOM);
					act("&+WA holy aura forms around you!&n", TRUE, ch, 0, 0,
					    TO_CHAR);
				}
				// Dharma up!
				    ...
				    IS_AFFECTED2(ch, AFF2_SOULSHIELD))
				{
					...
					send_to_char(
						"&+cYou cast your soul into the cosmos seeking &+Wrighteousness.\r\n",
						ch);

					add_event(event_holy_dharma,
						  (PULSE_VIOLENCE * number(3, 6)), ch, ch, 0, 0, 0,
						  0);
				}

				act("&+WThe aura surrounding your body flares for a moment, and drives you forth to\n"
				    "&+Weliminate the &+Lvile knight&+W!",
	...
	if (IS_FIGHTING(ch))
	{
		...
		if (GET_OPPONENT(opponent) != ch && CAN_ACT(opponent) &&
		    (GET_ALIGNMENT(opponent) <= -500))
		{
			...
			act("$N &+Wgrimaces with pure hatred, and suddenly focuses his &+RRAGE &+Wat&n $n&+W!",
			...
			P_char /*victim*/, P_obj /*obj*/)
{
	...
	{
		send_to_char(
			"&+LTo grow any more vile would require you to sprout horns and a tail.&n\n",
			ch);
		return;
	}
	else
	{
		...
		act("&+LAs you complete your incantation, you are surrounded by a sickly black cloud.\n"
		    ...
		    "&+Levil seeps into your very soul!",
		    FALSE, ch, 0, 0, TO_CHAR);
		act("&+LA sickly black cloud seems to coalesce in the room and flow towards $n&+L.\n"
		    "&+LSuddenly, $e lets out an unearthly &+rROAR&+L that leaves you shaken.&n",
		       ...
		       P_char victim, P_obj /*obj*/)
{
	...
	{
		send_to_char(
			"&+LPure &+revil&+L coalesces in a sickly black cloud, and seeps into your weapon!\n",
			ch);
...
bool dread_blade_proc(P_char ch, P_char victim)
{
	...
	spell_func_type spells[5] = { spell_disease, spell_blindness, spell_wither, spell_harm,
				      spell_unholy_word };
	spell_func_type spell_func;

	if (!IS_FIGHTING(ch) || !(victim = GET_OPPONENT(ch)) || !IS_ALIVE(victim) || !(room) ||
	    number(0, 32)) // 3%
		return false;
	...
	{
		if ((wpn = ch->equipment[pos]) && wpn->type == ITEM_WEAPON &&
		    wpn->type != ITEM_FIREWEAPON && CAN_SEE_OBJ(ch, wpn))
			break;
	}
	...
	act("&+LYour&n $p releases a torrent of pure foul &+revil!&n", TRUE, ch, wpn, vict,
	    TO_CHAR);
	act("$n's $p &+Lturns black and spews a torrent of pure foul &+revil&n &+Lat $N!", TRUE, ch,
	    wpn, vict, TO_NOTVICT);
...
bool holy_weapon_proc(P_char ch, P_char victim)
{
	...
	act("$n's $p &+Weminates a soft &+yg&+Yl&+Wow and $n &+Wlooks &+Yrefreshed&+W!", TRUE, ch,
	    wpn, NULL, TO_ROOM);
