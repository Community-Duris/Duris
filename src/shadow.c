...
void FreeShadowedData(P_char shadower, P_char shadowed)
{
	...
	{
		for (sh_ptr = shadowed->specials.shadow.who; sh_ptr->next; sh_ptr = sh_ptr->next)
		{
...
void MoveShadower(P_char ch, int to_room)
{
	...
	int                   noise_lev = 0, noise_lev_chart[] = {3, 5, 8, 13, 20, 30};
	...
	for (sh_ptr = ch->specials.shadow.who; sh_ptr; sh_ptr = sh_next)
	{
		...
		else if (sh_ptr->dir_last_move && (sh_ptr->dir_last_move == NOWHERE))
		{
			for (i = 0; i < NUM_EXITS; i++)
			{
				...
				if (world[ch->specials.shadow.room_last_in].dir_option[i] && world[ch->specials.shadow.room_last_in].dir_option[i]->to_room == to_room)
				{
				...
				if (number(1, 101) > fence)
				{
					switch (world[sh_ptr->shadower->in_room].sector_type)
					{
						...
						case SECT_HILLS:
							...
							{
								case 1:
									act("You hear the cracking of twigs from behind.", FALSE, ch, 0, 0, TO_CHAR);
									break;
								case 2:
									act("You suddenly hear loud rustling of branches from behind.", FALSE, ch, 0, 0, TO_CHAR);
									break;
								case 3:
									act("You hear muffled heavy breathing hidden in the thicket from behind.", FALSE, ch, 0, 0, TO_CHAR);
									break;
								default:
						...
						case SECT_UNDRWLD_MOUNTAIN:
							act("Somewhere behind you hear some small rocks tumble down the mountain.", FALSE, ch, 0, 0, TO_CHAR);
							break;
						...
						case SECT_UNDRWLD_SLIME:
							...
							{
								case 1:
									act("You suddenly hear a loud splash from behind.", FALSE, ch, 0, 0, TO_CHAR);
									break;
								case 2:
									act("You hear a faint plop from behind.", FALSE, ch, 0, 0, TO_CHAR);
									break;
								case 3:
									act("There are footsteps slightly splashing the water behind you.", FALSE, ch, 0, 0, TO_CHAR);
									break;
								default:
						...
						case SECT_UNDRWLD_LIQMITH:
							act("You're suddenly stricken with the &+Ceerie &+Bchill&n of being followed.", FALSE, ch, 0, 0, TO_CHAR);
							break;

						default: /* should never reach this case */
							snprintf(buf, MAX_STRING_LENGTH, "MoveShadower(): (real) room %d with unknown sector type %d.", sh_ptr->shadower->in_room, world[sh_ptr->shadower->in_room].sector_type);
							logit(LOG_DEBUG, buf);
							break;
					}
...
void spell_locate_object(int level, P_char ch, char *arg)
{
	...
	for (i = object_list; i && (j > 0) && !flag; i = i->next)
		if ((isname(arg, i->name) || isname(arg, i->short_description)) && CAN_SEE_OBJ(ch, i) && (!IS_SET(i->extra_flags, ITEM_NOLOCATE) || IS_TRUSTED(ch)))
		{
			...
			if ((OBJ_CARRIED(i) || OBJ_WORN(i)) && racewar(ch, i->loc.carrying))
				continue;
			if ((OBJ_CARRIED(i) || OBJ_WORN(i)) && IS_AFFECTED3(i->loc.carrying, AFF3_NON_DETECTION))
				continue;
			if (OBJ_CARRIED(i))
			{
...
void event_grapple(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	act("...and then, in rapid succession you slam your head twice into\r\n$S face smashing it to a pulp. ", FALSE, ch, 0, victim, TO_CHAR);
	act("...then the taste of blood fills your mouth as your\r\nteeth are shattered by $s forehead.", F ALSE, ch, 0, victim, TO_VICT);
	act("$n slams $s head twice into $N face in rapid\r\nsuccession crushing $S face.", FALSE, ch, 0, victim, TO_NOTVICT);
...
void do_grapple(P_char ch, char *arg, int cmd)
{
	...
	if (skl > number(0, 120))
	{
		...
		//                                                                                                             act("$n lunges forward, but you roll out of the way", FALSE, ch, 0, vict,
		//                                                                                                             TO_VICT);
		//                                                                                                                  act("$n lunges forward, but $N rolls out of the way.", FALSE, ch, 0, vict,
		//                                                                                                                  TO_NOTVICT);
		//                                                                                                                           CharWait(ch, PULSE_VIOLENCE * 1);
