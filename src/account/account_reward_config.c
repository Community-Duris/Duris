...
static void apply_value(const char *key, const char *value)
{
	...
	{
		long parsed;
		if (!parse_long_value(value, &parsed) || parsed < 0 ||
		    parsed > ACCOUNT_REWARD_COOLDOWN_MAX)
		{
			logit(LOG_STATUS,
	...
	if (!strcmp(key, "player.max.active.rewards"))
	{
		long parsed;
		if (!parse_long_value(value, &parsed) || parsed < 0 ||
		    parsed > ACCOUNT_REWARD_ACTIVE_MAX)
		{
			logit(LOG_STATUS,
	...
	if (!strcmp(key, "player.show.claim.ids"))
	{
		...
		{
			logit(LOG_STATUS, "Invalid account reward config %s=%s; retaining %s.", key,
			      value, configured_show_claim_ids ? "true" : "false");
			return;
		}
	...
	if (!strcmp(key, "pwipe.preserve"))
	{
		...
		{
			logit(LOG_STATUS, "Invalid account reward config %s=%s; retaining %s.", key,
			      value, configured_preserve_on_pwipe ? "true" : "false");
			return;
		}
...
void boot_account_reward_config(void)
{
	...
	{
		logit(LOG_STATUS,
		      "Account reward config unavailable; using cooldown=3600 seconds, player.max.active.rewards=0 (unlimited), player.show.claim.ids=true, pwipe.preserve=true.");
		return;
	}
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		{
			logit(LOG_STATUS, "Ignoring malformed account reward config line: %s",
			      line);
			continue;
		}
	...
	logit(LOG_STATUS,
	      ...
	      configured_cooldown_seconds, configured_max_active_rewards,
