...
int torment(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (CheckMultiProcTiming(ch))
	{
		...
		if (!number(0, 19))
		{
			act("Your $q glows dark and bites into $N's neck.", FALSE, obj->loc.wearing,
			    obj, vict, TO_CHAR);
			act("$n's $q glows dark as it bites into your neck.", FALSE,
			    obj->loc.wearing, obj, vict, TO_VICT);
			act("$n's $q glows dark and bites into $N's neck.", FALSE, obj->loc.wearing,
			    obj, vict, TO_NOTVICT);
...
int dragonkind(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (arg && (cmd == CMD_SAY))
	{
		if (isname(arg, "protect me"))
		{
			...
			if (obj->timer[0] + 60 <= curr_time)
			{
				...
				act("&+WThe $q &+Whums as the aura of &+gdragonkind&+W embraces $n.",
	...
	if (!number(0, 24))
	{
		...
		case 1:
			...
			for (vict = world[ch->in_room].people; vict; vict = temp)
			{
				...
				if (IS_ALIVE(vict))
				{
					if ((vict->group && (vict->group == ch->group)) ||
					    vict == ch)
					{
						continue;
					}
	...
	if (OBJ_WORN_BY(obj, ch))
	{
		...
		if (!number(0, 27))
		{
			P_char sweep_target, next_sweep_target;
			...
			for (sweep_target = world[ch->in_room].people; sweep_target;
			     sweep_target = next_sweep_target)
			{
				next_sweep_target = sweep_target->next_in_room;

				if (IS_NPC(sweep_target) &&
				    (!sweep_target->following || IS_NPC(sweep_target->following)) &&
				    (GET_OPPONENT(ch) != sweep_target) &&
				    (GET_OPPONENT(sweep_target) != ch))
				{
					continue;
				}
				if (IS_TRUSTED(sweep_target))
				{
					continue;
				}
				if (((IS_FIGHTING(sweep_target) &&
				      (GET_OPPONENT(sweep_target) == ch)) ||
				     !IS_FIGHTING(ch)) &&
				    !IS_DRAGON(sweep_target))
				{
					if (!StatSave(sweep_target, APPLY_AGI, -2))
					{
						/* fall down, go boom */
						SET_POS(sweep_target,
							POS_SITTING + GET_STAT(sweep_target));
						CharWait(sweep_target, PULSE_VIOLENCE * 2);
						act("The powerful sweep sends you crashing to the ground!",
						    FALSE, sweep_target, 0, 0, TO_CHAR);
						act("$n crashes to the ground!", FALSE,
						    sweep_target, 0, 0, TO_ROOM);
						/* don't want to kill them with it, but can mess them up BAD! */
						damage(ch, sweep_target,
						       MIN(dice(2, (GET_LEVEL(ch) / 5)) + 5,
							   GET_HIT(sweep_target) + 8),
						       TYPE_UNDEFINED);
					}
					else
					{
						send_to_char("You nimbly dodge the sweep!\n",
							     sweep_target);
						act("$N dodges your sweep.", 0, ch, 0, sweep_target,
						    TO_CHAR);
					}
				}
				if (!char_in_list(ch))
...
int lightning(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if ((cmd == CMD_RUB) && OBJ_WORN(obj) && (obj->timer[0] + 500 <= current_time))
	{
		...
		act("You rub your $q and begin to whirl around.", FALSE, obj->loc.wearing, obj, 0,
		    TO_CHAR);
		act("$n rubs $s $q and begins to whirl around.", FALSE, obj->loc.wearing, obj, 0,
		    TO_ROOM);
		...
		for (vict = world[ch->in_room].people; vict; vict = vict->next_in_room)
		{
			...
			{
				act("Your &+CWHIRLWIND picks up $N and tosses $M against the wall!",
				    FALSE, obj->loc.wearing, obj, vict, TO_CHAR);
				act("$n's &+CWHIRLWIND picks up $N and tosses $M against the wall!",
				    FALSE, obj->loc.wearing, obj, vict, TO_NOTVICT);
				act("$n's &+CWHIRLWIND picks you up and tosses you against the wall!",
...
int dispator(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	*/
	...
	if (!number(0, 15))
	{
		act("Your $q &+rsummons the power of hellfire at $N.", FALSE, obj->loc.wearing, obj,
		    vict, TO_CHAR);
		act("$n's $q &+rsummons the power of hellfire at you.", FALSE, obj->loc.wearing,
		    obj, vict, TO_VICT);
		act("$n's $q &+rsummons the power of hellfire at $N.", FALSE, obj->loc.wearing, obj,
		    vict, TO_NOTVICT);
...
int orb_of_the_sea(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_GOTHIT)
	{
		if (!((data = legacy_proc_arg<struct proc_data *>(arg)) && (victim = data->victim)))
		{
	...
	else
	{
		...
		else if (OBJ_WORN(obj) && (ch == obj->loc.wearing))
		{
			...
			if (!IS_NPC(ch) && !can_prime_class_use_item(ch, obj) &&
			    number(1, 100) <= count++ / 40)
			{
				for (loc = 0; loc < MAX_WEAR; loc++)
				{
					if (obj == ch->equipment[loc])
					{
						act("&+b$p&+b slips out of your hands!&n", FALSE,
						    ch, obj, NULL, TO_CHAR);
						act("&+b$p&+b slips out of $n&+b's hands.&n", TRUE,
						    ch, obj, NULL, TO_ROOM);
						unequip_char(ch, loc);
						obj_to_char(obj, ch);
	...
	if (OBJ_WORN_BY(obj, ch) && IS_ALIVE(victim) && !IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) &&
	    !IS_GH_GOLEM(victim) && GET_RACE(victim) != RACE_CONSTRUCT &&
	    !(strstr(victim->player.name, "_no_move_")))
	{
		// 1/30 chance.
		if (!number(0, 29))
		{
			act("Your $q lets out a banshee wail at $N!", FALSE, obj->loc.wearing, obj,
			    victim, TO_CHAR);
			act("$n's $q lets out a banshee wail at you!", FALSE, obj->loc.wearing, obj,
			    victim, TO_VICT);
			act("$n's $q lets out a banshee wail at $N!", FALSE, obj->loc.wearing, obj,
			    victim, TO_NOTVICT);
			...
			{
				act("&+r$N&+r grins evilly.&n", FALSE, obj->loc.wearing, obj,
				    victim, TO_CHAR);
				act("&+rYou grin evilly.&n", FALSE, obj->loc.wearing, obj, victim,
				    TO_VICT);
				act("&+r$N&+r grins evilly.&n", FALSE, obj->loc.wearing, obj,
				    victim, TO_NOTVICT);
				return FALSE;
			...
			if (!number(0, 2))
			{
				act("&+WA ghostly banshee appears in a puff of mist, and points at $N!",
				    FALSE, obj->loc.wearing, obj, victim, TO_CHAR);
				act("&+WA ghostly banshee appears in a puff of mist, and points at $N!",
				    FALSE, obj->loc.wearing, obj, victim, TO_NOTVICT);
				act("&+WA ghostly banshee appears in a puff of mist, and points at YOU!",
				    FALSE, obj->loc.wearing, obj, victim, TO_VICT);
				char_from_room(victim);
				new_room = real_room(number(WATERP_VNUM_BEGIN, WATERP_VNUM_END));
...
int mace(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	act("Your $q &+ysummons the power of the earth and rock at $N.", FALSE, obj->loc.wearing,
	    obj, vict, TO_CHAR);
	act("$n's $q &+ysummons the power of the earth and rock at you.", FALSE, obj->loc.wearing,
	    obj, vict, TO_VICT);
	act("$n's $q &+ysummons the power of the earth and rock at $N.", FALSE, obj->loc.wearing,
	    obj, vict, TO_NOTVICT);
	spell_earthquake(60, ch, 0, SPELL_TYPE_SPELL, vict, 0);
	if (is_char_in_room(vict, ch->in_room))
	{
		spell_damage(ch, vict, 406, SPLDAM_GENERIC, SPLDAM_NOSHRUG | SPLDAM_NODEFLECT,
			     &messages);
	}
	return (FALSE);
}
...
int flamberge(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if ((cmd == CMD_SAY) && (!(IS_FIGHTING(ch))))
	{
		...
		if (!strcmp(arg, "efreeti") && OBJ_WORN_BY(obj, ch))
		{
			...
			if (obj->timer[0] + 300 <= curr_time)
			{
				...
				{
					send_to_room(
						"&+wSm&+Lok&+we r&+Lis&+Wes&n from &+rthe holy flamberge&+w and slowly forms into the &+wim&+Wa&+Lge&n of an &+ref&+Rre&+ret&+Ri&n &+LUnq&+wu&+Lenchable&n who brings "
				...
				{
					send_to_room(
						"A &+wh&+Lea&+wt h&+La&+Wze&n from &+rthe holy flamberge&+w materializes into the &+wim&+Wa&+Lge&n of an &+LOrd&+we&+Lr of &+RFi&+re&+Rry He&+ra&+Rrt&n who calls on "
			...
			{
				send_to_room(
					"&+MAn &+ref&+Rre&+ret&+Ri &+Macolyte&+w snaps into your vision momentarilty then quickly &+Ldisappears&+w again, leaving only a faint trace of &+rheat&n.&n\n",
					ch->in_room);
	...
	if (cmd == CMD_REMOVE && OBJ_WORN(obj))
	{
		...
		if (!str_cmp(arg1, "all") || get_object_in_equip(ch, arg1, &loc) == obj)
		{
			...
			{
				act("&+rYour $q &+Rflares&+r slightly.&n", FALSE, ch, obj, NULL,
				    TO_CHAR);
				shown = TRUE;
			...
			if ((af = get_spell_from_char(ch, SPELL_HELLFIRE)))
			{
				if (!shown)
				{
					act("&+rYour $q &+Rflares&+r slightly.&n", FALSE, ch, obj,
					    NULL, TO_CHAR);
				}
				wear_off_message(ch, af);
				affect_remove(ch, af);
	...
	vict = legacy_proc_arg<P_char>(arg);
	// 1/30 chance.
	if (cmd != CMD_MELEE_HIT || !IS_ALIVE(ch) || number(0, 29) || !(room = ch->in_room) ||
	    !IS_ALIVE(vict))
	{
		return FALSE;
	}

	act("Your $q &+rsummons the power of fire and flames at $N.", FALSE, ch, obj, vict,
	    TO_CHAR);
...
int unholy_avenger_bloodlust(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (!number(0, 24) && CheckMultiProcTiming(ch) && IS_ALIVE(vict) && !IS_UNDEADRACE(vict))
	{
		...
		act("Your $q turns &+rblood red as it slashes into $N!", FALSE, ch, obj, vict,
		    TO_CHAR);
		act("$n's $q turns &+rblood red as it slashes into $N!", FALSE, ch, obj, vict,
		    TO_NOTVICT);
		act("$n's $q turns &+rblood red as it slashes into you!", FALSE, ch, obj, vict,
		    TO_VICT);
	...
	if (OBJ_WORN_BY(obj, ch) && vict)
	{
		if (!number(0, 24))
		{
			...
			if (is_char_in_room(vict, ch->in_room))
			{
				...
				{
					act("&+yThe earth and rock heals $N!", FALSE, 0, obj, vict,
					    TO_CHAR);
...
int unholy_avenger_bloodlust(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (!number(0, 24))
	{
		act("Your $q turns &+rblood red as it slashes into $N!", FALSE, ch, obj, vict,
		    TO_CHAR);
		act("&+rBlood red energy travels into your hand and up your arm, seeping into your veins!",
		    FALSE, ch, obj, vict, TO_CHAR);
		act("$n's $q turns &+rblood red as it slashes into $N!", FALSE, ch, obj, vict,
		    TO_NOTVICT);
		act("&+rBlood red energy travels into $n's hand and up $s arm, apparently strengthening $m.",
		    FALSE, ch, obj, vict, TO_NOTVICT);
		act("$n's $q turns &+rblood red as it slashes into you!", FALSE, ch, obj, vict,
		    TO_VICT);
		act("You feel yourself becoming weak as the &+rblood is sucked out of you!", FALSE,
		    ch, obj, vict, TO_VICT);
...
int nightbringer(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	vict = legacy_proc_arg<P_char>(arg);
	if (!dam || !IS_ALIVE(ch) || !OBJ_WORN_POS(obj, WIELD) || !OBJ_WORN_BY(obj, ch) ||
	    !IS_ALIVE(vict))
	{
		return FALSE;
	}
	...
	if (!number(0, 29))
	{
		...
		act("$n's $q glows brightly as it hits you.  You feel woozy.", FALSE, ch, obj, vict,
		    TO_VICT);
...
int purple_worm(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if (GET_OPPONENT(ch) && !pl)
	{
		if (!number(0, 9))
		{
			...
			if (!IS_TRUSTED(vict) && !(IS_NPC(vict) && IS_ELITE(vict)) &&
			    !IS_NEXUS_GUARDIAN(vict) && !IS_GH_GOLEM(vict) && !IS_OP_GOLEM(vict))
			{
				if (vict->in_room == ch->in_room)
				{
					act("You open your mouth and swallow $N whole!", FALSE, ch,
					    0, vict, TO_CHAR);
					...
					for (P_obj corpse_obj = world[ch->in_room].contents;
					     corpse_obj; corpse_obj = corpse_obj->next_content)
					{
						if (corpse_obj->value[3] == id)
						{
							obj_from_room(corpse_obj);
							obj_to_char(corpse_obj, ch);
							if (corpse_obj->type == ITEM_CORPSE &&
							    IS_SET(corpse_obj->value[1], PC_CORPSE))
							{
								writeCorpse(corpse_obj);
...
int piercer(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	    ...
	    MIN_POS(ch, POS_STANDING + STAT_RESTING))
	{
		for (tmp_ch = world[ch->in_room].people; tmp_ch; tmp_ch = next)
		{
			...
			if (IS_PC(tmp_ch) && CAN_SEE(ch, tmp_ch) && !IS_TRUSTED(tmp_ch) &&
			...
			if (vict && (ch->in_room == vict->in_room))
			{
				    ...
				    StatSave(vict, APPLY_AGI, -10))
				{
					...
					act("You crash to the ground, missing $N by a hair!", FALSE,
					    ch, 0, vict, TO_CHAR);
				}
				else
				{
  ...
  teleport_to(ch, real_room(obj->value[temp]));
	...
	do
	{
		to_room = number(0, top_of_world);
		 ...
		 IS_ROOM(to_room, ROOM_NO_TELEPORT) || world[to_room].sector_type == SECT_OCEAN ||
...
int githyanki(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (!number(0, 35))
	{
		...
		{
			// Kiss it goodbye
			act("A booming voice says 'Foolish mortal!! You can't contain the power of Gith for long!'",
			    FALSE, ch, obj, vict, TO_CHAR);
			act("A booming voice says 'Foolish mortal!! You can't contain the power of Gith for long!'",
			    FALSE, ch, obj, vict, TO_ROOM);
			act("Your $q explodes into a thousand pieces.", FALSE, ch, obj, 0, TO_CHAR);
			act("BOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOM! You are thrown from your feet!",
			    FALSE, ch, obj, vict, TO_CHAR);
			act("$n's $q explodes into a thousand pieces, knocking $m flat!", FALSE, ch,
			    obj, 0, TO_ROOM);
		...
		case 2:
			if (GET_LEVEL(vict) < 50)
			{
				...
				act("Your sword releases a blinding flash of white light!", FALSE,
				    ch, obj, vict, TO_CHAR);
				act("Your $q sings through the air as it lops $N's head off!",
				    FALSE, ch, obj, vict, TO_CHAR);
				act("$n's $q sings through the air as it lops $N's head off!",
				    FALSE, ch, obj, vict, TO_ROOM);
				act("$n's $q glows brightly and you see it coming towards your neck at an alarming rate of speed!",
				    FALSE, ch, obj, vict, TO_VICT);
				die(vict, ch);
				vict = NULL;
			}
		...
		case 3:
		case 4:
			act("&+cYour $q &+Csings &+cas it slashes into $N!", FALSE, ch, obj, vict,
			    TO_CHAR);
			act("&+cYou feel slightly invigorated!", FALSE, ch, obj, vict, TO_CHAR);
			act("$n&+c's $q &+Csings &+cas it slashes into $N!", FALSE, ch, obj, vict,
			    TO_NOTVICT);
			act("$n&+c's $q &+Csings &+cas it slashes into you!", FALSE, ch, obj, vict,
			    TO_VICT);
		...
		case 5:
			if (IS_PC(ch) && (temp = read_mobile(19790, VIRTUAL)))
			{
				...
				act("BOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOM!  There is a great clap of thunder!",
				    FALSE, temp, obj, 0, TO_ROOM);
				act("A deadly looking Githyanki Knight slowly fades into existence..",
				    FALSE, temp, obj, 0, TO_ROOM);
				act("$n screams 'I have come to reclaim our Silver Sword for Gith!'",
				    FALSE, temp, obj, 0, TO_ROOM);
...
int githpc_special_weap(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	{
		// 1/200 chance.
		if ((!number(0, 199)) && !CHAR_IN_JUSTICE_AREA(ch) && !CHAR_IN_TOWN(ch) &&
		    strlen(obj->name))
		{
			...
			if (j)
			{
				...
				if (IS_ALIVE(vict) && IS_PC(vict) && (vict->in_room != ch->in_room))
				{
					...
					if (!number(0, 6))
					{
						...
						{
							for (t_ch = world[vict->in_room].people;
							     t_ch; t_ch = t_ch->next_in_room)
							{
								if (IS_FIGHTING(t_ch) &&
								    (GET_OPPONENT(t_ch) == vict))
								{
									stop_fighting(t_ch);
								}
							}
						...
						act("&+W$n suddenly disappears in a flash of light!",
						    TRUE, vict, 0, 0, TO_ROOM);
						send_to_char(
							"&+WSuddenly, a flash of light obscures your vision.  You feel as though you are falling at an incredible rate, but yet you can hear nothing.  You find yourself "
							"elsewhere...\n",
							vict);
						char_from_room(vict);
						char_to_room(vict, ch->in_room, -1);
						act("&+WIn a flash of light, $n suddenly arrives.",
						    FALSE, vict, 0, 0, TO_ROOM);
					}
					else
					{
						act("&+WYour weapon glows with &+Bpower... &+Wthen suddenly disappears in a flash of light!",
						    FALSE, ch, 0, 0, TO_CHAR);
						act("&+W$n's&+W weapon glows with &+Bpower... &+Wthen suddenly disappears in a flash of light!",
						    FALSE, ch, 0, 0, TO_ROOM);
						obj_to_char(unequip_char(ch, WIELD), ch);
						obj_from_char(obj);
						obj_to_char(obj, vict);
						act("A bright flash of light erupts from your hands!!! When the light dies down, you realize $p has returned to your posession!",
						    FALSE, vict, 0, obj, TO_CHAR);
						act("A bright flash of light erupts from $n's hands!!!",
						    FALSE, vict, 0, 0, TO_ROOM);
						return TRUE;
					}
				}
		...
		else if (!number(0, 399))
		{
			...
			if (vict && (GET_LEVEL(vict) < 51) && !IS_TRUSTED(vict))
			{
				...
				{
					act("&+WIn a blinding burst of light, $n&+W's $q&+W suddenly stabs at $N&+W's head with incredible speed, instantly beheading $M!",
					    FALSE, ch, obj, vict, TO_NOTVICT);
					act("&+WIn a blinding burst of light, your $q&+W suddenly stabs at $N&+W's head with incredible speed, instantly beheading $M!",
					    FALSE, ch, obj, vict, TO_CHAR);
					act("&+WIn a blinding burst of light, $n&+W's $q&+W suddenly stabs at your head with incredible speed, instantly beheading you!",
					    FALSE, ch, obj, vict, TO_VICT);
				...
				{
					act("&+WIn a blinding burst of light, $n&+W's $q&+W suddenly smashes $N&+W's head with incredible speed, instantly beheading $M!",
					    FALSE, ch, obj, vict, TO_NOTVICT);
					act("&+WIn a blinding burst of light, your $q&+W suddenly smashes $N&+W's head with incredible speed, instantly beheading $M!",
					    FALSE, ch, obj, vict, TO_CHAR);
					act("&+WIn a blinding burst of light, $n&+W's $q&+W suddenly smashes your head with incredible speed, instantly beheading you!",
					    FALSE, ch, obj, vict, TO_VICT);
				...
				else
				{
					wizlog(MINLVLIMMORTAL,
					       "githpc_special_weap: Item '%s' vnum (%d) not recognized.\n",
...
int tiamat(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if (cmd == CMD_DEATH)
	{
		...
		logit(LOG_OBJ,
		      "Tiamat death: Heart decays in %d obj ticks = %d sec = %d:%02d:%02d.",
		      obj->value[0], obj->value[1], obj->value[2], obj->value[3], obj->value[4]);
	...
	if (ch->in_room == real_room(VROOM_TIAMAT_HOME))
	{
		...
		{
			act("$N &+Rgrowls in anger, as you approach $S treasure!", FALSE, pl, 0, ch,
			    TO_CHAR);
			act("$N &+Rgrowls in anger, as $n&+R makes an attempt at $S treasure!",
		...
		{
			act("&+LWith a mighty &+RROAR&+l, &+LTiamat shreds the blanket of silence!",
			    0, ch, 0, 0, TO_ROOM);
	...
	case 1:
		...
		for (vict = world[ch->in_room].people; vict; vict = next_ch)
		{
			...
			if (!IS_DRAGON(vict) && !affected_by_spell(vict, SKILL_BERSERK) &&
			    IS_FIGHTING(vict) && (GET_OPPONENT(vict) == ch))
			{
				if (!StatSave(vict, APPLY_AGI, -4))
				{
					...
					act("$n&+L crashes to the ground!", FALSE, vict, 0, 0,
					    TO_ROOM);
				}
				else
					act("&+LYou nimbly dodge the sweep!", FALSE, vict, 0, 0,
					    TO_CHAR);
			}
		}
		break;
	...
	case 2:
		...
		if (vict && !IS_DRAGON(vict) && !affected_by_spell(vict, SKILL_BERSERK) &&
		    IS_FIGHTING(vict) && (GET_OPPONENT(vict) == ch))
		{
			if (!StatSave(vict, APPLY_AGI, -4))
			{
	...
	default:
		...
		if (vict && !IS_DRAGON(vict) && !affected_by_spell(vict, SKILL_BERSERK) &&
		    IS_FIGHTING(vict) && (GET_OPPONENT(vict) == ch) && !IS_TRUSTED(vict))
		{
			struct damage_messages msgs = {
				"",
	...
	for (i = 1; i < 6; i++)
	{
		...
		case 2: /* Bite someone          */
			...
			if (vict)
			{
				snprintf(bufs[0], sizeof bufs[0], "You bite $N with your %s head.",
					 colors[i - 1]);
				snprintf(bufs[1], sizeof bufs[1],
					 "The %s head of $n lashes out and bites you.",
					 colors[i - 1]);
				snprintf(bufs[2], sizeof bufs[2], "$n bites $N with $m %s head.",
					 colors[i - 1]);
				snprintf(bufs[3], sizeof bufs[3],
					 "You bite $N with your %s head until they are dead.",
					 colors[i - 1]);
				snprintf(bufs[4], sizeof bufs[4],
					 "The %s head of $n bites you... to DEATH!", colors[i - 1]);
				snprintf(bufs[5], sizeof bufs[5],
					 "$n bites $N in two with $m %s head! $N is dead.",
					 colors[i - 1]);
				switch (i)
				{
				case 1: // Red head
					damtype = SPLDAM_FIRE;
					snprintf(
						dam_msg, sizeof dam_msg,
						"&+RFire courses through your blood as she bites deep!");
					break;
				case 2: // Black head
					damtype = SPLDAM_ACID;
					snprintf(
				...
				case 3: // Blue head
					...
					snprintf(
						dam_msg, sizeof dam_msg,
						"&+BYour heart skips a beat or three as her shocking teeth clamp down upon your arm!");
					break;
				case 4: // Green head
					damtype = SPLDAM_GAS;
					snprintf(
		...
		case 3:
			...
			{
			case 1:
				act("$n&+r's red head breathes &+Rfire&+r!&N", TRUE, ch, NULL, NULL,
				    TO_ROOM);
				funct = breath_weapon_fire;
				break;
			case 2:
				act("$n&+w's &+Lblack &+whead breathes &+Lacid&+w!&n", TRUE, ch,
				    NULL, NULL, TO_ROOM);
				funct = breath_weapon_acid;
				break;
			case 3:
				act("$n&+B's &+bblue &+Bhead breathes &=LBelectricity&N&+B!&N",
				    TRUE, ch, NULL, NULL, TO_ROOM);
				funct = breath_weapon_lightning;
				break;
			case 4:
				act("$n&+G's &+ggreen &+Ghead breathes &+gpoison gas&+G!&n", TRUE,
				    ch, NULL, NULL, TO_ROOM);
				funct = breath_weapon_poison;
				break;
			case 5:
				act("$n&+w's &+Wwhite &+whead breathes &+Wfrost&+w!&n", TRUE, ch,
				    NULL, NULL, TO_ROOM);
				funct = breath_weapon_frost;
				break;
			default:
				act("$n&+w's &+Lblack &+whead breathes &+Lblackness&+w!&n", TRUE,
				    ch, NULL, NULL, TO_ROOM);
				...
				if (!IS_DRAGON(vict))
				{
					...
					case 3:
						if (IS_AFFECTED(vict, AFF_HASTE))
						{
							...
							spell_damage(ch, vict, dice(20, 20),
								     SPLDAM_LIGHTNING,
								     SPLDAM_NODEFLECT |
									     SPLDAM_NOSHRUG,
								     &msgs);
					...
					case 5:
						if (has_skin_spell(vict))
						{
							...
							spell_damage(
								ch, vict, dice(5, 100), SPLDAM_COLD,
								SPLDAM_NODEFLECT | SPLDAM_NOSHRUG,
								&msgs);
						}
						break;
...
int tiamat_stinger(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	*/
	...
	if (!number(0, 15))
	{
		act("Your $q &+Glashes out in a blur of speed at $N.", FALSE, obj->loc.wearing, obj,
		    vict, TO_CHAR);
		act("$n's $q &+Gstrikes at you with a blur of speed!", FALSE, obj->loc.wearing, obj,
		    vict, TO_VICT);
		act("$n's $q &+Gstrikes out in a blur of speed at $N.", FALSE, obj->loc.wearing,
		    obj, vict, TO_ROOM);
...
int holy_mace(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	else if (cmd == CMD_GOTNUKED && !number(0, 3))
	{
		...
		act("$n's $p glows brilliantly, and a healing eldritch fire surrounds $s body!",
		    FALSE, ch, obj, NULL, TO_ROOM);
		act("Your $p glows brilliantly, and a healing eldritch fire surrounds you!", FALSE,
		    ch, obj, vict, TO_CHAR);
	...
	else if (cmd == CMD_GOTNUKED && !number(0, 3))
	{
		...
		if (GET_HIT(ch) > GET_MAX_HIT(ch))
		{
			GET_HIT(ch) = GET_MAX_HIT(ch);
			{
				...
				act("$n's $p glows brilliantly, and a healing eldritch fire surrounds $s body!",
				    FALSE, ch, obj, vict, TO_VICT);
				act("Your $p glows brilliantly, and a healing eldritch fire surrounds you!",
				    FALSE, ch, obj, vict, TO_CHAR);
...
int elvenkind_cloak(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (tch)
	{
		...
		spell_damage(ch, tch, data->dam, data->attacktype, data->flags | SPLDAM_NODEFLECT,
			     data->messages);
		return TRUE;
	}
...
void barb_proc_dwarven_ancestor([[maybe_unused]] int level, P_char ch, P_char victim)
{
	...
	if (!mob)
	{
		logit(LOG_DEBUG, "barb_proc_dwarven_ancestor(): mob %d not loadable",
		      DWARVEN_ANCESTOR_VNUM);
	...
	act("&+rIn a column of &+RROARING&+r fire, $N&+r makes $S entrance.", FALSE, ch, 0, mob,
	    TO_CHAR);
	act("&+rIn a column of &+RROARING&+r fire, $N&+r makes $S entrance.", FALSE, ch, 0, mob,
	    TO_ROOM);
	...
	for (i = number(1, 2); i; i--)
	{
		if (is_char_in_room(ch, ch->in_room) && is_char_in_room(victim, ch->in_room))
			...
			case 1:
				...
				break;
			case 2:
				act("$n &+Rlets out an ear-shattering war cry!", FALSE, mob, 0, 0,
				    TO_ROOM);
				do_war_cry(mob, 0, 0);
				break;
			default:
...
int barb(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (curr_race != GET_RACE(ch))
	{
		...
		if (GET_RACE(ch) == RACE_OGRE)
		{
			...
			act("$q begins to twist and writhe in $n's hands, his corruption seeping into it!",
			    FALSE, ch, obj, NULL, TO_ROOM);
			act("$q begins to twist and writhe in your hands, your will corrupting it!",
		...
		if (GET_RACE(ch) == RACE_OGRE)
		{
			...
			obj->short_description =
				str_dup("&+bthe mighty warhammer of the ogre chiefs&n");
		}
		else if (GET_RACE(ch) == RACE_BARBARIAN)
		{
			...
			obj->str_mask |= STRUNG_DESC2;
			obj->short_description =
				str_dup("&+ythe mystical warhammer of the barbarian kings&n");
		}
		else if (GET_RACE(ch) == RACE_MOUNTAIN || GET_RACE(ch) == RACE_DUERGAR)
		{
			...
			obj->short_description = str_dup(
				"&+Lthe warhammer of the &+yancient dwarven &+Rb&+ra&+Ltt&+Rl&+re&+Lr&+ra&+Rge&+Lr&+rs&n");
		}
		else if (GET_CLASS(ch, CLASS_BERSERKER))
		{
			...
			obj->str_mask |= STRUNG_DESC2;
			obj->short_description =
				str_dup("&+rthe great warhammer of the &+RRa&+rGe&+Rlo&+rRd&n");
		}
	}
	...
	if (arg && (cmd == CMD_SAY))
	{
		if (isname(arg, "power"))
		{
			...
			if ((obj->timer[0] + 60 <= curr_time) && (GET_RACE(ch) == RACE_BARBARIAN))
			{
				...
				act("Your $q hums as the spirits of your ancestors bestow you with power.",
				    FALSE, ch, obj, obj, TO_CHAR);

				act("$n says 'power'", TRUE, ch, obj, NULL, TO_ROOM);
				act("$n's $q bestows them with power.", TRUE, ch, obj, NULL,
				    TO_ROOM);
			...
			{
				act("You say 'power'", FALSE, ch, 0, 0, TO_CHAR);
				act("Your $q hums and you fill with &+RRAGE!", FALSE, ch, obj, obj,
				    TO_CHAR);
				act("$n says 'power'", TRUE, ch, obj, NULL, TO_ROOM);
				act("$n's $q bestows them with power.", TRUE, ch, obj, NULL,
				    TO_ROOM);
			...
			{
				act("You say 'power'", FALSE, ch, 0, 0, TO_CHAR);
				act("Your $q fills you with the power of the &+yancient dwarven battleragers!",
				    FALSE, ch, obj, NULL, TO_CHAR);
				act("$n says 'power'", TRUE, ch, obj, NULL, TO_ROOM);
				act("$n's $q bestows them with power.", TRUE, ch, obj, NULL,
				    TO_ROOM);
			...
			{
				act("You say 'power'", FALSE, ch, 0, 0, TO_CHAR);
				act("Your $q fills you with a &+rSuRGe of &+RBl&+rOOdL&+RusT!!!",
				    FALSE, ch, obj, obj, TO_CHAR);
				act("$n says 'power'", TRUE, ch, obj, NULL, TO_ROOM);
				act("$n's $q fills them with a &+rSuRGe of &+RBl&+rOOdL&+RusT!!!",
				    TRUE, ch, obj, NULL, TO_ROOM);
	    ...
	    (GET_RACE(ch) == RACE_MOUNTAIN || GET_RACE(ch) == RACE_DUERGAR) && (!number(0, 5)))
	{
		...
		if (BarbProcData.wielder !=
		    ch) /* the hammer changed hands. Or just came back from storage */
		{
			if (BarbProcData.hammer)
			{
				...
				{
					wizlog(MINLVLIMMORTAL,
					       "Possibly more than one instance of barbarian warhammer present in game!");
					logit(LOG_DEBUG,
					      "Possibly more than one instance of barbarian warhammer present in game!");
		...
		if (BarbProcData.damage)
		{
			...
			result =
				spell_damage(ch, vict, BarbProcData.damage, BarbProcData.attacktype,
					     ...
					     BarbProcData.messages_set ? &BarbProcData.messages :
									 &messages_deflect);
		    ...
		    data->messages->death_victim && data->messages->death_room)
		{
			...
			BarbProcData.messages.death_attacker =
				str_dup(data->messages->death_attacker);
	...
	if (IS_ALIVE(vict))
	{
		...
		if (!number(0, 24) && (GET_RACE(ch) == RACE_BARBARIAN) && CheckMultiProcTiming(ch))
		{
			...
			{
				spell_restoration((int)(GET_LEVEL(ch) - 5), ch, 0, SPELL_TYPE_SPELL,
						  ch, 0);
			}
		}
		...
		// return true;
		// } // 4%
		else if (!number(0, 24) && GET_CLASS(ch, CLASS_BERSERKER) &&
			 GET_CHAR_SKILL(ch, SKILL_BERSERK) > 0 && CheckMultiProcTiming(ch))
		{
			...
			act("$n's $q &+rsmashes into you, nearly knocking you off your feet!",
			    FALSE, obj->loc.wearing, obj, vict, TO_VICT);
			act("$n's $q &+rsmashes into $N&+r, causing them to scream in pain!", FALSE,
			    obj->loc.wearing, obj, vict, TO_NOTVICT);
			...
			{
				act("$n shatters $N's &+Lstone skin!&n", FALSE, ch, obj, vict,
				    TO_ROOM);
				act("$n shatters your &+Lstone skin!&n", FALSE, ch, obj, vict,
				    TO_VICT);
				act("You shatter $N's &+Lstone skin!&n", FALSE, ch, obj, vict,
				    TO_CHAR);
				affect_from_char(vict, SPELL_STONE_SKIN);
			}
			else if (affected_by_spell(vict, SPELL_BIOFEEDBACK))
			{
				act("$n pierces through $N's &+Gmisty barrier!&n", FALSE, ch, obj,
				    vict, TO_ROOM);
				act("$n pierces through your &+Gmisty barrier!&n", FALSE, ch, obj,
				    vict, TO_VICT);
				act("You pierce through $N's &+Gmisty barrier!&n", FALSE, ch, obj,
				    vict, TO_CHAR);
				affect_from_char(vict, SPELL_BIOFEEDBACK);
			}
			else if (affected_by_spell(vict, SPELL_SHADOW_SHIELD))
			{
				act("$n obliterates $N's &+Lshadowy shield!&n", FALSE, ch, obj,
				    vict, TO_ROOM);
				act("$n obliterates your &+Lshadowy shield!&n", FALSE, ch, obj,
				    vict, TO_VICT);
				act("You obliterate $N's &+Lshadowy shield!&n", FALSE, ch, obj,
				    vict, TO_CHAR);
		...
		if ((count <= 0) || IS_TRUSTED(ch))
		{
			...
			case IFRIT:
				...
				if (OUTSIDE(ch))
				{
					send_to_room(
						...
						room);
					send_to_room(
						"&+RBOOM!! Jera throws meteors at the whiners!\n",
						room);
					for (vict = world[room].people; vict; vict = next)
					{
						next = vict->next_in_room;
						if ((vict != ch) &&
						    !(ch->group && (ch->group == vict->group)) &&
						    number(0, 1))
						{
							int dam = 180 +
								  number(-20, 20); // ~40-50 damage
							spell_damage(ch, vict, dam, SPLDAM_FIRE,
								     SPLDAM_NODEFLECT |
									     SPLDAM_NOSHRUG,
								     0, NULL);
						}
				...
				else
				{
					...
					if (IS_ALIVE(ch))
					{
						spell_firestorm(60, ch, 0, SPELL_TYPE_STAFF, NULL,
								0);
					}
			...
			case DOOMTRAIN:
				...
				for (vict = world[room].people; vict; vict = next)
				{
					...
					{
						save = vict->specials
							       .apply_saving_throw[SAVING_SPELL];
						vict->specials.apply_saving_throw[SAVING_SPELL] =
							20;
						spell_wither(51, ch, 0, SPELL_TYPE_SPELL, vict, 0);
						spell_slow(51, ch, 0, SPELL_TYPE_SPELL, vict, 0);
						spell_poison(60, ch, 0, SPELL_TYPE_SPELL, vict, 0);
						spell_blindness(51, ch, 0, SPELL_TYPE_SPELL, vict,
								0);
						vict->specials.apply_saving_throw[SAVING_SPELL] =
							save;
			...
			case CARBUNCLE:
				...
				send_to_room(
					"Raxxel says 'Don't let anyone know I'm doing this!'\n",
					room);
...
int SeaKingdom_Tsunami(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (arg && (isname(arg, "trident") || isname(arg, "tsunami")))
	{
		...
		if (cmd == CMD_TAP)
		{
			if (obj->timer[0] + 300 <= curr_time)
			{
				     ...
				     vict = vict->next_in_room)
				{
					if (!affected_by_spell(vict, SPELL_VITALITY))
					{
						...
						{
							act("&+W$N&+L is &+Bvit&+Cal&+Wiz&+Ced by &+WTsu&+Bna&+bmi's&+W power&+W!",
							    FALSE, ch, obj, vict, TO_ROOM);
							spell_vitality(GET_LEVEL(ch), ch, 0,
								       SPELL_TYPE_SPELL, vict, 0);
						}
						else if (vict == ch)
						{
			...
			{
				act("You &+Ltap and tap and &+Wtap but no &+Bwa&+bter's comin outta &+Wthis &+Bs&+Ch&+Wa&+Cf&+Bt.",
				    TRUE, ch, obj, NULL, TO_CHAR);
			}
			return TRUE;
		}
		...
		{
			send_to_char(
				"You &+Lthrust&n, &+Lraise&n, and shake the trident madly but nothing happens.\n",
				ch);
			return TRUE;
		}
		...
		if (cmd == CMD_THRUST)
		{
			...
			{
				send_to_char(
					"The pressure of water around you would block ground water.\n",
					ch);
				return TRUE;
			}
		...
		else // cmd == CMD_RAISE
		{
			...
			{
				send_to_char(
					"You can't summon any water here; perhaps there's some in the ground?\n",
					ch);
				return FALSE;
			}
		...
		add_event(event_tsunamiwave, WAIT_SEC, ch, 0, 0, 0, &(ch->in_room),
			  sizeof(ch->in_room));
		obj->timer[1] = curr_time;
		return TRUE;
	}
...
int sevenoaks_longsword(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	// 1/33 chance.
	if (!number(0, 32) && spell_damage(ch, vict, dice(10, 24), SPLDAM_COLD,
					   SPLDAM_NOSHRUG | SPLDAM_NODEFLECT | RAWDAM_NOKILL,
...
int dagger_of_wind(P_obj /*obj*/, P_char ch, int cmd, char *arg)
{
	...
	if (!number(0, 14))
	{
		...
		for (vict = world[ch->in_room].people; vict; vict = vict->next_in_room)
		{
			...
			{
				act("&+C...suddenly, $n &+Cis engulfed in a sw&+cwi&+Wrl&+cin&+Cg &+Ltornado &+Cof &+Wwind... sweeping them from &+rbattle&+c!",
				    FALSE, ch, 0, vict, TO_ROOM);
				act("&+C...suddenly, $n &+Cis engulfed in a sw&+cwi&+Wrl&+cin&+Cg &+Ltornado &+Cof &+Wwind... sweeping them from &+rbattle&+c!",
				    FALSE, ch, 0, vict, TO_VICT);
				act("&+C...suddenly, you &+Care engulfed in a sw&+cwi&+Wrl&+cin&+Cg &+Ltornado &+Cof &+Wwind... and swept from &+rbattle&+c!",
	...
	if (OBJ_WORN_BY(obj, ch) && vict)
	{
		if (!number(0, 30))
		{
			act("Your $q &+Bvibrates then becomes very &+Cchilly!", FALSE,
			    obj->loc.wearing, obj, vict, TO_CHAR);
			act("$n's $q &+Bvibrates violently, directing &+Ccold&+B energy at you!",
			    FALSE, obj->loc.wearing, obj, vict, TO_VICT);
			act("$n's $q &+Bvibrates violently then explodes in &+Cchilly &+Bforce!",
			    FALSE, obj->loc.wearing, obj, vict, TO_NOTVICT);
...
int tendrils(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	*/
	...
	act("$n sidesteps $N's lunge only to slam $s face with an elbow!", TRUE, ch, 0, vict,
	    TO_NOTVICT);
	act("$n completely sidesteps your lunge only to slam $s elbow into your face!", TRUE, ch, 0,
	    vict, TO_VICT);
	act("You completely sidestep $N's lunge, only to slam your elbow into $s face!", TRUE, ch,
	    0, vict, TO_CHAR);
	/*
...
int Einjar(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd != CMD_GOTHIT || number(0, 4) || !(data = legacy_proc_arg<struct proc_data *>(arg)))
	{
		return FALSE;
	}
	vict = data->victim;
	...
	if (!number(0, 14) && GET_POS(vict) == POS_STANDING && !IS_TRUSTED(ch))
	{
		...
		act("You are knocked to the ground by $n's mighty bash!", FALSE, ch, 0, vict,
		    TO_VICT);
		act("$N is knocked to the ground by $n's mighty bash!", FALSE, ch, 0, vict,
		    TO_NOTVICT);
		SET_POS(vict, POS_SITTING + GET_STAT(vict));
		CharWait(vict, PULSE_VIOLENCE * 2);
	}
