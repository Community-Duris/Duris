...
int orb_of_the_sea(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if (OBJ_WORN_BY(obj, ch) && IS_ALIVE(victim) && !IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) && !IS_GH_GOLEM(victim) && GET_RACE(victim) != RACE_CONSTRUCT && !(strstr(victim->player.name, "_no_move_")))
  {
    // 1/30 chance.
    if (!number(0, 29))
    {
...
int elfgate(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  act("As you step into the $o, there is a blinding flash of light!", FALSE,
      ch, obj, 0, TO_CHAR);
  act("You are ripped through a dark and star-filled void, pain sears through",
  ...
  } while (IS_ROOM(to_room, ROOM_PRIVATE) || PRIVATE_ZONE(to_room) || IS_ROOM(to_room, ROOM_NO_TELEPORT) || world[to_room].sector_type == SECT_OCEAN || world[to_room].number < 110000 || world[to_room].number >= 210000);
  /*         !IS_MAP_ROOM(to_room)); */
  if (IS_FIGHTING(ch))
    stop_fighting(ch);
  if (IS_DESTROYING(ch))
...
int barb(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if ((cmd == CMD_GOTNUKED) && (GET_RACE(ch) == RACE_MOUNTAIN || GET_RACE(ch) == RACE_DUERGAR) && (!number(0, 5)))
  {
    ...
    if (data->messages && data->messages->attacker && data->messages->victim && data->messages->room && data->messages->death_attacker && data->messages->death_victim && data->messages->death_room)
    {
...
int tendrils(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  */
  ...
  act("$n completely sidesteps your lunge only to slam $s elbow into your face!",
      TRUE, ch, 0, vict, TO_VICT);
  act("You completely sidestep $N's lunge, only to slam your elbow into $s face!",
