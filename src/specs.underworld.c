			...
			for (tch = world[ch->in_room].people; tch; tch = tch_next)
			{
				    ...
				    !IS_DRAGON(tch))
				{
					if (!StatSave(tch, APPLY_AGI, -2))
					{
						...
						act("$n crashes to the ground!", FALSE, tch, 0, 0,
						    TO_ROOM);
					...
					else
					{
						...
						act("$N dodges your sweep.", 0, ch, 0, tch,
						    TO_CHAR);
					}
				}
				if (!char_in_list(ch))
	    ...
	    !(strstr(victim->player.name, "_no_move_")))
	{
		...
		if (!number(0, 29))
		{
			...
			if (!number(0, 2))
			{
				...
				new_room = new_room =
					real_room(number(WATERP_VNUM_BEGIN, WATERP_VNUM_END));
		...
		if (!number(0, 9))
		{
			    ...
			    !IS_NEXUS_GUARDIAN(vict) && !IS_GH_GOLEM(vict) && !IS_OP_GOLEM(vict))
			{
				if (vict->in_room == ch->in_room)
				{
					...
					for (P_obj obj = world[ch->in_room].contents; obj;
