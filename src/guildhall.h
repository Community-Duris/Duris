...
#define IS_GH_GOLEM(ch) (IS_NPC(ch) && (GET_VNUM(ch) == GH_GOLEM_WARRIOR || GET_VNUM(ch) == GH_GOLEM_CLERIC || GET_VNUM(ch) == GH_GOLEM_SORCERER))
...
struct GuildhallRoom
{
	...
	GuildhallRoom(int _template_vnum) : vnum(-1), guild(NULL), type(0), room(NULL), guildhall(NULL), template_vnum(_template_vnum) { GuildhallRoom(); }
