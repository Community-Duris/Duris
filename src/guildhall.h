...
#define IN_GH_ZONE(r_room) \
	(world[r_room].number >= GH_START_VNUM && world[r_room].number <= GH_END_VNUM)
...
#define IS_GH_GOLEM(ch)                                                                        \
	(IS_NPC(ch) && (GET_VNUM(ch) == GH_GOLEM_WARRIOR || GET_VNUM(ch) == GH_GOLEM_CLERIC || \
...
struct GuildhallRoom
{
	...
	virtual bool init();
	virtual bool deinit();
	virtual bool can_upgrade() const { return (this->type == GH_ROOM_TYPE_GENERIC); }

	bool save();
	bool valid();
	bool destroy();

	bool has_exit(int dir) const { return (this->exits[dir] >= 0); }
	...
	GuildhallRoom(int _template_vnum)
		: vnum(-1)
		, guild(NULL)
		, type(0)
...
struct EntranceRoom : public GuildhallRoom
{
	...
	EntranceRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_ENTRANCE)
		, door(NULL)
	{
...
struct InnRoom : public GuildhallRoom
{
	...
	InnRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_INN)
		, board(NULL)
	{
	}
};
...
struct PortalRoom : public GuildhallRoom
{
	...
	PortalRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_PORTAL)
	{
	}
};
...
struct HeartstoneRoom : public GuildhallRoom
{
	...
	HeartstoneRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_HEARTSTONE)
		, heartstone(NULL)
	{
	}
};
...
struct WindowRoom : public GuildhallRoom
{
	...
	WindowRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_WINDOW)
		, window(NULL)
	{
	}
};
...
struct HealRoom : public GuildhallRoom
{
	...
	HealRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_HEAL)
		, fountain(NULL)
	{
	}
};
...
struct BankRoom : public GuildhallRoom
{
	...
	BankRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_BANK)
		, counter(NULL)
	{
	}
};
...
struct TownPortalRoom : public GuildhallRoom
{
	...
	TownPortalRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_TOWN_PORTAL)
		, portal(NULL)
	{
	}
};
...
struct LibraryRoom : public GuildhallRoom
{
	...
	LibraryRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_LIBRARY)
		, tome(NULL)
	{
	}
};
...
struct CargoRoom : public GuildhallRoom
{
	...
	CargoRoom()
		: GuildhallRoom(GH_ROOM_TEMPLATE_CARGO)
		, board(NULL)
	{
	}
};
...
struct Guildhall
{
	...
	Guildhall()
		...
		, inn_room(NULL)
		, portal_room(NULL)
		, heartstone_room(NULL)
	{
	}

	~Guildhall() { this->clear_rooms(); }

	void clear_rooms()
	{
		// delete guildhall rooms
		for (vector<GuildhallRoom *>::iterator r = this->rooms.begin();
		     r != this->rooms.end(); r++)
		{
			delete (*r);
		}
