...
int fade_drusus(P_obj obj, P_char ch, int cmd, char *argument)
{
  ...
  if (!number(0, 30))
  {
    ...
    act("&+LA wisp of black shadow shoots forth, striking $N &+Lin the eyes!&N",
...
bool trident_charm(P_char ch, P_char victim)
{
  ...
  if (IS_NPC(victim) &&
      !GET_MASTER(victim) &&
      !NewSaves(victim, SAVING_PARA, (GET_LEVEL(ch) - GET_LEVEL(victim)) / 7) && ((GET_LEVEL(ch) - 10) > GET_LEVEL(victim)))
  {
...
int martelo_mstar(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  e_pos = OBJ_WORN_POS(obj, WIELD) ? WIELD : OBJ_WORN_POS(obj, SECONDARY_WEAPON) ? SECONDARY_WEAPON
                                                                                 : 0;
