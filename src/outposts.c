...
void show_outposts(P_char ch)
{
	...
	for (i = 0; i < static_cast<int>(buildings.size()); i++)
	{
				       ...
				       static_cast<unsigned int>(owner) == GET_ASSOC(ch)->get_id()))
		{
			snprintf(
				buff, MAX_STRING_LENGTH,
				"       &+LGateguards: &+c%d &+LPortal: &+c%-4s &+LArchers: &+c%-4s &+LMeurtriere: &+c%-4s&n\r\n",
...
void do_outpost(P_char ch, char *arg, int /*cmd*/)
{
	...
	if (IS_TRUSTED(ch) && !str_cmp("reset", buff2))
	{
		...
		{
			send_to_char(
				"Ask an overlord, this command will reset all active outposts.\n",
				ch);
			return;
		}
	...
	if (!str_cmp("drop", buff2))
	{
		...
		{
			send_to_char(
				"You need to be the leader of the guild to use this command.\r\n",
				ch);
			return;
		}
		int id = atoi(buff3);
	...
	if (!str_cmp("repair", buff2))
	{
		...
		if (rubble && OBJ_VNUM(rubble) == BUILDING_RUBBLE)
		{
			...
			if (!building)
			{
				...
				send_to_char(
					"Can't find building associated with outpost, tell a god.\r\n",
					ch);
				return;
			}
			op = building->get_mob();
			...
			{
				send_to_char(
					"You begin the arduous task of rebuilding the destroyed tower, claiming it for your guild!\r\n",
					ch);
			...
			{
				send_to_char(
					"You must belong to an association to claim an outpost.\r\n",
					ch);
				return;
			}
		...
		else
		{
			...
			if (op)
			{
				...
				for (tch = world[op->in_room].people; tch != NULL;
				     tch = tch->next_in_room)
				{
					if (GET_OPPONENT(tch) == op || IS_FIGHTING(op))
					{
						send_to_char(
							"You cannot repair an outpost being attacked!\r\n",
							ch);
						return;
					}
				}
				...
				send_to_char("You order repairs to begin upon the outpost.\r\n",
					     ch);
				act("$n orders repairs begin on the outpost.", TRUE, ch, 0, 0,
				    TO_ROOM);
				add_event(event_outpost_repair, PULSES_IN_TICK, op, NULL, NULL, 0,
					  NULL, 0);
				return;
			}
			else
			{
				send_to_char(
					"There is no rubble or outpost to repair here, go sac an outpost first!\r\n",
					ch);
				return;
			}
		}
	...
	if (!str_cmp("portal", buff2))
	{
		...
		{
			send_to_guild(building->get_guild(), "The Guild Banker",
				      "There are not enough guild funds to purchase a portal.");
			return;
		}
	...
	if (!str_cmp("golem", buff2))
	{
		...
		{
			send_to_guild(
				building->get_guild(), "The Guild Banker",
				"There are not enough guild funds to purchase an outpost golem.");
			return;
		}
	...
	if (!str_cmp("archers", buff2))
	{
		...
		if (!IS_TRUSTED(ch) && !(building->sub_money(cost / 1000, 0, 0, 0)))
		{
			send_to_guild(
				building->get_guild(), "The Guild Banker",
				"There are not enough guild funds to purchase outpost archers.");
			return;
		}
	...
	if (!str_cmp("meurtriere", buff2))
	{
		...
		if (!IS_TRUSTED(ch) && !(building->sub_money(cost / 1000, 0, 0, 0)))
		{
			send_to_guild(
				building->get_guild(), "The Guild Banker",
				"There are not enough guild funds to purchase an meurtriere.");
			return;
		}
	...
	if (!str_cmp("?", buff2) || !str_cmp("help", buff2))
	{
		...
		send_to_char(
			"Options Available: Archers, Drop, Golem, Meurtriere, Portal, Repair\r\n",
			ch);
		if (IS_TRUSTED(ch))
		{
			send_to_char(
				"&+CImmortal Options:&n\r\nOptions Available: Reload [id|all] (&+Lnot implemented&n), Reset[id|all]",
				ch);
		}
		return;
	}
...
void event_outpost_repair(P_char op, P_char /*vict*/, P_obj /*obj*/, void * /*data*/)
{
	...
	{
		send_to_guild(building->get_guild(), "The Guild Banker",
			      "There are not enough guild funds to complete outpost repairs.");
		return;
	}

	//  Add 10% of the buildings max hitpoints, up to max.
	GET_HIT(building->get_mob()) =
		BOUNDED(GET_HIT(building->get_mob()),
			GET_HIT(building->get_mob()) +
				(building_types[BUILDING_OUTPOST - 1].hitpoints / 10),
	...
	*/
	...
	for (int i = 0; i < building->size(); i++)
	{
		...
		for (P_char tch = world[building->gate_room()].people; tch != NULL; tch = next_tch)
		{
			...
			if (IS_PC(tch))
				send_to_char(
					"The outpost has been destroyed, you fall to the ground outside!\r\n",
					tch);
			char_from_room(tch);
	...
	{
		P_obj next_obj;
		for (P_obj tobj = world[building->gate_room()].contents; tobj != NULL;
		     tobj = next_obj)
		{
...
void outpost_create_wall(int location, int direction, int /*type*/)
{
	...
	if (create_walls(location, direction, NULL, 64, WALL_OUTPOST, value, -1,
			 "&+Wan outpost wall&n", "&+WAn outpost wall is here to the %s.&n", 0))
	{
		SET_BIT(world[location].dir_option[direction]->exit_info, EX_BREAKABLE);
		SET_BIT(VIRTUAL_EXIT((world[location].dir_option[direction])->to_room,
				     rev_dir[direction])
				->exit_info,
			EX_BREAKABLE);
	}
...
int outpost_generate_walls(Building *building, int /*type*/, int numgolem)
{
	...
	for (x = 0; x < NUM_EXITS; x++)
	{
		...
		if (x == gate)
		{
			...
			outpost_setup_gateguards(walllocal[x], OUTPOST_GATEGUARD_WAR, numgolem,
						 building);
...
int outpost_gateguard_proc(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if (get_outpost_owner(building) != GET_ASSOC(ch))
	{
		...
		act("$n blinks momentarily then accepts their new owners.", TRUE, ch, 0, 0,
		    TO_ROOM);
	}
	...
	if (blocked_dir < DIR_NORTH || blocked_dir >= NUM_EXITS)
	{
		logit(LOG_GUILDHALLS,
		      "outpost_gateguard_proc() assigned to %s in %d has an invalid blocking direction (%d)!",
	...
	if (pl && cmd == cmd_from_dir(blocked_dir))
	{
		...
		else if (allowed)
		{
			if (!number(0, 2))
			{
				act("$N stands impassively as you pass by.", FALSE, pl, 0, ch,
				    TO_CHAR);
				act("$N stands impassively as $n passes by.", FALSE, pl, 0, ch,
				    TO_NOTVICTROOM);
			}
			return FALSE;
		}
		...
		{
			act("$N glares at you and refuses to let you pass.", FALSE, pl, 0, ch,
			    TO_CHAR);
			act("$N glares at $n and refuses to let them pass.", FALSE, pl, 0, ch,
			    TO_NOTVICTROOM);
			return TRUE;
		}
	...
	if (pl && ((cmd == CMD_GOTHIT && !number(0, 15)) || cmd == CMD_HIT || cmd == CMD_KILL))
	{
		...
		for (P_desc i = descriptor_list; i; i = i->next)
			if (!i->connected && !is_silent(i->character, TRUE) &&
			    ...
			    (GET_ASSOC(i->character) == GET_ASSOC(ch)) && !IS_TRUSTED(i->character))
...
bool check_castle_walls(int from, int to)
{
	if ((world[to].sector_type == SECT_CASTLE_WALL &&
	    ...
	    (world[from].sector_type == SECT_CASTLE_WALL &&
	     (world[to].sector_type != SECT_CASTLE && world[to].sector_type != SECT_CASTLE_WALL)))
		return TRUE;
	else
		return FALSE;
}
	...
	for (guild_num = 1; guild_num <= MAX_ASC; guild_num++)
	{
		if (num_ops[guild_num] > 0)
		{
			...
			if (num_ops[guild_num] > 1)
			{
				...
				{
					deduct = deduct *
						 get_property("outpost.cost.upkeep.multi.modifier",
							      2.0);
				}
			}
			...
			if (!guild->sub_money(p, g, s, c))
			{
				...
				for (i = 1; i <= static_cast<int>(buildings.size()); i++)
				{
					...
					if (building->get_guild() == guild)
					{
						...
						snprintf(buff, MAX_STRING_LENGTH,
							 "Dropping %s&+C outpost.",
							 continent_name(world[building->location()]
										.continent));
...
int outpost_archer_attack(P_char ch, P_char vict)
{
	...
	if (number(1, 100) <= (int)get_property("outpost.archers.hit.chance", 60))
	{
		...
		if (!IS_TRUSTED(vict))
			GET_HIT(vict) -= dice((int)get_property("outpost.archers.dice.hit", 5),
					      (int)get_property("outpost.archers.dice.dam", 5));
		if (number(1, 100) < (int)(get_property("outpost.archers.hit.chance", 60) / 4))
		{
			GET_HIT(vict) -= (int)((GET_HIT(vict) / 10) *
					       get_property("outpost.archers.crit.multi", 1));
...
int outpost_meurtriere_attack(P_char ch)
{
	...
	snprintf(buf, MAX_STRING_LENGTH,
		 "&+LThe &+Rburning &+Loil splashes on you causing severe &+Rpain&+L!&n");
	snprintf(buf1, MAX_STRING_LENGTH,
		 "&+LThe &+Rburning &+Loil splashes on $n causing him severe &+Rpain&+L!");
	snprintf(buf2, MAX_STRING_LENGTH,
		 "You narrowly avoid being smothered in &+Rsearing &+Loil&n!");
	snprintf(buf3, MAX_STRING_LENGTH,
		 "$n narrowly avoids being smothered in &+Rsearing &+Loil&n!");
	...
	for (vict = world[ch->in_room].people; vict; vict = next_vict)
	{
		...
		if (number(1, 100) <= (int)get_property("outpost.meurtriere.hit.chance", 60))
		{
			...
			if (!IS_TRUSTED(vict))
				GET_HIT(vict) -=
					dice((int)get_property("outpost.meurtriere.dice.hit", 5),
					     (int)get_property("outpost.meurtriere.dice.dam", 5));
			if (number(1, 100) <
			    (int)(get_property("outpost.meurtriere.hit.chance", 60) / 4))
			{
				GET_HIT(vict) -=
					(int)((GET_HIT(vict) / 20) *
					      get_property("outpost.meurtriere.crit.multi", 1));
				send_to_char(
					"&+ROUCH! &+LThe oil seeps deep into your &+Rwounds&+L!&n\r\n",
					vict);
			}
			if (GET_HIT(vict) < -10)
			{
