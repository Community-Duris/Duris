...
static void zc_loader_reason(const zc_state &st, char *buf, size_t len)
{
	...
	case ZC_WHY_ROLL:
		snprintf(
			buf, len,
			"the %c at cmd %d loads on a %d%% roll (db.c:%s) - a dice throw, not a fact",
	...
	case ZC_WHY_POP:
		snprintf(
			buf, len,
			"the %c at cmd %d loads whenever the live population is under its limit of %d (db.c:%s)",
...
void do_zcheck(P_char ch, char *argument, [[maybe_unused]] int cmd)
{
	...
	{
		send_to_char(
			"No such zone - zcheck <zone#> takes a zone number as shown by zlist.\r\n",
			ch);
		return;
	}
	...
	send_to_char_f(ch, "/== &+WZCHECK&n: zone &+Y%d&n [index %d] %s&n ==\\\r\n", zd.number, zn,
		       zd.name ? zd.name : "(unnamed)");
	send_to_char_f(ch, "rooms #%d..#%d (%d rooms), file '%s'\r\n", zc_vnum(rb), zc_vnum(rt),
		       rt - rb + 1, zd.filename ? zd.filename : "?");
	...
	if (entrances.empty())
	{
		send_to_char_f(
			ch,
			"Entrances: &+ynone found&n - no other zone exits into this one; starting the walk at the lowest room #%d instead.\r\n",
			zc_vnum(rb));
	}
	else
	{
		...
		for (r = rb; r <= rt; r++)
		{
			...
			if (shown < ZC_MAX_LINES)
			{
				send_to_char_f(ch, "   #%-7d %s&n\r\n", zc_vnum(r),
					       world[r].name ? world[r].name : "(unnamed)");
				shown++;
			}
		}
		if (total > shown)
		...
		for (r = rb; r <= rt; r++)
		{
			...
			if (shown < ZC_MAX_LINES)
			{
				send_to_char_f(ch, "   &+R#%-7d&n %s&n - %s\r\n", zc_vnum(r),
					       world[r].name ? world[r].name : "(unnamed)",
					       any ? "its only exit loops back to itself" :
		...
		for (r = rb; r <= rt; r++)
			for (d = 0; d < NUM_EXITS; d++)
			{
				...
				if (shown < ZC_MAX_LINES)
				{
					send_to_char_f(
						ch,
						"   #%d --%s--> #%d, but #%d has no %s exit back\r\n",
						zc_vnum(r), zc_dir_name(d), zc_vnum(b), zc_vnum(b),
						zc_dir_name(rd));
					shown++;
				}
			}
	...
	{
		if (o->type != ITEM_SWITCH || o->R_num < 0)
			continue;
		zc_switch sw;
	 ...
	 * ---------------------------------------------------------------- */
	{
		...
		for (const auto &ds : doors)
		{
			...
			if (!ex)
			{
				...
				if (shown < ZC_MAX_LINES)
				{
					send_to_char_f(
						ch,
						"   [&+RBAD D ROW&n] #%d %s: the D reset targets an exit that does not exist (reset_zone disables it and logs, db.c:3606).\r\n",
			...
			if (ds.blocked)
			{
				...
				if (!found)
				{
					...
					if (shown < ZC_MAX_LINES)
					{
						send_to_char_f(
							ch,
							"   [&+RSEALED WALL&n] #%d %s: EX_BLOCKED wall and NO live ITEM_SWITCH targets it (item_switch is the only data-driven opener, specs.object.c:1331; it clears the near side only, :1405).%s A compiled proc or an unloaded switch prototype could still exist - zcheck cannot see those.\r\n",
							zc_vnum(ds.room), zc_dir_name(ds.dir),
							ds.secret ?
								" It is also SECRET: search cannot reveal it while blocked (actobj.c:5849)." :
								"");
						shown++;
					}
				}
			...
			const bool bash_ok = door && !pickproof && !nobash &&
					     ex->to_room != NOWHERE;
			...
			for (const auto &p : seen_pairs)
				if (p.a == pk.a && p.b == pk.b && p.key == pk.key &&
				    p.code == pk.code)
				{
					dup = true;
					break;
			...
			modes[0] = '\0';
			if (pick_ok)
				ml += snprintf(modes + ml, sizeof(modes) - (size_t)ml, "%spick",
					       ml ? ", " : "");
			if (bash_ok)
				ml += snprintf(modes + ml, sizeof(modes) - (size_t)ml, "%sdoorbash",
					       ml ? ", " : "");
			if (pass_ok)
				ml += snprintf(modes + ml, sizeof(modes) - (size_t)ml, "%spassdoor",
					       ml ? ", " : "");
			...
			if (!strcmp(sev, "UNWINNABLE"))
			{
				...
				if (key_meant)
					send_to_char_f(
						ch,
						"   [&+RUNWINNABLE&n] #%d %s: resets LOCKED needing key #%d, but no object prototype has that vnum (real_object = -1, so has_key can never match) and it is PICKPROOF (wld door-info 3, db.c:1376), which kills pick (actmove.c:2892), doorbash (innates.c:2852) and passdoor (actmove.c:1208) at once. Only a compiled proc or a studioproc trigger could open it - zcheck scans neither.\r\n",
						zc_vnum(ds.room), zc_dir_name(ds.dir), key);
				else
					send_to_char_f(
						ch,
						"   [&+RUNWINNABLE&n] #%d %s: resets LOCKED and NOTHING can open it - %s%s, and no mode survives: %s. Only a compiled proc or a studioproc trigger could open it - zcheck scans neither.\r\n",
						zc_vnum(ds.room), zc_dir_name(ds.dir),
						door ? (key < 0 ?
								"it has no keyhole (key < 0 refuses unlock, actmove.c:2708, and pick, actmove.c:2889)" :
								"no key is set") :
						       "the exit is not even a door (wld door-info 0 - key, pick and doorbash all need EX_ISDOOR)",
						pickproof ?
							"; it is PICKPROOF, which also kills doorbash and passdoor" :
							"",
						door ? "the lock survives every natural attempt" :
						       "passdoor is refused only on locked+pickproof, but nothing else applies to a non-door");
			}
			else if (!strcmp(sev, "KEY-BROKEN"))
			{
				...
				send_to_char_f(
					ch,
					"   [&+YKEY-BROKEN&n] #%d %s: needs key #%d but no object prototype has that vnum - the intended path is dead. Still openable by: %s. The D reset re-locks it every repop, so those are per-visit, not a fix.\r\n",
			...
			else /* NARROW */
			{
				...
				send_to_char_f(
					ch,
					"   [&+YNARROW&n] #%d %s: locked with no key path (%s)%s - only %s can open it. If that narrow availability is the design, this note is the receipt.\r\n",
					zc_vnum(ds.room), zc_dir_name(ds.dir),
					door ? (key < 0 ?
							"key < 0 = no keyhole, unlock and pick both refused" :
							"pickproof lock") :
					       "not a door, so key/pick/doorbash are all refused",
					nobash ?
						"; its keyword carries _nobash_, so doorbash bounces (innates.c:2852)" :
						"",
					modes);
			}
		}
		if (!listed)
			send_to_char(
				"   none - this zone's D resets leave nothing locked or walled that needs attention.\r\n",
				ch);
		else if (!shown)
	 ...
	 * ---------------------------------------------------------------- */
	{
			...
			for (i = 0; zd.cmd[i].command != 'S'; i++)
			{
				...
				case 'R':
					...
					if (rc.arg4 <= 0)
						snprintf(
							...
							rc.command, i, rc.command,
							mob_index[rc.arg1].virtual_number, rc.arg4,
						...
						snprintf(
							...
							rc.command, i, rc.command,
							mob_index[rc.arg1].virtual_number, rc.arg2,
							rc.arg2, zc_limit_line(rc.command),
							rc.command);
					else if (rc.command == 'M' && rc.arg4 != 100)
					{
						...
						snprintf(
							...
							i, mob_index[rc.arg1].virtual_number,
							rc.arg4, rc.arg4, rc.arg4);
					}
					break;
				...
				case 'G':
					...
					if (gate == ZC_NO)
						snprintf(
							...
							rc.command, i, rc.command,
							obj_index[rc.arg1].virtual_number, why);
						...
						snprintf(
							...
							obj_index[rc.arg1].virtual_number, why,
							rc.command == 'E' ?
								"3481, leaving the copy in limbo" :
								"3412, then extracting the copy");
					else if (rc.command == 'E' &&
						 (rc.arg3 <= 0 || rc.arg3 > CUR_MAX_WEAR))
						snprintf(
							line, sizeof(line),
						...
						snprintf(
							...
							rc.command, i, rc.command,
							obj_index[rc.arg1].virtual_number, rc.arg4,
							rc.arg4,
							rc.command == 'E' ? "3464-3470" :
									    "3397-3403");
				...
				case 'P':
				{
					...
					if (!inst)
						snprintf(
							...
							i, obj_index[rc.arg1].virtual_number,
							obj_index[rc.arg3].virtual_number);
					else if (inst->type != ITEM_CONTAINER &&
						 inst->type != ITEM_QUIVER &&
						 inst->type != ITEM_STORAGE &&
						 inst->type != ITEM_CORPSE)
						snprintf(
							line, sizeof(line),
				...
				case 'M':
				{
					...
					const zc_tri roll =
						(rc.arg4 <= 0) ?
							ZC_NO :
							(rc.arg4 >= 100 ? ZC_YES : ZC_MAYBE);
					const bool badroom =
						(rc.arg3 < 0 || rc.arg3 > top_of_world);
					...
					{
						b.last_cmd = b.last_mob_load =
							ZC_MAYBE; /* db.c:3247 vs :3250 */
				...
				case 'D':
					...
					if (rc.arg1 < 0 || rc.arg1 > top_of_world || rc.arg2 < 0 ||
					    rc.arg2 >= NUM_EXITS ||
					    !world[rc.arg1].dir_option[rc.arg2])
						b.last_cmd = ZC_NO;
					else
						b.last_cmd = ZC_YES;
					break;
				...
				case 'P': /* db.c:3311-3366 */
					b.last_cmd = (rc.arg1 < 0 || rc.arg3 < 0) ? ZC_NO :
										    ZC_MAYBE;
					break;
				...
				else
				{
					...
					b.last_mob_load =
						zc_join(b.last_mob_load, st.last_mob_load);
		...
		if (disabled_cmds)
			send_to_char_f(
				ch,
				"   (note: %d command%s already disabled by the loader at boot - bad vnums, see the boot log.)\r\n",
