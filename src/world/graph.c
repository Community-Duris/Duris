			...
			for (curr_dir = 0; curr_dir < NUM_EXITS; curr_dir++)
			{
				if (VALID_SHIP_EDGE(queue_head->room, curr_dir))
				{
					...
					bfs_enqueue(TOROOM(queue_head->room, curr_dir),
						    queue_head->dir, queue_head->step_no + 1);
				}
			}
			bfs_dequeue();
		}
	...
	for (curr_dir = 0; curr_dir < NUM_EXITS; curr_dir++)
	{
		...
		if (VALID_EDGE2(exit))
		{
			    ...
			    (!no_mob || !IS_ROOM(exit->to_room, ROOM_NO_TRACK)))
			{
				...
				if ((rr_targ ? IS_ROADRANGER_TARGET(exit->to_room) :
					       (exit->to_room == target)))
				{
					bfs_clear_queue();
					*ttl_steps = 1;
		...
		for (curr_dir = 0; curr_dir < NUM_EXITS; curr_dir++)
		{
			...
			if (VALID_EDGE2(exit))
			{
				...
				if (HOMETOWN_CHECK(curr_room, exit->to_room) &&
				    ...
				    (can_dispel || !IS_WALLED(curr_room, curr_dir) ||
				     (can_break && IS_BREAKABLE(curr_room, curr_dir))) &&
				    (!no_mob || !IS_ROOM(exit->to_room, ROOM_NO_TRACK)))
				{
					...
					// Enqueue it only if it passes the checks.
					bfs_enqueue(exit->to_room, queue_head->dir,
						    queue_head->step_no + 1);
				}
			}
		}
...
int how_close(int src, int target, int max_steps)
{
	...
	while (queue_head)
	{
		if (i++ > BFS_MAX_ROOMS)
		{
			...
			bfs_clear_queue();
			logit(LOG_DEBUG,
			      "graph.c: how_close(): Hit BFS_MAX_ROOMS going from room %d to %d",
			      src, target);
			return BFS_ERROR;
		}
		if (queue_head->room == target)
		...
		else if (queue_head->step_no > max_steps)
		{
			bfs_clear_queue();
			logit(LOG_DEBUG,
			      "graph.c: how_close(): Hit dim distance limit going from room %d to %d",
			      src, target);
			return BFS_NO_PATH;
		}
		else
	...
	while (!q.empty())
	{
		...
		for (int dir = 0; dir < NUM_EXITS; dir++)
		{
			if (VALID_SHIP_EDGE(cur_node->room_num, dir))
			{
				...
				cur_node =
					new BFSNode(TOROOM(cur_node->room_num, dir), cur_node, dir);
				nodes.push_back(cur_node);
				q.push(cur_node);
			}
		}
...
struct SearchNode
{
	...
	SearchNode(int _room_id, int _dist)
		: room_id(_room_id)
		, dist(_dist)
	{
	}
...
struct PathNode
{
	...
	PathNode(int _room_id, int _to_dir)
		: room_id(_room_id)
		, to_dir(_to_dir)
	{
	}
			       ...
			       int pcbase)
{
	...
	if (radius > RMFR_MAX_RADIUS)
	{
		wizlog(MINLVLIMMORTAL,
		       "radiate_message_from_room - called in %d with too high distance", src);
	...
	if (!dead_rmfr_pool)
	{
		dead_rmfr_pool = mm_create("RMFR", sizeof(RMFR_QUEUE_ITEM),
					   offsetof(RMFR_QUEUE_ITEM, next), 1);
	...
	while (rmfr_queue_head)
	{
		...
		{
			for (tar = world[rmfr_queue_head->room].people; tar;
			     tar = tar->next_in_room)
			{
				if (IS_NPC(tar))
					continue;
				...
				if (flags & RMFR_INCREASE_PC_DIFF_OVER_DISTANCE)
					chance -= distance_array[abs(rmfr_queue_head->x)]
								[abs(rmfr_queue_head->y)]
								[abs(rmfr_queue_head->z)] *
						  2;
		...
		for (curr_dir = 0; curr_dir < NUM_EXITS; curr_dir++)
		{
			if (rmfr_valid_exit(rmfr_queue_head->room, curr_dir, radius, flags,
					    RMFR_Q_OTHER))
			{
