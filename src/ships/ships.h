...
struct ShipData
{
	...
	int
	has_capital(); // returns which type of capital item (SLOT_WEAPON/SLOT_EQUIPMENT) or SLOT_EMPTY if none found.
	bool
	buy_check_capital(P_char ch); // Checks for capital item and sends message to ch if it does.
	...
	int db_id; // database id of ship, -1 if not saved yet
...
#define HAS_VALID_TARGET(shipdata)                                       \
	((shipdata->target != 0) && (!SHIP_SINKING(shipdata->target)) && \
	 (shipdata->target->race != shipdata->race))

// Cargo related stuff
#define SHIP_MAX_CARGO(shipdata) \
	((int)(SHIPTYPE_CARGO(SHIP_CLASS(shipdata)) * shipdata->crew.get_maxcargo_mod()))
...
#define SHIP_AVAIL_CARGO_LOAD(shipdata)                             \
	MIN((SHIP_MAX_CARGO(shipdata) - SHIP_CARGO_LOAD(shipdata)), \
...
#define SHIP_AVAIL_CARGO_SALVAGE(shipdata)                                              \
	MIN(((IS_MERCHANT(ship) ? SHIP_MAX_CARGO(ship) : (SHIP_MAX_WEIGHT(ship) / 5)) - \
	     SHIP_CARGO_LOAD(shipdata)),                                                \
	    (SHIP_AVAIL_WEIGHT(shipdata) / 2))
#define SHIP_MAX_CARGO_SALVAGE(shipdata) \
	(SHIP_CARGO_LOAD(shipdata) + SHIP_AVAIL_CARGO_SALVAGE(shipdata))
