...
int list_cargo(P_char ch, P_ship ship, bool owned)
{
	...
	send_to_char_f(ch, "%s %s per crate.\n", pad_ansi(cargo_type_name(portnum), 30).c_str(),
		       coin_stringv(cost, 3));
	...
	for (int i = 0; i < NUM_PORTS; i++)
	{
		...
		send_to_char_f(ch, "%s %s per crate.\n", pad_ansi(cargo_type_name(i), 30).c_str(),
			       coin_stringv(cost, 3));
	}
	...
	if (GET_ALIGNMENT(ch) <= MINCONTRAALIGN)
	{
		if (can_buy_contraband(ship, portnum))
		{
			...
			send_to_char_f(ch, "%s %s &+Lper crate.&n\n",
				       pad_ansi(contra_type_name(portnum), 30).c_str(),
				       coin_stringv(cost, 3));
		}
		...
		for (int i = 0; i < NUM_PORTS; i++)
		{
			if (can_buy_contraband(ship, i))
			{
				...
				send_to_char_f(ch, "%s %s &+Lper crate.&n\n",
					       pad_ansi(contra_type_name(i), 30).c_str(),
					       coin_stringv(cost));
			}
		}
		if (caption)
	...
	for (int i = 0; i < MAXSLOTS; i++)
	{
		...
		if (ship->slot[i].type == SLOT_CARGO)
		{
			...
			*/
			...
			{
				profit = (int)((((float)cost / (float)ship->slot[i].val1) - 1.00) *
					       100.0);
				send_to_char_f(
					ch, " %s &+Y%2d&n crates.\n",
					pad_ansi(cargo_type_name(ship->slot[i].index), 30).c_str(),
					ship->slot[i].val0);
		...
		else if (ship->slot[i].type == SLOT_CONTRABAND)
		{
			...
			*/
			...
			if (ship->slot[i].val1 > 0)
			{
				...
				send_to_char_f(
					ch, " %s &+Y%2d&n crates.\n",
					pad_ansi(contra_type_name(ship->slot[i].index), 30).c_str(),
					ship->slot[i].val0);
				send_to_char_f(ch, "   Paid %s.\n",
					       coin_stringv(ship->slot[i].val1, 4));
				send_to_char_f(ch, "   Sell %s (%d%% profit).\n",
...
int list_weapons(P_char ch, P_ship ship, int owned)
{
	...
	send_to_char(
		...
		ch);
	send_to_char(
		"&+y|&+Y    Name             Weight  Range   Damage Ammo Disper Sail Hull Sail Reload  Cost         |\n",
		ch);
	send_to_char(
		"&+y|&+Y                                                   sion  Hit  Dam  Dam   Time               |\n",
		ch);
	send_to_char(
		"&+y\\--------------------------------------------------------------------------------------------/\n",
		ch);
	for (int i = 0; i < MAXWEAPON; i++)
	{
		...
		if (weapon_data[i].fragments > 1)
		{
			if (weapon_data[i].min_damage == weapon_data[i].max_damage)
				sprintf(dam, "%d x %d", weapon_data[i].fragments,
					weapon_data[i].min_damage);
			else
				sprintf(dam, "%d x %d-%d", weapon_data[i].fragments,
					weapon_data[i].min_damage, weapon_data[i].max_damage);
		}
		else
		{
			if (weapon_data[i].min_damage == weapon_data[i].max_damage)
				...
				sprintf(dam, "%d-%d", weapon_data[i].min_damage,
					weapon_data[i].max_damage);
		}
		...
		send_to_char_f(
			...
			i + 1,
			ship_allowed_weapons[ship->m_class][i] &&
					(ship->frags >= weapon_data[i].min_frags ||
					 ship->crew.guns_skill >= weapon_data[i].min_crewexp) ?
				'W' :
				'L',
			weapon_data[i].name, weapon_data[i].weight, rng, dam, weapon_data[i].ammo,
			weapon_data[i].hit_arc, weapon_data[i].sail_hit, weapon_data[i].hull_dam,
			weapon_data[i].sail_dam, weapon_data[i].reload_time,
...
int list_equipment(P_char ch, P_ship ship, int owned)
{
	...
	for (int i = 0; i < MAXEQUIPMENT; i++)
	{
		...
		send_to_char_f(ch, " %-2d) %s%s      &+Y%2d&n %s&n\n", i + 1,
			       ship_allowed_equipment[ship->m_class][i] &&
					       (ship->frags >= equipment_data[i].min_frags) ?
				       "&+W" :
				       "&+L",
			       pad_ansi(equipment_data[i].name, 25, TRUE).c_str(), weight,
...
int list_hulls(P_char ch, P_ship ship, int owned)
{
	if (owned)
	{
		...
		send_to_char(
			...
			ch);
		send_to_char(
			"&+y|&+Y                 Load Cargo Passenger   Max Total  Weapons   Epic                &+y|\n",
			ch);
		send_to_char(
			"&+y|&+Y    Name         Cap.  Cap.      Cap. Speed Armor (F/S/R/P)  Cost   Cost         &+y|\n",
			ch);
		send_to_char(
			"&+y\\---------------------------------------------------------------------------------/\n",
			ch);
		for (int i = 0; i < MAXSHIPCLASS; i++)
		{
			...
			send_to_char_f(
				...
				i + 1, SHIPTYPE_NAME(i), SHIPTYPE_MAX_WEIGHT(i), SHIPTYPE_CARGO(i),
				SHIPTYPE_PEOPLE(i), SHIPTYPE_SPEED(i),
				ship_arc_properties[i].armor[SIDE_FORE] +
					ship_arc_properties[i].armor[SIDE_STAR] +
					ship_arc_properties[i].armor[SIDE_REAR] +
					ship_arc_properties[i].armor[SIDE_PORT],
		...
		*/
		...
		if (ship->timer[T_MAINTENANCE] > 0)
		{
			send_to_char_f(
				ch,
				"&+gYour ship is currently being worked on, please wait another &n%.1f&+g hours.&n\n",
				(float)ship->timer[T_MAINTENANCE] / 75.0);
		}
		else
		{
			send_to_char(
				"To repair, type '&+grepair <&+Ga&+grmor|&+Gi&+gnternal|&+Gs&+gail|&+Gw&+geapon>"
		...
		send_to_char(
			...
			ch);
		send_to_char(
			"To rename your ship for a price, type '&+gbuy &+Gr&+gename <new ship name>&n'.\n",
			ch);
	}
	else
	{
...
int summon_ship(P_char ch, P_ship ship, bool time_only)
{
	...
	if (IS_SET(ship->flags, SUMMONED))
	{
		if (time_only)
		{
			...
			if (ev && ev->obj == ship->shipobj)
			{
				...
				send_to_char_f(ch,
					       "&+gYour ship should arrive in &n%d &+ghour%s.&n\n",
					       time, (time == 1) ? "" : "s");
			}
			else
			{
				send_to_char(
					"&=LrCould not find summon ship event.  You might want to tell an Immortal.\n\r",
					ch);
			}
		}
		else
	...
	if (!time_only)
	{
		...
		{
			send_to_char(
				"&+gYou start to call your ship back from &+LDavy Jones Locker...&n\n",
				ch);
		}
	...
	{
		// 1/2 to 25 mud hrs.
		summontime = (SECS_PER_MUD_HOUR * WAIT_SEC * 50) /
			     MAX(get_maxspeed_without_cargo(ship), 2);
	...
	else
	{
		...
		summontime = (SECS_PER_MUD_HOUR * WAIT_SEC * 70) /
			     MAX((get_maxspeed_without_cargo(ship) - 20), 2);
	...
	if (pvp)
		send_to_char_f(
			ch,
			"&+YDue to dangerous conditions, it will take about &n%d &+Yhours for your ship to get here.&n\n",
		...
		send_to_char_f(
			ch,
			"&+gThanks for your business, it will take &n%d &+ghours for your ship to get here.&n\n",
			summontime / (SECS_PER_MUD_HOUR * WAIT_SEC));
	...
	if (!time_only)
	{
		...
		add_event(summon_ship_event, summontime, NULL, NULL, ship->shipobj, 0, buf,
			  strlen(buf) + 1);
	}
	return TRUE;
}
...
int sell_cargo_slot(P_char ch, P_ship ship, int slot, int rroom)
{
	...
	{
		send_to_char(
			"Your &+csea&+Cfaring&n heritage has earned you a &+Ybonus&n in your sale...&n\r\n",
			ch);
		cost *= 1.10;
	}
	if (ship->slot[slot].val1 != 0)
	{
		profit = (int)((((float)cost / (float)ship->slot[slot].val1) - 1.00) *
			       float(profit));
	}
	...
	{
		send_to_char(
			"&+gBecause your cargo is obviosly stolen, local merchants bargain the price down.&n\n",
			ch);
		cost = cost * 0.6;
	}
	...
	send_to_char_f(
		ch,
		"&+gYou sell &+Y%d&+g crates of &n%s &+gfor &n%s&+g, making a &n%d&+g%% profit.&n\n",
	...
	{
		send_to_char(
			"&+rCon&+Rgra&+Wtula&+Rtio&+rns! You have completed the &+RTrader&+r achievement!&n\n",
			ch);
	}
	paf->modifier += crates;
...
int sell_cargo(P_char ch, P_ship ship, int slot)
{
	...
	if (total_cost > 0)
	{
		...
		{
			send_to_char(
				"&+gYour &+bdip&+Blo&+bmat &+gstatus costs you a &+W10 &+gpercent cut on your profits.&n\n",
				ch);
		}
...
int sell_contra_slot(P_char ch, P_ship ship, int slot, int rroom)
{
	...
	{
		send_to_char(
			"&+gBecause your contraband is obviosly stolen, local merchants bargain the price down.&n\n",
			ch);
		cost = cost * 0.6;
	}
	...
	send_to_char_f(
		ch,
		"&+gYou sell &+Y%d &+gcrates of &n%s &+gfor &n%s&+g, making a &n%d&+g%% profit.&n\n",
...
int sell_slot(P_char ch, P_ship ship, int slot)
{
	...
	else if (is_diplomat_slot(ship, slot))
	{
		for (int i = 0; i < MAXSLOTS; i++)
		{
			...
			{
				send_to_char(
					"&+gYou cannot sell your diplomat flag with cargo onboard. Sell the cargo first!&n\n",
					ch);
				return TRUE;
			}
			else if (ship->slot[i].type == SLOT_CONTRABAND)
			{
				send_to_char(
					"&+gYou cannot sell your diplomat flag with contraband onboard. Sell the contraband first!&n\n",
					ch);
				return TRUE;
			}
		}
...
void check_contraband(P_ship ship, int to_room)
{
	...
	act_to_all_in_ship(
		ship, "&+BThe port authorities board the ship in search of &+Lcontraband&+B...&n");
	...
	for (int slot = 0; slot < MAXSLOTS; slot++)
	{
		if (ship->slot[slot].type == SLOT_CONTRABAND)
		{
			...
			{
				conf_chance =
					get_property("ship.contraband.baseConfiscationChance",
						     0.0) +
					(float)crates /
						2; // the more contraband you have, the bigger confiscation chance
				conf_chance -= sqrt(ship->frags) / 5.0;
				conf_chance +=
					(100.0 - conf_chance) *
					(1.0 -
					 total_load); // the more total cargo onboard, the less confiscation chance
				if (conf_chance > 100.0)
					conf_chance = 100.0;
				if (conf_chance < 0)
					conf_chance =
						5.0; // always a small chance of being confiscated
			}

			debug("SHIP: (%s) confiscation chance (%f).", SHIP_OWNER(ship),
			      conf_chance);
			...
			{
				sprintf(buf,
					"CONTRABAND: %s had &+W%d&n/&+W%d&n %s&n confiscated in %s [%d]",
				...
				act_to_all_in_ship_f(
					ship,
					"&+L...&+Band find &+Y%d &+Bcrates of &n%s&+B, which they confiscate.&n",
...
int sell_ship(P_char ch, P_ship ship, const char *arg)
{
	...
	{
		send_to_char(
			"&+RIf you are sure you want to sell your ship, type '&+gsell &+Gs&+ghip confirm&+R'.&n\n"
	...
	if (ship->location != ch->in_room)
	{
		...
		if (cost <= 0)
			cost = 1;
		send_to_char(
			"&+gSince your ship is not here, you only get half the price, salvage costs and all!&n\n",
			ch);
	}
	ADD_MONEY(ch, cost);
...
int repair_all(P_char ch, P_ship ship)
{
	...
	for (int j = 0; j < 4; j++)
	{
		int side_damage = (ship->maxarmor[j] - ship->armor[j]) +
				  (ship->maxinternal[j] - ship->internal[j]);
		if (side_damage > 0)
		{
	...
	send_to_char_f(
		ch,
		"&+gThank you for for your business, it will take &n%d &+ghours to complete this repair.&n\n",
		buildtime / 75);
...
int repair_sail(P_char ch, P_ship ship)
{
	...
	if (GET_MONEY(ch) < cost)
	{
		send_to_char_f(
			ch,
			"&+gThis &+Wsail &+gcosts &n%s &+gto repair!  Come back with enough gravy.&n\n",
			coin_stringv(cost));
		return TRUE;
	}
	...
	send_to_char_f(
		ch,
		"&+gThanks for your business, it will take &n%d &+ghours to complete this repair.&n\n",
		buildtime / 75);
	if (!IS_TRUSTED(ch) && BUILDTIME)
		ship->timer[T_MAINTENANCE] += buildtime;
	update_ship_status(ship);
...
int reload_ammo(P_char ch, P_ship ship, char *arg)
{
	...
	else if (isname(arg, "all"))
	{
		...
		for (int slot = 0; slot < MAXSLOTS; slot++)
		{
			...
			{
				weapons_to_reload[slot] = 1;
				cost += (weapon_data[ship->slot[slot].index].ammo -
					 ship->slot[slot].val1) *
					1000;
				buildtime += 75;
			}
	...
	send_to_char_f(
		ch,
		"&+gThanks for your business, it will take &n%d&+g hours to complete this reload.&n\n",
		buildtime / 75);
	if (!IS_TRUSTED(ch) && BUILDTIME)
		ship->timer[T_MAINTENANCE] += buildtime;
	update_ship_status(ship);
...
int rename_ship(P_char ch, P_ship ship, char *new_name)
{
	...
	send_to_char_f(ch,
		       "&+WCongratulations! From now on your ship will be known as '&n%s&+W'.&n\n",
		       arg2);
...
int buy_cargo(P_char ch, P_ship ship, char *arg)
{
	...
	if (SHIP_MAX_CARGO(ship) ==
	    0) // && !(IS_WARSHIP(ship) && isname( GET_NAME(ch), "Soldon")) )
	{
	...
	if (SHIP_AVAIL_CARGO_LOAD(ship) <= 0 &&
	    !(IS_WARSHIP(ship) && isname(GET_NAME(ch), "Soldon")))
	{
		send_to_char("&+gYou don't have any space left on your ship!&n\n", ch);
		return TRUE;
	}
	if (asked_for > SHIP_AVAIL_CARGO_LOAD(ship) &&
	    !(IS_WARSHIP(ship) && isname(GET_NAME(ch), "Soldon")))
	{
	...
	*/
	...
	if (GET_MONEY(ch) < cost)
	{
		send_to_char_f(
			ch,
			"&+gThis cargo load costs &n%s&+g.  Maybe you should do some babysitting first.&n\n",
			coin_stringv(cost));
		return TRUE;
	}
...
int buy_contra(P_char ch, P_ship ship, char *arg)
{
	...
	{
		send_to_char(
			"&+gContraband?! &+w*gasp* &+gHow could you even think that I would sell such things!&n\n",
			ch);
		return TRUE;
	}
	if (SHIP_MAX_CONTRA(ship) ==
	    0) //&& !(IS_WARSHIP(ship) && isname( GET_NAME(ch), "Soldon")) )
	{
	...
	int contra_max = int((float)SHIP_MAX_CONTRA(ship) *
			     ((float)SHIP_MAX_CARGO_LOAD(ship) / (float)SHIP_MAX_CARGO(ship)));
	...
	*/
	...
	{
		send_to_char(
			"&+gYou don't have any space left on your ship to hide contraband!&n\n",
			ch);
		return TRUE;
	}
	...
	*/
	if (asked_for > max_avail)
	{
		send_to_char_f(
			ch,
			"&+gYou only have space for &+Y%d&+g more crates of contraband to hide!&n\n",
			max_avail);
		return TRUE;
	}
	...
	*/
	...
	{
		send_to_char(
			"&+gYou do not have a free slot to fit this &+Lcontraband &+ginto!&n\n",
			ch);
		return TRUE;
	}
	...
	{
		send_to_char_f(ch,
			       "&+gThis contraband costs &n%s&+g; better go sell your sister.&n\n",
			       coin_stringv(cost));
		return TRUE;
	}
	...
	send_to_char(
		"&+gThank you for your 'purchase' &+w*snicker*&+g, your 'cargo' is loaded and set to go!&n\n",
		ch);
...
int buy_weapon(P_char ch, P_ship ship, char *arg1, char *arg2)
{
	...
	if (weapon_data[w].weight > SHIP_AVAIL_WEIGHT(ship))
	{
		send_to_char_f(
			ch,
			"&+gThat weapon weighs &+w%d&+g! Your ship can only support &+w%d&+g more!&n\n",
	...
	if (!ship_allowed_weapons[ship->m_class][w])
	{
		send_to_char_f(
			ch,
			"&+gSuch a great weapon can not be installed on such a small hull!&n\n");
		return TRUE;
	}
	...
	if (same_arc_weight + weapon_data[w].weight >
	    ship_arc_properties[ship->m_class].max_weapon_weight[arc])
	{
		send_to_char_f(
			ch,
			"&+gYour can not put more weight to this side! Maximum allowed: &+w%d&+g.&n\n",
			ship_arc_properties[ship->m_class].max_weapon_weight[arc]);
		return TRUE;
	}
	...
	{
		send_to_char(
			"&+gThat weapon cannot be placed in that position, try another one.&n\n",
			ch);
		return TRUE;
	}
	...
	{
		send_to_char(
			"&+gI'm sorry, but not just anyone can buy this weapon!  You must earn it!&n\n",
			ch);
		return TRUE;
	}
	...
	send_to_char_f(
		ch,
		"&+gThank you for your purchase, it will take &n%d &+ghours to install the part.&n\n",
		buildtime / 75);
	if (!IS_TRUSTED(ch) && BUILDTIME)
		ship->timer[T_MAINTENANCE] += buildtime;
	update_ship_status(ship);
...
int buy_equipment(P_char ch, P_ship ship, char *arg1)
{
	...
	if (equipment_data[equip_num].weight > SHIP_AVAIL_WEIGHT(ship))
	{
		send_to_char_f(
			ch,
			"&+gThat equipment weighs &+w%d&+g! Your ship can only support &+w%d&+g more!&n\n",
	...
	if (!ship_allowed_equipment[ship->m_class][equip_num])
	{
		send_to_char_f(
			ch,
			"&+gSuch marvelous equipment can not be installed on such a pitiful hull!&n\n");
		return TRUE;
	}
	...
	{
		send_to_char(
			"&+gI'm sorry, but not just anyone can buy this equipment!  You must earn it!&n\n",
			ch);
		return TRUE;
	}
	...
	for (j = 0; j < MAXSLOTS; j++)
	{
		if ((ship->slot[j].type == SLOT_EQUIPMENT) && (ship->slot[j].index == equip_num))
		{
			send_to_char_f(
				ch,
				"&+gYou already have the &n%s&+g! You can only have one such item.&n\n",
	...
	if (GET_MONEY(ch) < cost)
	{
		send_to_char_f(
			ch,
			"&+gThis equipment costs &n%s&+g, and you ain't got that much on ya!&n\n",
			coin_stringv(cost));
		return TRUE;
	}
	...
	*/
	...
	send_to_char_f(ch,
		       "Thank you for your purchase, it will take %d hours to install the %s.\r\n",
...
int buy_hull(P_char ch, P_ship ship, int owned, char *arg1, char *arg2)
{
		...
		if (cost >= 0)
		{
			    ...
			    GET_EPIC_POINTS(ch) < SHIPTYPE_EPIC_COST(hull_type))
			{
				if (SHIPTYPE_EPIC_COST(hull_type) > 0)
				{
					send_to_char_f(
						ch,
						"&+gThat upgrade costs &n%s &+gand &+W%d epics&+g!  Come back when you can afford it.&n\n",
				...
				else
				{
					send_to_char_f(
						ch,
						"&+gThat upgrade costs &n%s&+g!  Come back when you can afford it.&n\n",
						coin_stringv(cost));
				}
				return TRUE;
			}
	...
	else
	{
		    ...
		    GET_EPIC_POINTS(ch) < SHIPTYPE_EPIC_COST(hull_type))
		{
			...
			{
				send_to_char_f(ch,
					       "&+gThat hull costs &n%s &+gand &+W%d epics&+g!&n\n",
...
int swap_slots(P_char ch, P_ship ship, char *arg1, char *arg2)
{
	...
	{
		send_to_char(
			"&+YSyntax: '&+gbuy &+Gs&+gwap <slot number 1> <slot number 2>&+Y'.&n\n",
			ch);
		return TRUE;
	}
	int slot1 = atoi(arg1);
...
int ship_shop_proc(int room, P_char ch, int cmd, char *arg)
{
	...
	if ((cmd != CMD_SUMMON) && (cmd != CMD_LIST) && (cmd != CMD_BUY) && (cmd != CMD_RELOAD) &&
	    (cmd != CMD_REPAIR) && (cmd != CMD_SELL))
	{
		return FALSE;
	}
	...
	if (owned)
	{
		if (ship->location != ch->in_room)
		{
			if ((cmd != CMD_SUMMON) && (cmd != CMD_LIST) &&
			    !(cmd == CMD_SELL && isname(arg1, "ship")))
			{
				send_to_char(
					"&+gYour ship needs to be here to receive service.&n\n",
					ch);
				send_to_char_f(
					ch,
		...
		if ((cmd == CMD_SELL || cmd == CMD_SUMMON) && ship->timer[T_MAINTENANCE] > 0)
		{
			send_to_char_f(
				ch,
				"&+gYour ship is currently being worked on, please wait for another &n%.1f&+g hours.&n\n",
				(float)ship->timer[T_MAINTENANCE] / 75.0);
			return TRUE;
		}
		...
		if (cmd == CMD_SELL)
		{
			...
			{
				send_to_char(
					"&+gYour ship is currently pre-occupied in &+rcombat&+g!&n\n",
					ch);
				return TRUE;
			}
			if (*arg1)
		...
		if (cmd == CMD_REPAIR)
		{
			...
			send_to_char(
				"&+YSyntax: '&+grepair <&+Gall&+g|&+Ga&+grmor|&+Gi&+gnternal|&+Gs&+gail|&+Gw&+geapon>&+Y'.&n\n",
				ch);
			return TRUE;
		}
	...
	else if ((cmd == CMD_SUMMON) || (cmd == CMD_RELOAD) || (cmd == CMD_REPAIR) ||
		 (cmd == CMD_SELL))
	{
		send_to_char("&+gBut you do not own a ship!?&n\n", ch);
		return TRUE;
	...
	if (cmd == CMD_LIST)
	{
		...
		send_to_char(
			"Valid syntax is '&+glist <&+Gh&+gull/&+Gw&+geapon/&+Ge&+gquipment/&+Gc&+gargo>'\r\n",
			ch);
		return TRUE;
	}
	...
	if (cmd == CMD_BUY)
	{
		...
		{
			send_to_char(
				"Valid syntax: '&+gbuy &+Gh&+gull|&+Gca&+grgo|&+Gco&+gntraband|&+Gw&+geapon|&+Ge&+gquipment|&+Gr&+gename|&+Gs&+gwap>'.\r\n",
				ch);
			return TRUE;
		}
	}
...
int crew_shop_proc(int room, P_char ch, int cmd, char *arg)
{
	...
	if (IS_RACEWAR_EVIL(ch) && !IS_TRUSTED(ch))
	{
		for (unsigned i = 0; i < sizeof(good_crew_shops) / sizeof(int); i++)
		{
			...
			{
				send_to_char(
					"&+gNoone here wants to talk to such &+Rsuspicious &+gcharacter.&n\n",
					ch);
				return TRUE;
			}
		}
	...
	if (IS_RACEWAR_GOOD(ch) && !IS_TRUSTED(ch))
	{
		for (unsigned i = 0; i < sizeof(evil_crew_shops) / sizeof(int); i++)
		{
			...
			{
				send_to_char(
					"&+gNoone here wants to talk to such &+Ysuspicious &+gcharacter.\r\n",
					ch);
				return TRUE;
			}
		}
	...
	{
		send_to_char(
			"&+YYou ask around and find these people looking for employment:&N\n\n",
			ch);

		send_to_char(
			"                                         Base Skills     Base   Skill Modifiers   Frags  Hiring \n",
			ch);
		send_to_char(
			"    -========== Crews ==========- Lvl Deck/Guns/Repair Stamina Deck/Guns/Repair to hire   Cost  \n",
			ch);
		for (int i = n = 0; i < MAXCREWS; i++)
		{
			if (ship_crew_data[i].hire_room(world[room].number))
			{
				send_to_char_f(
					...
					++n, pad_ansi(ship_crew_data[i].name, 30).c_str(),
					...
					ship_crew_data[i].rpar_mod, ship_crew_data[i].hire_frags,
		...
		send_to_char("\r\n\r\n", ch);
		send_to_char(
			"                                                 Minimum  Skill   Skill     Frags        Hiring\r\n",
			ch);
		send_to_char(
			"-======= Chief members =======-     Speciality    skill    gain  modifier  to hire        cost \r\n\r\n",
			ch);
		...
		for (int i = 0; i < MAXCHIEFS; i++)
		{
			if (ship_chief_data[i].hire_room(world[room].number))
			{
				...
				char name_format[20];
				sprintf(name_format, "%%-%ds&N",
					(int)(strlen(ship_chief_data[i].name) +
					      (30 - strlen(strip_ansi(ship_chief_data[i].name)
								   .c_str()))));
				...
				send_to_char_f(ch, "       &+r%4d    &+W%20s\r\n",
					       ship_chief_data[i].hire_frags,
					       coin_stringv(ship_chief_data[i].hire_cost));
			}
		}
		send_to_char("\r\n\r\n", ch);
		look_crew(ch, ship);
	...
	if (cmd == CMD_HIRE)
	{
		...
		for (int i = 0; i < MAXCREWS; i++)
		{
			if (ship_crew_data[i].hire_room(world[room].number) && ++c == n)
			{
				...
				{
					send_to_char(
						"&+wNever heard of you and y'er crew looks weak, get lost!\r\n",
						ch);
					return TRUE;
				}
				...
				SUB_MONEY(ch, cost, 0);
				send_to_char(
					"Aye aye cap'n!  We'll be on yer ship before you board!\r\n",
					ch);
		...
		for (int i = 0; i < MAXCHIEFS; i++)
		{
			...
			{
				if (i == ship->crew.sail_chief || i == ship->crew.guns_chief ||
				    i == ship->crew.rpar_chief)
				{
					send_to_char("&+wYou already have this chief!\r\n", ch);
					return TRUE;
				}
				if (ship->frags < ship_chief_data[i].hire_frags &&
				    ((ship_chief_data[i].type == SAIL_CHIEF &&
				      ship->crew.sail_skill < ship_chief_data[i].min_skill) ||
				     (ship_chief_data[i].type == GUNS_CHIEF &&
				      ship->crew.guns_skill < ship_chief_data[i].min_skill) ||
				     (ship_chief_data[i].type == RPAR_CHIEF &&
				      ship->crew.rpar_skill < ship_chief_data[i].min_skill)))
				{
					send_to_char(
						"&+wI won't work with these greenhorns unless their captain is someone famous.\r\n",
						ch);
					return TRUE;
				}
				...
				SUB_MONEY(ch, cost, 0);
				send_to_char(
					"Aye aye cap'n!  I'll be on yer ship before you board!\r\n",
					ch);
				set_chief(ship, i);
				update_ship_status(ship);
				queue_ship_save(ship, "chief hire");
...
#define IS_MOONSTONE_FRAGMENT(obj) \
	(obj_index[obj->R_num].virtual_number == AUTOMATONS_MOONSTONE_FRAGMENT)
	...
	if (cmd == CMD_ASK)
	{
		...
		if (P_obj moonstone = has_moonstone(pl))
		{
			if (*arg2)
			{
				...
				{
					if (!ship || (ship->frags <
						      ship_crew_data[AUTOMATON_CREW].hire_frags))
					{
						send_to_char(
							"Erzul says '&+GI'm sorry, but you are not famous enough to be trusted with this crew!\r\n",
							pl);
						send_to_char(
							"&+G  But... if you &+Wask&+G for your &+Wreward&+G you will receive it!&N'\r\n",
							pl);
						return TRUE;
					}
					else
					{
						send_to_char_f(
							pl,
							"Erzul says '&+GOkay, i heard about you. I will trust you with my best creation!\r\n");
						send_to_char_f(
							pl,
							"&+G  But i need about &+W%s &+Gto start this work!&N'\r\n",
							coin_stringv(cost));
						...
						send_to_char_f(
							pl,
							"You hand over the &+Wmoonstone&N and the &+Wmoney&N.\r\n");
						SUB_MONEY(pl, cost, 0);
						...
						send_to_char(
							"Erzul says '&+GThey'll be at your ship by the time you get there!&N'\r\n",
							pl);
						return TRUE;
					}
				}
			...
			send_to_char(
				"Erzul says '&+GYou have it! You have the moonstone! &+WAsk&+G for your &+Wreward&+G and you will receive it!&N'\r\n",
				pl);
			if (ship && (ship->frags >= ship_crew_data[AUTOMATON_CREW].hire_frags))
			{
				send_to_char(
					"Erzul says '&+GI could even trust you with my &+Wautomatons &+Gif you &+Wask &+Gfor them!&N'\r\n",
					pl);
			}
			return TRUE;
		}
		if (*arg2)
		{
			...
			{
				send_to_char(
					"Erzul says '&+GThat cursed &+WXexos&N!  &+GNow I can't complete my masterpiece!'\r\n",
					pl);
				return TRUE;
			}
			...
			{
				send_to_char(
					"Erzul says '&+GYes, Xexos, the bastard who stole the &+Wmoonstone&N &+Gheart i've created!&N\r\n",
					pl);
				return TRUE;
			}
			...
			{
				send_to_char(
					"Erzul says '&+GThe moonstone was my life's work.  It's the power source I was\r\n",
					pl);
				send_to_char(
					"&+g  going to use to power my &+Wautomatons&N, &+gmy masterpiece!&N'",
					pl);
				return TRUE;
			}
			if (isname(arg2, "automaton automatons"))
			{
				send_to_char(
					"Erzul says '&+GYes, automatons, ultimate ship crew made of powerful and skilled golems.\r\n",
					pl);
				send_to_char(
					"  &+GIf only someone brought back my moonstone, I could astound the world with my greatest work!\r\n",
					pl);
				send_to_char(
					"  &+GThough it would &+Wcost&+G a fortune to make them...&N'\r\n",
					pl);
				return TRUE;
			}
			if (isname(arg2, "cost money"))
			{
				send_to_char_f(
					pl,
					"Erzul says '&+GHow much? I would need about &+W%s &+Gfor that work.\r\n",
					coin_stringv(cost));
				send_to_char_f(
					pl,
					"&+G  Does it even matter now, as i can not even start it!'\r\n");
				return TRUE;
			}
		}
		return FALSE;
