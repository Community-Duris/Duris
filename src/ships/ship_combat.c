	...
	for (i = 0; i < k; i++)
	{
		if (target == contacts[i].ship)
		{
			if (contacts[i].range <= 20.0 + (float)ship->crew.get_contact_range_mod())
			{
				...
				send_to_char_f(
					ch,
					"&+LID:[&+W%s&+L]&N %-20s                Class:&+y%s&N\r\n",
					SHIP_ID(target), SHIP_NAME(target),
					SHIP_CLASS_NAME(target));
				...
				send_to_char_f(
					...
					SHIP_ARMOR_COND(SHIP_MAX_FARMOR(target),
							SHIP_FARMOR(target)),
					SHIP_FARMOR(target), SHIP_MAX_FARMOR(target),
					SHIP_INTERNAL_COND(SHIP_MAX_FINTERNAL(target),
							   SHIP_FINTERNAL(target)),
					SHIP_FINTERNAL(target), SHIP_MAX_FINTERNAL(target),
					contacts[i].x, contacts[i].y, contacts[i].z);

				send_to_char_f(
					ch,
					"Starboard  %s%3d&N/&+G%-3d&N  %s%2d&N/&+g%-2d&N     Range  :&+W%-5.1f&N\r\n",
					SHIP_ARMOR_COND(SHIP_MAX_SARMOR(target),
							SHIP_SARMOR(target)),
					SHIP_SARMOR(target), SHIP_MAX_SARMOR(target),
					SHIP_INTERNAL_COND(SHIP_MAX_SINTERNAL(target),
							   SHIP_SINTERNAL(target)),
					SHIP_SINTERNAL(target), SHIP_MAX_SINTERNAL(target),
					contacts[i].range);

				send_to_char_f(
					ch,
					"Rear       %s%3d&N/&+G%-3d&N  %s%2d&N/&+g%-2d&N     Heading:&+W%d&N\r\n",
					SHIP_ARMOR_COND(SHIP_MAX_RARMOR(target),
							SHIP_RARMOR(target)),
					SHIP_RARMOR(target), SHIP_MAX_RARMOR(target),
					SHIP_INTERNAL_COND(SHIP_MAX_RINTERNAL(target),
							   SHIP_RINTERNAL(target)),
					SHIP_RINTERNAL(target), SHIP_MAX_RINTERNAL(target),
					(int)target->heading);

				send_to_char_f(
					ch,
					"Port       %s%3d&N/&+G%-3d&N  %s%2d&N/&+g%-2d&N     Bearing:&+W%-3d&N Arc:&+W%s&N\r\n",
					SHIP_ARMOR_COND(SHIP_MAX_PARMOR(target),
							SHIP_PARMOR(target)),
					SHIP_PARMOR(target), SHIP_MAX_PARMOR(target),
					SHIP_INTERNAL_COND(SHIP_MAX_PINTERNAL(target),
							   SHIP_PINTERNAL(target)),
					SHIP_PINTERNAL(target), SHIP_MAX_PINTERNAL(target),
				...
				send_to_char_f(
					...
					contacts[i].range < SCAN_RANGE ?
						(!SHIP_DOCKED(target) ?
							 ((target->race == EVILSHIP) ?
								  "&+RThis ship has an Evil flag.&N" :
							  (target->race == UNDEADSHIP) ?
								  "&+LThis ship has a tattered black flag.&N" :
							  (target->race == GOODIESHIP) ?
								  "&+YThis ship has a goodie flag.&N" :
							  (target->race == SQUIDSHIP) ?
								  "&+MThis ship has a very strange flag.&N" :
								  ...
								  (target->race == NPCSHIP) ?
								  "&nThis ship has a very plain flag.&N" :
							  (target->race == UNKNOWNSHIP) ?
								  "&+CThis ship has a foreign flag.&N" :
				...
				for (j = 0; j < MAXSLOTS; j++)
				{
					...
					{
						send_to_char_f(
							ch, "&+W[%d]&N &+W%-20s&N &+W%-9s\r\n", j,
							SHIP_WEAPON_DESTROYED(target, j) ?
								"&+LDestroyed&N" :
								target->slot[j].get_description(),
...
bool ship_gain_money(P_ship ship, P_ship target, int salvage, int bounty)
{
	...
	if (bounty > 0)
	{
		if (target->frags > 1000)
			act_to_all_in_ship_f(
				ship,
				"In addition to salvage, you recieve a bounty of %s for sinking THIS ship.\r\n",
				coin_stringv(bounty));
		else
			act_to_all_in_ship_f(
				ship,
...
bool sink_ship(P_ship ship, P_ship attacker)
{
	...
	if (attacker)
	{
		...
		logit(LOG_SHIP, "%s's ship sunk by %s.",
		      (ship->ownername) ? ship->ownername : "A pirate",
		      (attacker->ownername) ? attacker->ownername : "a pirate");
		statuslog(AVATAR, "%s's ship sunk by %s.",
			  (ship->ownername) ? ship->ownername : "A pirate",
			  (attacker->ownername) ? attacker->ownername : "a pirate");
	}
	else
	...
	if (IS_WATER_ROOM(ship->location))
	{
		...
		act_to_outside_ships(ship, ship, DEFAULT_RANGE, "&+W[%s]&N:%s &+Wstarts to sink!&N",
				     SHIP_ID(ship), SHIP_NAME(ship));
		act_to_all_in_ship(ship, "&=LRYOUR SHIP STARTS TO SINK!!&N");
	}
	else
	{
		act_to_outside(ship, DEFAULT_RANGE, "%s &+Wstarts to fall apart!&N",
			       SHIP_NAME(ship));
		act_to_outside_ships(ship, ship, DEFAULT_RANGE,
				     "&+W[%s]&N:%s &+Wstarts to fall apart!&N", SHIP_ID(ship),
				     SHIP_NAME(ship));
	...
	if (attacker)
	{
		...
		if (IS_NPC_SHIP(attacker))
		{
			if (attacker->npc_ai && attacker->npc_ai->escort &&
			    !IS_NPC_SHIP(attacker->npc_ai->escort))
		...
		if (gainer)
		{
			...
			for (i = 0; i < k; i++)
			{
				...
				if (contacts[i].ship->race == gainer->race ||
				    (contacts[i].ship->npc_ai && contacts[i].ship->npc_ai->escort &&
				     contacts[i].ship->npc_ai->escort->race == gainer->race))
				{
					fleet_size_total++;
				}
		...
		if (attacker->target == ship)
		{
			...
			if (attacker == cyrics_revenge &&
			    IS_NPC_SHIP(
				    ship)) // dirty hack to counter revenge's appearing in pirate fight
...
void volley_hit_event(P_char /*ch*/, P_char /*victim*/, P_obj /*obj*/, void *data)
{
	...
	if (target->timer[T_BSTATION] == 0)
		act_to_all_in_ship(target,
				   "&=LRYour crew scrambles to their battlestations&N!\r\n");
	...
	{ // we have a hit!
		...
		float range_mod = ((float)weapon_data[w_index].max_range - range) /
				  ((float)weapon_data[w_index].max_range -
				   (float)weapon_data[w_index].min_range);
		if (range_mod < 0.0)
			range_mod = 0.0;
		for (int f_no = 0; f_no < weapon_data[w_index].fragments; f_no++)
		{
			if (weapon_data[w_index].max_damage > 0)
			{
				...
				{
					damage = weapon_data[w_index].min_damage +
						 (int)((float)(weapon_data[w_index].max_damage -
							       weapon_data[w_index].min_damage) *
						       range_mod);
				}
				else
				{
					damage = number(weapon_data[w_index].min_damage,
							weapon_data[w_index].max_damage);
				}

				if ((SHIP_SAIL(target) > 0) &&
				    (number(0, 99) < weapon_data[w_index].sail_hit))
				{ // hitting sails
					damage =
						(int)((float)damage *
						      (float)weapon_data[w_index].sail_dam / 100.0);
					damage_sail(ship, target, damage);
				}
				else
				{ // hitting hull
					float hit_dir = your_bearing +
							number(-(weapon_data[w_index].hit_arc / 2),
							       (weapon_data[w_index].hit_arc / 2));
					if (hit_dir >= 360)
						hit_dir -= 360;
					else if (hit_dir < 0)
						hit_dir = 360 + hit_dir;
					...
					damage =
						(int)((float)damage *
						      (float)weapon_data[w_index].hull_dam / 100.0);
					damage_hull(ship, target, damage, hit_arc,
						    weapon_data[w_index].armor_pierce);
				}
			}
			//
		...
		if (IS_SET(weapon_data[w_index].flags, MINDBLAST))
		{
			...
			act_to_all_in_ship(
				target, "&+MYour crew is completely disoriented by the blast!&N");
			if (range <=
			    (weapon_data[w_index].max_range + weapon_data[w_index].min_range) / 2)
			{
				stun_all_in_ship(target, PULSE_VIOLENCE * 2);
				act_to_all_in_ship(
					target,
					"&+MYour mind reels from the blast, you huddle in pain!&N");
			}
			act_to_all_in_ship_f(
				ship, "You hit &+W[%s]&N:%s&N with a powerful &+Mmental blast&N!",
				SHIP_ID(target), SHIP_NAME(target));
			act_to_outside_ships(
				target, ship, DEFAULT_RANGE,
				"&+W[%s]&N:%s&N has been hit with a powerful &+Mmental blast&N!",
				SHIP_ID(target), SHIP_NAME(target));
			int stuntime = (int)(5.0 + 15.0 * range_mod);
			if (target == cyrics_revenge)
				stuntime /= 3; // demons are resistant to mindblast
	...
	{
		act_to_all_in_ship_f(ship, "You miss &+W[%s]&N:%s&N.", SHIP_ID(target),
				     SHIP_NAME(target));
		act_to_all_in_ship_f(target, "&+W[%s]&N:%s&N misses your ship.", SHIP_ID(ship),
				     SHIP_NAME(ship));
		act_to_outside_ships(target, ship, DEFAULT_RANGE,
				     "&+W[%s]&N:%s&N misses &+W[%s]&N:%s&N.", SHIP_ID(ship),
				     SHIP_NAME(ship), SHIP_ID(target), SHIP_NAME(target));
	}
	return;
}
...
int damage_sail(P_ship attacker, P_ship target, int dam)
{
	...
	if (attacker)
	{
		act_to_all_in_ship_f(
			attacker,
			"You hit &+W[%s]&N:%s&N for &+G%d&N %s of damage on the &+Wsails&N!",
			SHIP_ID(target), SHIP_NAME(target), dam, (dam == 1) ? "point" : "points");
	}
	act_to_all_in_ship_f(
		target, "&+WYour ship&N has been hit for &+R%d&N %s of damage on the &+Wsails&N!",
		dam, (dam == 1) ? "point" : "points");
	act_to_outside_ships(target, attacker, DEFAULT_RANGE,
			     "&+W[%s]&N:%s&N has been hit for %d %s of damage on the &+Wsails&N!",
			     SHIP_ID(target), SHIP_NAME(target), dam,
			     (dam == 1) ? "point" : "points");
...
int damage_hull(P_ship attacker, P_ship target, int dam, int arc, int armor_pierce)
{
	...
	{
		act_to_all_in_ship_f(
			attacker, "You hit &+W[%s]&N:%s&N for &+G%d&N %s of damage on the %s side!",
			SHIP_ID(target), SHIP_NAME(target), dam, (dam == 1) ? "point" : "points",
			get_arc_name(arc));
	}
	act_to_all_in_ship_f(target,
			     "&+WYour ship&N has been hit for &+R%d&N %s of damage on the %s side!",
			     dam, (dam == 1) ? "point" : "points", get_arc_name(arc));
	act_to_outside_ships(target, attacker, DEFAULT_RANGE,
			     "&+W[%s]&N:%s&N has been hit for %d %s of damage on the %s side!",
			     SHIP_ID(target), SHIP_NAME(target), dam,
			     (dam == 1) ? "point" : "points", get_arc_name(arc));
...
int ch_damage_hull(P_char attacker, P_ship target, int dam, int arc, int armor_pierce)
{
	...
	act_to_all_in_ship_f(target,
			     "&+WYour ship&N has been hit for &+R%d&N %s of damage on the %s side!",
			     dam, (dam == 1) ? "point" : "points", get_arc_name(arc));
	act_to_outside_ships(target, target, DEFAULT_RANGE,
			     "&+W[%s]&N:%s&N has been hit for %d %s of damage on the %s side!",
			     SHIP_ID(target), SHIP_NAME(target), dam,
			     (dam == 1) ? "point" : "points", get_arc_name(arc));
...
void damage_weapon(P_ship attacker, P_ship target, int arc, int dam)
{
	...
	for (j = 0; j < MAXSLOTS; j++)
	{
		if ((target->slot[j].type == SLOT_WEAPON) && (target->slot[j].position == arc) &&
		    !SHIP_WEAPON_DESTROYED(target, j))
			arc_weapons[i++] = j;
	}
	if (i > 0)
	{
		...
		{
			act_to_all_in_ship_f(target, " &+RYour &+W%s &+Rhas been destroyed!&N",
		...
		{
			act_to_all_in_ship_f(target, " &+WYour %s has been damaged!&N",
					     target->slot[w_index].get_description());
			if (attacker)
			{
...
void force_anchor(P_ship ship)
{
	...
	act_to_outside_ships(ship, ship, DEFAULT_RANGE,
			     "&+W[%s]&N:%s&N suddenly slows rapidly!\r\n", SHIP_ID(ship),
			     SHIP_NAME(ship));
...
void update_ship_status(P_ship ship, P_ship attacker)
{
	...
	if (!SHIP_DOCKED(ship))
	{
		if (SHIP_IMMOBILE(ship) && !currently_immobile)
		{
			...
			act_to_outside_ships(ship, ship, DEFAULT_RANGE,
					     "&+W[%s]&N:%s &+Ris immobilized!", SHIP_ID(ship),
					     SHIP_NAME(ship));
		}
		if (!SHIP_IMMOBILE(ship) && currently_immobile)
		{
...
int try_ram_ship(P_ship ship, P_ship target, float tbearing)
{
	...
	if (SHIP_FLYING(ship) != SHIP_FLYING(target))
	{
		if (SHIP_FLYING(ship))
			act_to_all_in_ship(
				ship, "&+WYou try to ram but pass above your target instead!&N");
		else
			act_to_all_in_ship(
				ship, "&+WYou try to ram but pass below your target instead!&N");
		if (IS_SET(ship->flags, RAMMING))
			REMOVE_BIT(ship->flags, RAMMING);
		return FALSE;
	}
	...
	if (counter_speed < 0)
	{
		...
		if (ram_speed <= 0)
		{
			act_to_all_in_ship(ship,
					   "&+WTarget speeds away from your attempt to ram it!&N");
			if (IS_SET(ship->flags, RAMMING))
				REMOVE_BIT(ship->flags, RAMMING);
			return FALSE;
		}
	...
	float speed_mod = (((float)ship->speed / (float)SHIPTYPE_SPEED(ship->m_class)) +
			   ((float)target->speed / (float)SHIPTYPE_SPEED(target->m_class)) * 3) /
			  4.0;
	...
	act_to_all_in_ship_f(ship, "You attempt to ram &+W[%s]&N: %s! Chance to hit: &+W%d%%&N",
			     SHIP_ID(target), SHIP_NAME(target), hit_chance);
	if (target->timer[T_BSTATION] == 0)
		act_to_all_in_ship(target,
				   "&=LRYour crew scrambles madly to battlestations!&N\r\n");
	target->timer[T_BSTATION] = BSTATION;

	set_pvp_on_passengers(ship);
	set_pvp_on_passengers(target);

	act_to_all_in_ship_f(target, "&+W[%s]&N: %s attempts to ram you!", SHIP_ID(ship),
			     SHIP_NAME(ship));
	act_to_outside_ships(ship, target, DEFAULT_RANGE,
			     "&+W[%s]&N:%s&N attempts to ram &+W[%s]&N:%s&N!", SHIP_ID(ship),
			     SHIP_NAME(ship), SHIP_ID(target), SHIP_NAME(target));
	...
	{ // Successful ram
		if (ram_speed < ship->speed / 2)
		{
			act_to_all_in_ship_f(
				ship,
				"&+yYour ship &+Llurches &+yas you &+rcrunch &+yinto the stern of &+W[%s]&N:%s&+y!&N",
				SHIP_ID(target), SHIP_NAME(target));
			act_to_all_in_ship_f(
				target,
				"&+W[%s]&N:%s &+rcrunches &+yinto the stern of your ship!&N",
				SHIP_ID(ship), SHIP_NAME(ship));
			act_to_outside_ships(
				target, ship, DEFAULT_RANGE,
				"&+W[%s]&N:%s &+rcrunches &+yinto the stern of &+W[%s]&N:%s&+y!&N",
				SHIP_ID(ship), SHIP_NAME(ship), SHIP_ID(target), SHIP_NAME(target));
		}
		else if (counter_speed > ram_speed / 2)
		{
			act_to_all_in_ship_f(
				ship,
				"&+yTimbers &+rcrunch &+yand &+Ycrack &+yas you &=LRcollide &+yhead on with &+W[%s]&N:%s&+y!&N",
				SHIP_ID(target), SHIP_NAME(target));
			act_to_all_in_ship_f(
				target,
				"&+yTimbers &+rcrunch &+yand &+Ycrack &+yas &+W[%s]&N:%s &=LRcollides &+yhead on with your ship!&N",
				SHIP_ID(ship), SHIP_NAME(ship));
			act_to_outside_ships(
				target, ship, DEFAULT_RANGE,
				"&+W[%s]&N:%s &+Rcollides &+y head on with &+W[%s]&N:%s&+y!&N",
				SHIP_ID(ship), SHIP_NAME(ship), SHIP_ID(target), SHIP_NAME(target));
		}
		else
		{
			act_to_all_in_ship_f(
				ship,
				"&+yTimbers &+rcrunch &+yand &+Ycrack &+yas you &=LRcrash &+yinto &+W[%s]&N:%s&+y!&N",
				SHIP_ID(target), SHIP_NAME(target));
			act_to_all_in_ship_f(
				target,
				"&+yTimbers &+rcrunch &+yand &+Ycrack &+yas &+W[%s]&N:%s &=LRcrashes &+yinto your ship!&N",
				SHIP_ID(ship), SHIP_NAME(ship));
			act_to_outside_ships(target, ship, DEFAULT_RANGE,
					     "&+W[%s]&N:%s &+Rcrashes &+yinto &+W[%s]&N:%s&+y!&N",
					     SHIP_ID(ship), SHIP_NAME(ship), SHIP_ID(target),
					     SHIP_NAME(target));
		}
		...
		ram_damage =
			(int)((float)ram_damage *
			      (0.1 +
			       0.4 * (float)ship->speed / (float)SHIPTYPE_SPEED(ship->m_class) +
			       0.5 * (float)ram_speed / (float)SHIPTYPE_SPEED(ship->m_class)));
		if (sarc == SIDE_FORE)
			ram_damage = (int)((float)ram_damage * 1.2);

		int counter_damage = (SHIP_HULL_WEIGHT(target) + 100) / 10;
		counter_damage =
			(int)((float)counter_damage *
			      (0.1 +
			       0.3 * (float)ship->speed / (float)SHIPTYPE_SPEED(ship->m_class) +
			       0.4 * (float)counter_speed /
				       (float)SHIPTYPE_SPEED(target->m_class)));
		if (tarc == SIDE_FORE)
		...
		while (ram_damage != 0 || counter_damage != 0)
		{
			...
			if (hit_target)
			{
				...
				else
				{
					...
					if (target_eram &&
					    hit_arc ==
						    SIDE_FORE) // equiped ram partially protects fore side
						dam = MAX(1, dam / 2);
			...
			{
				if (dam > counter_damage)
					dam = counter_damage;
				counter_damage -= dam;
				if (SHIP_HULL_WEIGHT(target) >= SHIP_HULL_WEIGHT(ship) &&
				    number(0, 99) < 10)
				{
		...
		float whole_crew_mod = (ship->crew.sail_mod_applied + ship->crew.guns_mod_applied +
					ship->crew.rpar_mod_applied) /
				       3.0;
		ship->timer[T_RAM] = MAX(1, (int)(50.0 * (1.0 - whole_crew_mod * 0.15)));
		ship->timer[T_RAM_WEAPONS] =
			MAX(1, (int)(25.0 * (1.0 - ship->crew.guns_mod_applied * 0.15)));
	}
	else
	{
...
int weaponsight(P_ship ship, int slot, int t_contact, P_char /*ch*/)
{
	...
	float ortogonal_speed =
		sin(tr_rad) * (float)target->speed; // targets ortogonal speed in 'ship units'

	// second, calculating projected change in relative position/heading
	float t_curr_bearing =
		contacts[t_contact].bearing - ship->heading; // target relative bearing this turn
	if (t_curr_bearing < 0)
		t_curr_bearing += 360;
	...
	float t_next_x = (float)contacts[t_contact].x + (target->x - 50.0) +
			 (t_next_speed * sin(rad)) / 150.000;
	float t_next_y = (float)contacts[t_contact].y + (target->y - 50.0) +
			 (t_next_speed * cos(rad)) / 150.000;
	...
	if (t_next_bearing < 0)
		t_next_bearing += 360;
	float t_next_range = range(s_next_x, s_next_y, ship->z, t_next_x, t_next_y,
				   target->z); // target range next turn

	// send_to_char_f(ch, "snt=%5.2f,snx=%5.2f,sny=%5.2f,snh=%5.2f,tnt=%5.2f,tnx=%5.2f,tny=%5.2f,tnh=%5.2f   tcb=%5.2f,tcr=%5.2f,tnb=%5.2f,tnr=%5.2f", s_next_turn, s_next_x, s_next_y, s_next_heading,
	// t_next_turn, t_next_x, t_next_y, t_next_heading, t_curr_bearing, t_curr_range, t_next_bearing, t_next_range);
	...
	{
		speed = ABS(ortogonal_speed) / 2 + ABS(angle_speed) * 3 +
			ABS(closing_speed); // catapults mostly depend on approaching speed
	}
	else
	{
		speed = ABS(ortogonal_speed) + ABS(angle_speed) * 4 +
			ABS(closing_speed) /
				4; // straight weapons almost exclusively depend on ortogonal and angle speed
	}
	...
	float size_to_speed_mod =
		1 +
		(size_mod / 100) / hit_chance; // the faster it goes, the more hullsize affects it
	...
	float range_mod =
		(max - t_curr_range) /
		(max *
		 0.75); // if range below max/4, there is no-penalty (and yes, too bad for longtom)
	if (range_mod < 0.0)
		range_mod = 0.0;
	if (range_mod > 1.0)
		range_mod = 1.0;

	miss_chance = max_range_miss -
		      (max_range_miss - min_range_miss) *
			      range_mod; // ok here we have our miss chance for the basic crew

	// send_to_char_f(ch, " hit_3=%5.2f", (1.0 - miss_chance) * 100);

	miss_chance /= (1.0 + ship->crew.guns_mod_applied);
	if (SHIP_FLYING(target))
		miss_chance = MIN(miss_chance * 1.5, 0.99);
...
int fire_weapon(P_ship ship, int w_num, int t_contact, int hit_chance, P_char ch)
{
	...
	act_to_outside(ship, DEFAULT_RANGE, "%s&N fires %s at %s!", ship->name,
		       ship->slot[w_num].get_description(), target->name);
	act_to_outside_ships(ship, target, DEFAULT_RANGE,
			     "&+W[%s]&N:%s&N fires %s at &+W[%s]&N:%s!", SHIP_ID(ship), ship->name,
			     ship->slot[w_num].get_description(), target->id, target->name);

	// setting volley
	float volley_time = (range / (float)weapon_data[w_index].max_range) *
			    (float)weapon_data[w_index].volley_time;
	...
	if (ship->slot[w_num].val1 > 0)
	{
		float reload_time = (float)weapon_data[w_index].reload_time *
				    (1.0 - ship->crew.guns_mod_applied * 0.15);
		reload_time /= ship->crew.get_stamina_mod();
		ship->slot[w_num].timer = MAX(1, (int)reload_time);
	}

	// reducing crew stamina
	ship->crew.reduce_stamina((float)weapon_data[w_index].weight / (SHIP_HULL_MOD(ship) / 10.0),
				  ship);
	if (HAS_VALID_TARGET(ship))
