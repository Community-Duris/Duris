...
void stun_all_in_ship(P_ship ship, int timer)
{
	...
	for (i = 0; i < ship->room_count; i++)
	{
		...
		{
			for (ch = world[real_room(SHIP_ROOM_NUM(ship, i))].people; ch; ch = ch->next_in_room)
			{
				if (ch && !IS_TRUSTED(ch))
				{
			...
			if (contacts[i].range <= 20.0 + (float)ship->crew.get_contact_range_mod())
			{
				...
				send_to_char_f(ch,
				               ...
				               : SHIP_DOCKED(target)   ? "&+yDOCKED&N"
				                                       : "",
				               contacts[i].range < SCAN_RANGE ? (!SHIP_DOCKED(target) ? ((target->race == EVILSHIP)     ? "&+RThis ship has an Evil flag.&N"
				...
				for (j = 0; j < MAXSLOTS; j++)
				{
					...
					{
						send_to_char_f(
							ch, "&+W[%d]&N &+W%-20s&N &+W%-9s\r\n", j, SHIP_WEAPON_DESTROYED(target, j) ? "&+LDestroyed&N" : target->slot[j].get_description(), target->slot[j].get_position_str());
					}
				}
			}
			else
...
int calc_frag_gain(P_ship ship) { return SHIP_HULL_WEIGHT(ship); }

int calc_bounty(P_ship target) { return (int)(target->frags * 10000 / get_property("ship.sinking.rewardDivider", 7.0)); }
...
bool sink_ship(P_ship ship, P_ship attacker)
{
	...
	if (attacker)
	{
		...
		logit(LOG_SHIP, "%s's ship sunk by %s.", (ship->ownername) ? ship->ownername : "A pirate", (attacker->ownername) ? attacker->ownername : "a pirate");
		statuslog(AVATAR, "%s's ship sunk by %s.", (ship->ownername) ? ship->ownername : "A pirate", (attacker->ownername) ? attacker->ownername : "a pirate");
	}
	else
	{
		...
		if (gainer)
		{
			...
			for (i = 0; i < k; i++)
			{
				...
				if (contacts[i].ship->m_class == SH_SLOOP)
					continue;
				if (contacts[i].ship->race == gainer->race || (contacts[i].ship->npc_ai && contacts[i].ship->npc_ai->escort && contacts[i].ship->npc_ai->escort->race == gainer->race))
				{
					fleet_size_total++;
				}
			...
			for (i = 0; i < k; i++)
			{
				...
				P_char ch2 = get_char2(str_dup(SHIP_OWNER(contacts[i].ship)));
				if ((contacts[i].ship == gainer) || (ch1 && ch2 && ch2->group && ch2->group == ch1->group))
				{
...
int damage_sail(P_ship attacker, P_ship target, int dam)
{
	...
	act_to_outside_ships(
		target, attacker, DEFAULT_RANGE, "&+W[%s]&N:%s&N has been hit for %d %s of damage on the &+Wsails&N!", SHIP_ID(target), SHIP_NAME(target), dam, (dam == 1) ? "point" : "points");
...
int ch_damage_hull(P_char attacker, P_ship target, int dam, int arc, int armor_pierce)
{
	...
	act_to_all_in_ship_f(target, "&+WYour ship&N has been hit for &+R%d&N %s of damage on the %s side!", dam, (dam == 1) ? "point" : "points", get_arc_name(arc));
	act_to_outside_ships(
		target, target, DEFAULT_RANGE, "&+W[%s]&N:%s&N has been hit for %d %s of damage on the %s side!", SHIP_ID(target), SHIP_NAME(target), dam, (dam == 1) ? "point" : "points", get_arc_name(arc));
