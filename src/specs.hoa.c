...
int trap_razor_hooks(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_DOWN)
	{
		...
		{
			act("&+LYou feel sharp and terrible pain as you traverse the walls.", TRUE,
			    ch, 0, 0, TO_CHAR);
...
int trap_tower1_para(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_UP)
	{
		...
		{
			act("&+wYou freeze in your tracks and gaze in awe at the sight of &+BRas&+bda&+Brn&+w.&n",
			    TRUE, ch, 0, 0, TO_CHAR);
			if (!number(0, 2))
...
int trap_tower2_sleep(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_UP)
	{
		...
		{
			act("&+LYou suddenly begin to feel very drowsy.&n", TRUE, ch, 0, 0,
			    TO_CHAR);
			if (!number(0, 2))
...
int illesarus(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	{
		// The damage
		act("&+LYou swing&n $q &+Lwith a broad two-handed arch at&n $N.&n", FALSE, ch, obj,
		    vict, TO_CHAR);
		act("$n &+Lswings&n $q &+Lwith a broad two-handed arch at you.&n", FALSE, ch, obj,
		    vict, TO_VICT);
		...
		if (limb)
		{
			...
			{
			case LIMB_ARM:
				act("$N &+Lgasps as your vorpal sword nearly severs $S arm!&n",
				    FALSE, ch, obj, vict, TO_CHAR);
				act("$q &+Lnearly severs your arm from your body!&n", FALSE, ch,
				    obj, vict, TO_VICT);
				act("$N &+Lgasps as the vorpal sword nearly severs $S arm!&n",
				...
				break;
			case LIMB_LEG:
				act("$N g&+Lasps as your vorpal sword nearly severs $S leg!&n",
				    FALSE, ch, obj, vict, TO_CHAR);
				act("$q &+Lnearly severs your leg from your body!&n", FALSE, ch,
				    obj, vict, TO_VICT);
				act("$N &+Lgasps as the vorpal sword nearly severs $S leg!&n",
				    FALSE, ch, obj, vict, TO_NOTVICT);
				if (!IS_TLEGLOCK(vict))
				{
					af.type = TAG_LEGLOCK;
...
int morkoth_mother(P_char ch, P_char tch, int cmd, char *arg)
{
	...
	if (!tch && !number(0, 4))
	{
		return shout_and_hunt(ch, 100,
				      "&+WChildren, come defend your mother against %s!&n\r\n",
				      NULL, helpers, 0, 0);
	}
	return FALSE;
}
...
int human_girl(P_char ch, P_char vict, int cmd, char *arg)
{
	...
	if (!number(0, 9))
	{
		...
		case 1:
			say(ch,
			    "Be warned, Aan does not take kindly to those who seek to defame his cathedral.");
			break;
		case 2:
			say(ch,
			    "Strife, Sin, and Death await those who do not bow to the power that is Aan.");
			break;
		}
		return FALSE;
	}
...
int hoa_plat(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	{
		act("&+LA large rumble of stones moving can be heard from the other room.&n", FALSE,
		    ch, 0, 0, TO_ROOM);
		act("&+LA large rumble of stones moving can be heard from the other room.&n", FALSE,
		    ch, 0, 0, TO_CHAR);
...
int hoa_death(P_char ch, P_char vict, int cmd, char *arg)
{
	...
	if (cmd == CMD_DEATH)
	{
		act("&+LDeath &+WSHRIEKS&+L as his power dissipates around this cathedral.&n",
	...
	if (IS_FIGHTING(ch))
	{
		...
		if (!number(0, 35) || IS_NPC(victim))
		{
			...
			act("&+LDeath smirks as he reaches out to touch $N, who suddenly collapses.&n",
			    FALSE, ch, 0, victim, TO_NOTVICT);
			act("&+LDeath smirks as he reaches out to touch you.  You simply collapse.&n",
	...
	if (ch && IS_FIGHTING(ch))
		if (!number(0, 4))
		{
			...
			if (!victim || GET_CLASS(victim, CLASS_PALADIN) || IS_TRUSTED(victim) ||
			    check_freedom_of_movement(vict, true))
				return FALSE;

			act("&+rS&+Ri&+rn &+Lgets ahold of your sight and gazes deep within your soul.\n"
			    "&+LSuddenly all your past sins seem to catch up to you!&n",
			    FALSE, ch, 0, victim, TO_VICT);
			act("&+rS&+Ri&+rn &+Lgets ahold of &N's sight and gazes deep within $S soul.\n"
			    "&+LSuddenly all $S past sins seem to catch up to $M!&n",
			    FALSE, ch, 0, victim, TO_NOTVICT);
