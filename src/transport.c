...
bool valid_flying_edge(int from_room, int dir) { return VALID_FLYING_EDGE(from_room, dir); }

void initialize_transport()
{
	for (int i = 0; transport_routes[i].origin_vnum; i++)
	{
		bool found_path = dijkstra(real_room0(transport_routes[i].origin_vnum), real_room0(transport_routes[i].destination_vnum), valid_flying_edge, transport_routes[i].path);
		if (!found_path)
		{
...
bool flying_transport_cmd_give(P_char ch, P_char victim, char *arg)
{
	...
	if (!ticket || obj_index[ticket->R_num].virtual_number != VNUM_OBJ_FLIGHT_PATH_TICKET || ticket->value[6] != mob_index[GET_RNUM(victim)].virtual_number)
	{
		return FALSE;
	}
...
int flying_transport(P_char ch, P_char victim, int cmd, char *arg)
{
	...
	else if (TRANSPORT_STATE(ch) == TRANSPORT_STATE_MOVING)
	{
		...
		if (cmd == CMD_LOOK || cmd == CMD_SCORE || cmd == CMD_INVENTORY || cmd == CMD_ATTRIBUTES || cmd == CMD_NEWS || cmd == CMD_PETITION || cmd == CMD_TOGGLE)
		{
			return FALSE;
		}
