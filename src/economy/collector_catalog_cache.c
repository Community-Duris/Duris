...
refresh_cache<collector::catalog> cache;
...
void publish_completed_generation()
{
	...
	if (!generation || generation == applied_generation)
		return;
	const collector::catalog *catalog = cache.get();
	if (!catalog)
		return;
	if (catalog->revision < collector_runtime_catalog_revision())
	{
		++stale_snapshots;
	...
	if (!collector_runtime_rebuild(*catalog))
	{
...
std::string collector_catalog_cache_status(void)
{
	return cache.status() + ", runtime " + (ready ? "ready" : "unavailable") +
	       ...
	       std::to_string(collector_runtime_catalog_revision()) + ", stale " +
	       std::to_string(stale_snapshots) + ", publication failures " +
	       std::to_string(publication_failures);
}
