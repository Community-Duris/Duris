...
int persistence_item_event_queue_enqueue(const char *line)
{
	...
	if (!q->events)
	{
					     ...
					     PERSISTENCE_EVENT_MAX_LEN))
		{
			logit("logs/log/debug",
			      "persistence_item_event_queue_enqueue: failed to allocate queue\n");
	...
	if (persistence_queue_line_too_long(q, line))
	{
		...
		logit("logs/log/debug",
		      "persistence_item_event_queue_enqueue: line too long for small queue slot; large queue unavailable\n");
		pthread_mutex_unlock(&persistence_item_event_queue_mutex);
		return 0;
	}
	...
	if (q->count >= q->capacity)
	{
		...
		{
			logit("logs/log/debug",
			      "persistence_item_event_queue_enqueue: failed to grow queue\n");
			q->dropped++;
			pthread_mutex_unlock(&persistence_item_event_queue_mutex);
			return 0;
		}
     ...
     * persistence_item_event_worker_stuck() on the main thread. */
		...
		if (persistence_item_event_queue.count > 0)
		{
			snprintf(line, sizeof(line), "%s",
				 persistence_item_event_queue
					 .events[persistence_item_event_queue.head]);
			persistence_item_event_worker_generation =
				persistence_item_event_queue
					.generations[persistence_item_event_queue.head];
			should_write = 1;
		}
		pthread_mutex_unlock(&persistence_item_event_queue_mutex);
		...
		write_ok = persistence_item_event_worker_writer ?
				   persistence_item_event_worker_writer(
					   line, persistence_item_event_worker_context) :
				   1;
...
int persistence_item_event_worker_stop(int drain_remaining)
{
	...
	stuck = was_running && persistence_item_event_worker_last_heartbeat != 0 &&
		(int)(time(NULL) - persistence_item_event_worker_last_heartbeat) >=
			PERSISTENCE_WORKER_HEARTBEAT_STUCK_SECS;
	persistence_item_event_worker_stop_pending_flag =
		was_running ? 1 : persistence_item_event_worker_stop_pending_flag;
	if (stuck)
		persistence_item_event_worker_is_running = 0;
...
int persistence_scalar_event_queue_enqueue(const char *line)
{
	...
	if (!q->events)
	{
		...
		{
			logit("logs/log/debug",
			      "persistence_scalar_event_queue_enqueue: failed to allocate queue\n");
	...
	if (persistence_queue_line_too_long(q, line))
	{
		...
		logit("logs/log/debug",
		      "persistence_scalar_event_queue_enqueue: line too long for small queue slot; large queue unavailable\n");
		pthread_mutex_unlock(&persistence_scalar_event_queue_mutex);
		return 0;
	}
...
int persistence_large_event_queue_enqueue(const char *line)
{
	...
	if (!q->events)
	{
		...
		{
			logit("logs/log/debug",
			      "persistence_large_event_queue_enqueue: failed to allocate queue\n");
	...
	{
		logit("logs/log/debug",
		      "persistence_large_event_queue_enqueue: line too long for queue slot\n");
		pthread_mutex_unlock(&persistence_large_event_queue_mutex);
		return 0;
	}
	...
	if (q->count >= q->capacity)
	{
		...
		{
			logit("logs/log/debug",
			      "persistence_large_event_queue_enqueue: failed to grow queue\n");
			q->dropped++;
			pthread_mutex_unlock(&persistence_large_event_queue_mutex);
			return 0;
		}
...
static void *persistence_scalar_event_worker_main(void *unused)
{
	...
	while (1)
	{
		...
		{
			snprintf(line, sizeof(line), "%s",
				 persistence_scalar_event_queue
					 .events[persistence_scalar_event_queue.head]);
			persistence_scalar_event_worker_generation =
				persistence_scalar_event_queue
					.generations[persistence_scalar_event_queue.head];
			should_write = 1;
		}
		pthread_mutex_unlock(&persistence_scalar_event_queue_mutex);
		...
		write_ok = persistence_scalar_event_worker_writer ?
				   persistence_scalar_event_worker_writer(
					   line, persistence_scalar_event_worker_context) :
				   1;
...
int persistence_scalar_event_worker_stop(int drain_remaining)
{
	...
	stuck = was_running && persistence_scalar_event_worker_last_heartbeat != 0 &&
		(int)(time(NULL) - persistence_scalar_event_worker_last_heartbeat) >=
			PERSISTENCE_WORKER_HEARTBEAT_STUCK_SECS;
	persistence_scalar_event_worker_stop_pending_flag =
		was_running ? 1 : persistence_scalar_event_worker_stop_pending_flag;
	if (stuck)
		persistence_scalar_event_worker_is_running = 0;
...
static void *persistence_large_event_worker_main(void *unused)
{
	...
	while (1)
	{
		...
		{
			snprintf(line, sizeof(line), "%s",
				 persistence_large_event_queue
					 .events[persistence_large_event_queue.head]);
			persistence_large_event_worker_generation =
				persistence_large_event_queue
					.generations[persistence_large_event_queue.head];
			should_write = 1;
		}
		pthread_mutex_unlock(&persistence_large_event_queue_mutex);
		...
		write_ok = persistence_large_event_worker_writer ?
				   persistence_large_event_worker_writer(
					   line, persistence_large_event_worker_context) :
				   1;
...
int persistence_large_event_worker_stop(int drain_remaining)
{
	...
	stuck = was_running && persistence_large_event_worker_last_heartbeat != 0 &&
		(int)(time(NULL) - persistence_large_event_worker_last_heartbeat) >=
			PERSISTENCE_WORKER_HEARTBEAT_STUCK_SECS;
	persistence_large_event_worker_stop_pending_flag =
		was_running ? 1 : persistence_large_event_worker_stop_pending_flag;
	if (stuck)
		persistence_large_event_worker_is_running = 0;
