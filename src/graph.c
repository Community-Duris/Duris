...
::byte find_first_step(int src, int target, long hunt_flags, int is_ship, int wagon_type, int *ttl_steps)
{
  ...
  for (curr_dir = 0; curr_dir < NUM_EXITS; curr_dir++)
  {
    ...
    if (VALID_EDGE2(exit))
    {
      ...
      if (HOMETOWN_CHECK(src, exit->to_room) && (can_fly || !NEEDS_FLY(src, curr_dir)) && (!stay_zone || SAME_ZONE(src, curr_dir)) && (can_dispel || !IS_WALLED(src, curr_dir) || (can_break && IS_BREAKABLE(src, curr_dir))) && (!no_mob || !IS_ROOM(exit->to_room, ROOM_NO_TRACK)))
      {
  ...
  while (queue_head)
  {
    ...
    for (curr_dir = 0; curr_dir < NUM_EXITS; curr_dir++)
    {
      ...
      if (VALID_EDGE2(exit))
      {
        // Mark it first so we only check it once.
        BFSMARK(exit->to_room);
        if (HOMETOWN_CHECK(curr_room, exit->to_room) && (can_fly || !NEEDS_FLY(curr_room, curr_dir)) && (!stay_zone || SAME_ZONE(curr_room, curr_dir)) && (can_dispel || !IS_WALLED(curr_room, curr_dir) || (can_break && IS_BREAKABLE(curr_room, curr_dir))) && (!no_mob || !IS_ROOM(exit->to_room, ROOM_NO_TRACK)))
        {
...
struct BFSNode
{
  BFSNode(int _room_num, BFSNode *_prev, int _to_here_dir) : room_num(_room_num),
                                                             prev(_prev),
                                                             next(NULL),
