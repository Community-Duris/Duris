...
static const struct hardcore_config defaults = {
    true, true,
    1, false, true, true, true, true,
    true, 50,
    2, 1.20f, 1.09f, 0.91f, 110, 1.60f, 1.50f,
    1000, 10000, 100, 2, 100, 25, 100000, true, true, 100};
...
static void set_int(const char *key, const char *value, int *field, int low, int high)
{
    ...
    {
        logit(LOG_STATUS, "Invalid hardcore config value %s=%s; retaining default.", key, value);
        return;
    }
    *field = (int)parsed;
}
...
{
#define BOOL_KEY(name, field) if (!strcmp(key, name)) { set_bool(key, value, &active.field); return; }
...
#define FLOAT_KEY(name, field, low, high) if (!strcmp(key, name)) { set_float(key, value, &active.field, low, high); return; }
    ...
    FLOAT_KEY("bonus.random.equipment.multiplier", bonus_random_equipment_multiplier, 0.0f, 10.0f)
...
void boot_hardcore_config(void)
{
    ...
    logit(LOG_STATUS,
          ...
          active.death_max_count,
          active.bonus_hp_per_level,
          active.bonus_healing_multiplier,
          active.bonus_damage_outgoing_multiplier,
          active.bonus_damage_incoming_multiplier);
}
