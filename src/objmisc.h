...
#define IS_METAL(mat)                                                                            \
	(mat == MAT_IRON || mat == MAT_STEEL || mat == MAT_COPPER || mat == MAT_BRASS ||         \
	 mat == MAT_ADAMANTIUM || mat == MAT_GOLD || mat == MAT_SILVER || mat == MAT_PLATINUM || \
	 mat == MAT_MITHRIL || mat == MAT_ELECTRUM || mat == MAT_TIN)
#define IS_STONE(mat)                                                                           \
	(mat == MAT_GRANITE || mat == MAT_GEM || mat == MAT_EMERALD || mat == MAT_SAPPHIRE ||   \
	 mat == MAT_OBSIDIAN || mat == MAT_MARBLE || mat == MAT_LIMESTONE || mat == MAT_RUBY || \
	 mat == MAT_DIAMOND || mat == MAT_CRYSTAL || mat == MAT_STONE || mat == MAT_CERAMIC)
#define IS_LEATHER(mat)                                                            \
	(mat == MAT_HIDE || mat == MAT_REPTILESCALE || mat == MAT_CURED_LEATHER || \
	 mat == MAT_LEATHER || mat == MAT_DRAGONSCALE)
#define IS_WOODEN(mat) \
	(mat == MAT_BAMBOO || mat == MAT_SOFTWOOD || mat == MAT_HARDWOOD || mat == MAT_BARK)
#define IS_CLOTH(mat)                                                                        \
	(mat == MAT_REEDS || mat == MAT_HEMP || mat == MAT_LEAVES || mat == MAT_PARCHMENT || \
	 mat == MAT_PAPER || mat == MAT_CLOTH || mat == MAT_FEATHER)
#define IS_PLASTIC(mat)                                                                       \
	(mat == MAT_WAX || mat == MAT_RUBBER || mat == MAT_GENERICFOOD || mat == MAT_FLESH || \
	 mat == MAT_SILICON)
#define IS_RIGID(mat)                                                            \
	(IS_METAL(mat) || IS_STONE(mat) || IS_WOODEN(mat) || mat == MAT_IVORY || \
	 mat == MAT_PEARL || mat == MAT_CHITINOUS || mat == MAT_BONE)
...
#define IS_BACKSTABBER(obj)                                                           \
	((obj)->type == ITEM_WEAPON &&                                                \
	 ((obj)->value[0] == WEAPON_DAGGER || (obj)->value[0] == WEAPON_SHORTSWORD || \
	  (obj)->value[0] == WEAPON_TRIDENT || (obj)->value[0] == WEAPON_HORN))

#define IS_FLAYING(obj)                \
	((obj)->type == ITEM_WEAPON && \
	 ((obj)->value[0] == WEAPON_FLAIL || (obj)->value[0] == WEAPON_WHIP))

#define IS_SWORD(obj)                                                                    \
	((obj)->type == ITEM_WEAPON &&                                                   \
	 ((obj)->value[0] == WEAPON_LONGSWORD || (obj)->value[0] == WEAPON_SHORTSWORD || \
...
#define IS_BLUDGEON(obj)                                                                    \
	((obj)->type == ITEM_WEAPON &&                                                      \
	 ((obj)->value[0] == WEAPON_HAMMER || (obj)->value[0] == WEAPON_MACE ||             \
	  (obj)->value[0] == WEAPON_CLUB || (obj)->value[0] == WEAPON_NUMCHUCKS ||          \
	  (obj)->value[0] == WEAPON_SPIKED_CLUB || (obj)->value[0] == WEAPON_SPIKED_MACE || \
	  (obj)->value[0] == WEAPON_STAFF))

#define IS_LANCE(obj) ((obj)->type == ITEM_WEAPON && (obj)->value[0] == WEAPON_LANCE)

#define IS_REACH_WEAPON(obj)                                                        \
	((obj)->type == ITEM_WEAPON && IS_SET((obj)->extra_flags, ITEM_TWOHANDS) && \
...
#define CAN_HURT(ch, obj, mob)                       \
	(!has_innate(mob, INNATE_WEAPON_IMMUNITY) || \
	 ((obj) && IS_SET((obj)->extra2_flags, ITEM2_MAGIC)) || (!(obj) && (GET_LEVEL(ch) >= 51)))

#define IS_SLAYING(obj, mob)                                                   \
	((IS_SET((obj)->extra2_flags, ITEM2_SLAY_GOOD) && IS_GOOD(mob)) ||     \
