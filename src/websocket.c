...
static int websocket_valid_utf8(const unsigned char *data, size_t len)
{
	...
	while (i < len)
	{
		...
		{
			if (i + 1 >= len || data[i] < 0xA0 || data[i] > 0xBF ||
		...
		{
			if (i + 1 >= len || data[i] < 0x80 || data[i] > 0xBF ||
			    data[i + 1] < 0x80 || data[i + 1] > 0xBF)
				return 0;
			i += 2;
		}
		...
		{
			if (i + 1 >= len || data[i] < 0x80 || data[i] > 0x9F ||
			    data[i + 1] < 0x80 || data[i + 1] > 0xBF)
				return 0;
			i += 2;
		}
		...
		{
			if (i + 2 >= len || data[i] < 0x90 || data[i] > 0xBF ||
			    data[i + 1] < 0x80 || data[i + 1] > 0xBF || data[i + 2] < 0x80 ||
			    data[i + 2] > 0xBF)
				return 0;
			i += 3;
		}
		...
		{
			if (i + 2 >= len || data[i] < 0x80 || data[i] > 0xBF ||
			    data[i + 1] < 0x80 || data[i + 1] > 0xBF || data[i + 2] < 0x80 ||
			    data[i + 2] > 0xBF)
				return 0;
			i += 3;
		...
		{
			if (i + 2 >= len || data[i] < 0x80 || data[i] > 0x8F ||
			    data[i + 1] < 0x80 || data[i + 1] > 0xBF || data[i + 2] < 0x80 ||
			    data[i + 2] > 0xBF)
				return 0;
			i += 3;
...
{
	const char *colon;
	if (!line || !*line || isspace((unsigned char)*line) || !(colon = strchr(line, ':')) ||
	    colon == line)
		return 0;
...
static int websocket_has_header_token(const char *value, const char *wanted, int strip_parameters)
{
	...
	while (cursor && *cursor)
	{
		...
		while (token_end > cursor && isspace((unsigned char)token_end[-1]))
			token_end--;
		if ((size_t)(token_end - cursor) == wanted_len &&
		    strncasecmp(cursor, wanted, wanted_len) == 0)
			return 1;
...
int websocket_parse_handshake(struct descriptor_data *d, const char *buf, size_t len)
{
	...
	/* Sec-WebSocket-Key must decode to exactly 16 bytes (RFC 6455 section 4.2.1). */
	{
		...
		while (key_len > 0 && isspace((unsigned char)ws_key[key_len - 1]))
			ws_key[--key_len] = '\0';
		if (key_len == WS_KEY_ENCODED_SIZE && ws_key[key_len - 2] == '=' &&
		    ws_key[key_len - 1] == '=')
		{
			int alphabet_ok = 1;
			...
			if (alphabet_ok)
			{
				decoded_len = EVP_DecodeBlock(
					decoded_key, (const unsigned char *)ws_key, (int)key_len);
				if (decoded_len == WS_KEY_DECODED_SIZE + 2 &&
				    decoded_key[WS_KEY_DECODED_SIZE] == 0 &&
				    decoded_key[WS_KEY_DECODED_SIZE + 1] == 0 &&
				    EVP_EncodeBlock(canonical_key, decoded_key,
						    WS_KEY_DECODED_SIZE) == WS_KEY_ENCODED_SIZE &&
	...
	if (!request_line_ok || !is_upgrade || !is_websocket || !version_ok || !key_ok ||
	    duplicate_invalid || malformed_header)
	{
		d->ws_deflate_requested = 0;
		return 0;
	}
...
int websocket_complete_handshake(struct descriptor_data *d, const char *key)
{
	...
	if (d->ws_deflate_requested)
	{
		...
		if (def && inf &&
		    deflateInit2(def, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -15, 8,
	...
	for (k = descriptor_list; k; k = next_k)
	{
		    ...
		    k->connected != CON_PLAYING && strcmp(k->host, d->host) == 0)
		{
			statuslog(
				56,
				"WebSocket: Kicking stale connection from %s (new connection established)",
				k->host);

			ws_send_system(k, "kicked",
				       "New connection established from your browser.");
...
static int websocket_queue_output(struct descriptor_data *d, const unsigned char *data, size_t len,
				  int control)
{
	...
	limit = control ? WS_CONTROL_OUTPUT_RESERVE :
			  WS_MAX_OUTPUT_BYTES - WS_CONTROL_OUTPUT_RESERVE;
...
static int websocket_flush_queue(struct descriptor_data *d, unsigned char *buffer,
				 size_t *queued_len, size_t *offset)
{
	size_t flushed = 0;
...
int websocket_flush_output(struct descriptor_data *d)
{
	...
	if (!d || d->descriptor < 0)
		return -1;
	result = websocket_flush_queue(d, d->ws_control_output_buffer, &d->ws_control_output_len,
				       &d->ws_control_output_offset);
	if (result <= 0)
		return result;
	...
	result = websocket_flush_queue(d, d->ws_output_buffer, &d->ws_output_len,
				       &d->ws_output_offset);
	if (result <= 0)
		return result;
...
static int websocket_send_frame(struct descriptor_data *d, int opcode, const void *data, size_t len)
{
	    ...
	    len >= WS_COMPRESS_THRESHOLD)
	{
		...
		if (compressed)
		{
			...
			if (deflate(strm, Z_SYNC_FLUSH) == Z_OK)
			{
				...
				if (payload_len >= 4 && compressed[payload_len - 4] == 0x00 &&
	...
	if (frame_len > WS_MAX_OUTPUT_BYTES)
		return WS_OUTPUT_QUEUE_FULL;
	if (frame_len > (opcode >= WS_OPCODE_CLOSE ?
				 WS_CONTROL_OUTPUT_RESERVE :
				 WS_MAX_OUTPUT_BYTES - WS_CONTROL_OUTPUT_RESERVE))
		return WS_OUTPUT_QUEUE_FULL;
...
int websocket_send_close(struct descriptor_data *d, int code, const char *reason)
{
	...
	if (code > 0)
	{
		...
		if (reason)
		{
			...
			while (reason_len > 0 &&
			       !websocket_valid_utf8((const unsigned char *)reason, reason_len))
				reason_len--;
...
int websocket_parse_frame(struct descriptor_data *d, const char *buf, size_t len, char **payload,
			  size_t *payload_len, int *opcode, int *fin_out)
{
	size_t offset = 0;
	size_t frame_len;
	size_t data_len;
	...
	*opcode = -1;
	...
	if (op != WS_OPCODE_CONTINUATION && op != WS_OPCODE_TEXT && op != WS_OPCODE_BINARY &&
	    op != WS_OPCODE_CLOSE && op != WS_OPCODE_PING && op != WS_OPCODE_PONG)
		return websocket_input_error(d, WS_CLOSE_PROTOCOL_ERROR);
	...
	else if (data_len == 127)
	{
		...
		uint64_t full_len = ((uint64_t)(unsigned char)buf[2] << 56) |
	...
	if (data_len > 0)
	{
		*payload = (char *)malloc(data_len + 1);
		...
		if (op == WS_OPCODE_CLOSE)
		{
			...
			if (data_len >= 2)
			{
				unsigned int close_code =
					((unsigned int)(unsigned char)(*payload)[0] << 8) |
				...
				if (!websocket_valid_utf8((unsigned char *)*payload + 2,
							  data_len - 2))
				{
					free(*payload);
					*payload = NULL;
		...
		{
			if (!d->ws_inflate_stream ||
			    (rsv1 && op != WS_OPCODE_TEXT && op != WS_OPCODE_BINARY) ||
			    (!rsv1 && !d->ws_compressed_message))
				return websocket_input_error(d, WS_CLOSE_PROTOCOL_ERROR);
			if (rsv1)
			...
			if (inflated && input)
			{
				    ...
				    strm->avail_out > 0 && strm->avail_in == 0)
				{
					...
					if (!new_payload)
					{
						...
						inflateReset(strm);
						d->ws_compressed_message = 0;
						return websocket_input_error(
							d, WS_CLOSE_INTERNAL_ERROR);
					}
	...
	*opcode = op;
	...
	if (op == WS_OPCODE_CLOSE)
	{
		...
		if (*payload_len >= 2)
		{
			close_code = ((unsigned char)(*payload)[0] << 8) |
		...
		if (json)
		{
			...
			else if (type && strcmp(type, "gmcp") == 0)
			{
				...
				if (pkg_item && cJSON_IsString(pkg_item) && pkg_item->valuestring)
				{
					...
					if (package_len <= GMCP_MAX_PACKAGE_SIZE &&
					    package_len + 2 <= GMCP_MAX_INPUT_SIZE)
					{
						char *data_str =
							data_item ?
								cJSON_PrintUnformatted(data_item) :
								NULL;
						size_t data_len = data_str ? strlen(data_str) : 0;
						if (!data_str || data_len <= GMCP_MAX_INPUT_SIZE -
										     package_len -
										     2)
						{
							size_t gmcp_len =
								package_len +
								(data_str ? data_len + 2 : 1);
...
int websocket_process_input(struct descriptor_data *d)
{
	...
	if (!d->ws_handshake_done)
	{
		...
		if (result > 0 && trailing_bytes > 0)
		{
			...
			memcpy(d->ws_fragment_buffer, d->ws_handshake_buffer + header_bytes,
			       trailing_bytes);
	...
	while (offset < d->ws_fragment_len && frames_processed < WS_MAX_FRAMES_PER_READ)
	{
		    ...
		    opcode == WS_OPCODE_PONG)
		{
			if (opcode == WS_OPCODE_CLOSE)
			{
				...
				{
					memmove(d->ws_fragment_buffer,
						d->ws_fragment_buffer + offset,
		...
		if (opcode == WS_OPCODE_CONTINUATION)
		{
			...
			{
				if (d->ws_message_len > WS_MAX_MESSAGE_SIZE ||
				    payload_len > WS_MAX_MESSAGE_SIZE - d->ws_message_len)
				{
					free(payload);
					return websocket_input_error(d, WS_CLOSE_MESSAGE_TOO_BIG);
				}
				...
				memcpy(d->ws_message_buffer + d->ws_message_len, payload,
				       payload_len);
