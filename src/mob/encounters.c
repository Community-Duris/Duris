...
void random_encounters(P_char ch)
{
	    ...
	    world[ch->in_room].sector_type == SECT_UNDRWLD_MUSHROOM)
	{
		...
		if (mobs_vnum != 0)
		{
			...
			if (!(mob = read_mobile(mobs_vnum, VIRTUAL)))
			{
				logit(LOG_MOB, "Random Encounters, mob #%d not in file!",
				      mobs_vnum);
				return;
			}
...
P_obj ran_magical(P_char mob)
{
	...
	const char *lights[] = { "&+ytorch", "&+Llantern", "&+Wstrange fungus",
				 "&+Wglowing crystal" };
	int nlights = 3;
	const char *scrolls[] = { "tattered",	  "small", "&+Ldusty",	"vellum",
				  "&+yparchment", "frail", "&+Wglowing" };
	int nscrolls = 6;
	const char *potions[] = { "&+Ldark",	"light", "&+ggreen", "&+Yyellow", "&+Bbubbly",
				  "&+Lsmoking", "clear", "cloudy",   "&+mpurple" };
	int npotions = 8;
	const char *bags[] = { "&+yleather",	 "&+ybrown",   "&+Lblack",  "&+gsnakeskin",
			       "large",		 "&+Wruffled", "&+mvelvet", "torn",
			       "&+Rdragonscale", "&+yfurred",  "silverish" };
	int nbags = 10;
	const char *drinks[] = { "&+ybottle",	"flagon",	    "jar",   "&+Yvase",
				 "&+Lwineskin", "&+Wdrinking horn", "&+ymug" };
	int ndrinks = 6;
	const char *foods[] = { "pastry",	    "&+yloaf of bread",
				"&+Ltravel ration", "&+Rleg of beef",
				"&+Ljuicy steak",   "&+gpear",
				"&+Rcarrot",	    "&+ysuspicious-looking mushroom",
				"&+Rapple",	    "&+Lwyvern jerky",
				"&+Ldragon-gut",    "&+ypiece of bread" };
	int nfoods = 11;
...
P_obj ran_obj(P_char mob, ulong which)
{
	...
	const char *earrings[] = { "&+Wdiamond", "&+gemerald",	  "&+ytopaz", "&+ywooden",
				   "&+gjade",	 "&+Wwhite gold", "&+Lonyx",  "tin",
				   "glass",	 "&+Wmarble",	  "&+Lblack", "&+Lgranite" };
	int nearrings = 11;
	const char *rings[] = { "tarnished", "jeweled",	  "silver",	"&+Ygolden", "&+Wshiny",
				"dull",	     "sparkling", "attractive", "&+Lblack",  "glass" };
	int nrings = 9;
	...
	int nfeets = 4;
	const char *shields[] = { "dented", "round", "oval",	 "center-grip",	  "heater",
				  "door",   "shiny", "embossed", "coffin-shaped", "curved" };
	int nshields = 9;
	const char *weps[] = { "axe",  "sword", "mace", "club",	 "bardiche", "pike",
	...
	case ITEM_WEAR_FINGER: /* 1293 */
		...
		else if (GET_LEVEL(mob) > 24)
		{
			...
			if ((obj->affected[1].location == 7) ||
			    (obj->affected[1].location == 8) || /* can't use these */
			    (obj->affected[1].location == 6) || (obj->affected[1].location == 15) ||
	...
	case ITEM_WEAR_NECK: /* 1285 */
		...
		if (!(obj = read_object(1285, VIRTUAL)))
			return NULL;
		obj->str_mask = (STRUNG_KEYS | STRUNG_DESC1 | STRUNG_DESC2);
		snprintf(buf, MAX_STRING_LENGTH, "a %s%s%s&n", color[temp3], generics[temp2],
			 necks[temp]);
		obj->short_description = str_dup(buf);
		snprintf(buf, MAX_STRING_LENGTH, "A %s%s%s &nlies here.", color[temp3],
			 generics[temp2], necks[temp]);
		obj->description = str_dup(buf);
	...
	case ITEM_WEAR_BODY: /* 1286 */
		...
		if (!(obj = read_object(1286, VIRTUAL)))
			return NULL;
		obj->str_mask = (STRUNG_KEYS | STRUNG_DESC1 | STRUNG_DESC2);
		snprintf(buf, MAX_STRING_LENGTH, "a %s%s%s&n", color[temp3],
			 generics[number(0, nbodys)], bodys[temp]);
		obj->short_description = str_dup(buf);
		snprintf(buf, MAX_STRING_LENGTH, "A %s%s%s &nlies here.", color[temp3],
			 generics[temp2], bodys[temp]);
		obj->description = str_dup(buf);
	...
	case ITEM_WEAR_LEGS: /* 1296 */
		...
		if (!(obj = read_object(1296, VIRTUAL)))
			return NULL;
		obj->str_mask = (STRUNG_KEYS | STRUNG_DESC1 | STRUNG_DESC2);
		snprintf(buf, MAX_STRING_LENGTH, "some %s%sleggings&n", color[temp3],
			 generics[temp]);
		obj->short_description = str_dup(buf);
	...
	case ITEM_WEAR_FEET: /* 1297 */
		...
		if (!(obj = read_object(1297, VIRTUAL)))
			return NULL;
		obj->str_mask = (STRUNG_KEYS | STRUNG_DESC1 | STRUNG_DESC2);
		snprintf(buf, MAX_STRING_LENGTH, "a pair of %s%s%s&n", color[temp3],
	...
	case ITEM_WEAR_ARMS: /* 1289 */
		...
		if (!(obj = read_object(1289, VIRTUAL)))
			return NULL;
		obj->str_mask = (STRUNG_KEYS | STRUNG_DESC1 | STRUNG_DESC2);
		snprintf(buf, MAX_STRING_LENGTH, "a pair of %s%ssleeves&n", color[temp3],
			 generics[temp]);
		obj->short_description = str_dup(buf);
	...
	case ITEM_WEAR_ABOUT: /* 1287 */
		...
		obj->short_description = str_dup(buf);
		snprintf(buf, MAX_STRING_LENGTH, "A %s%scloak &nlies here.", color[temp3],
			 generics[temp]);
		obj->description = str_dup(buf);
	...
	case ITEM_WEAR_WAIST: /* 1288 */
		...
		obj->short_description = str_dup(buf);
		snprintf(buf, MAX_STRING_LENGTH, "A %s%sbelt &nlies here.", color[temp3],
			 generics[temp]);
		obj->description = str_dup(buf);
	...
	case ITEM_WEAR_WRIST: /* 1291 */
		...
		obj->short_description = str_dup(buf);
		snprintf(buf, MAX_STRING_LENGTH, "A %s%sbracelet &nlies here.", color[temp3],
			 generics[temp]);
		obj->description = str_dup(buf);
	...
	case ITEM_WEAR_HANDS: /* 1292 */
		...
		obj->short_description = str_dup(buf);
		snprintf(buf, MAX_STRING_LENGTH, "Some %s%sgloves &nlie here.", color[temp3],
			 generics[temp]);
		obj->description = str_dup(buf);
	...
	case ITEM_WIELD: /* 1294 */
		...
		obj->short_description = str_dup(buf);
		snprintf(buf, MAX_STRING_LENGTH, "A %s%s &nlies here.", generics[temp2],
			 weps[temp]);
		obj->description = str_dup(buf);
	...
	obj->extra_flags = obj->extra_flags & !ITEM_NOSELL & !ITEM_NOLOCATE & !ITEM_NOIDENTIFY &
			   !ITEM_TRANSIENT;
...
void random_gems(P_obj gem, int base_value)
{
	...
	int get_out = FALSE;
	char buf[MAX_STRING_LENGTH];
	static int bv_table[13][2] = { { 25, 10 },    { 50, 50 },    { 70, 100 },   { 90, 500 },
				       { 99, 1000 },  { 100, 5000 }, { 0, 10000 },  { 0, 25000 },
				       { 0, 50000 },  { 0, 100000 }, { 0, 250000 }, { 0, 500000 },
				       { 0, 1000000 } };
	static float decreased_bv_table[6] = { 5.0, 1.0, 0.5, 0.25, 0.05 };
	const char *properties[6][14] = {
		{ /* Ornamental Stones, base value 1 GP */
		  "&+bazurite&n", "&+yba&n&+Bn&n&+yded a&N&+Rg&N&+yate&n", "&+Bblue quartz&n",
		  "&+Weye a&n&+Lg&n&+Wate&n", "&+Lhematite&n",
		  "&+bla&+Bp&n&+bis la&n&+Bz&n&+buli&n",
		  "&+gm&+Ga&n&+gl&+Ga&n&+gc&+Gh&n&+gi&+Gt&N&+ge&n",
		  "&+Wmo&n&+gs&+Ws ag&n&+ga&+Wte&n", "&+Lobsidian&n", "&+Rrhodochrosite&n",
		  "&+ytiger &+Yeye&n", "&+Bturquiose&n", "", "" },

		{ /* Semi-precious Stones, base value 5 GP */
		  "&+Yamber&n", "&+Rcarnelian&n", "&+Wchalcedony&n", "&+gchrysoprase&n",
		  "&+ycitrine&n", "&+bjasper&n", "&+Wmoon&n&+Bstone&n", "&+Lonyx&n", "rock crystal",
		  "&+Rs&n&+War&n&+Rdo&n&+Wny&n&+Rx&n", "smoky quartz",
		  "&+Rstar &n&+Wrose&n&+R quartz&n", "&+Bzircon&n", "" },

		{ /* Fancy Stones, base value 10 GP */
		  "&+yamber&n", "&+Yalexandrite&n", "&+mamethyst&n", "", "&+gchrysoberyl&n",
		  "&+Rcoral&n", "&+rgarnet&n", "&+gjade&n", "&+Ljet&n", "&+Wpearl&n", "",
		  "&+gspinel&n", "", "&+Btourmaline&n" },

		{ /* Fancy Stones, base value 50 GP */
		  "", "", "", "&+gaquamarine&n", "", "", "&+rgarnet&n", "", "", "&+Wpearl&n",
		  "&+gperidot&n", "&+Rspinel&n", "&+Ytopaz&n", "" },

		{ /* Gem Stones, base value 100 GP */
		  "&+Lblack opal&n", "", "", "&+gemerald&n", "&+Rfire opal", "", "&+Wopal&n",
		  "&+Moriental amethyst&n", "", "&+Yoriental topaz&n", "", "&+Bsapphire&n",
		  "&+Rstar &n&+Wr&n&+Ruby&n", "&+Bstar &n&+Ws&n&+Bapphire&n" },

		{ /* Gem Stones, base value 500 GP */
		  "&+Lblack opal&n", "&+Lblack sapphire&n", "&+Wdiamond&n", "&+gemerald&n",
		  "&+rfire opal&n", "&+Rjacinth&n", "&+Wopal&n", "&+Boriental amethyst&n",
		  "&+goriental emerald&n", "&+Yoriental topaz&n", "&+Rruby&n", "&+Bsapphire&n",
		  "&+Rstar &n&+Wr&n&+Ruby&n", "&+Bstar &n&+Ws&n&+Bapphire&n" }
	};
	...
	snprintf(buf, MAX_STRING_LENGTH, "A %s lies here.",
		 properties[orig_base_value_place][prop]);
...
void random_jewelry(P_obj obj, int base_value)
{
	...
	const char *kind[51] = { "anklet",     "arm band", "belt",
				 "box(small)", "bracelet", "brooch",
				 "buckle",     "chain",	   "chalice",
				 "choker",     "clasp",	   "collar",
				 "comb",       "coronet",  "crown",
				 "decanter",   "diadem",   "earring",
				 "fob",	       "goblet",   "headband(fillet)",
				 "idol",       "locket",   "medal",
				 "medallion",  "necklace", "pendant",
				 "pin",	       "orb",	   "ring",
				 "sceptre",    "seal",	   "statuette",
				 "tiara",      "bracers",  "cup",
				 "dagger",     "flask",	   "helm",
				 "horn",       "mirror",   "periapt",
				 "phylactery", "rod",	   "scarab",
				 "skull",      "shield",   "staff",
