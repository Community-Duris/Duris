...
bool poll_has_voted(const char *account_name, int poll_id)
{
  ...
  return false;
#else
  if (!account_name || !*account_name) return false;

  string acct_esc = escape_str(account_name);
  MYSQL_RES *res = db_query("SELECT id FROM poll_votes WHERE poll_id = %d AND account_name = '%s' LIMIT 1",
...
{
  if (!poll_wizards.count(ch)) return;
