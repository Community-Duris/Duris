...
#define GET_CHAR_SKILL(ch, num) ( \
    IS_AFFECTED5(ch, AFF5_MENTAL_ANGUISH) ? (MIN(GET_CHAR_SKILL_P((ch), (num)), 10)) : GET_CHAR_SKILL_P(ch, (num)))
...
#define HAS_VEGETATION(sect) (sect == SECT_FIELD || sect == SECT_FOREST || sect == SECT_HILLS || sect == SECT_MOUNTAIN || sect == SECT_UNDRWLD_WILD || sect == SECT_DESERT || sect == SECT_ARCTIC || sect == SECT_SWAMP || sect == SECT_UNDRWLD_MOUNTAIN || sect == SECT_UNDRWLD_SLIME || sect == SECT_UNDRWLD_MUSHROOM || sect == SECT_SNOWY_FOREST || sect == SECT_UNDRWLD_WILD || sect == SECT_UNDRWLD_LOWCEIL || sect == SECT_CITY || sect == SECT_UNDRWLD_CITY)
...
#define IS_UNDERWATER(c) ((world[c->in_room].sector_type == SECT_UNDERWATER) || (world[c->in_room].sector_type == SECT_UNDERWATER_GR) || (world[c->in_room].sector_type == SECT_WATER_PLANE) || (IS_WATER_ROOM(c->in_room) && c->specials.z_cord < 0) || (IS_ROOM(ch->in_room, ROOM_UNDERWATER)))

#define IS_WATER_ROOM(r) (world[r].sector_type == SECT_UNDRWLD_WATER || world[r].sector_type == SECT_UNDRWLD_NOSWIM || world[r].sector_type == SECT_OCEAN || world[r].sector_type == SECT_UNDERWATER || world[r].sector_type == SECT_UNDERWATER_GR || world[r].sector_type == SECT_WATER_NOSWIM || world[r].sector_type == SECT_WATER_SWIM || world[r].sector_type == SECT_WATER_PLANE)

#define IS_UNDERWORLD(r) (world[r].sector_type == SECT_UNDRWLD_CITY || world[r].sector_type == SECT_UNDRWLD_INSIDE || world[r].sector_type == SECT_UNDRWLD_WILD || world[r].sector_type == SECT_UNDRWLD_WATER || world[r].sector_type == SECT_UNDRWLD_NOSWIM || world[r].sector_type == SECT_UNDRWLD_NOGROUND || world[r].sector_type == SECT_UNDRWLD_MOUNTAIN || world[r].sector_type == SECT_UNDRWLD_SLIME || world[r].sector_type == SECT_UNDRWLD_LOWCEIL || world[r].sector_type == SECT_UNDRWLD_LIQMITH || world[r].sector_type == SECT_UNDRWLD_MUSHROOM)

#define NORMAL_PLANE(r) (world[r].sector_type != SECT_FIREPLANE && world[r].sector_type != SECT_WATER_PLANE && world[r].sector_type != SECT_EARTH_PLANE && world[r].sector_type != SECT_AIR_PLANE && world[r].sector_type != SECT_ETHEREAL && world[r].sector_type != SECT_ASTRAL && world[r].sector_type != SECT_NEG_PLANE)
...
#define GET_HEIGHT(ch) ((ush_int)((IS_AFFECTED3(ch, AFF3_ENLARGE) ? ((ch)->player.height * 1.5) : IS_AFFECTED3(ch, AFF3_REDUCE) ? ((ch)->player.height / 1.5) \
                                                                                                                                : (ch)->player.height)))

#define GET_WEIGHT(ch) (IS_AFFECTED3(ch, AFF3_ENLARGE) ? ((ch)->player.weight * 2) : IS_AFFECTED3(ch, AFF3_REDUCE) ? ((ch)->player.weight / 2) \
                                                                                                                   : (ch)->player.weight)
...
#define GET_PLYR(a) (IS_MORPH(a) ? MORPH_ORIG(a) : ((a)->desc && (a)->desc->original) ? (a)->desc->original \
                                                                                      : (a))
...
#define IS_CARRYING_W(ch, rider) ( \
    (ch)->specials.carry_weight + COIN_WEIGHT(GET_COPPER(ch), GET_SILVER(ch), GET_GOLD(ch), GET_PLATINUM(ch)) + (((rider = GET_RIDER(ch)) == NULL) ? 0 : (rider->player.weight + rider->specials.carry_weight)) + COIN_WEIGHT(GET_COPPER(rider), GET_SILVER(rider), GET_GOLD(rider), GET_PLATINUM(rider)))
...
#define CAN_CARRY_OBJ(ch, obj, rider) ( \
    ((IS_CARRYING_W(ch, rider) + GET_OBJ_WEIGHT(obj)) <= CAN_CARRY_W(ch)) && ((IS_CARRYING_N(ch) + 1) <= CAN_CARRY_N(ch)))
...
#define OBJ_FALLING(o) ((o) && !IS_SET((o)->extra_flags, ITEM_LEVITATES) && OBJ_ROOM(o) && ((world[(o)->loc.room].sector_type == SECT_NO_GROUND) || (world[(o)->loc.room].sector_type == SECT_UNDRWLD_NOGROUND) || (world[(o)->loc.room].chance_fall >= number(1, 100)) || (o->z_cord > 0)))
...
#define IS_CONTROLLER_RACE(race) ((race >= RACE_HUMAN && race <= RACE_PLAYER_MAX && race != RACE_ILLITHID) || race == RACE_AGATHINON || race == RACE_HUMANOID || race == RACE_HALFORC || race == RACE_FAERIE || race == RACE_VAMPIRE || race == RACE_RAKSHASA || race == RACE_ANGEL || race == RACE_SNOW_OGRE || race == RACE_LYCANTH || race == RACE_ZOMBIE || race == RACE_PRIMATE || race == RACE_ELADRIN)
...
#define USES_COMMUNE(ch) ((IS_NPC(ch) || GET_PRIME_CLASS(ch, CLASS_DRUID) || GET_PRIME_CLASS(ch, CLASS_RANGER) || GET_PRIME_CLASS(ch, CLASS_DRAGOON) || (!(IS_CASTER_CLASS(ch->player.m_class) || USES_MANA(ch)) && (GET_SECONDARY_CLASS(ch, CLASS_DRUID) || GET_SECONDARY_CLASS(ch, CLASS_RANGER)))))
...
#define CAN_SPEAK(ch) (IS_HUMANOID(ch) || IS_DRAGON(ch) || IS_DRAGONKIN(ch) || IS_DEMON(ch) || IS_GIANT(ch) || ((IS_PC_PET(ch) || IS_PC(ch)) && !IS_ANIMAL(ch)) || IS_UNDEAD(ch) || IS_EFREET(ch) || IS_TITAN(ch))
...
#define OLD_RACE_GOOD(race, align) ((race == RACE_HUMAN) || (race == RACE_GREY) || (race == RACE_MOUNTAIN) || (race == RACE_BARBARIAN) || (race == RACE_GNOME) || (race == RACE_HALFLING) || (race == RACE_HALFELF) || (race == RACE_CENTAUR) || (race == RACE_GITHZERAI) || (race == RACE_AGATHINON) || (race == RACE_WOODELF) || (race == RACE_FIRBOLG) || (race == RACE_ELADRIN) || (OLD_RACE_NEUTRAL(race) && (align >= 0)))

#define OLD_RACE_EVIL(race, align) ((race == RACE_DROW) || (race == RACE_DUERGAR) || (race == RACE_GITHYANKI) || (race == RACE_OGRE) || (race == RACE_GOBLIN) || (race == RACE_ORC) || (race == RACE_OROG) || (race == RACE_TROLL) || (race == RACE_KUOTOA) || (race == RACE_KOBOLD) || (race == RACE_DRIDER) || (OLD_RACE_NEUTRAL(race) && (align < 0)))

#define OLD_RACE_PUNDEAD(race) ((race == RACE_LICH) || (race == RACE_PVAMPIRE) || (race == RACE_PDKNIGHT) || (race == RACE_SHADE) || (race == RACE_REVENANT) || (race == RACE_PSBEAST) || (race == RACE_WIGHT) || (race == RACE_GARGOYLE) || (race == RACE_PHANTOM))
...
#define IS_NECRO_GOLEM(ch) IS_NPC(ch) && (GET_RACE(ch) == RACE_GOLEM) && (GET_VNUM(ch) == golem_data[0].vnum || GET_VNUM(ch) == golem_data[1].vnum || GET_VNUM(ch) == golem_data[2].vnum || GET_VNUM(ch) == golem_data[3].vnum)
...
#define INFRA_INVIS_RACE(race) (race == RACE_UNDEAD || race == RACE_GHOST || race == RACE_VAMPIRE || race == RACE_LICH || race == RACE_PDKNIGHT || race == RACE_ZOMBIE || race == RACE_SPECTRE || race == RACE_SKELETON || race == RACE_WRAITH || race == RACE_SHADOW || race == RACE_DRACOLICH || race == RACE_PVAMPIRE || race == RACE_SHADE || race == RACE_REVENANT || race == RACE_PSBEAST || race == RACE_WIGHT || race == RACE_GARGOYLE || race == RACE_PHANTOM || race == RACE_AQUATIC_ANIMAL || race == RACE_PARASITE || race == RACE_GOLEM || race == RACE_SNAKE || race == RACE_ARACHNID || race == RACE_PLANT || race == RACE_SLIME || race == RACE_A_ELEMENTAL || race == RACE_INSECT || race == RACE_W_ELEMENTAL || race == RACE_V_ELEMENTAL || race == RACE_I_ELEMENTAL || race == RACE_E_ELEMENTAL || (IS_AFFECTED4(obj, AFF4_VAMPIRE_FORM) && !GET_CLASS(obj, CLASS_THEURGIST)) || IS_RACEWAR_UNDEAD(obj))

// This is done in reverse 'cause it's faster that way.
#define HAS_LUNGS(race) (race != RACE_SHADE && race != RACE_REVENANT && race != RACE_LICH && race != RACE_PVAMPIRE && race != RACE_PDKNIGHT && race != RACE_PSBEAST && race != RACE_WIGHT && race != RACE_PHANTOM && race != RACE_GARGOYLE && race != RACE_F_ELEMENTAL && race != RACE_A_ELEMENTAL && race != RACE_UNDEAD && race != RACE_VAMPIRE && race != RACE_W_ELEMENTAL && race != RACE_E_ELEMENTAL && race != RACE_GHOST && race != RACE_GOLEM && race != RACE_PLANT && race != RACE_DRACOLICH && race != RACE_SLIME && race != RACE_CONSTRUCT && race != RACE_ZOMBIE && race != RACE_SPECTRE && race != RACE_SKELETON && race != RACE_WRAITH && race != RACE_V_ELEMENTAL && race != RACE_I_ELEMENTAL && race != RACE_SHADOW && race != RACE_AVATAR)
...
#define COMBAT_PULSE(ch) (ch->specials.base_combat_round + ch->points.combat_pulse * ((.04 / 3) * ch->specials.base_combat_round * ch->specials.base_combat_round - (1.10 / 3) * ch->specials.base_combat_round + 2.73))
...
#define CALMCHANCE(ch) (GET_C_CHA(ch) / ((GET_C_CHA(ch) > 160) ? 4 : (GET_C_CHA(ch) < 80) ? 9 \
                                                                                          : 7))
...
#define MAGICDAMBONUS(ch) ((GET_C_STR(ch) < 121) ? 100 : (GET_C_STR(ch) < 141) ? 110 \
                                                     : (GET_C_STR(ch) < 181)   ? 120 \
                                                                               : 130)

// Quest items and containers (maybe containing quest items) have 100% load.
#define ITEM_LOAD_CHECK(item, ival, zone_percent) ((IS_OBJ_STAT2(item, ITEM2_QUESTITEM) || (item->type == ITEM_CONTAINER)) ? zone_percent : item_load_check(item, ival, zone_percent))
