...
void spell_enchant(int level, P_char ch, P_char victim, P_obj obj)
{
	...
	if (((GET_ITEM_TYPE(obj) == ITEM_WEAPON) || (GET_ITEM_TYPE(obj) == ITEM_DRINKCON) || (GET_ITEM_TYPE(obj) == ITEM_NOTE)) && !IS_SET(obj->extra_flags, ITEM_MAGIC) && !(obj->enchant.prepared))
	{
		...
		if (IS_ARTIFACT(obj) ||
		    !(j = (GET_ITEM_TYPE(obj) == ITEM_WEAPON && (GET_CLASS(ch) == CLASS_CONJURER || GET_CLASS(ch) == CLASS_SUMMONER)) ||
		          (GET_ITEM_TYPE(obj) == ITEM_DRINKCON && GET_CLASS(ch) == CLASS_CLERIC) ||
		          (GET_ITEM_TYPE(obj) == ITEM_NOTE && (GET_CLASS(ch) == CLASS_CONJURER || GET_CLASS(ch) == CLASS_SUMMONER || GET_CLASS(ch) == CLASS_SORCERER || GET_CLASS(ch) == CLASS_NECROMANCER))) &&
		        !IS_TRUSTED(ch))
		{
		...
		else
		{
			...
			if (number(0, 9) < 3)
			{
				act("A flash of &+Wlight&N and &+G$p glows green&N.", FALSE, ch, obj, 0, TO_CHAR);
				//      SET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
				//      SET_BIT(obj->extra_flags, ITEM_ANTI_EVIL);
			}
		}
...
void spell_enchant_weapon(int level, P_char ch, P_char victim, P_obj obj)
{
	...
	if ((GET_ITEM_TYPE(obj) == ITEM_WEAPON) && (obj->enchant.prepared) && IS_SET(obj->extra_flags, ITEM_MAGIC))
	{
		...
		obj->enchant.magic += MAX(0, 10 * obj->affected[1].modifier - GET_LEVEL(ch) / 5);
		...
		if ((number(1, 200) < obj->enchant.magic) || (plus_six && !(six_possible)) || (max_magic(obj) < obj->enchant.magic))
		{
			act("&+R$p cannot hold this much magic, and explodes.", FALSE, ch, obj, 0, TO_CHAR);
...
void cast_spell_to_object(int level, P_char ch, P_obj obj, int spell)
{
	    ...
	    !obj->value[6])
	{
		...
		if ((number(1, 200) < obj->enchant.magic) || (max_magic(obj) < obj->enchant.magic))
		{
			act("&+R$p cannot hold this much magic, and explodes.", FALSE, ch, obj, 0, TO_CHAR);
			act("&+R$p, held by $n, &+Rcannot hold this much magic, and explodes.", FALSE, ch, obj, 0, TO_ROOM);
			obj_from_char(obj);
...
void spell_permanence(int level, P_char ch, P_char victim, P_obj obj)
{
	...
	act("$p glows &+Bblue&N as all the magic becomes bonded with it.", FALSE, ch, obj, 0, TO_CHAR);
...
with it.",
	...
	if (GET_ITEM_TYPE(obj) == ITEM_WEAPON)
	{
		if ((obj->value[1] * obj->value[2]) < ((IS_SET(obj->extra_flags, ITEM_TWOHANDS)) ? 28 : 14))
		{
			(IS_SET(obj->extra_flags, ITEM_TWOHANDS)) ? (obj->value[1] = 4) : (obj->value[1] = 2);
			obj->value[2] = 7;
		}
...
int player_enchant_proc(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (obj->loc.wearing == ch)
	{
		if (!number(0, 20))
		{
			...
			{
				case SPELL_MAGIC_MISSILE:
					act("Your $q glows brightly as magical arrows streak out of it.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly as magical arrows streak out of it!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
					cast_magic_missile(level, ch, 0, SPELL_TYPE_SPELL, vict, 0);
					break;
				case SPELL_DISPEL_MAGIC:
					act("Your $q glows brightly as a beam of energy streaks out of it.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly as a beam of energy streaks out of it!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
					cast_dispel_magic(level, ch, 0, SPELL_TYPE_SPELL, vict, 0);
					break;
				case SPELL_LIGHTNING_BOLT:
					act("Your $q glows brightly as a bolt of lightning streaks out of it.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly as a bolt of lightning streaks out of it!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
					cast_lightning_bolt(level, ch, 0, SPELL_TYPE_SPELL, vict, 0);
					break;
				case SPELL_FIREBALL:
					act("Your $q glows brightly as a ball of fire streaks out of it.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly as a ball of fire streaks out of it!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
					cast_fireball(level, ch, 0, SPELL_TYPE_SPELL, vict, 0);
					break;
				case SPELL_CURE_LIGHT:
					act("Your $q glows brightly as healing energy envelops you.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly as healing energy envelops them!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
				...
				case SPELL_CURE_CRITIC:
					...
					break;
				case SPELL_CONTINUAL_LIGHT:
					act("Your $q glows brightly and lights the room.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly and lights the room!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
					cast_continual_light(level, ch, 0, SPELL_TYPE_SPELL, vict, 0);
					break;
				case SPELL_DARKNESS:
					act("Your $q glows brightly as darkness cloaks the room.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly as darkness cloaks the room!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
					cast_darkness(level, ch, 0, SPELL_TYPE_SPELL, vict, 0);
					break;
				case SPELL_MOLEVISION:
					act("Your $q glows brightly as a hazy outline seeps out of it.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly as a hazy outline seeps out of it!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
					cast_molevision(level, ch, 0, SPELL_TYPE_SPELL, vict, 0);
					break;
				case SPELL_EARTHEN_RAIN:
					if (number(0, 1))
					{
						act("Your $q glows brightly as a beam of energy shoots out of it.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
						act("$n's $q glows brightly as a beam of energy shoots out of it.", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
				...
				case SPELL_SOUL_DISTURBANCE:
					...
					break;
				case SPELL_ENERGY_DRAIN:
					act("Your $q glows brightly as a dark beam shoots out of it.", FALSE, obj->loc.wearing, obj, 0, TO_CHAR);
					act("$n's $q glows brightly as a dark beam shoots out of it!", FALSE, obj->loc.wearing, obj, 0, TO_ROOM);
