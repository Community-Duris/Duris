...
#ifdef _PFILE_
/* The _DE_ arm above declares this; the _PFILE_ arm needs it for the
   SKILL_CREATE/SPELL_ADD macros below. */
int currentSkill = 0;
#endif
...
#else
void SPELL_CREATE_MSG(const char *Name, int Index, int Beats, unsigned int Targets,
...
void SPELL_CREATE(const char *Name, int Index, int Beats, unsigned int Targets,
		  void (*Spell_pointer)(int, P_char, char *, int, P_char, P_obj))
{
...
#define POISON_CREATE(Name, Index, Spell_pointer)                                             \
	SPELL_CREATE(Name, Index, 0, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO | TAR_POISON, \
		     (spell_func)Spell_pointer)

void SKILL_CREATE_WITH_MESSAGES(const char *Name, int Index, unsigned int Type,
				const char *wearoff_char, const char *wearoff_room)
{
	SKILL_CREATE(Name, Index, Type);
...
void SPELL_CREATE2(const char *Name, int Index, int Beats, int Targets,
		   void (*Spell_pointer)(int, P_char, char *, int, P_char, P_obj),
		   const char *Wear_off, const char *wear_off_room)
{
	SPELL_CREATE_MSG(Name, Index, Beats, Targets, Spell_pointer, Wear_off);
...
void initialize_skills()
{
	...
	SPELL_CREATE("water to life", SPELL_WATER_TO_LIFE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
		     spell_water_to_life);
	...
	SPELL_CREATE("ethereal grounds", SPELL_ETHEREAL_GROUNDS, PULSE_SPELLCAST * 4, TAR_IGNORE,
		     spell_ethereal_grounds);
	SPEC_SPELL_ADD(CLASS_CONJURER, 11, SPEC_AIR);

	SPELL_CREATE("electrical execution", SPELL_ELECTRICAL_EXECUTION,
		     (int)(PULSE_SPELLCAST * 1.5), TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
		     spell_electrical_execution);
	...
	SPELL_CREATE2("earthen tomb", SPELL_EARTHEN_TOMB, PULSE_SPELLCAST * 4,
		      TAR_IGNORE | TAR_NOCOMBAT, cast_earthen_tomb,
	...
	/* Cleric Specs */
	SPELL_CREATE("lesser sanctuary", SPELL_LESSER_SANCTUARY, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
		     spell_sanctuary);
	SPEC_SPELL_ADD(CLASS_CLERIC, 11, SPEC_HOLYMAN);

	SPELL_CREATE("aura of rebirth", SPELL_AURA_OF_REBIRTH, PULSE_SPELLCAST * 10, TAR_IGNORE,
		     cast_area_resurrect);
	SPEC_SPELL_ADD(CLASS_CLERIC, 12, SPEC_HEALER);

	SPELL_CREATE_MSG("battletide", SPELL_BATTLETIDE, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
	...
	SKILL_CREATE_WITH_MESSAGES("war cry", SKILL_WAR_CRY, TAR_PHYS, "Your battle frenzy fades.",
				   NULL);
	SKILL_ADD(CLASS_BERSERKER, 26, 100);

	SKILL_CREATE_WITH_MESSAGES("roar of heroes", SKILL_ROAR_OF_HEROES, TAR_PHYS,
				   "Your heroic urges slowly fade away.", NULL);

	/* Anti-paladin Specs */
	SPELL_CREATE_MSG("spawn", SPELL_SPAWN, PULSE_SPELLCAST * 3, TAR_SELF_ONLY, spell_spawn,
			 "&+LThe aura of death slowly fades&n.");

	SPELL_CREATE_MSG("dread blade", SPELL_DREAD_BLADE, PULSE_SPELLCAST * 3, TAR_SELF_ONLY,
			 spell_dread_blade,
			 "&+LThe putrid &+raura&+L surrounding your weapon fades.&n");
	...
	SPELL_CREATE("group heal", SPELL_GROUP_HEAL, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_group_heal);
	...
	*/
	...
	SPELL_CREATE_MSG("shadow projection", SPELL_SHADOW_PROJECTION, PULSE_SPELLCAST * 6,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_shadow_projection,
			 "&+LYour material form realigns with this plane.");
	...
	SPELL_CREATE("obtenebration", SPELL_OBTENEBRATION, PULSE_SPELLCAST * 2.5,
		     TAR_AREA | TAR_OFFAREA | TAR_AGGRO, spell_obtenebration);
	SPEC_SPELL_ADD(CLASS_SORCERER, 12, SPEC_SHADOW);
	...
	/* Monk Specs */
	SKILL_CREATE_WITH_MESSAGES("fist of dragon", SKILL_FIST_OF_DRAGON, TAR_MENTAL,
				   "The touch of the dragon leaves your soul.", "");
	SPEC_SKILL_ADD(CLASS_MONK, 46, 100, SPEC_WAYOFDRAGON);

	SKILL_CREATE_WITH_MESSAGES("tiger palm", SKILL_TIGER_PALM, TAR_MENTAL,
				   "Your tiger palm concentration leaves you.", "");
	...
	SKILL_CREATE_WITH_MESSAGES("whirlwind", SKILL_WHIRLWIND, TAR_PHYS,
				   "You slow down exhausted.", "$n slows down exhausted.");
	SPEC_SKILL_ADD(CLASS_RANGER, 30, 100, SPEC_BLADEMASTER);

	SPELL_CREATE("healing salve", SPELL_HEALING_SALVE, PULSE_SPELLCAST, TAR_CHAR_ROOM,
		     spell_healing_salve);
	SPEC_SPELL_ADD(CLASS_CLERIC, 6, SPEC_HEALER);

	SPELL_CREATE_MSG("plague", SPELL_PLAGUE, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_plague,
			 "Your body managed to defeat the disease.");
	SPEC_SPELL_ADD(CLASS_CLERIC, 11, SPEC_ZEALOT);

	SPELL_CREATE("banish", SPELL_BANISH, PULSE_SPELLCAST * 2, TAR_AREA | TAR_OFFAREA,
		     spell_banish);
	...
	SPELL_CREATE("natures calling", SPELL_NATURES_CALLING, PULSE_SPELLCAST * 3, TAR_IGNORE,
		     spell_natures_call);
	...
	SPELL_CREATE("call deva", SPELL_CALL_DEVA, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT,
		     spell_call_deva);
	...
	SPELL_CREATE_MSG("obscuring mist", SPELL_OBSCURING_MIST, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_obscuring_mist,
	...
	SPELL_CREATE("mirage", SPELL_MIRAGE, PULSE_SPELLCAST * 8, TAR_IGNORE | TAR_NOCOMBAT,
		     spell_mirage);
	SPELL_ADD(CLASS_ILLUSIONIST, 12);

	/* end specs */

	SPELL_CREATE("pleasantry", SPELL_PLEASANTRY, PULSE_SPELLCAST, TAR_CHAR_ROOM | TAR_AGGRO,
		     spell_pleasantry);
	...
	SPELL_CREATE_MSG(
		"learning", SPELL_LEARNING, PULSE_SPELLCAST, TAR_SELF_ONLY, spell_learning,
	...
	SPELL_CREATE_MSG("blackmantle", SPELL_BMANTLE, PULSE_SPELLCAST * 4,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_blackmantle,
			 "&+LThe &+bnegative energy droplets &+Ldissipate harmlessly.");
	...
	SPELL_CREATE("create greater dracolich", SPELL_CREATE_GREATER_DRACOLICH,
		     PULSE_SPELLCAST * 10, TAR_OBJ_ROOM | TAR_NOCOMBAT,
		     spell_create_greater_dracolich);
	SPELL_ADD(CLASS_NECROMANCER, 12);

	SPELL_CREATE("call avatar", SPELL_CALL_AVATAR, PULSE_SPELLCAST * 10,
		     TAR_OBJ_ROOM | TAR_NOCOMBAT, spell_call_avatar);
	...
	SPELL_CREATE_MSG("regeneration", SPELL_REGENERATION, PULSE_SPELLCAST * 3 / 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_regeneration,
	...
	SPELL_CREATE("depressed earth", SPELL_DEPRESSED_EARTH, PULSE_SPELLCAST * 4,
		     TAR_IGNORE | TAR_NOCOMBAT | TAR_SPIRIT, cast_depressed_earth);
	...
	SPELL_CREATE("transmute rock to mud", SPELL_TRANS_ROCK_MUD, PULSE_SPELLCAST * 4,
		     TAR_IGNORE | TAR_NOCOMBAT, cast_transmute_rock_mud);
	...
	SPELL_CREATE("hurricane", SPELL_HURRICANE, PULSE_SPELLCAST * 4, TAR_AREA | TAR_AGGRO,
		     cast_hurricane);
	...
	SPELL_CREATE_MSG("blood to stone", SPELL_BLOODTOSTONE, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, cast_bloodstone,
			 "Your blood flows normally again.");
	SPEC_SPELL_ADD(CLASS_DRUID, 11, SPEC_STORM);
	...
	SPELL_CREATE_MSG("waves of fatigue", SPELL_WAVES_FATIGUE, PULSE_SPELLCAST * 4,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_waves_fatigue,
	...
	SPELL_CREATE_MSG("accelerated healing", SPELL_ACCEL_HEALING, (PULSE_SPELLCAST * 3) / 2,
			 TAR_CHAR_ROOM, spell_accel_healing, "You no longer regenerate.");
	...
	SPELL_CREATE_MSG("sap nature", SPELL_SAP_NATURE, PULSE_SPELLCAST * 3 / 2, TAR_SELF_ONLY,
			 spell_sap_nature, "&+yThe &+Genergy&+y ceases to flow.&n");
	SPELL_ADD(CLASS_BLIGHTER, 6);
	...
	SPELL_CREATE_MSG("armor", SPELL_ARMOR, PULSE_SPELLCAST, TAR_CHAR_ROOM, spell_armor,
			 "You feel less &+Wprotected&n.");
	SPELL_ADD(CLASS_CLERIC, 1);
	SPELL_ADD(CLASS_PALADIN, 3);
	SPELL_ADD(CLASS_ANTIPALADIN, 3);

	SPELL_CREATE_MSG("virtue", SPELL_VIRTUE, PULSE_SPELLCAST, TAR_CHAR_ROOM, spell_virtue,
	...
	SPELL_CREATE("group teleport", SPELL_GROUP_TELEPORT, PULSE_SPELLCAST, TAR_IGNORE,
	...
	SPELL_CREATE("excogitate", SPELL_EXCOGITATE, PULSE_SPELLCAST / 2, TAR_CHAR_ROOM,
		     spell_excogitate);
	...
	SPELL_CREATE_MSG("fire aura", SPELL_FIRE_AURA, PULSE_SPELLCAST * 6,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_fire_aura,
			 "&+BYour &+rfiery aura &+Breturns to normal.&N");
	...
	SPELL_CREATE("wormhole", SPELL_WORMHOLE, PULSE_SPELLCAST * 6, TAR_CHAR_WORLD | TAR_NOCOMBAT,
		     spell_wormhole);
	SPELL_ADD(CLASS_MINDFLAYER, 9);
	...
	SPELL_CREATE("molecular agitation", SPELL_MOLECULAR_AGITATION, PULSE_SPELLCAST * 3,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_molecular_agitation);
	...
	SPELL_CREATE("spinal corruption", SPELL_SPINAL_CORRUPTION, PULSE_SPELLCAST * 3,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_spinal_corruption);
	...
	SPELL_CREATE_MSG("adrenaline control", SPELL_ADRENALINE_CONTROL, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY, spell_adrenaline_control,
			 "&+rYour rush of adrenaline runs dry.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 1);
	SPELL_ADD(CLASS_PSIONICIST, 1);

	SPELL_CREATE_MSG("aura sight", SPELL_AURA_SIGHT, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
			 spell_aura_sight, "&+CThe auras in your vision disappear.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 3);
	SPELL_ADD(CLASS_PSIONICIST, 3);

	SPELL_CREATE_MSG("awe", SPELL_AWE, PULSE_SPELLCAST * 4, TAR_CHAR_ROOM, spell_awe,
...
#ifndef _DE_
	skills[SPELL_MENTAL_ANGUISH].wear_off_room[0] =
		"$n straightens up and begins fighting more confidently.";
#endif
	SPELL_ADD(CLASS_MINDFLAYER, 8);
	...
	SPELL_CREATE_MSG("biofeedback", SPELL_BIOFEEDBACK, (PULSE_SPELLCAST * 5) / 2, TAR_SELF_ONLY,
			 spell_biofeedback, "&+GThe green mist around your body fades.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 7);
	SPELL_ADD(CLASS_PSIONICIST, 11);

	SPELL_CREATE_MSG("displacement", SPELL_DISPLACEMENT, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
			 spell_displacement, "&+WYour body shifts back into this space-time.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 8);
	SPEC_SPELL_ADD(CLASS_PSIONICIST, 9, SPEC_PSYCHEPORTER);

	SPELL_CREATE("cell adjustment", SPELL_CELL_ADJUSTMENT, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
		     spell_cell_adjustment);
	SPELL_ADD(CLASS_MINDFLAYER, 2);
	SPELL_ADD(CLASS_PSIONICIST, 6);

	SPELL_CREATE_MSG("combat mind", SPELL_COMBAT_MIND, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
	...
	SPELL_CREATE("create sound", SPELL_CREATE_SOUND, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_create_sound);
	SPELL_ADD(CLASS_MINDFLAYER, 2);
	SPELL_ADD(CLASS_PSIONICIST, 6);

	SPELL_CREATE("death field", SPELL_DEATH_FIELD, PULSE_SPELLCAST * 3, TAR_AREA | TAR_OFFAREA,
		     spell_death_field);
	SPELL_ADD(CLASS_MINDFLAYER, 10);
	SPELL_ADD(CLASS_PSIONICIST, 10);
	...
	SPELL_CREATE("ectoplasmic form", SPELL_ECTOPLASMIC_FORM, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
	...
	SPELL_CREATE("psionic wave blast", SPELL_PSIONIC_WAVE_BLAST, PULSE_SPELLCAST,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_CHAR_RANGE | TAR_AGGRO,
		     spell_psionic_wave_blast);
	// SPELL_ADD(CLASS_PSIONICIST, 11);
	// SPELL_ADD(CLASS_MINDFLAYER, 11);

	SPELL_CREATE_MSG("energy containment", SPELL_ENERGY_CONTAINMENT, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY, spell_energy_containment,
			 "&+YYou can no longer absorb energy.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 4);
	SPELL_ADD(CLASS_PSIONICIST, 4);

	SPELL_CREATE_MSG("enhance armor", SPELL_ENHANCE_ARMOR, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
	...
	SPELL_CREATE_MSG("enhance strength", SPELL_ENHANCED_STR, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_enhanced_strength,
			 "&+CYour muscles return to normal.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 2);
	SPELL_ADD(CLASS_PSIONICIST, 2);

	SPELL_CREATE_MSG("enhance dexterity", SPELL_ENHANCED_DEX, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_enhanced_dexterity,
			 "&+yYour coordination decreases.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 2);
	SPELL_ADD(CLASS_PSIONICIST, 2);

	SPELL_CREATE_MSG("enhance agility", SPELL_ENHANCED_AGI, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_enhanced_agility,
			 "&+CYou feel a bit more clumsy.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 2);
	SPELL_ADD(CLASS_PSIONICIST, 2);

	SPELL_CREATE_MSG("enhance constitution", SPELL_ENHANCED_CON, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_enhanced_constitution,
			 "&+YYour vitality drains away.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 2);
	SPELL_ADD(CLASS_PSIONICIST, 2);

	SPELL_CREATE_MSG("flesh armor", SPELL_FLESH_ARMOR, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
	...
	SPELL_CREATE_MSG("inertial barrier", SPELL_INERTIAL_BARRIER, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY, spell_inertial_barrier,
	...
	SPELL_CREATE_MSG("intellect fortress", SPELL_INTELLECT_FORTRESS, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY, spell_intellect_fortress,
	...
	SPELL_CREATE_MSG("flight", SPELL_POWERCAST_FLY, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
			 spell_fly, "&+CYour feet slowly descend to the ground.&n");
	SPELL_ADD(CLASS_MINDFLAYER, 5);
	SPELL_ADD(CLASS_PSIONICIST, 6);
	SPEC_SPELL_ADD(CLASS_REAVER, 9, SPEC_SHOCK_REAVER);

	SPELL_CREATE("confuse", SPELL_CONFUSE, PULSE_SPELLCAST * 4, TAR_AREA | TAR_AGGRO,
		     spell_confuse);
	SPELL_ADD(CLASS_MINDFLAYER, 9);
	SPEC_SPELL_ADD(CLASS_PSIONICIST, 12, SPEC_PYROKINETIC);

	SPELL_CREATE_MSG("mind blank", SPELL_MIND_BLANK, PULSE_SPELLCAST * 8, TAR_SELF_ONLY,
	...
	SPELL_CREATE_MSG("cannibalize", SPELL_CANNIBALIZE, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
	...
	*/
	...
	SPELL_CREATE_MSG("tower of iron will", SPELL_TOWER_IRON_WILL, PULSE_SPELLCAST * 4,
			 TAR_SELF_ONLY, spell_tower_iron_will,
			 "&+YYou just lost your tower of iron will!&N");
	SPELL_ADD(CLASS_MINDFLAYER, 7);
	SPELL_ADD(CLASS_PSIONICIST, 7);

	SPELL_CREATE("celerity", SPELL_CELERITY, PULSE_SPELLCAST * 2, TAR_SELF_ONLY | TAR_NOCOMBAT,
		     spell_celerity);
	//  SPELL_ADD(CLASS_PSIONICIST, 5);
	...
	SPELL_CREATE_MSG("chill touch", SPELL_CHILL_TOUCH, PULSE_SPELLCAST * 7 / 9,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_chill_touch,
			 "You feel less &+Bchilled&n.");
	...
	SPELL_CREATE_MSG("cone of cold", SPELL_CONE_OF_COLD, PULSE_SPELLCAST * 4 / 3,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_CHAR_RANGE | TAR_AGGRO,
			 spell_cone_of_cold, "You feel less &+Bcold&n.");
	...
	SPELL_CREATE_MSG("wither", SPELL_WITHER, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_wither,
			 "You feel less &+Lwithered&n.");
	SPELL_ADD(CLASS_ANTIPALADIN, 8);
	SPELL_ADD(CLASS_NECROMANCER, 4);
	...
	SPELL_CREATE("bigbys clenched fist", SPELL_BIGBYS_CLENCHED_FIST, PULSE_SPELLCAST * 5 / 3,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_bigbys_clenched_fist);
	...
	SPELL_CREATE("solbeeps missile barrage", SPELL_MISSILE_BARRAGE, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_solbeeps_missile_barrage);
	...
	SPELL_CREATE("mordenkainens lucubration", SPELL_MORDENKAINENS_LUCUBRATION,
		     PULSE_SPELLCAST * 4, TAR_SELF_ONLY, spell_mordenkainens_lucubration);
	...
	SPELL_CREATE("bigbys crushing hand", SPELL_BIGBYS_CRUSHING_HAND, PULSE_SPELLCAST * 19 / 9,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_bigbys_crushing_hand);
	SPELL_ADD(CLASS_SORCERER, 10);
	...
	SPELL_CREATE_MSG("wizard eye", SPELL_WIZARD_EYE, PULSE_SPELLCAST,
			 TAR_CHAR_WORLD | TAR_SELF_NONO | TAR_NOCOMBAT, spell_wizard_eye,
			 "Your magical eyesight dissipates.");
	SPELL_ADD(CLASS_SORCERER, 4);

	SPELL_CREATE_MSG("clairvoyance", SPELL_CLAIRVOYANCE, PULSE_SPELLCAST * 2,
			 TAR_CHAR_WORLD | TAR_SELF_NONO | TAR_NOCOMBAT, spell_clairvoyance,
			 "Your clairvoyant vision dissipates.");
	...
	SPELL_CREATE_MSG("rejuvenate major", SPELL_REJUVENATE_MAJOR, PULSE_SPELLCAST * 3,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_rejuvenate_major,
			 "You feel &+Lolder&n.");
	SPELL_ADD(CLASS_NECROMANCER, 6);
	SPELL_ADD(CLASS_THEURGIST, 6);

	SPELL_CREATE("age", SPELL_AGE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM | TAR_NOCOMBAT,
		     spell_age);
	SPELL_ADD(CLASS_NECROMANCER, 9);
	SPELL_ADD(CLASS_THEURGIST, 9);

	SPELL_CREATE_MSG("rejuvenate minor", SPELL_REJUVENATE_MINOR, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_rejuvenate_minor,
			 "you feel &+Lolder&n.");
	SPELL_ADD(CLASS_NECROMANCER, 4);
	SPELL_ADD(CLASS_THEURGIST, 4);

	SPELL_CREATE_MSG("ray of enfeeblement", SPELL_RAY_OF_ENFEEBLEMENT, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_ray_of_enfeeblement,
	...
	SPELL_CREATE("earthquake", SPELL_EARTHQUAKE, PULSE_SPELLCAST * 2, TAR_OFFAREA | TAR_AGGRO,
		     spell_earthquake);
	...
	SPELL_CREATE("call woodland beings", SPELL_CALL_WOODLAND, PULSE_SPELLCAST * 3,
		     TAR_IGNORE | TAR_NOCOMBAT, spell_call_woodland_beings);
	...
	SPELL_CREATE_MSG("flame strike", SPELL_FLAMESTRIKE, PULSE_SPELLCAST + 1,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_CHAR_RANGE | TAR_AGGRO,
			 spell_flamestrike, "The &+rbrand&n begins to fade...");
	SPELL_ADD(CLASS_CLERIC, 6);
	SPEC_SPELL_ADD(CLASS_CLERIC, 0, SPEC_HEALER);
	SPELL_ADD(CLASS_BLIGHTER, 6);
	...
	SPELL_CREATE_MSG("mielikki vitality", SPELL_MIELIKKI_VITALITY, PULSE_SPELLCAST * 3,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_mielikki_vitality,
			 "&+GMielikki's blessing fades.&n");
	SPELL_ADD(CLASS_DRUID, 7);
	// SPELL_ADD(CLASS_RANGER, 8);

	SPELL_CREATE_MSG("faluzures vitality", SPELL_FALUZURES_VITALITY, PULSE_SPELLCAST * 3,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_faluzures_vitality,
			 "&+yFa&+Lluz&+yure&+L's blessing fades.&n");
	SPELL_ADD(CLASS_BLIGHTER, 7);

	SPELL_CREATE_MSG("bless", SPELL_BLESS, PULSE_SPELLCAST,
			 TAR_OBJ_INV | TAR_OBJ_EQUIP | TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_bless,
	...
	SPELL_CREATE_MSG("blindness", SPELL_BLINDNESS, PULSE_SPELLCAST * 3 / 2,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_blindness,
	...
	SPELL_CREATE_MSG("minor globe of invulnerability", SPELL_MINOR_GLOBE, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM, spell_minor_globe,
	...
	SPELL_CREATE_MSG("charm person", SPELL_CHARM_PERSON, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_SELF_NONO | TAR_AGGRO, spell_charm_person,
			 "You regain your senses.");

	SPELL_CREATE_MSG("infravision", SPELL_INFRAVISION, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_infravision,
			 "Your &+rinfravision&n dissipates.");
	SPELL_ADD(CLASS_SORCERER, 4);
	SPELL_ADD(CLASS_CONJURER, 4);
	SPELL_ADD(CLASS_SUMMONER, 4);
	SPELL_ADD(CLASS_RANGER, 7);
	...
	SPELL_CREATE("harmonic resonance", SPELL_HARMONIC_RESONANCE, PULSE_SPELLCAST * 2,
		     TAR_AREA | TAR_OFFAREA | TAR_AGGRO, spell_harmonic_resonance);
	...
	SPELL_CREATE("sandstorm", SPELL_SANDSTORM, PULSE_SPELLCAST * 3, TAR_OFFAREA | TAR_AGGRO,
		     spell_sandstorm);
	SPELL_ADD(CLASS_BLIGHTER, 12);

	SPELL_CREATE("spore burst", SPELL_SPORE_BURST, PULSE_SPELLCAST * 3, TAR_OFFAREA | TAR_AGGRO,
		     spell_spore_burst);
	SPEC_SPELL_ADD(CLASS_DRUID, 11, SPEC_WOODLAND);
	...
	SPELL_CREATE_MSG("mage flame", SPELL_MAGE_FLAME, (PULSE_SPELLCAST * 3) / 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_mage_flame,
			 "Your mage flame slowly fades into nothingness.");
	SPELL_ADD(CLASS_CONJURER, 3);
	SPELL_ADD(CLASS_SUMMONER, 3);

	SPELL_CREATE_MSG("holy light", SPELL_HOLY_LIGHT, (PULSE_SPELLCAST * 3) / 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_holy_light,
			 "Your holy light slowly fades into nothingness.");
	SPELL_ADD(CLASS_THEURGIST, 9);

	SPELL_CREATE("globe of darkness", SPELL_GLOBE_OF_DARKNESS, PULSE_SPELLCAST * 2,
		     TAR_SELF_ONLY | TAR_NOCOMBAT, spell_globe_of_darkness);
	SPELL_ADD(CLASS_NECROMANCER, 9);
	...
	SPELL_CREATE_MSG("pulchritude", SPELL_PULCHRITUDE, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_ANIMAL, spell_pulchritude,
	...
	SPELL_CREATE("cure blind", SPELL_CURE_BLIND, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
		     spell_cure_blind);
	SPELL_ADD(CLASS_CLERIC, 3);
	...
	SPELL_CREATE2("wandering woods", SPELL_WANDERING_WOODS, PULSE_SPELLCAST * 4,
		      TAR_IGNORE | TAR_NOCOMBAT, spell_wandering_woods,
	...
	SPELL_CREATE("devitalize", SPELL_DEVITALIZE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
		     spell_devitalize);
	...
	SPELL_CREATE("invigorate", SPELL_INVIGORATE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
		     spell_invigorate);
	SPELL_ADD(CLASS_PALADIN, 5);
	SPELL_ADD(CLASS_ANTIPALADIN, 5);
	SPELL_ADD(CLASS_RANGER, 5);
	...
	SPELL_CREATE_MSG("dispel lifeforce", SPELL_DISPEL_LIFEFORCE, PULSE_SPELLCAST * 3 / 2,
			 ...
			 spell_dispel_lifeforce, "&+wYou feel the life surge back into you");
	...
	SPELL_CREATE_MSG("water breathing", SPELL_WATERBREATH, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_waterbreath,
			 "Your gills retract back into your neck.");
	SPELL_ADD(CLASS_CLERIC, 7);
	//  SPELL_ADD(CLASS_DRUID, 7);

	SPELL_CREATE_MSG("farsee", SPELL_FARSEE, PULSE_SPELLCAST * 5 / 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_farsee,
			 "You cannot see as far anymore.");
	SPELL_ADD(CLASS_SORCERER, 4);
	...
	SPELL_CREATE_MSG("fear", SPELL_FEAR, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_fear,
			 "You feel less cowardly.");
	SPELL_ADD(CLASS_CLERIC, 6);
	SPELL_ADD(CLASS_ANTIPALADIN, 5);

	SPELL_CREATE("recharger", SPELL_RECHARGER, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
		     spell_recharger);
	/*  SPELL_ADD(CLASS_PSIONICIST, 2);*/

	SPELL_CREATE("cure light", SPELL_CURE_LIGHT, PULSE_SPELLCAST, TAR_CHAR_ROOM,
		     spell_cure_light);
	SPELL_ADD(CLASS_CLERIC, 1);
	//  SPELL_ADD(CLASS_DRUID, 1);
	SPELL_ADD(CLASS_PALADIN, 3);
	...
	SPELL_CREATE("cure disease", SPELL_CURE_DISEASE, PULSE_SPELLCAST, TAR_CHAR_ROOM,
		     spell_cure_disease);
	SPELL_ADD(CLASS_CLERIC, 4);
	...
	SPELL_CREATE("invoke negative energy", SPELL_INVOKE_NEG_ENERGY, PULSE_SPELLCAST * 4 / 3,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_invoke_negative_energy);
	SPELL_ADD(CLASS_WARLOCK, 3);
	...
	SPELL_CREATE("channel negative energy", SPELL_CHANNEL_NEG_ENERGY, PULSE_SPELLCAST * 5 / 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_channel_negative_energy);
	...
	SPELL_CREATE("natures touch", SPELL_NATURES_TOUCH, PULSE_SPELLCAST * 3 / 2, TAR_CHAR_ROOM,
		     spell_natures_touch);
	SPELL_ADD(CLASS_DRUID, 5);
	//  SPELL_ADD(CLASS_RANGER, 7);

	SPELL_CREATE("drain nature", SPELL_DRAIN_NATURE, PULSE_SPELLCAST * 3 / 2, TAR_CHAR_ROOM,
		     spell_drain_nature);
	SPELL_ADD(CLASS_BLIGHTER, 5);

	SPELL_CREATE("sticks to snakes", SPELL_STICKS_TO_SNAKES, PULSE_SPELLCAST * 3 / 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_sticks_to_snakes);
	SPELL_ADD(CLASS_DRUID, 2);

	SPELL_CREATE("full heal", SPELL_FULL_HEAL, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
		     spell_full_heal);
	SPELL_ADD(CLASS_CLERIC, 7);

	SPELL_CREATE_MSG("vitalize undead", SPELL_VITALIZE_UNDEAD, PULSE_SPELLCAST * 7 / 3,
			 TAR_CHAR_ROOM, spell_vitalize_undead,
			 "Your enhanced strength drains away.");
	SPELL_ADD(CLASS_NECROMANCER, 6);

	SPELL_CREATE_MSG("vitalize soul", SPELL_VITALIZE_SOUL, PULSE_SPELLCAST * 7 / 3,
			 TAR_CHAR_ROOM, spell_vitalize_undead, "Your enhanced spirit drains away.");
	SPELL_ADD(CLASS_THEURGIST, 6);
	...
	SPELL_CREATE_MSG("detect evil", SPELL_DETECT_EVIL, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_detect_evil,
	...
	SPELL_CREATE_MSG("detect good", SPELL_DETECT_GOOD, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_detect_good,
	...
	SPELL_CREATE_MSG("detect magic", SPELL_DETECT_MAGIC, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_detect_magic,
	...
	SPELL_CREATE_MSG("negative energy barrier", SPELL_NEG_ENERGY_BARRIER, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY, spell_negative_energy_barrier,
	...
	SPELL_CREATE("lesser resurrect", SPELL_LESSER_RESURRECT, PULSE_SPELLCAST * 5,
		     TAR_OBJ_ROOM | TAR_NOCOMBAT | TAR_SPIRIT, spell_lesser_resurrect);
	SPELL_ADD(CLASS_CLERIC, 7);
	...
	SPELL_CREATE_MSG("death blessing", SPELL_DEATH_BLESSING, PULSE_SPELLCAST * 7 / 3,
			 TAR_IGNORE, spell_death_blessing, "You feel less vitalized.");
	SPELL_ADD(CLASS_WARLOCK, 12);

	SPELL_CREATE("preserve", SPELL_PRESERVE, PULSE_SPELLCAST * 2, TAR_OBJ_ROOM | TAR_NOCOMBAT,
		     spell_preserve);
	SPELL_ADD(CLASS_CLERIC, 2);
	SPELL_ADD(CLASS_NECROMANCER, 1);
	SPELL_ADD(CLASS_THEURGIST, 1);
	...
	SPELL_CREATE("unmaking", SPELL_UNMAKING, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT,
		     spell_unmaking);
	SPELL_ADD(CLASS_WARLOCK, 4);
	...
	*/
	...
	SPELL_CREATE_MSG("invisibility", SPELL_INVISIBILITY, PULSE_SPELLCAST * 4 / 3,
			 TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_OBJ_EQUIP | TAR_NOCOMBAT,
	...
	SPELL_CREATE_MSG("concealment", SPELL_CONCEALMENT, PULSE_SPELLCAST * 4 / 3,
			 TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_OBJ_EQUIP | TAR_NOCOMBAT,
			 spell_concealment, "You slowly fade back into visibility.");
	SPELL_ADD(CLASS_NECROMANCER, 5);
	SPELL_ADD(CLASS_SORCERER, 3);
	SPELL_ADD(CLASS_CONJURER, 3);
	SPELL_ADD(CLASS_SUMMONER, 3);
	...
	SPELL_CREATE_MSG("protection from evil", SPELL_PROTECT_FROM_EVIL, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM, spell_protection_from_evil,
	...
	SPELL_CREATE_MSG("protection from good", SPELL_PROTECT_FROM_GOOD, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM, spell_protection_from_good,
	...
	SPELL_CREATE_MSG("righteous aura", SPELL_RIGHTEOUS_AURA, PULSE_SPELLCAST * 3 / 2,
			 TAR_SELF_ONLY, spell_righteous_aura,
			 "&+WThe power of your god leaves your body, and you feel weaker.&n");
	SPELL_ADD(CLASS_PALADIN, 12);

	SPELL_CREATE_MSG(
		"bleak foeman", SPELL_BLEAK_FOEMAN, PULSE_SPELLCAST * 3 / 2, TAR_SELF_ONLY,
		spell_bleak_foeman,
	...
	SPELL_CREATE("remove curse", SPELL_REMOVE_CURSE, PULSE_SPELLCAST * 4,
		     TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_EQUIP | TAR_OBJ_ROOM | TAR_NOCOMBAT,
		     spell_remove_curse);
	SPELL_ADD(CLASS_PALADIN, 6);
	SPELL_ADD(CLASS_CLERIC, 4);
	//  SPELL_ADD(CLASS_DRUID, 5);
	...
	SPELL_CREATE("dispel magic", SPELL_DISPEL_MAGIC, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_WALL |
			     TAR_AGGRO,
		     spell_dispel_magic);
	SPELL_ADD(CLASS_CONJURER, 3);
	SPELL_ADD(CLASS_SUMMONER, 3);
	...
	SPELL_CREATE("tranquility", SPELL_TRANQUILITY, PULSE_SPELLCAST * 3, TAR_IGNORE,
		     spell_tranquility);
	SPELL_ADD(CLASS_DRUID, 5);

	SPELL_CREATE_MSG("strength", SPELL_STRENGTH, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_strength,
			 "You feel less &+Cstrong&n.");
	SPELL_ADD(CLASS_NECROMANCER, 4);
	SPELL_ADD(CLASS_SORCERER, 2);
	SPELL_ADD(CLASS_CONJURER, 3);
	SPELL_ADD(CLASS_SUMMONER, 3);
	...
	SPELL_CREATE_MSG("dexterity", SPELL_DEXTERITY, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_dexterity,
			 "You feel less &+Ldexterous&n.");
	//  SPELL_ADD(CLASS_SORCERER, 3);
	//  SPELL_ADD(CLASS_CONJURER, 3);
	//  SPELL_ADD(CLASS_SUMMONER, 3);

	SPELL_CREATE_MSG("agility", SPELL_AGILITY, PULSE_SPELLCAST * 3,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_agility, "You feel less &+cagile&n.");
	SPELL_ADD(CLASS_SORCERER, 3);
	SPELL_ADD(CLASS_CONJURER, 3);
	SPELL_ADD(CLASS_SUMMONER, 3);

	SPELL_CREATE("summon", SPELL_SUMMON, PULSE_SPELLCAST * 2, TAR_CHAR_WORLD | TAR_NOCOMBAT,
		     spell_summon);
	SPELL_ADD(CLASS_CLERIC, 4);
	//  SPELL_ADD(CLASS_DRUID, 4);
	...
	SPELL_CREATE_MSG("dazzle", SPELL_DAZZLE, PULSE_SPELLCAST * 2, TAR_SELF_ONLY, spell_dazzle,
			 "You feel the energy seep out of you.");
	SPELL_ADD(CLASS_RANGER, 10);

	SPELL_CREATE_MSG("blur", SPELL_BLUR, PULSE_SPELLCAST * 2, TAR_SELF_ONLY, spell_blur,
			 "&+BYour motions slow to normal.");
	SPELL_ADD(CLASS_RANGER, 11);

	SPELL_CREATE_MSG("pass without trace", SPELL_PASS_WITHOUT_TRACE, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY, spell_pass_without_trace,
			 "&+gThe forest closes in around you.&n");
	...
	SPELL_CREATE_MSG("hellfire", SPELL_HELLFIRE, PULSE_SPELLCAST, TAR_SELF_ONLY, spell_hellfire,
	...
	SPELL_CREATE_MSG("stornogs metamagic shroud", SPELL_STORNOGS_LOWERED_RES,
			 PULSE_SPELLCAST * 3, TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO,
	...
	SPELL_CREATE2("stornogs shimmering starshell", SPELL_STARSHELL, (PULSE_SPELLCAST * 3) / 2,
		      TAR_IGNORE, spell_starshell, "&+WThe &+Yblazing&N&+W shell dissipates.",
	...
	SPELL_CREATE_MSG("decaying flesh", SPELL_DECAYING_FLESH, PULSE_SPELLCAST * 1,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_decaying_flesh,
			 "The &+gdecay&n finally leaves your body.");
	...
	SPELL_CREATE("cloak of fear", SPELL_CLOAK_OF_FEAR, PULSE_SPELLCAST, TAR_OFFAREA | TAR_AGGRO,
		     spell_cloak_of_fear);
	SPELL_ADD(CLASS_NECROMANCER, 10);
	SPELL_ADD(CLASS_THEURGIST, 10);
	...
	SPELL_CREATE_MSG("haste", SPELL_HASTE, (PULSE_SPELLCAST * 3) / 2, TAR_CHAR_ROOM,
			 spell_haste, "The world speeds up around you.");
	SPELL_ADD(CLASS_SORCERER, 7);
	SPELL_ADD(CLASS_CONJURER, 7);
	SPELL_ADD(CLASS_SUMMONER, 7);
	SPELL_ADD(CLASS_NECROMANCER, 8);
	...
	SPELL_CREATE("word of recall", SPELL_WORD_OF_RECALL, 0, TAR_SELF_ONLY,
	...
	SPELL_CREATE("group recall", SPELL_GROUP_RECALL, PULSE_SPELLCAST * 6, TAR_IGNORE,
		     spell_group_recall);
	SPELL_ADD(CLASS_CLERIC, 11);
	// SPELL_ADD(CLASS_WARLOCK, 11);

	SPELL_CREATE("remove poison", SPELL_REMOVE_POISON, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_NOCOMBAT,
		     spell_remove_poison);
	SPELL_ADD(CLASS_CLERIC, 3);
	SPELL_ADD(CLASS_PALADIN, 4);
	SPELL_ADD(CLASS_WARLOCK, 3);
	...
	SPELL_CREATE_MSG("minor paralysis", SPELL_MINOR_PARALYSIS, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_minor_paralysis,
			 "You can move again.");
	SPELL_ADD(CLASS_SORCERER, 4);

	SPELL_CREATE_MSG("major paralysis", SPELL_MAJOR_PARALYSIS, PULSE_SPELLCAST * 3,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_major_paralysis,
			 "You can move again.");

	SPELL_CREATE_MSG("slowness", SPELL_SLOW, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_slow,
	...
	SPELL_CREATE("conjure elemental", SPELL_CONJURE_ELEMENTAL, PULSE_SPELLCAST * 4, TAR_IGNORE,
		     spell_conjour_elemental);
	SPELL_ADD(CLASS_CONJURER, 5);

	SPELL_CREATE("mirror image", SPELL_MIRROR_IMAGE, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_mirror_image);
	SPELL_ADD(CLASS_CONJURER, 2);
	SPELL_ADD(CLASS_SUMMONER, 2);
	...
	SPELL_CREATE("conjure greater elemental", SPELL_CONJURE_GREATER_ELEMENTAL,
		     PULSE_SPELLCAST * 4, TAR_IGNORE, spell_conjour_greater_elemental);
	SPELL_ADD(CLASS_CONJURER, 11);
	// SPEC_SPELL_ADD(CLASS_CONJURER, 11, SPEC_AIR);
	// SPEC_SPELL_ADD(CLASS_CONJURER, 11, SPEC_FIRE);
	// SPEC_SPELL_ADD(CLASS_CONJURER, 11, SPEC_WATER);
	...
	SPELL_CREATE("vitalize mana", SPELL_VITALIZE_MANA, PULSE_SPELLCAST, TAR_SELF_ONLY,
		     spell_vitalize_mana);

	SPELL_CREATE_MSG("sense follower", SPELL_SENSE_FOLLOWER, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_sense_follower,
	...
	SPELL_CREATE_MSG("sense life", SPELL_SENSE_LIFE, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_sense_life,
	...
	SPELL_CREATE("continual light", SPELL_CONTINUAL_LIGHT, PULSE_SPELLCAST * 3,
		     TAR_OBJ_ROOM | TAR_OBJ_INV | TAR_AREA, spell_continual_light);
	SPELL_ADD(CLASS_CLERIC, 6);
	//  SPELL_ADD(CLASS_DRUID, 6);
	SPELL_ADD(CLASS_PALADIN, 8);
	SPELL_ADD(CLASS_ILLUSIONIST, 2);
	...
	SPELL_CREATE2("consecrate land", SPELL_CONSECRATE_LAND, PULSE_SPELLCAST * 3,
		      TAR_IGNORE | TAR_SPIRIT, spell_consecrate_land,
	...
	SPELL_CREATE2("desecrate land", SPELL_DESECRATE_LAND, PULSE_SPELLCAST * 3,
		      TAR_IGNORE | TAR_SPIRIT, spell_desecrate_land,
	...
	SPELL_CREATE2("forbiddance", SPELL_FORBIDDANCE, PULSE_SPELLCAST * 3,
		      TAR_IGNORE | TAR_SPIRIT, spell_forbiddance,
		      "&+LYour seal in this area is broken.&n",
	...
	SPELL_CREATE_MSG("protection from fire", SPELL_PROTECT_FROM_FIRE, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM, spell_protection_from_fire,
	...
	SPELL_CREATE_MSG("protection from cold", SPELL_PROTECT_FROM_COLD, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM, spell_protection_from_cold,
	...
	SPELL_CREATE_MSG("protection from living", SPELL_PROTECT_FROM_LIVING, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM, spell_protection_from_living,
	...
	SPELL_CREATE_MSG("protection from animals", SPELL_PROTECT_FROM_ANIMAL, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_SPIRIT, spell_protection_from_animals,
	...
	SPELL_CREATE_MSG("protection from lightning", SPELL_PROTECT_FROM_LIGHTNING, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM, spell_protection_from_lightning,
	...
	SPELL_CREATE("iluck", SPELL_PERM_INCREASE_LUCK, PULSE_SPELLCAST,
		     TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_perm_increase_luck);
	...
	SPELL_CREATE("prismatic spray", SPELL_PRISMATIC_SPRAY, PULSE_SPELLCAST * 2,
		     TAR_AREA | TAR_OFFAREA | TAR_AGGRO, spell_prismatic_spray);
	SPELL_ADD(CLASS_CONJURER, 8);
	SPELL_ADD(CLASS_SUMMONER, 8);
	/*  SPELL_ADD(CLASS_SORCERER, 7);*/
	...
	SPELL_CREATE("color spray", SPELL_COLOR_SPRAY, PULSE_SPELLCAST * 5 / 3,
		     TAR_AREA | TAR_OFFAREA | TAR_AGGRO, spell_color_spray);
	SPELL_ADD(CLASS_SORCERER, 6);
	SPELL_ADD(CLASS_CONJURER, 6);
	SPELL_ADD(CLASS_SUMMONER, 6);

	SPELL_CREATE("incendiary cloud", SPELL_INCENDIARY_CLOUD, (int)(PULSE_SPELLCAST * 3),
		     TAR_AREA | TAR_OFFAREA | TAR_AGGRO, spell_incendiary_cloud);
	SPELL_ADD(CLASS_SORCERER, 9);
	...
	SPELL_CREATE_MSG("guardian spirits", SPELL_GUARDIAN_SPIRITS, PULSE_SPELLCAST * 7,
			 TAR_SELF_ONLY | TAR_NOCOMBAT | TAR_SPIRIT, spell_guardian_spirits,
	...
	SPELL_CREATE_MSG("faerie fire", SPELL_FAERIE_FIRE, PULSE_SPELLCAST * 3 / 2,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_faerie_fire,
	...
	SPELL_CREATE("power word blind", SPELL_PWORD_BLIND, PULSE_SPELLCAST / 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_INSTACAST | TAR_AGGRO, spell_pword_blind);
	SPELL_ADD(CLASS_SORCERER, 7);

	SPELL_CREATE_MSG("power word stun", SPELL_PWORD_STUN, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_pword_stun,
	...
	SPELL_CREATE_MSG("feeblemind", SPELL_FEEBLEMIND, PULSE_SPELLCAST * 2,
			 TAR_FIGHT_VICT | TAR_CHAR_ROOM | TAR_AGGRO, spell_feeblemind,
	...
	*/
	SPELL_CREATE_MSG("silence", SPELL_SILENCE, PULSE_SPELLCAST * 2,
			 TAR_FIGHT_VICT | TAR_CHAR_ROOM | TAR_AGGRO, spell_silence,
			 "You are able to speak again.");
	SPELL_ADD(CLASS_CLERIC, 9);
	//  SPELL_ADD(CLASS_DRUID, 6);

	SPELL_CREATE("turn undead", SPELL_TURN_UNDEAD, PULSE_SPELLCAST, TAR_IGNORE,
		     spell_turn_undead);
	SPELL_ADD(CLASS_CLERIC, 1);
	SPELL_ADD(CLASS_PALADIN, 5);
	SPELL_ADD(CLASS_AVENGER, 8);

	SPELL_CREATE("command undead", SPELL_COMMAND_UNDEAD, 0, TAR_FIGHT_VICT | TAR_CHAR_ROOM,
	...
	SPELL_CREATE("life ward", SPELL_LIFE_WARD, PULSE_SPELLCAST * 4, TAR_IGNORE | TAR_NOCOMBAT,
		     cast_life_ward);
	SPELL_ADD(CLASS_WARLOCK, 10);
	...
	SPELL_CREATE_MSG("self comprehension", SPELL_SELF_COMPREHENSION, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_comprehend_languages,
	...
	SPELL_CREATE_MSG("comprehend languages", SPELL_COMPREHEND_LANGUAGES, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_comprehend_languages,
  ...
  SPELL_ADD(CLASS_BARD, 6);
	...
	SPELL_CREATE_MSG("charm animal", SPELL_CHARM_ANIMAL, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_SELF_NONO, spell_charm_animal,
			 "You regain your senses.");
	...
	SPELL_CREATE_MSG("dharma", SPELL_HOLY_DHARMA, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
			 spell_holy_dharma,
			 "&+cThe &+Cdivine &+cinspiration departs from your soul.&n");
	SPELL_ADD(CLASS_PALADIN, 9);
	...
	SPELL_CREATE("mass heal", SPELL_MASS_HEAL, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_mass_heal);
	SPELL_ADD(CLASS_CLERIC, 8);

	SPELL_CREATE_MSG("true seeing", SPELL_TRUE_SEEING, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_true_seeing,
			 "&+MYour vision &n&+mdulls.&n");
	SPELL_ADD(CLASS_CLERIC, 11);

	SPELL_CREATE_MSG("shadow vision", SPELL_SHADOW_VISION, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_shadow_vision,
			 "&+MYour vision &n&+mdulls.&n");
	SPELL_ADD(CLASS_WARLOCK, 11);
	SPELL_ADD(CLASS_BLIGHTER, 8);

	SPELL_CREATE_MSG("animal vision", SPELL_ANIMAL_VISION, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_animal_vision,
			 "You vision &+ydulls.&n");
	SPELL_ADD(CLASS_DRUID, 8);
	SPELL_ADD(CLASS_RANGER, 11);

	SPELL_CREATE_MSG("scent of the bloodhound", SPELL_BLOODHOUND, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT | TAR_ANIMAL, spell_bloodhound,
			 "Your nose feels &+Gclogged&n.&n");
	...
	SPELL_CREATE("animal growth", SPELL_ANIMAL_GROWTH, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM | TAR_ANIMAL, spell_animal_growth);
	SPEC_SPELL_ADD(CLASS_DRUID, 8, SPEC_WOODLAND);
	SPEC_SPELL_ADD(CLASS_SHAMAN, 7, SPEC_ANIMALIST);

	SPELL_CREATE_MSG("natures blessing", SPELL_NATURES_BLESSING, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY, spell_natures_blessing,
	...
	SPELL_CREATE_MSG("reduce", SPELL_REDUCE, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM, spell_reduce,
			 "&+MYou return to your normal size.&n");
	SPELL_ADD(CLASS_SORCERER, 11);
	...
	SPELL_CREATE_MSG("wolfspeed", SPELL_WOLFSPEED, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_ANIMAL, spell_wolfspeed,
	...
	SPELL_CREATE_MSG("pantherspeed", SPELL_PANTHERSPEED, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_ANIMAL, spell_pantherspeed,
	...
	SPELL_CREATE_MSG("bearstrength", SPELL_BEARSTRENGTH, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_ANIMAL, spell_bearstrength,
	...
	SPELL_CREATE_MSG("lionrage", SPELL_LIONRAGE, PULSE_SPELLCAST * 3,
			 TAR_CHAR_ROOM | TAR_ANIMAL, spell_lionrage,
	...
	SPELL_CREATE_MSG("elephantstrength", SPELL_ELEPHANTSTRENGTH, PULSE_SPELLCAST * 4,
			 TAR_CHAR_ROOM | TAR_ANIMAL, spell_elephantstrength,
	...
	SPELL_CREATE_MSG("ravenflight", SPELL_RAVENFLIGHT, PULSE_SPELLCAST * 3,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_ANIMAL, spell_ravenflight,
	...
	SPELL_CREATE("greater ravenflight", SPELL_GREATER_RAVENFLIGHT, PULSE_SPELLCAST * 4,
		     TAR_IGNORE | TAR_NOCOMBAT | TAR_ANIMAL, spell_greater_ravenflight);
	SPELL_ADD(CLASS_SHAMAN, 9);

	SPELL_CREATE("greater pythonsting", SPELL_GREATER_PYTHONSTING, PULSE_SPELLCAST * 4,
		     TAR_AREA | TAR_AGGRO | TAR_ANIMAL, spell_greater_pythonsting);
	...
	SPELL_CREATE("beastform", SPELL_BEASTFORM, PULSE_SPELLCAST * 3,
		     TAR_SELF_ONLY | TAR_NOCOMBAT | TAR_ANIMAL, spell_beastform);
	//  SPELL_ADD(CLASS_SHAMAN, 5);

	/* elemental spells */

	SPELL_CREATE_MSG(
		"elemental affinity", SPELL_ELEM_AFFINITY, PULSE_SPELLCAST * 4,
		TAR_SELF_ONLY | TAR_NOCOMBAT | TAR_ELEMENTAL, spell_elemental_affinity,
	...
	SPELL_CREATE("elemental fury", SPELL_ELEM_FURY, PULSE_SPELLCAST * 4,
		     TAR_AREA | TAR_OFFAREA | TAR_AGGRO | TAR_ELEMENTAL, spell_elemental_fury);
	SPEC_SPELL_ADD(CLASS_SHAMAN, 12, SPEC_ELEMENTALIST);
	...
	SPELL_CREATE("flameburst", SPELL_FLAMEBURST, PULSE_SPELLCAST,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO | TAR_ELEMENTAL, spell_flameburst);
	...
	SPELL_CREATE_MSG("cold ward", SPELL_COLD_WARD, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_ELEMENTAL, spell_cold_ward,
			 "&+cYou no longer feel protected from the cold.");
	SPELL_ADD(CLASS_SHAMAN, 3);

	SPELL_CREATE_MSG("fire ward", SPELL_FIRE_WARD, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_ELEMENTAL, spell_fire_ward,
			 "&+rYou no longer feel protected from fire.");
	SPELL_ADD(CLASS_SHAMAN, 3);
	...
	SPELL_CREATE_MSG("greater earthen grasp", SPELL_GREATER_EARTHEN_GRASP, PULSE_SPELLCAST * 2,
			 TAR_AREA | TAR_AGGRO | TAR_ELEMENTAL, spell_greater_earthen_grasp,
	...
	SPELL_CREATE("scathing wind", SPELL_SCATHING_WIND, PULSE_SPELLCAST * 3,
		     TAR_AREA | TAR_OFFAREA | TAR_AGGRO | TAR_ELEMENTAL, spell_scathing_wind);
	SPELL_ADD(CLASS_SHAMAN, 8);

	SPELL_CREATE("earthen rain", SPELL_EARTHEN_RAIN, PULSE_SPELLCAST * 3,
		     TAR_AREA | TAR_OFFAREA | TAR_AGGRO | TAR_ELEMENTAL, spell_earthen_rain);
	SPELL_ADD(CLASS_SHAMAN, 10);
	...
	SPELL_CREATE_MSG(
		"indomitability", SPELL_INDOMITABILITY, PULSE_SPELLCAST * 2,
		TAR_SELF_ONLY | TAR_SPIRIT, spell_indomitability,
	...
	SPELL_CREATE_MSG("spirit armor", SPELL_SPIRIT_ARMOR, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_SPIRIT, spell_spirit_armor,
			 "&+mYou feel less protected.");
	SPELL_ADD(CLASS_SHAMAN, 1);

	SPELL_CREATE("transfer wellness", SPELL_TRANSFER_WELLNESS, PULSE_SPELLCAST,
		     TAR_CHAR_ROOM | TAR_SPIRIT, spell_transfer_wellness);
	SPELL_ADD(CLASS_SHAMAN, 1);

	SPELL_CREATE("sustenance", SPELL_SUSTENANCE, PULSE_SPELLCAST,
		     TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_SPIRIT, spell_sustenance);
	// SPELL_ADD(CLASS_SHAMAN, 2);
	...
	SPELL_CREATE_MSG("spirit sight", SPELL_SPIRIT_SIGHT, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_SPIRIT, spell_spirit_sight,
			 "&+LYour spirit sight fades.");
	SPELL_ADD(CLASS_SHAMAN, 6);

	SPELL_CREATE_MSG("sense spirit", SPELL_SENSE_SPIRIT, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT | TAR_SPIRIT, spell_sense_spirit,
			 "&+LYour sense of spirits disappears.");
	SPELL_ADD(CLASS_SHAMAN, 5);

	SPELL_CREATE_MSG("malison", SPELL_MALISON, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO | TAR_SPIRIT, spell_malison,
			 "You feel less cursed.");
	SPELL_ADD(CLASS_SHAMAN, 5);

	SPELL_CREATE("wellness", SPELL_WELLNESS, PULSE_SPELLCAST, TAR_IGNORE | TAR_SPIRIT,
		     spell_wellness);
	SPELL_ADD(CLASS_SHAMAN, 6);

	SPELL_CREATE("greater mending", SPELL_GREATER_MENDING, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM | TAR_SPIRIT, spell_greater_mending);
	SPELL_ADD(CLASS_SHAMAN, 6);

	SPELL_CREATE("spirit anguish", SPELL_SPIRIT_ANGUISH, 2 * PULSE_SPELLCAST,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO | TAR_SPIRIT, spell_spirit_anguish);
	SPELL_ADD(CLASS_SHAMAN, 6);
	...
	SPELL_CREATE_MSG("eshabalas vitality", SPELL_ESHABALAS_VITALITY, PULSE_SPELLCAST,
			 TAR_SELF_ONLY, spell_eshabalas_vitality,
	...
	SPELL_CREATE_MSG("cegilunes searing blade", SPELL_CEGILUNE_BLADE, PULSE_SPELLCAST * 4 / 3,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_cegilunes_searing_blade,
	...
	SPELL_CREATE_MSG("kanchelsis fury", SPELL_KANCHELSIS_FURY, (PULSE_SPELLCAST * 3) / 2,
			 TAR_SELF_ONLY, spell_kanchelsis_fury,
			 "&+LYou feel the world speeding up around you.&n");
	SPELL_ADD(CLASS_REAVER, 8);

	SPELL_CREATE("blood alliance", SPELL_BLOOD_ALLIANCE, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM | TAR_SELF_NONO, spell_blood_alliance);

	SPELL_CREATE_MSG("ilienzes flaming sword", SPELL_ILIENZES_FLAME_SWORD,
			 PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY, spell_ilienzes_flame_sword,
	...
	SPELL_CREATE_MSG("thryms icerazor", SPELL_THRYMS_ICERAZOR, PULSE_SPELLCAST * 4 / 3,
			 TAR_SELF_ONLY, spell_thryms_icerazor,
			 "&+CYou feel weaker as &+LThrym's &+Cessence leaves you.&n");
	SPEC_SPELL_ADD(CLASS_REAVER, 11, SPEC_ICE_REAVER);

	SPELL_CREATE_MSG("girilals granite hammer", SPELL_GIRILALS_GRANITE_HAMMER,
			 PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY, spell_girilals_granite_hammer,
			 "&+LThe &+ydust &+Lsurrounding your weapon blows away..&n");
	SPEC_SPELL_ADD(CLASS_REAVER, 11, SPEC_EARTH_REAVER);

	SPELL_CREATE_MSG("ileshs smashing fury", SPELL_ILESHS_SMASHING_FURY,
			 PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY | TAR_NOCOMBAT,
	...
	SPELL_CREATE_MSG("kostchtchies chilling implosion", SPELL_CHILLING_IMPLOSION,
			 PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY | TAR_NOCOMBAT,
	...
	SPELL_CREATE_MSG("lliendils stormshock", SPELL_LLIENDILS_STORMSHOCK,
			 PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY, spell_lliendils_stormshock,
			 "&+BThe power of &+Lstorms &+Bleaves your weapon.&n");
	...
	SPELL_CREATE_MSG("stormcallers fury", SPELL_STORMCALLERS_FURY, PULSE_SPELLCAST * 4 / 3,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_stormcallers_fury,
			 "&+BThe call of the &+Lstorm&+B leaves your weapon.&n");
	...
	SPELL_CREATE_MSG("lightning shield", SPELL_LIGHTNINGSHIELD, PULSE_SPELLCAST * 3 / 2,
			 TAR_SELF_ONLY, spell_lightning_shield,
			 "&+YThe maelstrom of energy surrounding your body fades.");
	...
	SPELL_CREATE("illusionary wall", SPELL_ILLUSIONARY_WALL, PULSE_SPELLCAST * 4,
		     TAR_IGNORE | TAR_NOCOMBAT, spell_illusionary_wall);
	...
	SPELL_CREATE("stunning visions", SPELL_STUNNING_VISIONS, PULSE_SPELLCAST,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_stunning_visions);
	SPELL_ADD(CLASS_ILLUSIONIST, 6);

	SPELL_CREATE("reflection", SPELL_REFLECTION, PULSE_SPELLCAST, TAR_CHAR_ROOM,
		     spell_reflection);
	SPELL_ADD(CLASS_ILLUSIONIST, 6);

	SPELL_CREATE("mask", SPELL_MASK, PULSE_SPELLCAST * 3, TAR_IGNORE | TAR_NOCOMBAT,
		     spell_mask);
	SPELL_ADD(CLASS_ILLUSIONIST, 8);
	...
	SPELL_CREATE_MSG("nightmare", SPELL_NIGHTMARE, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_nightmare,
			 "You feel less frightend.");
	SPELL_ADD(CLASS_ILLUSIONIST, 6);

	SPELL_CREATE_MSG(
		"shadow shield", SPELL_SHADOW_SHIELD, PULSE_SPELLCAST * 4 / 3, TAR_SELF_ONLY,
		spell_shadow_shield,
	...
	SPELL_CREATE_MSG("detect illusion", SPELL_DETECT_ILLUSION, PULSE_SPELLCAST * 4 / 3,
			 TAR_SELF_ONLY, spell_detect_illusion,
	...
	SPELL_CREATE_MSG("nonexistence", SPELL_NONEXISTENCE, PULSE_SPELLCAST * 4,
			 TAR_IGNORE | TAR_NOCOMBAT, spell_nonexistence,
			 "You slowly fade back into visibility.");
	...
	SPELL_CREATE("titan", SPELL_TITAN, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_titan);
	SPELL_ADD(CLASS_ILLUSIONIST, 11);

	SPELL_CREATE_MSG("delirium", SPELL_DELIRIUM, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_delirium,
			 "&+WYou feel less &+Gconfused&n");
	SPELL_ADD(CLASS_ILLUSIONIST, 10);

	SPELL_CREATE("flicker", SPELL_FLICKER, PULSE_SPELLCAST * 5, TAR_IGNORE | TAR_NOCOMBAT,
		     spell_flicker);
	SPELL_ADD(CLASS_ILLUSIONIST, 11);

	SPELL_CREATE("greater flicker", SPELL_GREATER_FLICKER, PULSE_SPELLCAST * 7,
		     TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_greater_flicker);
	// SPELL_ADD(CLASS_ILLUSIONIST, 12);

	// Ethermancer

	SPELL_CREATE_MSG("vapor armor", SPELL_VAPOR_ARMOR, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_vapor_armor,
			 "&+CYou feel the protection of the winds dissapate.");
	SPELL_ADD(CLASS_ETHERMANCER, 1);

	SPELL_CREATE("frost beam", SPELL_FROST_BEAM, PULSE_SPELLCAST * 3 / 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_frost_beam);
	SPELL_ADD(CLASS_ETHERMANCER, 10);

	SPELL_CREATE_MSG("faerie sight", SPELL_FAERIE_SIGHT, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_faerie_sight,
	...
	SPELL_CREATE_MSG("vapor strike", SPELL_VAPOR_STRIKE, PULSE_SPELLCAST,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_vapor_strike,
	...
	SPELL_CREATE_MSG("frost bolt", SPELL_FROST_BOLT, PULSE_SPELLCAST * 3 / 4,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_frost_bolt,
			 "The &+Cice&n wracking your body melts.");
	SPELL_ADD(CLASS_ETHERMANCER, 3);
	...
	SPELL_CREATE_MSG("ethereal form", SPELL_ETHEREAL_FORM, PULSE_SPELLCAST * 3,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_ethereal_form,
	...
	SPELL_CREATE("ethereal recharge", SPELL_ETHEREAL_RECHARGE, PULSE_SPELLCAST * 3 / 2,
		     TAR_CHAR_ROOM, spell_ethereal_recharge);
	SPELL_ADD(CLASS_ETHERMANCER, 8);
	...
	SPELL_CREATE("forked lightning", SPELL_FORKED_LIGHTNING, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_forked_lightning);
	SPEC_SPELL_ADD(CLASS_ETHERMANCER, 7, SPEC_TEMPESTMAGUS);

	SPELL_CREATE("ice spikes", SPELL_ICE_SPIKES, PULSE_SPELLCAST * 3 / 2,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_ice_spikes);
	SPEC_SPELL_ADD(CLASS_ETHERMANCER, 7, SPEC_FROSTMAGUS);
	...
	SPELL_CREATE("purge", SPELL_PURGE, PULSE_SPELLCAST * 3, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
		     spell_purge);
	SPELL_ADD(CLASS_ETHERMANCER, 10);
	...
	SPELL_CREATE_MSG("negative feedback barrier", SPELL_NEG_ARMOR, PULSE_SPELLCAST * 4 / 3,
			 TAR_SELF_ONLY, spell_negative_feedback_barrier,
			 "&+LThe negative field encasing you fades away...&n");
	SPEC_SPELL_ADD(CLASS_ETHERMANCER, 8, SPEC_STARMAGUS);

	SPELL_CREATE("etheric gust", SPELL_ETHERIC_GUST, PULSE_SPELLCAST, TAR_SELF_ONLY,
		     spell_etheric_gust);
	SPEC_SPELL_ADD(CLASS_ETHERMANCER, 8, SPEC_TEMPESTMAGUS);

	SPELL_CREATE_MSG("induce tupor", SPELL_INDUCE_TUPOR, PULSE_SPELLCAST * 4,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_induce_tupor,
			 "You awake from your magical slumber.");
	SPELL_ADD(CLASS_ETHERMANCER, 10);

	SPELL_CREATE2("tempest terrain", SPELL_TEMPEST_TERRAIN, PULSE_SPELLCAST * 6,
		      TAR_IGNORE | TAR_NOCOMBAT, spell_tempest_terrain,
	...
	SPELL_CREATE_MSG("planetary alignment", SPELL_PLANETARY_ALIGNMENT, PULSE_SPELLCAST * 5,
			 TAR_SELF_ONLY, spell_planetary_alignment, "Your feel weaker.");
	SPEC_SPELL_ADD(CLASS_ETHERMANCER, 9, SPEC_STARMAGUS);
	...
	// Frost Magus Spec
	SPELL_CREATE("squall", SPELL_TEMPEST, PULSE_SPELLCAST * 4, TAR_OFFAREA | TAR_AGGRO,
		     spell_tempest);
	...
	SPELL_CREATE_MSG("ethereal alliance", SPELL_ETHEREAL_ALLIANCE, PULSE_SPELLCAST * 5,
			 TAR_CHAR_ROOM, spell_ethereal_alliance,
			 "&+cYour alliance has been severed.");
	SPEC_SPELL_ADD(CLASS_ETHERMANCER, 11, SPEC_TEMPESTMAGUS);

	SPELL_CREATE("greater ethereal recharge", SPELL_GREATER_ETHEREAL, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM, spell_greater_ethereal_recharge);
	...
	SPELL_CREATE("doom blade", SPELL_DOOM_BLADE, PULSE_SPELLCAST * 8, TAR_IGNORE | TAR_NOCOMBAT,
		     spell_doom_blade);
	SPEC_SPELL_ADD(CLASS_NECROMANCER, 8, SPEC_REAPER);
	SPEC_SPELL_ADD(CLASS_BLIGHTER, 6, SPEC_RUINER);

	SPELL_CREATE("holy blade", SPELL_HOLY_BLADE, PULSE_SPELLCAST * 8, TAR_IGNORE | TAR_NOCOMBAT,
		     spell_doom_blade);
	...
	SPELL_CREATE_MSG("atonement", SPELL_ATONEMENT, PULSE_SPELLCAST * 3, TAR_SELF_ONLY,
			 spell_atonement,
			 "&+WYou sense that your past sins once again need forgiving.&n");
	SPELL_ADD(CLASS_AVENGER, 7);

	SPELL_CREATE_MSG("celestial aura", SPELL_CELESTIAL_AURA, PULSE_SPELLCAST * 5, TAR_SELF_ONLY,
			 spell_celestial_aura,
			 "&+WYou feel your prayers will go unanswered once again.&n");
	...
	*/
	...
	SKILL_CREATE_WITH_MESSAGES("heroism", SKILL_HEROISM, TAR_MENTAL,
	...
	SKILL_CREATE_WITH_MESSAGES("song of heroism", SONG_HEROISM, TAR_MENTAL,
				   "You no longer feel like a hero.", NULL);
	SKILL_ADD(CLASS_BARD, 41, 100);
	...
	SPELL_CREATE("airy water", SPELL_AIRY_WATER, PULSE_SPELLCAST * 4, TAR_IGNORE,
		     spell_airy_water);
	SPELL_ADD(CLASS_CONJURER, 7);
	SPELL_ADD(CLASS_SUMMONER, 7);

	SPELL_CREATE_MSG("globe of invulnerability", SPELL_GLOBE, PULSE_SPELLCAST * 3,
			 TAR_CHAR_ROOM, spell_globe,
	...
	*/
	...
	SPELL_CREATE("embalm", SPELL_EMBALM, PULSE_SPELLCAST * 3, TAR_OBJ_ROOM | TAR_NOCOMBAT,
		     spell_embalm);
	SPELL_ADD(CLASS_NECROMANCER, 3);
	SPELL_ADD(CLASS_THEURGIST, 3);
	...
	SPELL_CREATE_MSG("angelic countenance", SPELL_ANGELIC_COUNTENANCE, PULSE_SPELLCAST * 2,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_vampire,
			 "The holy shine about you dimishes.");
	SPELL_ADD(CLASS_THEURGIST, 10);
	...
	SPELL_CREATE_MSG("divine blessing", SPELL_DIVINE_BLESSING, PULSE_SPELLCAST * 3,
			 TAR_SELF_ONLY, spell_divine_blessing,
	...
	SPELL_CREATE_MSG("elemental form", SPELL_ELEMENTAL_FORM, PULSE_SPELLCAST * 4,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_elemental_form,
			 "&+BYou return to your normal form.&N");

	SPELL_CREATE_MSG("elemental aura", SPELL_ELEMENTAL_AURA, PULSE_SPELLCAST * 4,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_elemental_aura,
			 "&+BYour aura returns to normal.&N");
	SPELL_ADD(CLASS_DRUID, 11);
	SPELL_ADD(CLASS_BLIGHTER, 11);
	SPEC_SPELL_ADD(CLASS_SUMMONER, 10, SPEC_MENTALIST);
	// SPEC_SPELL_ADD(CLASS_DRUID, 11, SPEC_STORM);

	SPELL_CREATE("wind tunnel", SPELL_WIND_TUNNEL, PULSE_SPELLCAST * 1,
		     TAR_IGNORE | TAR_NOCOMBAT | TAR_ELEMENTAL, spell_wind_tunnel);
	SPEC_SPELL_ADD(CLASS_ETHERMANCER, 7, SPEC_TEMPESTMAGUS);
	SPEC_SPELL_ADD(CLASS_DRUID, 8, SPEC_STORM);
	SPEC_SPELL_ADD(CLASS_SHAMAN, 9, SPEC_ELEMENTALIST);

	SPELL_CREATE("binding wind", SPELL_BINDING_WIND, PULSE_SPELLCAST * 1,
		     TAR_IGNORE | TAR_NOCOMBAT | TAR_ELEMENTAL, spell_binding_wind);
	SPEC_SPELL_ADD(CLASS_ETHERMANCER, 9, SPEC_TEMPESTMAGUS);
	SPEC_SPELL_ADD(CLASS_DRUID, 10, SPEC_STORM);
	SPEC_SPELL_ADD(CLASS_SHAMAN, 11, SPEC_ELEMENTALIST);
	...
	SPELL_CREATE("protect undead", SPELL_PROT_UNDEAD, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
		     spell_prot_undead);
	SPELL_ADD(CLASS_NECROMANCER, 7);

	SPELL_CREATE("protect soul", SPELL_PROTECT_SOUL, PULSE_SPELLCAST * 2, TAR_CHAR_ROOM,
		     spell_prot_undead);
	SPELL_ADD(CLASS_THEURGIST, 7);

	SPELL_CREATE_MSG("protection from undead", SPELL_PROT_FROM_UNDEAD, PULSE_SPELLCAST * 2,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_prot_from_undead,
			 "Your field of &+Yliving energy&n dissipates&n.");
	SPELL_ADD(CLASS_NECROMANCER, 1);
	SPELL_ADD(CLASS_THEURGIST, 1);
	...
	SPELL_CREATE("heal undead", SPELL_HEAL_UNDEAD, PULSE_SPELLCAST * 3 / 2, TAR_CHAR_ROOM,
		     spell_heal_undead);
	SPELL_ADD(CLASS_NECROMANCER, 5);
	SPELL_ADD(CLASS_WARLOCK, 5);

	SPELL_CREATE("mend soul", SPELL_MEND_SOUL, PULSE_SPELLCAST * 3 / 2, TAR_CHAR_ROOM,
		     spell_mend_soul);
	SPELL_ADD(CLASS_THEURGIST, 5);

	SPELL_CREATE("greater heal undead", SPELL_GREATER_HEAL_UNDEAD, PULSE_SPELLCAST * 2,
		     TAR_CHAR_ROOM, spell_greater_heal_undead);
	SPELL_ADD(CLASS_WARLOCK, 7);

	SPELL_CREATE_MSG("entangle", SPELL_ENTANGLE, PULSE_SPELLCAST * 1,
			 TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_entangle,
			 "You are able to move normally again.");
	//  SPELL_ADD(CLASS_DRUID, 8);
	SPEC_SPELL_ADD(CLASS_DRUID, 8, SPEC_WOODLAND);
	SPEC_SPELL_ADD(CLASS_BLIGHTER, 8, SPEC_SCOURGE);

	SPELL_CREATE("create spring", SPELL_CREATE_SPRING, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_create_spring);
	SPELL_ADD(CLASS_DRUID, 4);

	SPELL_CREATE("create pond", SPELL_CREATE_POND, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_create_pond);
	SPELL_ADD(CLASS_BLIGHTER, 4);

	SPELL_CREATE_MSG("barkskin", SPELL_BARKSKIN, PULSE_SPELLCAST, TAR_CHAR_ROOM, spell_barkskin,
			 "Your skin loses its &+ybarklike &ntexture.");
	SPELL_ADD(CLASS_DRUID, 1);
	SPELL_ADD(CLASS_RANGER, 5);

	SPELL_CREATE_MSG("thornskin", SPELL_THORNSKIN, PULSE_SPELLCAST, TAR_CHAR_ROOM,
	...
	SPELL_CREATE_MSG("curse of yzar", SPELL_CURSE_OF_YZAR, PULSE_SPELLCAST, TAR_CHAR_ROOM,
			 spell_curse_of_yzar,
			 "You &+Wscream&n in &+Rpain&n as the &+Yflesh&n falls from your bones.");

	SPELL_CREATE("rested", SPELL_REST, PULSE_SPELLCAST, TAR_CHAR_ROOM, spell_rest);
	// Immortal only spell

	SPELL_CREATE("moonwell", SPELL_MOONWELL, PULSE_SPELLCAST * 7, TAR_CHAR_WORLD | TAR_NOCOMBAT,
		     spell_moonwell);
	SPELL_ADD(CLASS_DRUID, 10);
	...
	SPELL_CREATE("moonstone", SPELL_MOONSTONE, PULSE_SPELLCAST * 6, TAR_IGNORE | TAR_NOCOMBAT,
		     spell_moonstone);
	SPELL_ADD(CLASS_DRUID, 12);

	SPELL_CREATE("bloodstone", SPELL_BLOODSTONE, PULSE_SPELLCAST * 6, TAR_IGNORE | TAR_NOCOMBAT,
		     spell_bloodstone);
	SPELL_ADD(CLASS_BLIGHTER, 12);

	SPELL_CREATE("create dracolich", SPELL_CREATE_DRACOLICH, PULSE_SPELLCAST * 9,
		     TAR_OBJ_ROOM | TAR_NOCOMBAT, spell_create_dracolich);
	SPELL_ADD(CLASS_NECROMANCER, 11);

	SPELL_CREATE("call titan", SPELL_CALL_TITAN, PULSE_SPELLCAST * 9,
		     TAR_OBJ_ROOM | TAR_NOCOMBAT, spell_call_titan);
	SPELL_ADD(CLASS_THEURGIST, 11);
	...
	SKILL_CREATE_WITH_MESSAGES(
		"throat crush", SKILL_THROAT_CRUSH, TAR_PHYS,
	...
	SKILL_CREATE_WITH_MESSAGES(
		"infuriate", SKILL_INFURIATE, TAR_PHYS,
	...
	*/
	...
	SKILL_ADD(CLASS_ASSASSIN, 20, 90);
	...
	SPELL_CREATE("ghastly touch", SPELL_GHASTLY_TOUCH, PULSE_SPELLCAST * 2 / 3,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_ghastly_touch);

	SPELL_CREATE("infernal fury", SPELL_INFERNAL_FURY, PULSE_SPELLCAST * 2 / 3, TAR_SELF_ONLY,
		     spell_infernal_fury);
	...
	SPELL_CREATE_MSG("animae cicatrix", SPELL_ANIMAE_CICATRIX, PULSE_SPELLCAST, TAR_SELF_ONLY,
			 spell_animae_cicatrix,
			 "The &+Gdr&+Lag&+Gon&n god's scar on your soul fades.");
	SPELL_ADD(CLASS_DRAGOON, 3);

	SPELL_CREATE("ensis unguis", SPELL_ENSIS_UNGUIS, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_ensis_unguis);
	SPELL_ADD(CLASS_DRAGOON, 4);

	SPELL_CREATE("lancea cineralae", SPELL_LANCEA_CINERALAE, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_lancea_cineralae);
	SPEC_SPELL_ADD(CLASS_DRAGOON, 4, SPEC_DRAGON_LANCER);

	SPELL_CREATE("simulacrum anguis", SPELL_SIMULACRUM_ANGUIS, PULSE_SPELLCAST * 2, TAR_IGNORE,
		     spell_simulacrum_anguis);
	SPEC_SPELL_ADD(CLASS_DRAGOON, 4, SPEC_DRAGON_PRIEST);

	SPELL_CREATE_MSG(
		"stigmata draconica", SPELL_STIGMATA_DRACONICA, PULSE_SPELLCAST * 3, TAR_SELF_ONLY,
	...
	SPELL_CREATE_MSG("aegis squamarum", SPELL_DRAKESCALE_AEGIS, PULSE_SPELLCAST * 3,
			 TAR_SELF_ONLY, spell_drakescale_aegis,
			 "The scales penetrating your flesh subside, easing your &+rpain&n.");
	SPELL_ADD(CLASS_DRAGOON, 6);

	SPELL_CREATE_MSG("judicium fidei", SPELL_JUDICIUM_FIDEI, PULSE_SPELLCAST * 3, TAR_SELF_ONLY,
			 spell_judicium_fidei,
			 "The stench of &+rburning&n flesh fades as your skin cools.");
	SPELL_ADD(CLASS_DRAGOON, 7);

	SPELL_CREATE("igneus vitae", SPELL_IGNEUS_VITAE, PULSE_SPELLCAST * 3, TAR_SELF_ONLY,
		     spell_igneus_vitae);
	SPEC_SPELL_ADD(CLASS_DRAGOON, 8, SPEC_DRAGON_PRIEST);

	SPELL_CREATE_MSG("sanguinis ignis", SPELL_SANGUINIS_IGNIS, PULSE_SPELLCAST * 4,
			 TAR_SELF_ONLY, spell_sanguinis_ignis,
			 "Your blood cools as the &+Gdr&+Lag&+Gon&n god's &+rpower&n fades.");
	SPEC_SPELL_ADD(CLASS_DRAGOON, 8, SPEC_DRAGON_HUNTER);

	SPELL_CREATE_MSG("sanctum draconis", SPELL_SANCTUM_DRACONIS, PULSE_SPELLCAST * 4,
			 TAR_SELF_ONLY, spell_sanctum_draconis,
			 "Your resolve cools as the &+Gdr&+Lag&+Gon&n god's &+rpower&n fades.");
	SPEC_SPELL_ADD(CLASS_DRAGOON, 8, SPEC_DRAGON_LANCER);

	SPELL_CREATE("vivernae concordia", SPELL_VIVERNAE_CONCORDIA, PULSE_SPELLCAST * 4,
		     TAR_SELF_ONLY, spell_vivernae_concordia);
	SPEC_SPELL_ADD(CLASS_DRAGOON, 9, SPEC_DRAGON_HUNTER);

	SPELL_CREATE_MSG("pactum serpentis", SPELL_PACTUM_SERPENTIS, PULSE_SPELLCAST * 4,
			 TAR_SELF_ONLY, spell_pactum_serpentis,
			 "Your soul cools as the &+Gdr&+Lag&+Gon&n god's &+rpower&n fades.");
	SPEC_SPELL_ADD(CLASS_DRAGOON, 9, SPEC_DRAGON_PRIEST);

	SPELL_CREATE("ritus draconum", SPELL_RITUS_DRACONUM, PULSE_SPELLCAST * 4, TAR_SELF_ONLY,
		     spell_ritus_draconum);
	SPEC_SPELL_ADD(CLASS_DRAGOON, 9, SPEC_DRAGON_LANCER);

	SPELL_CREATE("edictum cineris", SPELL_EDICTUM_CINERIS, PULSE_SPELLCAST * 4,
		     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_AGGRO, spell_edictum_cineris);
	SPELL_ADD(CLASS_DRAGOON, 10);

	SPELL_CREATE_MSG("sigillum negati", SPELL_SIGILLUM_NEGATI, PULSE_SPELLCAST * 5, TAR_IGNORE,
			 spell_sigillum_negati,
			 "The &+Lnegation&n &+ysigil&n branded on your &+rflesh&n begins to fade.");
	SPELL_ADD(CLASS_DRAGOON, 11);

	SPELL_CREATE("draconic apotheosis", SPELL_DRACONIC_APOTHEOSIS, PULSE_SPELLCAST * 5,
		     TAR_SELF_ONLY | TAR_NOCOMBAT, spell_draconic_apotheosis);
	SPELL_ADD(CLASS_DRAGOON, 12);

	SPELL_CREATE_MSG("freedom of movement", SPELL_FREEDOM_OF_MOVEMENT, PULSE_SPELLCAST * 4,
			 TAR_SELF_ONLY | TAR_NOCOMBAT, spell_freedom_of_movement,
			 "&+yYour movement is no longer unhindered!&n");
	SPELL_ADD(CLASS_DRUID, 10);
	SPELL_ADD(CLASS_BLIGHTER, 10);
	SPELL_ADD(CLASS_CLERIC, 10);
	SPELL_ADD(CLASS_BARD, 10);
	...
	SPELL_CREATE_MSG("lesser guardian ward", SPELL_LESSER_GUARDIAN_WARD, PULSE_SPELLCAST * 4,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_lesser_guardian_ward,
			 "&+CThe spirits no longer protect you.&n");
	// SPELL_ADD(CLASS_SHAMAN, 5);

	SPELL_CREATE_MSG("guardian ward", SPELL_GUARDIAN_WARD, PULSE_SPELLCAST * 6,
			 TAR_CHAR_ROOM | TAR_NOCOMBAT, spell_guardian_ward,
			 "&+CThe spirits no longer protect you.&n");
	// SPELL_ADD(CLASS_SHAMAN, 7);

	SPELL_CREATE_MSG("greater guardian ward", SPELL_GREATER_GUARDIAN_WARD, PULSE_SPELLCAST * 8,
	...
	SPELL_CREATE_MSG("vital intercession", SPELL_VITAL_INTERCESSION, PULSE_SPELLCAST * 3 / 2,
			 TAR_CHAR_ROOM, spell_vital_intercession,
			 "&+WThe healing energies surrounding you falter!");
	SPEC_SPELL_ADD(CLASS_CLERIC, 6, SPEC_HOLYMAN);

	SPELL_CREATE("holy intercession", SPELL_HOLY_INTERCESSION, PULSE_SPELLCAST * 4, TAR_IGNORE,
		     spell_holy_intercession);
	SPEC_SPELL_ADD(CLASS_CLERIC, 10, SPEC_HOLYMAN);

	SPELL_CREATE_MSG("warring zeal", SPELL_WARRING_ZEAL, PULSE_SPELLCAST * 2, TAR_SELF_ONLY,
			 spell_warring_zeal, "The sound of the wardrums subsides.");
...
void create_tags()
{
	...
	// TRAPS
	TAG_CREATE_WITH_MESSAGES(
		"crippled", TAG_CRIPPLED, "&+yYou feel &+Ystrength &+yreturn to your legs.&n",
	...
	TAG_CREATE_WITH_MESSAGES(
		"phantasmal form", TAG_PHANTASMAL_FORM,
	...
	TAG_CREATE_WITH_MESSAGES("alliance proposal", TAG_ALLIANCE,
				 "&+bYou revoke your proposed alliance.", "");
	TAG_CREATE_WITH_MESSAGES("enhance healing", TAG_ENHANCE_HEALING,
				 "&+WYou no longer feel looked over by a greater being.", "");
	TAG_CREATE_WITH_MESSAGES("item set bonus", TAG_SETPROC,
				 "You no longer feel any spirit's support.", "");

	TAG_CREATE_WITH_MESSAGES("divine force affect", TAG_DIVINE_FORCE_AFFECT,
				 "&+YThe divine force leaves you!",
				 "$n &+Llooks momentarily drained.");
	TAG_CREATE_WITH_MESSAGES("divine force timer", TAG_DIVINE_FORCE_TIMER,
				 "&+WYou are ready to embrace the divine.",
				 "$n's features &+wsoften considerably.");
	TAG_CREATE_WITH_MESSAGES("broken arm", TAG_ARMLOCK, "&+WYour arm has healed.&n",
				 "$n's arm has healed.&n");
	TAG_CREATE_WITH_MESSAGES("broken leg", TAG_LEGLOCK, "&+WYour leg has healed.&n",
				 "$n's leg has healed.&n");
	TAG_CREATE_WITH_MESSAGES("arrow bleed", TAG_ARROW_BLEED,
				 "&+WYour bleeding wound has healed.&n",
				 "$n's bleeding wound has healed.&n");
	TAG_CREATE("summon spawn/ally", TAG_SPAWN);

	TAG_CREATE_WITH_MESSAGES("rested bonus", TAG_RESTED,
				 "You sigh as you start to feel a bit tired.",
				 "$n sighs and stares off into the sky.");
	TAG_CREATE_WITH_MESSAGES("well-rested bonus", TAG_WELLRESTED,
				 "You sigh as you start to feel a bit tired.",
				 "$n sighs and stares off into the sky.");
	...
	TAG_CREATE_WITH_MESSAGES("squidrage timer", TAG_SQUIDRAGE,
				 "You feel ready to &+rr&+Rag&+re&n again!", "");
