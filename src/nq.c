...
struct nq_quest
{
	...
	struct nq_actor
		*actor; // global quest actors, can be empty when loading quest for the first time
	struct nq_actor_template
		*actor_template[NQ_MAX_ACTORS]; // templates for instance associated actors
...
void nq_free_instance(struct nq_instance *instance, P_char ch)
{
	...
	while ((actor = instance->actor))
	{
		...
		if (actor->loaded && char_in_list(actor->ch))
		{
			if (ch)
			{
				snprintf(buf, 512, "Extracting $N in room [%d].",
					 actor->ch->in_room);
 ...
 */
int nq_find_actor(P_char mob, struct nq_quest **quest, struct nq_instance **instance,
		  struct nq_actor **actor)
{
	*quest = NULL;
	*instance = NULL;
...
void nq_reward_player(struct nq_action *action, struct nq_instance *instance,
		      struct nq_quest *quest, P_char ch, P_char mob, bool kill)
{
	struct nq_item *item;
	...
	if (action->reward->phrase)
	{
		...
		{ // add entry to the quest log
			...
			if (!t_tag)
			{
				...
				if (instance->log)
				{
					for (t_tag = instance->log; t_tag->next;
					     t_tag = t_tag->next)
						;
			  ...
			  bool kill)
{
	...
	mob_cash = mob->points.cash[0] + 10 * mob->points.cash[1] + 100 * mob->points.cash[2] +
		   1000 * mob->points.cash[3];
	...
	for (action = actor->action; action; action = action->next)
	{
		...
		if (was_accepted || !action->reward->tag)
		{
			if (nq_test_single_action(action, instance, quest, ch, mob, phrase, kill))
			{
			 ...
			 quest->allowed_races[GET_RACE(ch)] != 0 && i < quest->allowed_instances &&
			 (!quest->skill ||
			  ch->only.pc->skills[quest->skill->skill].taught == quest->skill->value) &&
			 nq_test_single_action(action, instance, quest, ch, mob, phrase, kill))
		{
