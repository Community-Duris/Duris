...
int getNumbBodyLocsbyRace(const int race)
{
	...
	{
		logit(LOG_DEBUG, "getNumbBodyLocsbyRace(): unrecognized physiology type (race %d)", race);
...
int getNumbBodyLocsbyPhysType(const int physType)
{
	switch (physType)
	{
		...
		default:
			logit(LOG_DEBUG, "getNumbBodyLocsbyPhysType(): unrecognized physiology type (phys %d)", physType);
			return NUMB_HUMANOID_BODY_LOCS; // we're doomed...
	}
}
...
int getPhysTypebyRace(const int race)
{
	...
	{
		logit(LOG_DEBUG, "getPhysTypebyRace(): unrecognized race (race %d)", race);
...
const char *getBodyLocStrn(const int loc, const P_char ch) { return (physBodyLocTables[GET_PHYS_TYPE(ch)])[loc].bodyLocName; }
...
const int getBodyLocMaxHP(const P_char ch, const int loc)
{
	...
	return (int)((physBodyLocTables[GET_PHYS_TYPE(ch)])[loc].maxHPmult * (float)max_hp);
}
...
int getCharToHitValClassandLevel(const P_char ch)
{
	...
	if (IS_RIDING(ch) && ((GET_CLASS(ch) == CLASS_PALADIN) || (GET_CLASS(ch) == CLASS_ANTIPALADIN)))
	{
		if (lvl < 26)
			val += 0;
		else if (lvl < 46)
...
int getChartoHitSkillMod(const int wpn_skl_lvl) { return (wpn_skl_lvl >> 1); }
...
int getVictimtoHitMod(const P_char ch, const P_char victim)
{
	...
	if ((IS_AFFECTED(victim, AFF_PROTECT_EVIL) && IS_EVIL(ch) && !IS_EVIL(victim)) || (IS_AFFECTED(victim, AFF_PROTECT_GOOD) && IS_GOOD(ch) && !IS_GOOD(victim)))
...
int getBodyLocTargettingtoHitMod(const P_char ch, const P_char victim, const int body_loc_target, const int weaptype)
{
	...
	mod = (physBodyLocTables[GET_PHYS_TYPE(ch)])[body_loc_target].bodyLocTargettingMod;
	pos = (physBodyLocTables[GET_PHYS_TYPE(ch)])[body_loc_target].bodyLocOverall;
	...
	else if ((pos == BODY_LOC_OVERALL_VERYHIGH) || (pos == BODY_LOC_OVERALL_HIGH))
	{
		if (chsize < victsize)
...
int getNPCweaponSkillLevel(const P_char ch, const int wpn_skill)
{
	...
	if (((chcl == CLASS_ROGUE) || (chcl == CLASS_ASSASSIN)) && (wpn_skill == SKILL_DAGGER))
		val += (lvl >> 1);

	if (((chcl == CLASS_SORCERER) || (chcl == CLASS_CONJURER) || (chcl == CLASS_SUMMONER) || (chcl == CLASS_PSIONICIST) || (chcl == CLASS_NECROMANCER)) && (wpn_skill == SKILL_STAFF))
		val += (lvl >> 2);
...
{
	if (!vict || !AWAKE(vict) || IS_AFFECTED2(vict, AFF2_MINOR_PARALYSIS) || IS_AFFECTED2(vict, AFF2_MAJOR_PARALYSIS) || IS_AFFECTED(vict, AFF_BOUND) || IS_AFFECTED(vict, AFF_KNOCKED_OUT) ||
	    (GET_STAT(vict) <= STAT_SLEEPING) || IS_AFFECTED2(vict, AFF2_STUNNED) || (attacker && !CAN_SEE(vict, attacker) && GET_CHAR_SKILL(vict, SKILL_BLINDFIGHTING) > number(1, 300)))
		return FALSE;
...
int bodyLocisUpperArms(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_HUMANOID:
			return ((loc == BODY_LOC_HUMANOID_UPPER_LEFT_ARM) || (loc == BODY_LOC_HUMANOID_UPPER_RIGHT_ARM) || (loc == BODY_LOC_HUMANOID_LOWER_LEFT_ARM) ||
			        (loc == BODY_LOC_HUMANOID_LOWER_RIGHT_ARM) || (loc == BODY_LOC_HUMANOID_LEFT_ELBOW) || (loc == BODY_LOC_HUMANOID_RIGHT_ELBOW));

		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_LEFT_UPPER_ARM) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_RIGHT_UPPER_ARM) ||
			        (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_LEFT_UPPER_ARM) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_RIGHT_UPPER_ARM) ||
			        (loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_LEFT_ELBOW) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_RIGHT_ELBOW));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_UPPER_LEFT_ARM) || (loc == BODY_LOC_CENTAUR_UPPER_RIGHT_ARM) || (loc == BODY_LOC_CENTAUR_LOWER_LEFT_ARM) || (loc == BODY_LOC_CENTAUR_LOWER_RIGHT_ARM) ||
			        (loc == BODY_LOC_CENTAUR_LEFT_ELBOW) || (loc == BODY_LOC_CENTAUR_RIGHT_ELBOW));

		case PHYS_TYPE_WINGED_HUMANOID:
			return ((loc == BODY_LOC_WINGED_HUMANOID_UPPER_LEFT_ARM) || (loc == BODY_LOC_WINGED_HUMANOID_UPPER_RIGHT_ARM) || (loc == BODY_LOC_WINGED_HUMANOID_LOWER_LEFT_ARM) ||
			        (loc == BODY_LOC_WINGED_HUMANOID_LOWER_RIGHT_ARM) || (loc == BODY_LOC_WINGED_HUMANOID_LEFT_ELBOW) || (loc == BODY_LOC_WINGED_HUMANOID_RIGHT_ELBOW));
...
int bodyLocisUpperWrists(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_HUMANOID:
			return ((loc == BODY_LOC_HUMANOID_LEFT_WRIST) || (loc == BODY_LOC_HUMANOID_RIGHT_WRIST));

		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_LEFT_WRIST) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_RIGHT_WRIST));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_LEFT_WRIST) || (loc == BODY_LOC_CENTAUR_RIGHT_WRIST));

		case PHYS_TYPE_WINGED_HUMANOID:
			return ((loc == BODY_LOC_WINGED_HUMANOID_LEFT_WRIST) || (loc == BODY_LOC_WINGED_HUMANOID_RIGHT_WRIST));
...
int bodyLocisUpperHands(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_HUMANOID:
			return ((loc == BODY_LOC_HUMANOID_LEFT_HAND) || (loc == BODY_LOC_HUMANOID_RIGHT_HAND));

		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_LEFT_HAND) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_RIGHT_HAND));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_LEFT_HAND) || (loc == BODY_LOC_CENTAUR_RIGHT_HAND));

		case PHYS_TYPE_WINGED_HUMANOID:
			return ((loc == BODY_LOC_WINGED_HUMANOID_LEFT_HAND) || (loc == BODY_LOC_WINGED_HUMANOID_RIGHT_HAND));
...
int bodyLocisEar(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_HUMANOID:
			return ((loc == BODY_LOC_HUMANOID_LEFT_EAR) || (loc == BODY_LOC_HUMANOID_RIGHT_EAR));

		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_LEFT_EAR) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_RIGHT_EAR));

		case PHYS_TYPE_QUADRUPED:
			return ((loc == BODY_LOC_QUADRUPED_LEFT_EAR) || (loc == BODY_LOC_QUADRUPED_RIGHT_EAR));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_LEFT_EAR) || (loc == BODY_LOC_CENTAUR_RIGHT_EAR));

		case PHYS_TYPE_WINGED_HUMANOID:
			return ((loc == BODY_LOC_WINGED_HUMANOID_LEFT_EAR) || (loc == BODY_LOC_WINGED_HUMANOID_RIGHT_EAR));

		case PHYS_TYPE_WINGED_QUADRUPED:
			return ((loc == BODY_LOC_WINGED_QUADRUPED_LEFT_EAR) || (loc == BODY_LOC_WINGED_QUADRUPED_RIGHT_EAR));
...
int bodyLocisUpperShoulders(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_HUMANOID:
			return ((loc == BODY_LOC_HUMANOID_LEFT_SHOULDER) || (loc == BODY_LOC_HUMANOID_RIGHT_SHOULDER));

		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_LEFT_SHOULDER) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_RIGHT_SHOULDER));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_LEFT_SHOULDER) || (loc == BODY_LOC_CENTAUR_RIGHT_SHOULDER));

		case PHYS_TYPE_WINGED_HUMANOID:
			return ((loc == BODY_LOC_WINGED_HUMANOID_LEFT_SHOULDER) || (loc == BODY_LOC_WINGED_HUMANOID_RIGHT_SHOULDER));

		case PHYS_TYPE_INSECTOID:
			return ((loc == BODY_LOC_INSECTOID_FRONT_LEFT_UPPER_JOINT) || (loc == BODY_LOC_INSECTOID_FRONT_RIGHT_UPPER_JOINT));

		case PHYS_TYPE_ARACHNID:
			return ((loc == BODY_LOC_ARACHNID_FRONT_LEFT_UPPER_JOINT) || (loc == BODY_LOC_ARACHNID_FRONT_RIGHT_UPPER_JOINT));
...
int bodyLocisLegs(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_HUMANOID:
			return ((loc == BODY_LOC_HUMANOID_UPPER_LEFT_LEG) || (loc == BODY_LOC_HUMANOID_LEFT_KNEE) || (loc == BODY_LOC_HUMANOID_LOWER_LEFT_LEG) || (loc == BODY_LOC_HUMANOID_UPPER_RIGHT_LEG) ||
			        (loc == BODY_LOC_HUMANOID_RIGHT_KNEE) || (loc == BODY_LOC_HUMANOID_LOWER_RIGHT_LEG));

		case PHYS_TYPE_QUADRUPED:
			return ((loc == BODY_LOC_QUADRUPED_UPPER_LEFT_FRONT_LEG) || (loc == BODY_LOC_QUADRUPED_LEFT_FRONT_KNEE) || (loc == BODY_LOC_QUADRUPED_LOWER_LEFT_FRONT_LEG) ||
			        (loc == BODY_LOC_QUADRUPED_UPPER_RIGHT_FRONT_LEG) || (loc == BODY_LOC_QUADRUPED_RIGHT_FRONT_KNEE) || (loc == BODY_LOC_QUADRUPED_LOWER_RIGHT_FRONT_LEG));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_UPPER_LEFT_FRONT_LEG) || (loc == BODY_LOC_CENTAUR_LEFT_FRONT_KNEE) || (loc == BODY_LOC_CENTAUR_LOWER_LEFT_FRONT_LEG) ||
			        (loc == BODY_LOC_CENTAUR_UPPER_RIGHT_FRONT_LEG) || (loc == BODY_LOC_CENTAUR_RIGHT_FRONT_KNEE) || (loc == BODY_LOC_CENTAUR_LOWER_RIGHT_FRONT_LEG));

		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_LEFT_LEG) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LEFT_KNEE) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_LEFT_LEG) ||
			        (loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_RIGHT_LEG) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_RIGHT_KNEE) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_RIGHT_LEG));

		case PHYS_TYPE_WINGED_HUMANOID:
			return ((loc == BODY_LOC_WINGED_HUMANOID_UPPER_LEFT_LEG) || (loc == BODY_LOC_WINGED_HUMANOID_LEFT_KNEE) || (loc == BODY_LOC_WINGED_HUMANOID_LOWER_LEFT_LEG) ||
			        (loc == BODY_LOC_WINGED_HUMANOID_UPPER_RIGHT_LEG) || (loc == BODY_LOC_WINGED_HUMANOID_RIGHT_KNEE) || (loc == BODY_LOC_WINGED_HUMANOID_LOWER_RIGHT_LEG));

		case PHYS_TYPE_WINGED_QUADRUPED:
			return ((loc == BODY_LOC_WINGED_QUADRUPED_UPPER_LEFT_FRONT_LEG) || (loc == BODY_LOC_WINGED_QUADRUPED_LEFT_FRONT_KNEE) || (loc == BODY_LOC_WINGED_QUADRUPED_LOWER_LEFT_FRONT_LEG) ||
			        (loc == BODY_LOC_WINGED_QUADRUPED_UPPER_RIGHT_FRONT_LEG) || (loc == BODY_LOC_WINGED_QUADRUPED_RIGHT_FRONT_KNEE) || (loc == BODY_LOC_WINGED_QUADRUPED_LOWER_RIGHT_FRONT_LEG));
...
int bodyLocisFeet(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_HUMANOID:
			return ((loc == BODY_LOC_HUMANOID_LEFT_FOOT) || (loc == BODY_LOC_HUMANOID_LEFT_ANKLE) || (loc == BODY_LOC_HUMANOID_RIGHT_FOOT) || (loc == BODY_LOC_HUMANOID_RIGHT_ANKLE));

		case PHYS_TYPE_QUADRUPED:
			return ((loc == BODY_LOC_QUADRUPED_LEFT_FRONT_HOOF) || (loc == BODY_LOC_QUADRUPED_LEFT_FRONT_ANKLE) || (loc == BODY_LOC_QUADRUPED_RIGHT_FRONT_HOOF) ||
			        (loc == BODY_LOC_QUADRUPED_RIGHT_FRONT_ANKLE));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_LEFT_FRONT_HOOF) || (loc == BODY_LOC_CENTAUR_LEFT_FRONT_ANKLE) || (loc == BODY_LOC_CENTAUR_RIGHT_FRONT_HOOF) ||
			        (loc == BODY_LOC_CENTAUR_RIGHT_FRONT_ANKLE));

		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_LEFT_FOOT) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LEFT_ANKLE) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_RIGHT_FOOT) ||
			        (loc == BODY_LOC_FOUR_ARMED_HUMANOID_RIGHT_ANKLE));

		case PHYS_TYPE_WINGED_HUMANOID:
			return ((loc == BODY_LOC_WINGED_HUMANOID_LEFT_FOOT) || (loc == BODY_LOC_WINGED_HUMANOID_LEFT_ANKLE) || (loc == BODY_LOC_WINGED_HUMANOID_RIGHT_FOOT) ||
			        (loc == BODY_LOC_WINGED_HUMANOID_RIGHT_ANKLE));

		case PHYS_TYPE_WINGED_QUADRUPED:
			return ((loc == BODY_LOC_WINGED_QUADRUPED_LEFT_FRONT_HOOF) || (loc == BODY_LOC_WINGED_QUADRUPED_LEFT_FRONT_ANKLE) || (loc == BODY_LOC_WINGED_QUADRUPED_RIGHT_FRONT_HOOF) ||
...
int bodyLocisLowerArms(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_LEFT_LOWER_ARM) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_UPPER_RIGHT_LOWER_ARM) ||
			        (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_LEFT_LOWER_ARM) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_RIGHT_LOWER_ARM) ||
			        (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_LEFT_ELBOW) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_RIGHT_ELBOW));
...
int bodyLocisLowerWrists(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_LEFT_WRIST) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_RIGHT_WRIST));
...
int bodyLocisLowerHands(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_FOUR_ARMED_HUMANOID:
			return ((loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_LEFT_HAND) || (loc == BODY_LOC_FOUR_ARMED_HUMANOID_LOWER_RIGHT_HAND));
...
int bodyLocisHorseBody(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_QUADRUPED:
			return ((loc == BODY_LOC_QUADRUPED_FRONT_BODY) || (loc == BODY_LOC_QUADRUPED_REAR_BODY));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_FRONT_HORSE_BODY) || (loc == BODY_LOC_CENTAUR_REAR_HORSE_BODY));

		case PHYS_TYPE_WINGED_QUADRUPED:
			return ((loc == BODY_LOC_WINGED_QUADRUPED_FRONT_BODY) || (loc == BODY_LOC_WINGED_QUADRUPED_REAR_BODY));
...
int bodyLocisRearLegs(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_QUADRUPED:
			return ((loc == BODY_LOC_QUADRUPED_UPPER_LEFT_REAR_LEG) || (loc == BODY_LOC_QUADRUPED_LOWER_LEFT_REAR_LEG) || (loc == BODY_LOC_QUADRUPED_UPPER_RIGHT_REAR_LEG) ||
			        (loc == BODY_LOC_QUADRUPED_LOWER_RIGHT_REAR_LEG) || (loc == BODY_LOC_QUADRUPED_LEFT_REAR_KNEE) || (loc == BODY_LOC_QUADRUPED_RIGHT_REAR_KNEE));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_UPPER_LEFT_REAR_LEG) || (loc == BODY_LOC_CENTAUR_LOWER_LEFT_REAR_LEG) || (loc == BODY_LOC_CENTAUR_UPPER_RIGHT_REAR_LEG) ||
			        (loc == BODY_LOC_CENTAUR_LOWER_RIGHT_REAR_LEG) || (loc == BODY_LOC_CENTAUR_LEFT_REAR_KNEE) || (loc == BODY_LOC_CENTAUR_RIGHT_REAR_KNEE));

		case PHYS_TYPE_WINGED_QUADRUPED:
			return ((loc == BODY_LOC_WINGED_QUADRUPED_UPPER_LEFT_REAR_LEG) || (loc == BODY_LOC_WINGED_QUADRUPED_LOWER_LEFT_REAR_LEG) || (loc == BODY_LOC_WINGED_QUADRUPED_UPPER_RIGHT_REAR_LEG) ||
			        (loc == BODY_LOC_WINGED_QUADRUPED_LOWER_RIGHT_REAR_LEG) || (loc == BODY_LOC_WINGED_QUADRUPED_LEFT_REAR_KNEE) || (loc == BODY_LOC_WINGED_QUADRUPED_RIGHT_REAR_KNEE));
...
int bodyLocisRearFeet(const int physType, const int loc)
{
	...
	{
		case PHYS_TYPE_QUADRUPED:
			return ((loc == BODY_LOC_QUADRUPED_LEFT_REAR_HOOF) || (loc == BODY_LOC_QUADRUPED_LEFT_REAR_ANKLE) || (loc == BODY_LOC_QUADRUPED_RIGHT_REAR_HOOF) ||
			        (loc == BODY_LOC_QUADRUPED_RIGHT_REAR_ANKLE));

		case PHYS_TYPE_CENTAUR:
			return ((loc == BODY_LOC_CENTAUR_LEFT_REAR_HOOF) || (loc == BODY_LOC_CENTAUR_LEFT_REAR_ANKLE) || (loc == BODY_LOC_CENTAUR_RIGHT_REAR_HOOF) || (loc == BODY_LOC_CENTAUR_RIGHT_REAR_ANKLE));

		case PHYS_TYPE_WINGED_QUADRUPED:
			return ((loc == BODY_LOC_WINGED_QUADRUPED_LEFT_REAR_HOOF) || (loc == BODY_LOC_WINGED_QUADRUPED_LEFT_REAR_ANKLE) || (loc == BODY_LOC_WINGED_QUADRUPED_RIGHT_REAR_HOOF) ||
...
int calcChDamagetoVict(P_char ch, P_char victim, P_obj weap, const int body_loc, const int wpn_skl, const int wpn_skl_lvl, const int hit_type, const int crit_hit)
{
	...
	if (weap && (GET_ITEM_TYPE(weap) == ITEM_WEAPON) && isBackstabber(weap) && ((hit_type == SKILL_BACKSTAB) || (hit_type == SKILL_CIRCLE)))
	{
...
int getCharParryVal(const P_char vict, const P_char attacker, const int body_loc_target, const P_obj weapon)
{
	...
	if (IS_PC(vict))
		parry_skill = ((GET_CHAR_SKILL(vict, SKILL_PARRY) / 2) + (GET_CHAR_SKILL(vict, getWeaponSkillNumb(weapon)) / 2));
	...
	*/
	...
	if (attacker && (IS_AFFECTED(attacker, AFF_HASTE) || IS_AFFECTED3(attacker, AFF3_BLUR)))
	{
		//    chance -= 5;
		mod -= 0.05;
	}
