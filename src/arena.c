...
const char *game_type[] = {
	"Capture the Flag", "Jail Break", "King of the Hill", "Team DeathMatch", "IT",
	"Deathmatch",	    "\n"
};

const char *arena_stage[] = { "&+GAccepting players&N",	   "&+rPre Match prep&N",
			      "&=LRMatch in progress&N",   "&=LRMatch in progress&N",
			      "&+LPost match aftermath&N", "&+LPost match aftermath&N" };
const char *death_type[] = { "Winner Takes ALL", "Even Trade", "Entrance Fee, Winner takes Pot",
			     "Free Play", "\n" };
...
const char *arena_death_msg(P_obj /*p_weapon*/)
{
	return "killed";
}
...
void players_from_map()
{
	...
	for (i = arena.map.startroom; i <= arena.map.endroom; i++)
	{
		for (ch = world[real_room(i)].people; ch; ch = next_ch)
		{
			...
			{
				char_from_room(ch);
				char_to_room(ch, real_room(arena_start_room[arena_team(ch)][0]),
					     -1);
			}
		}
	}
		...
		if (d->character != NULL)
		{
			if (CHAR_IN_ARENA(d->character))
			{
				...
				char_to_room(
					d->character,
					real_room(
						arena_hometown_location[arena_team(d->character)]),
					-1);
			}
		}
	}
...
void show_stats_to_char(P_char ch)
{
	...
	else
	{
		...
		snprintf(tbuf, MAX_STRING_LENGTH,
			 "&+LGame Type: &+W%-20s &+LDeath Mode: &+W%s\r\n&N", game_type[arena.type],
			 death_type[arena.deathmode]);
		send_to_char(tbuf, ch);
		countg = 0;
		counte = 0;
...
int arenaobj_proc(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_ENTER)
	{
		...
		else
		{
			...
			char_to_room(
				ch,
				real_room(arena_hometown_location[IS_RACEWAR_EVIL(ch) ? EVIL :
											GOODIE]),
				-1);
			act("$n steps out of $p.", TRUE, ch, obj, 0, TO_ROOM);
			return TRUE;
