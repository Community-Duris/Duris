...
int check_nexus_bonus(P_char ch, int amount, int type)
{
	...
	{
		newamnt = MAX(nexus_bonus_data[type].minval,
			      (int)((double)amount * (double)get_property(buff, 0.100)));
		...
		case NEXUS_BONUS_CARGO:
			snprintf(buff2, MAX_STRING_LENGTH,
				 "&+yEnlil&n grants you an additional %s&n.\r\n",
...
bool nexus_stone_touch(P_obj stone, P_char ch)
{
	...
	for (i = world[ch->in_room].people; i; i = next)
	{
		...
		if (IS_NPC(i))
		{
			if (GET_VNUM(i) == MOB_NEUTRAL_GUARDIAN ||
			    GET_VNUM(i) == MOB_GOOD_GUARDIAN || GET_VNUM(i) == MOB_EVIL_GUARDIAN)
			{
	...
	if (IS_RACEWAR_GOOD(ch))
	{
		...
		if (STONE_ALIGN(stone) >= STONE_ALIGN_GOOD)
		{
			...
			statuslog(57, "Nexus stone (%d) turned to good by %s", STONE_ID(stone),
				  GET_NAME(ch));
			logit(LOG_STATUS, "Nexus stone (%d) turned to good by %s", STONE_ID(stone),
			      GET_NAME(ch));
	...
	else if (IS_RACEWAR_EVIL(ch))
	{
		...
		if (STONE_ALIGN(stone) <= STONE_ALIGN_EVIL)
		{
			...
			statuslog(57, "Nexus stone (%d) turned to evil by %s", STONE_ID(stone),
				  GET_NAME(ch));
			logit(LOG_STATUS, "Nexus stone (%d) turned to evil by %s", STONE_ID(stone),
			      GET_NAME(ch));
		...
		if (ch->group)
		{
			for (struct group_list *gl = ch->group; gl; gl = gl->next)
			{
				if (gl->ch->in_room == ch->in_room)
					check_boon_completion(gl->ch, NULL, STONE_ID(stone),
							      BOPT_NEXUS);
			}
		}
		else
	...
	if (!IS_TRUSTED(ch))
	{
		add_event(event_nexus_stone_hum,
			  (get_property("nexusStones.touchTimerSecs", 10) * WAIT_SEC), 0, 0, stone,
			  0, 0, 0);
	}
...
int nexus_stone(P_obj stone, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_PERIODIC)
	{
		...
		if (!sage && STONE_TURNED(stone) &&
		    (STONE_SAGE_TIMER(stone) <
		     (time_now - (int)get_property("nexusStones.sage.respawnWaitSecs", 3600))))
		{
	...
	if (ch->group)
	{
		for (struct group_list *gl = ch->group; gl; gl = gl->next)
		{
			...
			if (gl->ch->in_room == ch->in_room)
			{
				...
				{
					send_to_char(
						"The &+rGods of Duris&n are very pleased with your success.\n",
						gl->ch);
					send_to_char(
...
void nexus_guardian_nuke(P_char ch, P_char victim, int dam)
{
	...
	act("$n&+W's surface seems to ripple...\n            &+W...and a &+YMASSIVE &+Wburst of energy is released!&n",
...
void nexus_guardian_energy_burst(P_char ch, bool show_message = true)
{
	...
	for (tch = world[ch->in_room].people; tch; tch = next)
	{
		...
		if (should_area_hit(ch, tch))
		{
			...
			{
				act("You deftly slip through the wave of energy.", FALSE, ch, 0,
				    tch, TO_VICT);
				act("$N deftly slips through the wave of energy.", FALSE, ch, 0,
				    tch, TO_NOTVICT);
				continue;
			}
			...
			{
				act("The wave slams into you, throwing from the room!", FALSE, ch,
				    0, tch, TO_VICT);
				act("The wave slams into $N, throwing $M from the room!", FALSE, ch,
				    0, tch, TO_NOTVICT);
				int target_room = world[tch->in_room].dir_option[door]->to_room;
				char_from_room(tch);
				if (char_to_room(tch, target_room, -1))
				{
					act("$n flies in, crashing on the floor!", TRUE, tch, 0, 0,
					    TO_ROOM);
			...
			{
				act("The wave slams into you, knocking you off your feet!", FALSE,
				    ch, 0, tch, TO_VICT);
				act("The wave slams into $N, knocking $M off $S feet!", FALSE, ch,
				    0, tch, TO_NOTVICT);
...
int nexus_stone_guardian(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	...
	if (cmd == CMD_DEATH)
	{
		if (stone_id == -1)
		{
			debug("nexus_stone_guardian(CMD_DEATH): couldn't find stone_id (%d)!",
			      stone_id);
			logit(LOG_DEBUG,
			      "nexus_stone_guardian(CMD_DEATH): couldn't find stone_id (%d)!",
			      stone_id);
		}
		...
		if (!ns)
		{
			debug("nexus_stone_guardian(CMD_DEATH): couldn't find stone (%d)!",
			      stone_id);
			logit(LOG_DEBUG,
			      "nexus_stone_guardian(CMD_DEATH): couldn't find stone (%d)!",
			      stone_id);
		}
...
P_char load_guardian(int stone_id, int rroom_id, int align)
{
	...
	if (!mace)
	{
		logit(LOG_DEBUG, "load_guardian(): guardian mace %d not loadable.",
		      OBJ_GUARDIAN_MACE);
...
int nexus_sage_ask(P_char ch, P_char pl, char *arg)
{
	...
	if (current_stat >= 100)
	{
		act("$n says, 'You have already surpassed my ability to train you!'", FALSE, ch, 0,
		    pl, TO_VICT);
	}
	else
	{
		snprintf(
			...
			"$n says, 'If you have acquired truly epic experiences, type \"train\" to learn from my considerable %s.'",
...
int nexus_sage_train(P_char ch, P_char pl, char * /*arg*/)
{
	...
	if (current_stat >= 100)
	{
		act("$n says, 'You have already surpassed my ability to teach you!'", FALSE, ch, 0,
		    pl, TO_VICT);
		return TRUE;
	}
	...
	{
		act("$n says, 'You have not learned enough. Come back later when you are ready!'",
...
int nexus_sage(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if (cmd == CMD_DEATH)
	{
		...
		if (stone_id == -1)
		{
			...
			logit(LOG_DEBUG, "nexus_sage(CMD_DEATH): couldn't find stone_id (%d)!",
			      stone_id);
			return TRUE;
		}
		...
		if (!ns)
		{
			...
			logit(LOG_DEBUG, "nexus_sage(CMD_DEATH): couldn't find stone (%d)!",
			      stone_id);
			return TRUE;
		}
		...
		{
			GET_HIT(guardian) =
				(int)(GET_HIT(guardian) *
				      get_property("nexusStones.sage.death.guardian.hit.modifier",
						   0.500));
			act("$n&n's presence trembles slightly, as $s energy is siphoned away.",
			    FALSE, guardian, 0, 0, TO_ROOM);
		}
...
bool load_nexus_stone(int stone_id, const char *stone_name, int room_vnum, int align)
{
	...
	if (!stone)
	{
		logit(LOG_DEBUG, "load_nexus_stone(): nexus stone obj %d not loadable.",
		      OBJ_NEXUS_STONE);
...
void reload_nexus_stone(P_char ch, int stone_id)
{
	...
	for (P_char tch = character_list; tch; tch = tch->next)
	{
		if ((IS_NEXUS_GUARDIAN(tch) || IS_NEXUS_SAGE(tch)) &&
		    guardian_stone_id(tch) == stone_id)
		{
			delete_chars.push_back(tch);
		}
	}
