...
bool is_allowed_race_spec(int race, uint m_class, int spec)
{
	...
	for (int i = 0; allowed_race_specs[i].race; i++)
	{
		if (allowed_race_specs[i].race == race &&
		    ...
		    (allowed_race_specs[i].spec == SPEC_ALL || allowed_race_specs[i].spec == spec))
		{
			return TRUE;
		}
	}
...
void do_specialize(P_char ch, char *argument, int /*cmd*/)
{
	...
	{
		if (IS_DRAGOON(ch) && IS_NPC(teacher) &&
		...
		if (IS_PC(teacher) && IS_TRUSTED(teacher) &&
		    is_linked_to(ch, teacher, LNK_CONSENT) && ch != teacher)
		{
			break;
		}
	}
	...
	{
		mobsay(teacher,
		       "Please stop trying to crash the game by ordering pets to specialize.");
		return;
	}
	...
	for (i = 0; i < MAX_SPEC; i++)
	{
		...
		if (is_abbrev(argument, strip_ansi(GET_SPEC_NAME(ch->player.m_class, i)).c_str()))
		{
			if (IS_DRAGOON(ch))
			{
				if (i + 1 == SPEC_DRAGON_HUNTER &&
				    !(GET_CLASS(teacher, CLASS_RANGER) ||
				      GET_CLASS(teacher, CLASS_MERCENARY)))
				{
					snprintf(
						buf, MAX_STRING_LENGTH,
						"I cannot teach you to follow the path of the %s&n!",
						GET_SPEC_NAME(ch->player.m_class, i));
					mobsay(teacher, buf);
					return;
				}

				if (i + 1 == SPEC_DRAGON_PRIEST &&
			...
			snprintf(buf, MAX_STRING_LENGTH,
				 "From this day onwards you will follow the path of the %s&n!",
				 GET_SPEC_NAME(ch->player.m_class, i));
			ch->player.spec = i + 1;
			update_skills(ch);
			mobsay(teacher, buf);
