...
int wh_corpse_to_object(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	if (cmd == CMD_DEATH)
	{
		...
		if (!(obj))
		{
			logit(LOG_OBJ, "winterhaven_object: death object for mob %d doesn't exist",
			      GET_VNUM(ch));
			return FALSE;
		}
...
int wh_corpse_decay(P_obj obj, P_char ch, int cmd, char * /*args*/)
{
	...
	if (obj->value[0]-- <= 0)
	{
		...
		if (!corpse)
		{
			logit(LOG_OBJ, "wh_corpse_decay: unable to load obj #%d",
			      VOBJ_WH_ROTTING_CORPSE);
			return FALSE;
		}
...
int storm_legplates(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (IS_ALIVE(GET_OPPONENT(ch)) && ch->in_room == GET_OPPONENT(ch)->in_room)
	{
		if (arg && (cmd == CMD_SAY))
		{
			if (isname(arg, "storm"))
			{
				...
				if (obj->timer[0] + 600 <= curr_time)
				{
					if (OUTSIDE(ch))
					{
						act("&+WYou say '&+wI summon the &+Lst&+wo&+Lrm&+w!&+W'&n",
						    TRUE, ch, obj, NULL, TO_CHAR);
						act("&+LA m&+Wassive &+Ls&+Wtatic &+Lc&+wharge &+Lb&+Wuilds &+Li&+Wn $q &+La&+Ws &+Ly&+Wou &+Ls&+Wtomp &+Ly&+wour &+Lf&+weet!&n",
						    TRUE, ch, obj, obj, TO_CHAR);
						act("$n says '&+wI summon the &+Lst&+wo&+Lrm&+w!&+W'&n",
						    TRUE, ch, obj, NULL, TO_NOTVICT);
						act("&+LA m&+Wassive &+Ls&+Wtatic &+Lc&+wharge &+Lb&+Wuilds &+Li&+Wn $q &+La&+Ws $N &+Ls&+Wtomps $s &+Lf&+weet!&n",
						    TRUE, ch, obj, NULL, TO_NOTVICT);
						act("$n says '&+wI summon the &+Lst&+wo&+Lrm&+w!&+W'&n",
		...
		if (cmd == CMD_GOTHIT && !number(0, 29))
		{
			...
			act("$q &+wsizzles with &+Yenergy &+was $N tries to hit you.&n", TRUE, ch,
			    obj, vict, TO_CHAR);
			act("$q &+wsizzles with &+Yenergy &+was you try to hit $n.&n", TRUE, ch,
			    obj, vict, TO_VICT);
			act("$q &+wsizzles with &+Yenergy &+was $N &+wtries to hit $n&+w.&n", TRUE,
	...
	if (!IS_FIGHTING(ch))
	{
		...
		if (arg && (cmd == CMD_STOMP))
		{
			if (isname(arg, "ground"))
			{
				...
				if (obj->timer[0] + 600 <= curr_time)
				{
					act("$q &+wsends a burst of &+Yelectricity &+wthrough your body.&n",
					    TRUE, ch, obj, obj, TO_CHAR);
					act("You feel &+Bvitalized &+wand &+Cenergized&n.", TRUE,
					    ch, obj, obj, TO_CHAR);
					act("$q &+wsends a burst of &+Yelectricity &+wthrough $n's body.&n",
					    TRUE, ch, obj, NULL, TO_ROOM);
					act("$n looks &+Bvitalized &+wand &+Cenergized&n.", TRUE,
					    ch, obj, NULL, TO_ROOM);
...
int blur_shortsword(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_SAY))
		{
			if (isname(arg, "blur"))
			{
				...
				if (obj->timer[0] + 600 <= curr_time)
				{
					...
					act("&+L$n&+L's $q &+Lslows down time and freezes you in place!&n",
					    TRUE, ch, obj, vict, TO_VICT);
					act("&+L...$n leaps at you and deals a series of &+cvicious &+Lattacks!&n",
					    TRUE, ch, obj, vict, TO_VICT);

					act("&+L$n&+L's $q &+Lslows down time and freezes&n $N &+Lin place!&n",
					...
					act("$p &+Cglows &+Las it touches $n&+L's &+Csoul&+L!&n",
		...
		if (cmd == CMD_GOTHIT && !number(0, 24))
		{
			...
			act("&+LAn &+Bicy &+chaze &+Lbillows out from $q&+L, &+Bslowing &+Ldown &n$N&+L's attack.",
			    TRUE, ch, obj, vict, TO_CHAR | ACT_NOTTERSE);
			act("&+L...you swiftly dodge &n$N&+L's attack and turn to deliver a &+Cvicious &+Lstrike!",
			    TRUE, ch, obj, vict, TO_CHAR | ACT_NOTTERSE);

			act("&+LAn &+Bicy &+chaze &+Lbillows out from $q+L, &+Bslowing &+Ldown your attack.&n",
			    TRUE, ch, obj, vict, TO_VICT | ACT_NOTTERSE);
			act("&+L...$n &+Lswiftly dodges your attack, $m turns to deliver a &+Cvicious &+Lstrike!&n",
			    TRUE, ch, obj, vict, TO_VICT | ACT_NOTTERSE);
			...
			act("$p &+Cglows &+Las it touches your &+Csoul&+L!&n", FALSE, ch, obj, NULL,
			    TO_CHAR);
			act("$p &+Cglows &+Las it touches $n&+L's &+Csoul&+L!&n", FALSE, ch, obj,
			    NULL, TO_ROOM);
			...
			case 2:
				...
				{
					spell_arieks_shattering_iceball(35, ch, 0, SPELL_TYPE_SPELL,
									vict, 0);
				}
				break;
			case 3:
	...
	if (!IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_RUB))
		{
			if (isname(arg, "misty"))
			{
				...
				if (obj->timer[0] + 600 <= curr_time)
				{
					act("&+LYour $q &+Chums &+Lloudly and surrounds you in a &+Cmisty &+chaze&+L.&n",
					    TRUE, ch, obj, NULL, TO_CHAR);
					act("&+L$n&+L's $q &+Chums &+Lloudly and surrounds $m in a &+Cmisty &+chaze&+L.&n",
					    TRUE, ch, obj, NULL, TO_ROOM);
	...
	if (IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_SAY))
		{
			if (isname(arg, "volo"))
			{
				...
				if (obj->timer[0] + 600 <= curr_time)
				{
					act("&+WYou say '&+cI s&+Lummon &+ct&+Lhe &+cT&+Lraveler!&+W'&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+cY&+Lou &+Lthrust $q &+Linto $N&+L's &+rfl&+Re&+rsh &+Land send a &+ct&+Lor&+cr&+Len&+ct &+Lof dark &+cp&+Llanar &+ce&+Lnergy into $S &+cs&+Lou&+cl&+L!&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+cV&+Lo&+cl&+Lo &+ct&+Lhe &+cT&+Lraveler reaches from beyond the &+cP&+Llanar &+cR&+Lealms and grabs a hold of $N&+L's soul!&n",
					    TRUE, ch, obj, vict, TO_CHAR);

					act("$n says '&+cI s&+Lummon &+ct&+Lhe &+cT&+Lraveler!&+W'&n",
					    TRUE, ch, obj, vict, TO_NOTVICT);
					...
					act("$n &+Lthrust $q &+Linto your &+rfl&+Re&+rsh &+Land send a &+ct&+Lor&+cr&+Len&+ct &+Lof dark &+cp&+Llanar &+ce&+Lnergy into your &+cs&+Lou&+cl&+L!&n",
					    TRUE, ch, obj, vict, TO_VICT);
					act("&+cV&+Lo&+cl&+Lo &+ct&+Lhe &+cT&+Lraveler reaches from beyond the &+cP&+Llanar &+cR&+Lealms and grabs a hold of YOUR soul!&n",
					    TRUE, ch, obj, vict, TO_VICT);

					spell_teleport(50, ch, 0, 0, vict, 0);

					act("$n slowly fades out of existence.&n", TRUE, ch, obj,
					    vict, TO_ROOM);

					char_from_room(ch);
					char_to_room(ch, vict->in_room, -1);
					act("&+cV&+Lo&+cl&+Lo &+ct&+Lhe &+cT&+Lraveler grins wickedly as he throws you back into combat!&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+cV&+Lo&+cl&+Lo &+ct&+Lhe &+cT&+Lraveler grins wickedly as he throws you back into combat!&n",
					    TRUE, ch, obj, vict, TO_VICT);
					attack(ch, vict);
		...
		if (cmd == CMD_MELEE_HIT && IS_ALIVE(vict) && !number(0, 32) &&
		    CheckMultiProcTiming(ch) && !IS_ELITE(vict) && !IS_GREATER_RACE(vict))
		{
			switch (number(0, 2))
			{
			case 0:
				act("&+LYour $q &+Lsinks deep into &n$N&+L's &+rfl&+Re&+rsh&+L!&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("$n's $q &+Lsinks deep into &n$N&+L's &+rfl&+Re&+rsh&+L!&n",
				    TRUE, ch, obj, vict, TO_NOTVICT);
				act("$n's $q &+Lsinks deep into your &+rfl&+Re&+rsh&+L!&n", TRUE,
				    ch, obj, vict, TO_VICT);

				spell_energy_drain(GET_LEVEL(ch), ch, 0, SPELL_TYPE_SPELL, vict, 0);
				break;
			case 1:
				act("&+LYour $q &+cg&+Llows &+cb&+Lrill&+ci&+Lantly!&n", TRUE, ch,
				    obj, vict, TO_CHAR);
				act("&+LYou feel the &+cwounds &+Land &+cfatigue &+Lof your body melt away.&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("$n's $q &+cg&+Llows &+cb&+Lrill&+ci&+Lantly!&n", TRUE, ch, obj,
				    vict, TO_ROOM);
				act("$n's $q &+cg&+Llows &+cb&+Lrill&+ci&+Lantly!&n", TRUE, ch, obj,
				    vict, TO_VICT);
				...
				break;
			case 2:
				act("&+LYour $q &+cf&+Llares &+cw&+Lith &+cp&+Lower and strikes at the back $N&+L's of legs!&n",
				    FALSE, ch, obj, vict, TO_CHAR);
				act("$N &+Lis stunned and falls to $s knees in agony!&n", FALSE, ch,
				    obj, vict, TO_CHAR);

				act("$n's $q &+cf&+Llares &+cw&+Lith &+cp&+Lower!&n", FALSE, ch,
				    obj, vict, TO_ROOM);
				act("$N &+Lis stunned and falls to $s knees in agony!&n", FALSE, ch,
				    obj, vict, TO_ROOM);
				...
				act("$n's $q &+cf&+Llares &+cw&+Lith &+cp&+Lower!&n", FALSE, ch,
				    obj, vict, TO_VICT);
				act("You are stunned and fall to your knees in agony!&n", FALSE, ch,
				    obj, vict, TO_VICT);
...
int snowogre_warhammer(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (arg && (cmd == CMD_SAY))
	{
		if (isname(arg, "fury"))
		{
			...
			if (obj->timer[0] + 600 <= curr_time)
			{
				...
				act("&+WYou whisper to $p &+Wunleashing the &+rF&+Ru&+rr&+Ry &+Wof the Snow &+bOgre &+Wkings!&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("$n says '&+rf&+Ru&+rr&+Ry&n'", TRUE, ch, obj, vict, TO_ROOM);
				act("$n&+W's $q &+Wunleashes the &+rF&+Ru&+rr&+Ry &+Wof the Snow &+bOgre &+WKings!&n",
				...
				{
					spell_enlarge(GET_LEVEL(ch), ch, 0, SPELL_TYPE_SPELL, ch,
						      0);
				}
				...
				if (!has_skin_spell(ch))
				{
					...
					{
					case 0:
						spell_shadow_shield(GET_LEVEL(ch), ch, 0,
								    SPELL_TYPE_SPELL, ch, 0);
						break;
					case 1:
						spell_biofeedback(GET_LEVEL(ch), ch, 0,
								  SPELL_TYPE_SPELL, ch, 0);
						break;
					case 2:
						spell_stone_skin(GET_LEVEL(ch), ch, 0,
								 SPELL_TYPE_SPELL, ch, 0);
						break;
					default:
						break;
	...
	if (cmd == CMD_MELEE_HIT && CanDoFightMove(ch, GET_OPPONENT(ch)) && !IS_IMMOBILE(ch) &&
	    !number(0, 32) && CheckMultiProcTiming(ch) && !IS_ELITE(vict))
	{
		...
		{
		case 0:
			act("$p &+Wsuddenly unleashes the &+wF&+ru&+Rr&+Ly &+Wof the &+LV&+rol&+Rc&+ran&+Lo&n",
			    TRUE, ch, obj, vict, TO_CHAR);
			act("$p &+Wsuddenly unleashes the &+wF&+ru&+Rr&+Ly &+Wof the &+LV&+rol&+Rc&+ran&+Lo&n",
			    TRUE, ch, obj, vict, TO_NOTVICT);
			act("$p &+Wsuddenly unleashes the &+wF&+ru&+Rr&+Ly &+Wof the &+LV&+rol&+Rc&+ran&+Lo &+Wupon you!&n",
			    TRUE, ch, obj, vict, TO_VICT);
			...
			break;
		case 1:
			act("$p &+Wsuddenly unleashes the &+wF&+yu&+Yr&+wy &+Wof the &+wD&+ye&+Yse&+yr&+wt&n",
			    TRUE, ch, obj, vict, TO_CHAR);
			act("$p &+Wsuddenly unleashes the &+wF&+yu&+Yr&+wy &+Wof the &+wD&+ye&+Yse&+yr&+wt&n",
			    TRUE, ch, obj, vict, TO_NOTVICT);
			act("$p &+Wsuddenly unleashes the &+wF&+yu&+Yr&+wy &+Wof the &+wD&+ye&+Yse&+yr&+wt &+Wupon you!&n",
			    TRUE, ch, obj, vict, TO_VICT);

			act("A powerful &+Ysand&+ystorm &npummels $N!&n", TRUE, ch, obj, vict,
			    TO_CHAR);
			act("A powerful &+Ysand&+ystorm &npummels $N!&n", TRUE, ch, obj, vict,
			    TO_NOTVICT);
			act("A powerful &+Ysand&+ystorm &npummels you!&n", TRUE, ch, obj, vict,
			    TO_VICT);

			if (affected_by_spell(vict, SPELL_STONE_SKIN))
			{
				act("&+yThe &+Ysand&+ystorm grinds away $N&+y's &+Lstone skin&+y!&n",
				    TRUE, ch, obj, vict, TO_NOTVICT);
				act("&+yThe &+Ysand&+ystorm grinds away your &+Lstone skin&+y!",
				    TRUE, ch, obj, vict, TO_VICT);
				act("&+yThe &+Ysand&+ystorm grinds away $N&+y's &+Lstone skin&+y!",
				    TRUE, ch, obj, vict, TO_CHAR);
				affect_from_char(vict, SPELL_STONE_SKIN);
			}
			if (affected_by_spell(vict, SPELL_SHADOW_SHIELD))
			{
				act("&+yThe &+Ysand&+ystorm grinds away $N&+y's &+Lshadow shield&+y!&n",
				    TRUE, ch, obj, vict, TO_NOTVICT);
				act("&+yThe &+Ysand&+ystorm grinds away your &+Lshadow shield&+y!",
				    TRUE, ch, obj, vict, TO_VICT);
				act("&+yThe &+Ysand&+ystorm grinds away $N&+y's &+Lshadow shield&+y!",
				    TRUE, ch, obj, vict, TO_CHAR);
			...
			{
				act("&+yThe &+Ysand&+ystorm grinds away $N&+y's &+Gbiofeedback&+y!&n",
				    TRUE, ch, obj, vict, TO_NOTVICT);
				act("&+yThe &+Ysand&+ystorm grinds away your &+Gbiofeedback&+y!",
				    TRUE, ch, obj, vict, TO_VICT);
				act("&+yThe &+Ysand&+ystorm grinds away $N&+y's &+Gbiofeedback&+y!",
				    TRUE, ch, obj, vict, TO_CHAR);
			...
			{
				act("&+yThe &+Ysand&+ystorm shatters $N&+y's &+rglobe of invulnerability&+y!&n",
				    TRUE, ch, obj, vict, TO_NOTVICT);
				act("&+yThe &+Ysand&+ystorm shatters your &+rglobe of invulnerability&+y!",
				    TRUE, ch, obj, vict, TO_VICT);
				act("&+yThe &+Ysand&+ystorm shatters $N&+y's &+rglobe of invulnerability&+y!",
				    TRUE, ch, obj, vict, TO_CHAR);
			...
			break;
		case 2:
			act("$p &+Wsuddenly unleashes the &+wF&+cu&+Cr&+wy &+Wof the &+wgl&+ca&+Cc&+ci&+wer&n",
			    TRUE, ch, obj, vict, TO_CHAR);
			act("$p &+Wsuddenly unleashes the &+wF&+cu&+Cr&+wy &+Wof the &+wgl&+ca&+Cc&+ci&+wer&n",
			    TRUE, ch, obj, vict, TO_NOTVICT);
			act("$p suddenly unleashes the &+wF&+cu&+Cr&+wy &+Wof the &+wgl&+ca&+Cc&+ci&+wer &+Wupon you!&n",
			    TRUE, ch, obj, vict, TO_VICT);
			...
			{
			case 0:
				spell_arieks_shattering_iceball(50, ch, 0, SPELL_TYPE_SPELL, vict,
								0);
				break;
			case 1:
			...
			break;
		case 3:
			act("&+W$p &+Win your hands which suddenly unleashes the &+lF&+Bu&+Cr&+By &+Wof the &+bs&+Bq&+Cua&+Bl&+bl&n",
			    TRUE, ch, obj, vict, TO_CHAR);
			act("$p &+Wsuddenly unleashes the &+lF&+Bu&+Cr&+By &+Wof the &+bs&+Bq&+Cua&+Bl&+bl&n",
			    TRUE, ch, obj, vict, TO_NOTVICT);
			act("$p &+Wsuddenly unleashes the &+lF&+Bu&+Cr&+By &+Wof the &+bs&+Bq&+Cua&+Bl&+bl &+Wupon you!&n",
			    TRUE, ch, obj, vict, TO_VICT);
			...
			break;
		case 4:
			act("&+WYour $q &+wsuddenly &+Wglows brightly!&n", TRUE, ch, obj, vict,
			    TO_CHAR);
			act("&+W$n's $q &+wsuddenly &+Wglows brightly!&n", TRUE, ch, obj, vict,
			    TO_ROOM);
			...
			{
				spell_greater_mending(GET_LEVEL(ch), ch, 0, SPELL_TYPE_SPELL, ch,
						      0);
			}
			else
			{
...
int deathseeker_mace(P_obj obj, P_char ch, int cmd, char *arg)
{
		...
		if (arg && (cmd == CMD_SAY))
		{
			if (isname(arg, "death"))
			{
				...
				if (obj->timer[0] + 600 <= curr_time)
				{
					...
					{
					case 0:
						spell_shadow_shield(50, ch, 0, SPELL_TYPE_SPELL, ch,
								    0);
						break;
					case 1:
						spell_biofeedback(50, ch, 0, SPELL_TYPE_SPELL, ch,
								  0);
						break;
					case 2:
						spell_stone_skin(50, ch, 0, SPELL_TYPE_SPELL, ch,
								 0);
						break;
					}
					...
					case 0:
						...
						break;
					case 1:
						spell_cannibalize(50, ch, 0, SPELL_TYPE_SPELL, ch,
								  0);
						break;
					case 2:
						spell_combat_mind(50, ch, 0, SPELL_TYPE_SPELL, ch,
								  0);
						break;
					}
			...
			if (!strcmp(Command, "seek") && *viewperson)
			{
				...
				if (target && target != ch && IS_PC(target) && !IS_TRUSTED(target))
				{
					...
					if (obj->timer[0] + 600 <= curr_time)
					{
						...
						{
						case 0:
							act("&nYou say 'I &+Rd&+re&+Rm&+ra&+Rn&+rd &nto know the whereabouts of this mortal's &+Rs&+ro&+Ru&+Rl&n..&n'",
							    TRUE, ch, obj, vict, TO_CHAR);
							act("&+LYou grip $q &+Lfirmly in your hand and call upon the &+Rp&+ro&+Rw&+re&+Rr&+rs &+Lof &+BD&+Lar&+Bk&+Lness to aid you in your &+Rs&+re&+Ra&+rr&+Rc&+rh&+L!",
							    TRUE, ch, obj, vict, TO_CHAR);
							act("$n says 'I &+Rd&+re&+Rm&+ra&+Rn&+rd &nto know the whereabouts of this mortal's &+Rs&+ro&+Ru&+Rl&n..&n'",
							    TRUE, ch, obj, vict, TO_ROOM);
						...
						case 1:
							act("&nYou say 'I &+Rd&+re&+Rm&+ra&+Rn&+rd &nto know the whereabouts of this mortal's &+Rs&+ro&+Ru&+Rl&n..&n'",
							    TRUE, ch, obj, vict, TO_CHAR);
							act("&+L$q &+rr&+Re&+rf&+Ru&+rs&+Re&+rs &+Lyour command and channels the &+Rp&+ro&+Rw&+re&+Rr&+rs &+Lof &+BD&+Lar&+Bk&+Lness upon you!",
							    TRUE, ch, obj, vict, TO_CHAR);
							act("$n says 'I &+Rd&+re&+Rm&+ra&+Rn&+rd &nto know the whereabouts of this mortal's &+Rs&+ro&+Ru&+Rl&n..&n'",
							    TRUE, ch, obj, vict, TO_ROOM);
	...
	if (cmd == CMD_MELEE_HIT && IS_ALIVE(vict) && !number(0, 32) && CheckMultiProcTiming(ch))
	{
		...
		{
		case 0:
			act("&+LYour $q &+Llets loose a &+rPo&+RWe&+rRF&+ruL &+rSc&+RRe&+raM&+L!&n",
			    TRUE, ch, obj, vict, TO_CHAR);
			act("&+W$n's $q &+Llets loose a &+rPo&+RWe&+rRF&+ruL &+rSc&+RRe&+raM&+L!&n",
			    TRUE, ch, obj, vict, TO_VICT);
			act("&+W$n's $q &+Llets loose a &+rPo&+RWe&+rRF&+ruL &+rSc&+RRe&+raM&+L!$n",
			    TRUE, ch, obj, vict, TO_NOTVICT);
		...
		case 1:
			...
			break;
		case 2:
			act("&+LYour $q &+Llets loose a &+rPo&+RWe&+rRF&+ruL &+mSc&+RRe&+ma&+rM&+L!&n",
			    TRUE, ch, obj, vict, TO_CHAR);
			act("&+W$n's $q &+Llets loose a &+rPo&+RWe&+rRF&+ruL &+mSc&+RRe&+ma&+rM&+L!&n",
			    TRUE, ch, obj, vict, TO_VICT);
			act("&+W$n's $q &+Llets loose a &+rPo&+RWe&+rRF&+ruL &+mSc&+RRe&+ma&+rM&+L!$n",
			    TRUE, ch, obj, vict, TO_NOTVICT);
	...
	if (cmd == CMD_MELEE_HIT && IS_ALIVE(vict) && !number(0, 34) && CheckMultiProcTiming(ch))
	{
		if (room == vict->in_room)
		{
			...
			case 1:
				...
				spell_psychic_crush(GET_LEVEL(ch), ch, 0, SPELL_TYPE_SPELL, vict,
						    0);
				break;
			default:
				break;
...
int dagger_ra(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_PERIODIC)
	{
		...
		if (!CHAR_IN_NO_MAGIC_ROOM(ch))
		{
			...
			if (obj->timer[0] + 60 <= curr_time)
			{
				...
				{
					act("$n&+L's $q &+rvi&+Rbra&+rtes &+Lsoftly.&n", TRUE, ch,
					    obj, NULL, TO_ROOM);
					act("&+LYour $q &+rvi&+Rbra&+rtes &+Lsoftly.&n", TRUE, ch,
					    obj, NULL, TO_CHAR);
	...
	if (arg && (cmd == CMD_SAY))
	{
		if (isname(arg, "ra"))
		{
			...
			if (obj->timer[1] + 750 <= curr_time)
			{
				...
				if (affected_by_spell(ch, SPELL_COLDSHIELD))
				{
					act("&+LThe &+rflames &+Lmelt away $n&+L's &+Bicy &+Lshield&+L.&n",
					    TRUE, ch, obj, vict, TO_ROOM);
					act("&+LThe &+rflames &+Lmelt away your &+Bicy &+Lshield&+L.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
	...
	if (cmd == CMD_MELEE_HIT && !number(0, 49) && CheckMultiProcTiming(ch))
	{
		...
		act("&+LYour $q &+Lchannels the &+Wb&+Yro&+Wke&+Yn &+Wr&+Ya&+Wy&+Ys &+Lof &+Wm&+Yor&+Wni&+Yng &+rS&+Ru&+rnr&+Ri&+rs&+Re &+Lat&n $N&+L.&n",
		    TRUE, ch, obj, vict, TO_CHAR);
		act("$n's $q &+Lchannels the &+Wb&+Yro&+Wke&+Yn &+Wr&+Ya&+Wy&+Ys &+Lof &+Wm&+Yor&+Wni&+Yng &+rS&+Ru&+rnr&+Ri&+rs&+Re &+Lat&n you&+L.&n",
		    TRUE, ch, obj, vict, TO_VICT);
		act("$n's $q &+Lchannels the &+Wb&+Yro&+Wke&+Yn &+Wr&+Ya&+Wy&+Ys &+Lof &+Wm&+Yor&+Wni&+Yng &+rS&+Ru&+rnr&+Ri&+rs&+Re &+Lat&n $N&+L.&n",
		    TRUE, ch, obj, vict, TO_NOTVICT);
...
int newbie_spellup_mob(P_char ch, P_char /*victim*/, int cmd, char * /*arg*/)
{
	...
	int ShamBeneSpells[] = { SPELL_SPIRIT_ARMOR,
				 SPELL_PANTHERSPEED,
				 SPELL_HAWKVISION,
				 SPELL_FIRE_WARD,
				 SPELL_COLD_WARD,
	...
	int DruidBeneSpells[] = { SPELL_BARKSKIN,     SPELL_FORTITUDE,
				  SPELL_AID,	      SPELL_PROTECT_FROM_ANIMAL,
				  SPELL_REGENERATION, 0 };

	int SorcBeneSpells[] = { SPELL_DETECT_MAGIC, SPELL_STRENGTH, SPELL_AGILITY, SPELL_LEVITATE,
				 0 };
	...
	for (P_char tch = world[ch->in_room].people; tch; tch = tch->next)
	{
		...
		if (!IS_PC(tch) || GET_LEVEL(tch) > 35 || !number(0, 2) ||
		...
		if (GET_HIT(tch) < (int)(GET_MAX_HIT(tch) * 0.75))
		{
			...
			{
				return MobCastSpell(ch, tch, 0, SPELL_GREATER_MENDING,
						    GET_LEVEL(ch));
			}
...
int welfare_well(int /*room*/, P_char ch, int cmd, char *arg)
{
	...
	if ((cmd == CMD_GET) || (cmd == CMD_TAKE))
	{
		...
		if (bits && (well->type == ITEM_STORAGE))
		{
			...
			if (obj)
				logit(LOG_DONATION, "&n%s: (%d) %s.", GET_NAME(ch),
				      obj_index[obj->R_num].virtual_number, obj->short_description);
		}
	}
	return FALSE;
}
...
int wh_janitor(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	...
	if (ch->in_room == well_room)
	{
		...
		if (!found_well)
		{
			debug("wh_janitor(): can't find donation well [%d] in its room [%d] - loading a new one.",
			      WELL, WELL_ROOM);
	...
	if (!loaded)
	{
		...
		if (1 /*!number(0, 5)*/)
		{
			...
			for (a = 0; a < NUM_EXITS; a++)
				...
				{
					for (o = world[EXIT(ch, a)->to_room].contents; o;
					     o = o->next_content)
					{
						if (CAN_WEAR(o, ITEM_TAKE) && CAN_CARRY_OBJ(ch, o))
						{
							act("$n notices some garbage nearby.",
...
int wh_guard(P_char ch, P_char victim, int cmd, char * /*arg*/)
{
	...
	if (!victim && IS_FIGHTING(ch) && EVIL_RACE(GET_OPPONENT(ch)))
		return shout_and_hunt(ch, 20, "&+YCome to my aid! We are being invaded!&n", NULL,
				      helpers, 0, 0);
	return FALSE;
}
...
int demon_slayer(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (IS_UNDEAD(ch))
	{
		...
		{
			act("An aura of &+Ldeath &nseems to drain $n's strength.", TRUE, ch, obj,
			    vict, TO_ROOM);
			act("An aura of &+Ldeath &ndrains you of your strength.", TRUE, ch, obj,
			    vict, TO_CHAR);
	...
	if (IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_SAY))
		{
			if (isname(arg, "bel"))
			{
				...
				{
					act("&+WYou scream '&+rBel&+L! I call upon you to &+rS&+Rl&+rA&+Ry my foe!&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+WYou thrust $q &+Winto the ground!&n", TRUE, ch, obj,
					    vict, TO_CHAR);
					act("&+LDarkness quickly swallows up all available light and the room suddenly turns to silence.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+LThe ghostly image of &+rBel &+Lsuddenly appears and reaches it's hand into the ground.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+YBoom! &+LA giant pillar of &+Cenergy &+Lerupts from beneath your feet, engulfing $N &+Lin a &+rc&+Ro&+Yl&+Bo&+Cr&+Yf&+Ru&+rl &+Lhaze!&n",
					    TRUE, ch, obj, vict, TO_CHAR);

					act("$n screams '&+rBel&+L! I call upon you to &+rS&+Rl&+rA&+Ry my foe!'&n",
					    TRUE, ch, obj, vict, TO_NOTVICT);
					act("$n &+Lthrusts $q &+Winto the ground!&n", TRUE, ch, obj,
					    vict, TO_NOTVICT);
					act("&+LDarkness quickly swallows up all available light and the room suddenly turns to silence.&n",
					    TRUE, ch, obj, vict, TO_NOTVICT);
					act("&+LThe ghostly image of &+rBel &+Lsuddenly appears and reaches it's hand into the ground.&n",
					...
					act("&+LThe ghostly image of &+rBel &+Lsuddenly appears and reaches it's hand into the ground.&n",
					    TRUE, ch, obj, vict, TO_VICT);
					act("&+YBoom! &+LA giant pillar of &+Cenergy &+Lerupts from the ground, engulfing &+wyou &+Lin a &+rc&+Ro&+Yl&+Bo&+Cr&+Yf&+Ru&+rl &+Lhaze!&n",
					    TRUE, ch, obj, vict, TO_VICT);

					switch (number(1, 6))
					{
					case 1:
						spell_pword_blind(60, ch, 0, SPELL_TYPE_SPELL, vict,
								  0);
						break;
					case 2:
						spell_pword_stun(56, ch, 0, SPELL_TYPE_SPELL, vict,
								 0);
						break;
					case 3:
						spell_pword_stun(56, ch, 0, SPELL_TYPE_SPELL, vict,
					...
					case 4:
						spell_bigbys_clenched_fist(
							45, ch, 0, SPELL_TYPE_SPELL, vict, 0);
						break;
					case 5:
						spell_firebrand(50, ch, 0, SPELL_TYPE_SPELL, vict,
								0);
						break;
					case 6:
						spell_chaotic_ripple(56, ch, 0, SPELL_TYPE_SPELL,
								     vict, 0);
						break;
					}
					act("&+LThe ghostly image of &+RBel &+Lfades out of existence and the world slowly returns to normal.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+LThe ghostly image of &+RBel &+Lfades out of existence and the world slowly returns to normal.&n",
					    TRUE, ch, obj, vict, TO_NOTVICT);
					act("&+LThe ghostly image of &+RBel &+Lfades out of existence and the world slowly returns to normal.&n",
	...
	if (arg && (cmd == CMD_SAY))
	{
		if (isname(arg, "euronymous"))
		{
			...
			{
				act("&+WYou say '&+rEuronymous&+L! Bring me &+mP&+Mo&+mW&+Me&+mr &+Lor bring me &+rD&+Re&+rA&+Rt&+rH&+L!'&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("&+WYou thrust $q &+Winto the ground!&n", TRUE, ch, obj, vict,
				    TO_CHAR);
				act("&+LA dark &+bportal &+Lappears before you, and &+rEuronymous, the Prince of Demons &+Lsteps out.&n",
				    TRUE, ch, obj, vict, TO_CHAR);

				act("$n says '&+rEuronymous&+L! Bring me &+mP&+Mo&+mW&+Me&+mr &+Lor bring me &+rD&+Re&+rA&+Rt&+rH&+L!&n'&n",
				    TRUE, ch, obj, vict, TO_ROOM);
				act("$n &+Lthrusts $q &+Winto the ground!&n", TRUE, ch, obj, vict,
				    TO_ROOM);
				act("&+LA dark &+bportal &+Lappears before you, and &+rEuronymous, the Prince of Demons &+Lsteps out.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
				...
				if (rand <= 31)
				{
					act("&+rThe Prince of Demons &+Llooks at you and says, &+W'&+CProt&+Bec&+Ction&+W'&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+rThe Prince of Demons &+Llooks at $n and says, &+W'&+CProt&+Bec&+Ction&+W'&n",
					    TRUE, ch, obj, vict, TO_ROOM);
				...
				{
					act("&+rThe Prince of Demons &+Llooks at you and says, &+W'&+mSp&+Me&+med&+W'&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+rThe Prince of Demons &+Llooks at $n and says, &+W'&+mSp&+Me&+med&+W'&n",
					    TRUE, ch, obj, vict, TO_ROOM);

					spell_enhanced_agility(56, ch, 0, SPELL_TYPE_SPELL, ch, 0);
					spell_enhanced_dexterity(56, ch, 0, SPELL_TYPE_SPELL, ch,
								 0);
				...
				{
					act("&+rThe Prince of Demons &+Llooks at you and says, &+W'&+bPo&+Bw&+ber&+W'&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+rThe Prince of Demons &+Llooks at $n and says, &+W'&+bPo&+Bw&+ber&+W'&n",
					    TRUE, ch, obj, vict, TO_ROOM);
				...
				{
					act("&+rThe Prince of Demons &+grins at you and says, &+W'&+rF&+Ru&+rr&+Ry&+W'&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+rThe Prince of Demons &+grins at $n and says, &+W'&+rF&+Ru&+rr&+Ry&+W'&n",
					    TRUE, ch, obj, vict, TO_ROOM);

					spell_hellfire(56, ch, 0, SPELL_TYPE_SPELL, ch, 0);

					act("&+ROuch&+L! The demonic &+rflames &+RSEAR &+Lyour &+rfl&+Re&+rsh&+L!&n",
					    TRUE, ch, obj, vict, TO_CHAR);
				...
				{
					act("&+rThe Prince of Demons &+Llooks at you and laughs, &+W'&+rD&+Re&+ra&+Rt&+rh&+W? &+mU&+Mn&+md&+Me&+ma&+Mt&+mh&+L!'&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+rEuronymous &+Lleaps forward and bites you in the neck, sinking his fangs deep into your flesh!",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+LAs the &+Rblood &+Lflows out of your body, you begin to see the world in a different light.",
					    TRUE, ch, obj, vict, TO_CHAR);

					act("&+rThe Prince of Demons &+Llooks at $n and laughs, &+W'&+rD&+Re&+ra&+Rt&+rh&+W? &+mU&+Mn&+md&+Me&+ma&+Mt&+mh&+L!'&n",
					    TRUE, ch, obj, vict, TO_ROOM);
					act("&+rEuronymous suddenly leaps forward and sinks his fangs into the neck of $n, sending blood flying.&n",
					    TRUE, ch, obj, vict, TO_ROOM);
					act("$n &+Lbegins to look rather &+wpale &+Las the last drops of &+Rblood &+Ldrip to the floor.&n",
					    TRUE, ch, obj, vict, TO_ROOM);

					spell_vampire(56, ch, 0, SPELL_TYPE_SPELL, ch, 0);
					act("&nAs life departs from your body, you feel strained.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					GET_HIT(ch) = 50;
				}

				act("&+rEuronymous &+Lgrins at you, steps into the &+bportal&+L, and vanishes.&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("&+rEuronymous &+Lgrins at $n, steps into the &+bportal&+L, and vanishes.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
		...
		else if (isname(arg, "juiblex") || isname(arg, "jubilex"))
		{
			...
			if (obj->timer[0] + 450 <= curr_time && obj->timer[1] + 600 <= curr_time)
			{
				act("&+WYou say '&+rJuiblex&+L! Grant me strength!&+W'&n", TRUE, ch,
				    obj, vict, TO_CHAR);
				act("&+WYou thrust $q &+Winto the ground!&n", TRUE, ch, obj, vict,
				    TO_CHAR);
				act("&+LA thin line of &+ggreen &+Glight &+Lcuts into the &+Cair&+L, revealing a &+gportal &+Lto another &+Gdimension&+L.",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("&+LSuddenly, &+rJuiblex &+Lleaps into this reality and conjures up a &+rgreen &+Gmist&+L.&n",
				    TRUE, ch, obj, vict, TO_CHAR);

				act("$n says '&+rJuiblex&+L! Grant me strength!&+W'&n", TRUE, ch,
				    obj, vict, TO_ROOM);
				act("$n &+Lthrusts $q &+Winto the ground!&n", TRUE, ch, obj, vict,
				    TO_ROOM);
				act("&+LA thin line of &+ggreen &+Glight &+Lcuts into the &+Cair&+L, revealing a &+gportal &+Lto another &+Gdimension&+L.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
				act("&+LSuddenly, &+rJuiblex &+Lleaps into this reality and conjures up a &+rgreen &+Gmist&+L.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
				...
				if (rand >= 76)
				{
					act("You are bathed in an extremely powerful healing aura.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("$n is bathed in an extremely powerful healing aura.&n",
					    TRUE, ch, obj, vict, TO_ROOM);
					GET_HIT(ch) += 400;
				...
				{
					act("You are bathed in a strong healing aura.&n", TRUE, ch,
					    obj, vict, TO_CHAR);
					act("$n is bathed in a strong healing aura.&n", TRUE, ch,
					    obj, vict, TO_ROOM);
					GET_HIT(ch) += 300;
					spell_invigorate(50, ch, 0, SPELL_TYPE_SPELL, ch, 0);
				}
				...
				{
					act("You are bathed in a healing aura.&n", TRUE, ch, obj,
					    vict, TO_CHAR);
					act("$n is bathed in a healing aura.&n", TRUE, ch, obj,
					    vict, TO_ROOM);
					GET_HIT(ch) += 200;
					spell_invigorate(50, ch, 0, SPELL_TYPE_SPELL, ch, 0);
				}
				...
				{
					act("You are bathed in a light healing aura.&n", TRUE, ch,
					    obj, vict, TO_CHAR);
					act("$n is bathed in a light healing aura.&n", TRUE, ch,
					    obj, vict, TO_ROOM);
					GET_HIT(ch) += 100;
					spell_invigorate(50, ch, 0, SPELL_TYPE_SPELL, ch, 0);
				}

				act("&+LAs the &+ggreen &+Gmist &+Lsubsides, &+rJuiblex &+Lflashes you a wicked grin, steps into the portal, and vanishes.&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("&+LAs the &+ggreen &+Gmist &+Lsubsides, &+rJuiblex &+Lflashes $n a wicked grin, steps into the portal, and vanishes.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
	...
	if (cmd == CMD_MELEE_HIT && IS_ALIVE(vict) && !number(0, 32) && CheckMultiProcTiming(ch))
	{
		if (GET_RACE(vict) == RACE_DEMON)
		{
			...
			{
			case 0:
				act("&+LYour $q &+rfl&+Rar&+res &+Las it channels the powers of &+rBel &+Lupon your foe!&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("&+L$n's $q &+rfl&+Rar&+res &+Las it channels the powers of &+rBel &+Lupon you!&n",
				    TRUE, ch, obj, vict, TO_VICT);
				act("&+L$n's $q &+rfl&+Rar&+res &+Las it channels the powers of &+rBel &+Lupon $N!&n",
				    TRUE, ch, obj, vict, TO_NOTVICT);
				...
				break;
			case 1:
				act("&+LYour $q &+bbl&+Cu&+brs &+Las it channels the powers of &+rEuronymous &+Lupon your foe!&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("&+L$n's $q &+bbl&+Cu&+brs &+Las it channels the powers of &+rEuronymous &+Lupon you!&n",
				    TRUE, ch, obj, vict, TO_VICT);
				act("&+L$n's $q &+bbl&+Cu&+brs &+Las it channels the powers of &+rEuronymous &+Lupon $N!&n",
				    TRUE, ch, obj, vict, TO_NOTVICT);
				...
				break;
			case 2:
				act("&+LYour $q &+ggl&+Go&+gws &+Las it channels the powers of &+rJuiblex &+Lupon your foe!&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("&+L$n's $q &+ggl&+Go&+gws &+Las it channels the powers of &+rJuiblex &+Lupon you!&n",
				    TRUE, ch, obj, vict, TO_VICT);
				act("&+L$n's $q &+ggl&+Go&+gws &+Las it channels the powers of &+rJuiblex &+Lupon $N!&n",
				    TRUE, ch, obj, vict, TO_NOTVICT);
		...
		else
		{
			...
			case 1:
				...
				break;
			case 2:
				act("&+LYour $q &+ggl&+Go&+gws &+Las it channels the powers of &+rJubliex &+Lupon your foe!&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("&+L$n's $q &+ggl&+Go&+gws &+Las it channels the powers of &+rJuiblex &+Lupon you!&n",
				    TRUE, ch, obj, vict, TO_VICT);
				act("&+L$n's $q &+ggl&+Go&+gws &+Las it channels the powers of &+rJuiblex &+Lupon $N!&n",
...
int helmet_vampires(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (!IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_WORSHIP))
		{
			if (isname(arg, "dead"))
			{
				...
				{
					act("&+WYou kneel down to the &+Lground &+Wand utter a prayer for the &+rd&+Ra&+rm&+Rn&+re&+Rd&+W.&n",
					    TRUE, ch, obj, NULL, TO_CHAR);
					act("&+WYour $q &+Wbegins to &+rg&+Rlo&+rw &+Wand a strange &+bm&+Bis&+bt &+Wbegins filling the &+Cair&+W.&n",
					    TRUE, ch, obj, NULL, TO_CHAR);
					act("&+WAs the &+bm&+Bis&+bt &+Wthickens, your nostrils are filled with the smell of &+gde&+Gca&+gyi&+Gng &+rf&+Rl&+re&+Rs&+rh&+W.&n",
					    TRUE, ch, obj, NULL, TO_CHAR);
					act("&+W... the &+yground &+Wseems to be coming alive.&n",
					    TRUE, ch, obj, NULL, TO_CHAR);

					act("$n &+Wkneels down to the &+Lground &+Wand utters a prayer for the &+rd&+Ra&+rm&+Rn&+re&+Rd&+W.&n",
					    TRUE, ch, obj, NULL, TO_ROOM);
					act("$n's $q &+Wbegins &+rg&+Rlo&+rwi&+Rng &+Wand a strange &+bm&+Bis&+bt &+Wbegins filling the &+Cair&+W.&n",
					    TRUE, ch, obj, NULL, TO_ROOM);
					act("&+WAs the &+bm&+Bis&+bt &+Wthickens, your nostrils are filled with the smell of &+gde&+Gca&+gyi&+Gng &+rf&+Rl&+re&+Rs&+rh&+W.&n",
					    TRUE, ch, obj, NULL, TO_ROOM);
					act("&+W... the &+yground &+Wseems to be coming alive.&n",
					...
					*/
					...
					act("&+WAs you rise to your feet, the &+gf&+Gou&+gl &+bm&+Bis&+bt &+Wlooks &+Ldrained &+Wand begins to fade away.&n",
					    FALSE, ch, 0, NULL, TO_CHAR);
					act("&+WAs &n rises to $s feet, the &+gf&+Gou&+gl &+bm&+Bis&+bt &+Wlooks &+Ldrained &+Wand begins to fade away.&n",
	...
	if (IS_FIGHTING(ch) && !IS_IMMOBILE(ch) && IS_ALIVE(GET_OPPONENT(ch)) &&
	    ch->in_room == GET_OPPONENT(ch)->in_room)
	{
		if (arg && (cmd == CMD_PRAY))
		{
			if (isname(arg, "koztk"))
			{
				...
				{
					act("You utter a prayer for '&+rKoztk&n'&n", TRUE, ch, obj,
					    vict, TO_CHAR);
					act("$q &+rhums &+Land turns your eyes a deep &+rred&+L.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+L...an uncontrollable &+ranger &+Lstarts building within you.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+L...&nyou &+Lgasp as your skin turns &+Wpale &+Land a vicious set of &+rfangs &+Lburst out of your upper jaw.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+L...&nyou &+Lleap forward and sink your fangs into $N's neck, sending &+rblood &+Leverywhere.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+L...&nyou &+Lfeel &+Renergy &+Ldraining from $N &+Land flowing into you!&n",
					    TRUE, ch, obj, vict, TO_CHAR);

					act("$n utters a prayer for '&+rKoztk&n'&n", TRUE, ch, obj,
					    vict, TO_NOTVICT);
					act("$n's $q &+rhums &+Land turns $s eyes a deep &+rred&+L.&n",
					    TRUE, ch, obj, vict, TO_NOTVICT);
					act("&+L...&n$n &+Lgasps as $s skin turns pale and a pair of fangs burst out of $s mouth.&n",
					    TRUE, ch, obj, vict, TO_NOTVICT);
					act("&+L...&n$n &+Lleaps forward and sinks $s fangs into $N's neck, sending &+rblood &+Leveryone.&n",
					    TRUE, ch, obj, vict, TO_NOTVICT);
					act("&+L...&n$n &+Lseems to drain $N's &+rstrength&+L.&n",
					    TRUE, ch, obj, vict, TO_NOTVICT);
					...
					act("&+L...&n$n &+Lgasps as $s skin turns pale and a pair of fangs burst out of $s mouth.&n",
					    TRUE, ch, obj, vict, TO_VICT);
					act("&+L...&n$n &+Lleaps forward and sinks $s fangs into your neck, sending &+rblood &+Leveryone.&n",
					    TRUE, ch, obj, vict, TO_VICT);
					act("&+L...&nYou &+Lfeel your &+rstrength &+Land &+Blifeforce &+Lflow into $n.&n",
					    TRUE, ch, obj, vict, TO_VICT);
					...
					case 0:
						...
						vamp(ch, dam / 2,
						     (int)(GET_MAX_HIT(ch) * VAMPPERCENT(ch)));
						break;
					case 1:
					...
					act("You grin with satisfaction.&n", TRUE, ch, obj, vict,
					    TO_CHAR);

					act("$N is stunned!&n", TRUE, ch, obj, vict, TO_NOTVICT);
					act("$n grins with satisfaction.&n", TRUE, ch, obj, vict,
					    TO_NOTVICT);
...
int buckler_saints(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_PERIODIC)
	{
		...
		if (!CHAR_IN_NO_MAGIC_ROOM(ch))
		{
			...
			if (obj->timer[1] + 180 <= curr_time2)
			{
				...
				if (GET_HIT(ch) < GET_MAX_HIT(ch))
				{
					act("&+LYour $q &+Mhums &nsoftly and fills your soul with a warm feeling.",
					    TRUE, ch, obj, 0, TO_CHAR);
					act("$n&+L's $q &+Mhums &nsoftly.", TRUE, ch, obj, 0,
					    TO_ROOM);
	...
	if (arg && (cmd == CMD_SAY))
	{
		if (isname(arg, "gauce"))
		{
			...
			{
				act("&+WYou utter a prayer for &+YSaint &+WGauce.&n", TRUE, ch, obj,
				    vict, TO_CHAR);
				act("&+WYour $q hums loudly, sending waves of &+csoothing &+Wsensations through the &+Cair&+W.&n",
				    TRUE, ch, obj, vict, TO_CHAR);

				act("$n &+Wutters a prayer for &+YSaint &+WGauce.&n", TRUE, ch, obj,
				    vict, TO_ROOM);
				act("$n's $q hums loudly, sending waves of &+csoothing &+Wsensations through the &+Cair&+W.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
		...
		else if (isname(arg, "macavor"))
		{
			...
			if (obj->timer[0] + 450 <= curr_time)
			{
				act("&+WYou utter a prayer for &+YSaint &+WMacavor.&n", TRUE, ch,
				    obj, vict, TO_CHAR);
				act("&+WYour $q hums loudly, sending waves of &+csoothing &+Wsensations through the &+Cair&+W.&n",
				    TRUE, ch, obj, vict, TO_CHAR);

				act("$n &+Wutters a prayer for &+YSaint &+WMacavor.&n", TRUE, ch,
				    obj, vict, TO_ROOM);
				act("$n's $q hums loudly, sending waves of &+csoothing &+Wsensations through the &+Cair&+W.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
		...
		else if (isname(arg, "verdonnaly"))
		{
			...
			if (obj->timer[0] + 450 <= curr_time)
			{
				act("&+WYou utter a prayer for &+YSaint &+WVerdonnaly.&n", TRUE, ch,
				    obj, vict, TO_CHAR);
				act("&+WYour $q hums loudly, sending waves of &+csoothing &+Wsensations through the &+Cair&+W.&n",
				    TRUE, ch, obj, vict, TO_CHAR);

				act("$n &+Wutters a prayer for &+YSaint &+WVerdonnaly.&n", TRUE, ch,
				    obj, vict, TO_ROOM);
				act("$n's $q hums loudly, sending waves of &+csoothing &+Wsensations through the &+Cair&+W.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
	...
	if (IS_FIGHTING(ch))
	{
		if (cmd == CMD_GOTHIT && !number(0, 24))
		{
			...
			act("$q &nsings with &+Yholy &npower as $N attempts to hit you.&n", TRUE,
			    ch, obj, vict, TO_CHAR | ACT_NOTTERSE);
			act("$q &nsings with &+Yholy &npower as you attempt to hit $n.&n", TRUE, ch,
			    obj, vict, TO_VICT | ACT_NOTTERSE);
			act("$q &nsings with &+Yholy &npower as $N attempts to hit $n.&n", TRUE, ch,
			    obj, vict, TO_NOTVICT | ACT_NOTTERSE);
...
int rapier_penetration(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (IS_FIGHTING(ch))
	{
...

		{
			...
			act("$n&+L's $q &+mpulsates &+Lwith energy!&n", TRUE, ch, obj, vict,
			    TO_NOTVICT);
			...
			if (number(1, 10) > 5)
			{
				if (affected_by_spell(vict, SPELL_STONE_SKIN))
				{
					act("$N's &+Lstone skin &ncrumbles to the ground!&n", TRUE,
					    ch, obj, vict, TO_CHAR);
			...
			if (number(1, 10) > 5)
			{
				if (affected_by_spell(vict, SPELL_GLOBE))
				{
					act("$N's &+Rglobe of invulnerability &nshatters to pieces!&n",
					    TRUE, ch, obj, vict, TO_CHAR);
			...
			if (number(1, 10) > 5)
			{
				if (affected_by_spell(vict, SPELL_SHADOW_SHIELD))
				{
					act("&nWaves of energy destroy $N's &+Lshadow shield&n!",
					    TRUE, ch, obj, vict, TO_CHAR);
			...
			if (number(1, 10) > 5)
			{
				if (affected_by_spell(vict, SPELL_BIOFEEDBACK))
				{
					act("The thick &+Ggreen &+gmist &nsurrounding $N turns pale and fades away.",
					    TRUE, ch, obj, vict, TO_CHAR);
			...
			if (number(1, 10) > 5)
			{
				if (affected_by_spell(vict, SPELL_FIRESHIELD))
				{
					act("The &+rflames &nsurrounding $N flicker out of existence.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
			...
			if (number(1, 10) > 5)
			{
				if (affected_by_spell(vict, SPELL_COLDSHIELD))
				{
					act("The &+Bice &nsurrounding $N melt away.&n", TRUE, ch,
					    obj, vict, TO_CHAR);
...
int gauntlets_legend(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (IS_FIGHTING(ch) && (cmd == CMD_PERIODIC) && !number(0, 9))
	{
		...
		{
			act("$q &nsend a &+csoothing &nsensation up your arm.&n.", TRUE, ch, obj,
			    vict, TO_CHAR);
		...
		if (GET_HIT(vict) < (GET_MAX_HIT(vict) / 4) &&
		    GET_HIT(vict) > (GET_MAX_HIT(vict) / 20))
		{
			act("A &+rpulse &nof &+Menergy &nshoots from&n $q&+M, striking&n $N &+Min the chest&n.",
			    TRUE, ch, obj, vict, TO_CHAR);
			act("A &+rpulse &nof &+Menergy &nshoots from&n $q&+M, striking&n $N &+Min the chest&n.",
			    TRUE, ch, obj, vict, TO_NOTVICT);
			act("A &+rpulse &nof &+Menergy &nshoots from&n $q, &+Mstriking you in the chest&n.",
			    TRUE, ch, obj, vict, TO_VICT);

			GET_HIT(vict) -= (GET_MAX_HIT(vict) / 25);
		}
		else if (GET_HIT(vict) < (GET_MAX_HIT(vict) / 2) &&
			 GET_HIT(vict) > (GET_MAX_HIT(vict) / 10))
		{
			act("A &+rpulse &nof &+Menergy &nshoots from $q, striking $N in the chest&n.",
			    TRUE, ch, obj, vict, TO_CHAR);
			act("A &+rpulse &nof &+Menergy &nshoots from $q, striking $N in the chest&n.",
			    TRUE, ch, obj, vict, TO_NOTVICT);
			act("A &+rpulse &nof &+Menergy &nshoots from $q, striking you in the chest&n.",
			    TRUE, ch, obj, vict, TO_VICT);
	...
	if (!IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_CLAP))
		{
			if (isname(arg, "hands"))
			{
				...
				if (obj->timer[0] + 600 <= curr_time)
				{
					act("&nAs you clap your hands, $q begin to tremble with power.&n",
					    TRUE, ch, obj, vict, TO_CHAR);
					act("&+LDarkness &ncreeps up and begins to surround you.&n",
					    TRUE, ch, obj, vict, TO_CHAR);

					act("&nAs $n claps $s hands, $q begin to tremble with power.&n",
					    TRUE, ch, obj, vict, TO_ROOM);
					act("&+LDarkness &ncreeps up and begins to surround you.&n",
					    TRUE, ch, obj, vict, TO_ROOM);
...
int attribute_scroll(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (!IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_RECITE))
		{
			...
			{
				act("&+WYou begin reciting the words on $q, which quickly transforms and reveals its true identity.&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("$n begins reciting the words on $q, which quickly transforms and reveals its true identity.&n",
				    TRUE, ch, obj, vict, TO_ROOM);
...
int earring_powers(P_obj obj, P_char ch, int cmd, char * /*arg*/)
{
	...
	if (cmd == CMD_ENCRUST)
	{
		...
		if (IS_ALIVE(ch))
		{
			act("&nYour $q prevents encrust magic from working.&n", TRUE, ch, obj, NULL,
			    TO_CHAR);
		}
		return TRUE;
	}
...
int gladius_backstabber(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (IS_FIGHTING(ch))
	{
		...
		if (cmd == CMD_MELEE_HIT && !number(0, 24) && CheckMultiProcTiming(ch))
		{
			...
			act("&nYou quickly vanish and reappear behind $N!", TRUE, ch, obj, vict,
			    TO_CHAR);
			act("$n's $q &nbegins to &+mhum&n.", TRUE, ch, obj, vict, TO_NOTVICT);
			act("$n quickly vanishes and reappears behind $N!", TRUE, ch, obj, vict,
			    TO_NOTVICT);
			act("$n's $q &nbegins to &+mhum&n.", TRUE, ch, obj, vict, TO_VICT);
			act("$n quickly vanishes and reappears behind you!", TRUE, ch, obj, vict,
			    TO_VICT);
...
int damnation_staff(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_GOTHIT && !number(0, 19))
	{
		...
		act("&+LYour $q &+mflares &+rbril&+Rli&+rantly &+Las it intercepts &n$N's &+Lattack and &+rsav&+Rag&+rely &+Lstrikes back!&n",
		    TRUE, ch, obj, vict, TO_CHAR);
		act("$n's $q &+mflares &+rbril&+Rli&+rantly &+Las it intercepts &nyour &+Lattack and &+rsav&+Rag&+rely &+Lstrikes back!&n",
		    TRUE, ch, obj, vict, TO_VICT);
		act("$n's $q &+mflares &+rbril&+Rli&+rantly &+Las it intercepts &n$N's &+Lattack and &+rsav&+Rag&+rely &+Lstrikes back!&n",
		    TRUE, ch, obj, vict, TO_NOTVICT);
...
int elemental_wand(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (!IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_RUB))
		{
			if (isname(arg, "wand"))
			{
				act("&+WYou rub $q and it quickly begins to transform in your hands.&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("$n rubs $q and it begins to transform in $n hands.&n", TRUE,
				    ch, obj, vict, TO_ROOM);
...
int nuke_damnation(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_GOTNUKED && !number(0, 19))
	{
		...
		act("&+LYour $q &+mflares &+rbril&+Rli&+rantly &+Las &n$N's &+Lspell is tranformed and deflected!&n",
		    TRUE, ch, obj, vict, TO_CHAR);
		act("$n's $q &+mflares &+rbril&+Rli&+rantly &+Las your spell is tranformed and deflected back at you!&n",
		    TRUE, ch, obj, vict, TO_VICT);
		act("$n's $q &+mflares &+rbril&+Rli&+rantly &+Las &n$N's &+Lspell is tranformed and deflected!&n",
		    TRUE, ch, obj, vict, TO_NOTVICT);
		...
		if (isname(arg, "frost"))
		{
			...
			if (obj->timer[0] + 600 <= curr_time)
			{
				...
				if (can_conjure_greater_elem(ch, GET_LEVEL(ch)))
				{
					if (elesize > 94)
					{
						act("You whisper '&+bFr&+Co&+bst&n' to your $q...",
						    FALSE, ch, obj, 0, TO_CHAR);
						act("&+cEnormous crystals of &+Cice &+cstart forming and condensing into the form of an elemental.&n",
						    FALSE, ch, obj, obj, TO_CHAR);

						act("$n whispers '&+bFr&+Co&+bst&n' to $s $q...&N",
						    TRUE, ch, obj, NULL, TO_ROOM);
						act("&+cEnormous crystals of &+Cice &+cstart forming and condensing into the form of an elemental.",
					...
					else if (elesize > 89)
					{
						...
						act("$n whispers '&+bFr&+Co&+bst&n' to $q...&N",
						    TRUE, ch, obj, NULL, TO_ROOM);
						act("&+cEnormous crystals of &+Cice &+cstart forming and condensing into the form of an elemental.",
						    FALSE, ch, obj, obj, TO_ROOM);
					...
					else if (elesize > 70)
					{
						...
						act("$n whispers '&+bFr&+Co&+bst&n' to $q...&N",
						    TRUE, ch, obj, NULL, TO_ROOM);
						act("&+cLarge crystals of &+Cice &+cstart forming and condensing into the form of an elemental.",
						    FALSE, ch, obj, obj, TO_ROOM);
				...
				if (elesize <= 70)
				{
					act("You whisper '&+bFr&+Co&+bst&n' to your $q...", FALSE,
					    ch, obj, 0, TO_CHAR);
					act("&+cSmall crystals of &+Cice &+cstart forming and condensing into the form of an elemental.&n",
					    FALSE, ch, obj, obj, TO_CHAR);

					act("$n whispers '&+bFr&+Co&+bst&n' to $q...&N", TRUE, ch,
					    obj, NULL, TO_ROOM);
					act("&+cSmall crystals of &+Cice &+cstart forming and condensing into the form of an elemental.",
					    FALSE, ch, obj, obj, TO_ROOM);
				...
				{
					act("&=LBTHERE IS NO ICE ELEMENTAL, TELL A GOD!!&N", FALSE,
					    ch, obj, obj, TO_CHAR);
					debug("collar_frost: missing mental (%s) or room (%d).",
					      icemental ? J_NAME(icemental) : "NULL", ch->in_room);
					return FALSE;
				}
				if (IS_ROOM(ch->in_room, ROOM_SINGLE_FILE))
				{
					send_to_char(
						"&+cThe elemental failed to arrive. This area is too narrow.\r\n",
						ch);
				...
				sum = dice(GET_LEVEL(icemental) * 4, 8) +
				      (GET_LEVEL(icemental) * 3);
				...
				GET_MAX_HIT(icemental) = GET_HIT(icemental) =
					icemental->points.base_hit = sum;
				icemental->points.base_hitroll = icemental->points.hitroll =
					GET_LEVEL(icemental) / 3;
				icemental->points.base_damroll = icemental->points.damroll =
					GET_LEVEL(icemental) / 3;
			...
			{
				act("&+cOnly a slight &+Cchill &+cfills the air.&n", FALSE, ch, obj,
				    obj, TO_CHAR);
				act("&+cOnly a slight &+Cchill &+cfills the air.&n", FALSE, ch, obj,
				    obj, TO_ROOM);
			...
			if (obj->timer[0] + 600 <= curr_time)
			{
				...
				if (elesize <= 89)
				{
					...
					act("&+rSmall pilars of &+Rflames &+rstart forming and condensing into the form of an elemental.&n",
					    FALSE, ch, obj, obj, TO_CHAR);
					act("$n whispers '&+rFl&+Ram&+res&n' to $s $q...&N", TRUE,
					    ch, obj, NULL, TO_ROOM);
					act("&+cSmall crystals of &+Cice &+cstart forming and condensing into the form of an elemental.&n",
					    FALSE, ch, obj, obj, TO_ROOM);
				...
				if (!firemental || ch->in_room == NOWHERE)
				{
					act("&+LTHERE IS NO FIRE ELEMENTAL, TELL A GOD!!&N", FALSE,
					    ch, obj, obj, TO_CHAR);
					debug("collar_frost: missing mental (%s) or room (%d).",
					      firemental ? J_NAME(firemental) : "NULL",
					      ch->in_room);
					return FALSE;
				}
				...
				{
					send_to_char(
						"&+RThe elemental failed to arrive. This area is too narrow.\r\n",
						ch);
					return FALSE;
				}
				...
				sum = dice(GET_LEVEL(firemental) * 4, 8) +
				      (GET_LEVEL(firemental) * 3);
				...
				GET_MAX_HIT(firemental) = GET_HIT(firemental) =
					firemental->points.base_hit = sum;
				firemental->points.base_hitroll = firemental->points.hitroll =
					GET_LEVEL(firemental) / 3;
				firemental->points.base_damroll = firemental->points.damroll =
					GET_LEVEL(firemental) / 3;
...
int tiamat_human_to_rareloads(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	if (cmd == CMD_DEATH)
	{
		...
		act("&nAs &+La pitiful slave &nfalls to the ground, his corpse quickly disolves, leaving only a few valuable artifacts...&n",
		    FALSE, ch, obj, obj, TO_ROOM);
...
int dragonnia_heart(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	...
	if (cmd == CMD_DEATH)
	{
		...
		{
			logit(LOG_EXIT, "dragonnia_heart: could not load heart vnum %d",
			      VOBJ_WH_DRAGONHEART_DRAGONNIA);
			return FALSE;
		}

		obj_to_room(obj, ch->in_room);

		act("&+WWith her very last breath, &+GDragonnia &+Wcloses her eyes.&n", FALSE, ch,
		    obj, obj, TO_ROOM);
		act("&+GDragonnia&+W's body begins to shimmer brilliantly, her flesh becoming more and more translucent!&n",
		    FALSE, ch, obj, obj, TO_ROOM);
		act("&+WAs the light subsides, only a few pieces of her once great body remain.&n",
		    FALSE, ch, obj, obj, TO_ROOM);
		...
		logit(LOG_OBJ,
		      "Dragonnia death: Heart decays in %d obj ticks = %d sec = %d:%02d:%02d.",
		      obj->value[0], obj->value[1], obj->value[2], obj->value[3], obj->value[4]);
...
int dragon_heart_decay(P_obj obj, P_char ch, int cmd, char *args)
{
	...
	if (cmd == CMD_EXAMINE)
	{
		...
		if (obj == decayed_heart)
		{
			...
			case 2:
				send_to_char(
					"&+WYou still have over &+B2&+W days before this &+rheart&+W goes bad.&n\n",
					ch);
				break;
			case 1:
				send_to_char(
					"&+WYou have over &+B1&+W day before this &+rheart&+W goes bad.&n\n",
					ch);
				break;
			case 0:
				send_to_char(
					"&+WYou have less than &+B1&+W day before this &+rheart&+W goes bad.  Hurry!&n\n",
					ch);
				break;
			default:
				send_to_char_f(
					ch,
					"&+WIt seems this &+rheart&+W will last for &+B%d&+W days, no worries.&n\n",
					(obj->value[0] * PULSE_MOBILE) /
						(WAIT_SEC * SECS_PER_MUD_DAY));
				break;
			}
			return TRUE;
		}
	...
	if (!obj->value[0]--)
	{
		...
		{
			logit(LOG_EXIT, "wh_corpse_decay: unable to load decayed heart #%d.",
			      VOBJ_WH_DRAGONHEART_ROTTED);
			return FALSE;
		}
		...
		else if (OBJ_WORN(obj))
		{
			...
			if (loc == MAX_WEAR)
			{
				debug("Dragon heart '%s' %d supposedly equipped but not!",
				      OBJ_SHORT(obj), OBJ_VNUM(obj));
				logit(LOG_OBJ, "Dragon heart '%s' %d supposedly equipped but not!",
				      OBJ_SHORT(obj), OBJ_VNUM(obj));
				obj_to_char(decayed_heart, carrier);
			}
			else
			{
				if (loc != HOLD)
				{
					debug("Dragon heart '%s' %d equipped in non-hold slot %d!",
					      OBJ_SHORT(obj), OBJ_VNUM(obj), loc);
					logit(LOG_OBJ,
					      "Dragon heart '%s' %d equipped in non-hold slot %d!",
					      OBJ_SHORT(obj), OBJ_VNUM(obj), loc);
				}
				// Remove the heart.
		...
		case VOBJ_WH_DRAGONHEART_BAHAMUT:
			...
			break;
		default:
			debug("dragon_heart_decay: Unknown item decaying - '%s' %d?!?",
			      OBJ_SHORT(obj), OBJ_VNUM(obj));
			break;
		}
...
int lanella_heart(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	if (cmd == CMD_DEATH)
	{
		...
		act("&nAs &+MLanella &nfalls to the ground, her body begins to decay.&n", FALSE, ch,
		    obj, obj, TO_ROOM);
...
int lancer_gift(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (!IS_FIGHTING(ch))
	{
		if (arg && (cmd == CMD_OPEN))
		{
			if (isname(arg, "gift"))
			{
				act("&+WYou open &n$q &+Wand the wrappings fall to the floor.&n",
				    TRUE, ch, obj, vict, TO_CHAR);
				act("$n opens &n$q &+Wand the wrappings fall to the floor.&n", TRUE,
				    ch, obj, vict, TO_ROOM);
...
int cerberus_load(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	if (cmd == CMD_DEATH)
	{
		...
		act("&+yA HUGE rockworm gasps and begins to turn to dust.&n", FALSE, ch, NULL, NULL,
		    TO_ROOM);
		extract_char(ch);
