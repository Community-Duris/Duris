...
int piergeiron(P_char ch, P_char pl, int cmd, char *arg)
{
		...
		if (ch->in_room != real_room(3306))
		{
			if ((GET_STAT(ch) > STAT_INCAP) && !IS_FIGHTING(ch))
				if (GET_STAT(ch) == STAT_SLEEPING)
					act("$n wakes up, looking extremely annoyed.", TRUE, ch, 0,
					    0, TO_ROOM);
			SET_POS(ch, POS_STANDING + STAT_NORMAL);
			act("$n says, 'Damn, lost again, and almost time for work too!", TRUE, ch,
			    0, 0, TO_ROOM);
			...
			act("$n slowly fades into existence, looking extremely annoyed.", TRUE, ch,
			    0, 0, TO_ROOM);
			act("$n sits behind his desk, and closes his eyes for a quick nap.", TRUE,
			    ch, 0, 0, TO_ROOM);
	...
	if ((cmd != 0) && pl)
	{
		...
		case CMD_ROSE:
			...
			{
				act("$N tells you, 'Now, now. None of that mushy stuff with me, pal.'",
				    FALSE, pl, 0, ch, TO_CHAR);
				act("$N tells $n, 'None of the mushy stuff with me, pal.'", TRUE,
				    pl, 0, ch, TO_ROOM);
			}
			return TRUE;
			break;
		...
		case CMD_TRIP:
			if ((c_obj != ch) || (!IS_AWAKE(ch)) || (IS_FIGHTING(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N snarls at you, 'Watch it, bub, or I just might have to teach you a lesson!'",
			    FALSE, pl, 0, ch, TO_CHAR);
			act("$N snarls at $n, 'Watch it, bub, or I just might have to teach you a lesson!'",
		...
		case CMD_BIRD:
			if ((c_obj != ch) || (!IS_AWAKE(ch)) || (IS_FIGHTING(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N grumbles at you, 'Every damn place you go, someone has to be a clown.'",
			    FALSE, pl, 0, ch, TO_CHAR);
			act("$N grumbles at $n, 'Every damn place you go, someone has to be a clown.'",
			    TRUE, pl, 0, ch, TO_ROOM);
			return TRUE;
			break;
		case CMD_GRIN:
	...
	case 'S':
		...
		SET_POS(ch, POS_SITTING + STAT_SLEEPING);
		act("$n lies down and quickly falls asleep behind his desk.", FALSE, ch, 0, 0,
		    TO_ROOM);
		move = FALSE;
		break;
	...
	case 'c':
		act("$n says, 'Greetings, friends!  Another fine day in Verzanan.'", FALSE, ch, 0,
		    0, TO_ROOM);
	...
	case 'e':
		act("$n says, 'Looks like another busy day in the port!  Keep the people in order, men.'",
	...
	case 'g':
		act("Kalara says, 'Same as always, old man, and you come by anyway.'", FALSE, ch, 0,
		    0, TO_ROOM);
	...
	case 'h':
		...
		act("$n says, 'I'm off to find Khelben. Take care, Kalara.'", FALSE, ch, 0, 0,
		    TO_ROOM);
		act("$n bows deeply.", FALSE, ch, 0, 0, TO_ROOM);
		break;

	case 'i':
		act("The watchman says, 'Nothing happening here at the bazaar, sir.'", FALSE, ch, 0,
		    0, TO_ROOM);
	...
	case 'j':
		...
		act("The watchman says, 'Nothing unusual sir, same as always.'", FALSE, ch, 0, 0,
		    TO_ROOM);
		break;

	case 'k':
		act("$n says, 'Anyone here seen Khelben?  Damn slippery mages are impossible to find..'",
		    FALSE, ch, 0, 0, TO_ROOM);
	...
	case 'm':
		...
		act("$n says, 'You got it, old man. Need it to wake up.'", FALSE, ch, 0, 0,
		    TO_ROOM);
		break;

	case 'n':
		act("The waiter pours a shot of whiskey. ", FALSE, ch, 0, 0, TO_ROOM);
		act("The waiter gives the shot to the tired Lord of Verzanan.", FALSE, ch, 0, 0,
		    TO_ROOM);
		break;

	case 'o':
		act("$n swallows the shot in a single gulp, and hands back the glass.", FALSE, ch,
		    0, 0, TO_ROOM);
		act("$n says, 'Ahhhh, that hit the spot.'", FALSE, ch, 0, 0, TO_ROOM);
		break;

	case 'p':
		act("$n says, 'Well, back to work, boys. See ya later on.'", FALSE, ch, 0, 0,
		    TO_ROOM);
	...
	case 'A':
		act("$n says, 'The east gate of Verzanan is now closed!'", FALSE, ch, 0, 0,
		    TO_ROOM);
		act("$n says, 'You may reopen the gates at sunrise, men.'", FALSE, ch, 0, 0,
		    TO_ROOM);
		act("The guards nod in acknowledgment.", FALSE, ch, 0, 0, TO_ROOM);
		break;

	case 'B':
		act("$n says, 'The north gate of Verzanan is now closed!'", FALSE, ch, 0, 0,
		    TO_ROOM);
		act("$n says, 'You may reopen the gates at sunrise, men.'", FALSE, ch, 0, 0,
		    TO_ROOM);
		act("The guards nod in acknowledgment.", FALSE, ch, 0, 0, TO_ROOM);
	...
	case 'F':
		act("$n says, 'The west gate of Verzanan is now closed!'", FALSE, ch, 0, 0,
		    TO_ROOM);
		act("$n says, 'You may reopen the gates at sunrise, men.'", FALSE, ch, 0, 0,
		    TO_ROOM);
		act("The guards nod in acknowledgment.", FALSE, ch, 0, 0, TO_ROOM);
	...
	if (!pl)
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 1:
				...
				mobsay(ch,
				       "No where else in the world can you find such splendor!");
				break;
			case 2:
				mobsay(ch, "Ooo, smells like someone is cooking up a fine meal.");
				act("$n peers around with a hungry look on his face.", TRUE, ch, 0,
				    0, TO_ROOM);
				break;
			case 3:
			...
			case 4:
				...
				mobsay(ch,
				       "Scum hides like a rat and corrodes my city from within.");
			...
			case 5:
				...
				break;
			case 6:
				act("$n adjusts his uniform to look more authoritative.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			case 7:
				mobsay(ch,
				...
				break;
			case 8:
				act("$n trips over a small stone, but recovers gracefully.", TRUE,
				    ch, 0, 0, TO_ROOM);
			...
			case 11:
				...
				break;
			case 12:
				act("$n smirks arrogantly, obviously proud of his station.", TRUE,
				    ch, 0, 0, TO_ROOM);
				break;
			case 13:
...
int piergeiron_guard(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((MIN_POS(ch, POS_STANDING + STAT_NORMAL)) && !number(0, 4))
	{
		...
		{
		case 2:
			act("$n looks around cautiously, watching for any possible threat to the lord.",
			    TRUE, ch, 0, 0, TO_ROOM);
			break;
		case 3:
			...
			break;
		case 4:
			act("$n keeps his hand on his sword hilt while near you.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 5:
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 4:
				...
				break;
			case 5:
				act("$n gazes off onto the horizon, looking for something.", TRUE,
				    ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 4:
				...
				break;
			case 5:
				act("$n whirls, listening intently for some unheard sound.", TRUE,
				    ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
...
int drunk_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
			case 5:
				act("$n stumbles and nearly falls, lost in his drunken stupor.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
			...
			case 3:
				...
				break;
			case 4:
				act("$n turns green and nearly hurls, but amazingly recovers.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
...
int drunk_two(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
			case 5:
				act("$n begins singing loudly, though his awful tone makes you cringe.",
				    TRUE, ch, 0, 0, TO_ROOM);
			...
			case 2:
				...
				break;
			case 3:
				act("$n points at you and laughs uncontrollably for several minutes..",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("$n flips you the bird and mumbles something incoherent under his breath.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
...
int drunk_three(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
			case 5:
				act("$n shouts annoyingly, 'Where is that damn bartender!'", TRUE,
				    ch, 0, 0, TO_ROOM);
				break;
			case 2:
				...
				break;
			case 3:
				act("$n loses his balance and falls to the ground, cursing all the while.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("$n stares off into space, seemingly lost in some mindless thought.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
...
int cat_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
			case 5:
				act("$n approaches and bumps your leg, looking for attention.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("$n looks you up and down, probably sizing up whether or not you're worth the effort.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
...
int merchant_two(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if ((MIN_POS(ch, POS_STANDING + STAT_NORMAL)) && (ch->in_room == real_room(5400)))
		{
			...
			{
			case 5:
				act("$n looks impatient, as if he's waited years for his ship to come in.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				...
				break;
			case 3:
				act("$n stares out the door, scanning the harbor for his ship.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				mobsay(ch,
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("$n screws up a sword maneuver, blushing furiously.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			default:
				break;
...
int mercenary_three(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
			case 5:
				act("$n takes a long draught from his mug.", TRUE, ch, 0, 0,
				    TO_ROOM);
				break;
			case 2:
				...
				break;
			case 3:
				act("$n lets off a roaring belch that echoes around the room.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int casino_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
			case 2:
				act("$n moves some gambling chips around so fast you almost can't follow his movements.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				act("$n shuffles the cards with the ease of a skilled pro.", TRUE,
				    ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int casino_three(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	...
	if (ch->in_room != real_room(GET_HOME(ch)))
	{
		...
		/*
		   Muahahahah
		 */
		act("$n looks around dazedily, and says 'Where the HELL am I?'", FALSE, ch, 0, 0,
		    TO_ROOM);
		act("$n says 'Damn!  Luna'll KILL me, I gotta get back to work!'", FALSE, ch, 0, 0,
		    TO_ROOM);
	...
	if (evil && (GET_ALIGNMENT(GET_OPPONENT(evil)) >= 0))
	{
		...
		act("$n yells, 'HEY! No fighting in here, dead beat!'", FALSE, ch, 0, evil,
		    TO_ROOM);
		act("The bouncer picks you and tosses you out of the bar!", FALSE, evil, 0, ch,
		    TO_CHAR);
		...
		act("$N is thrown from the tavern, landing in a heap on the ground!", FALSE, ch, 0,
		    evil, TO_NOTVICT);
		act("The bouncer snarls at you, 'Next time I'll break your neck, punk!'", FALSE,
		    evil, 0, ch, TO_CHAR);
		act("The bouncer snarls at $N, 'Next time I'll break your neck, punk!'", FALSE, ch,
		    0, evil, TO_NOTVICT);
...
int casino_four(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (IS_FIGHTING(ch))
		{
			...
			{
			case 5:
				act("$n growls angrily, a look of rage on his face.", TRUE, ch, 0,
				    0, TO_ROOM);
				break;
			case 2:
...
int guard_one(P_char ch, P_char /*pl*/, int cmd, char * /*arg*/)
{
	...
	if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
		case 6:
			act("$n drinks deep from his bottle, and then lets out a roaring belch.",
			    TRUE, ch, 0, 0, TO_ROOM);
			break;
		case 2:
			mobsay(ch,
		...
		case 3:
			...
			break;
		case 4:
			act("$n says, 'Hey, bartender! Bring me another, dammit.'", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 5:
			act("$n laughs heartily, nearly falling off the barstool.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		default:
			break;
	...
	if (evil && (GET_ALIGNMENT(GET_OPPONENT(evil)) >= 0))
	{
		...
		act("$n jumps off the stool and joins in the fight with a drunken grin.", FALSE, ch,
		    0, 0, TO_ROOM);
	...
	else
	{
		if (GET_STAT(ch) == STAT_SLEEPING)
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n twitches in his sleep, probably having a dream.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			default:
				break;
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 4:
				...
				break;
			case 2:
				act("$n digs a small nut from the grass and begins eating it.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				act("$n wriggles its little tail while looking at you.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			default:
				break;
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n looks at you with eyes both angry and hateful.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			case 4:
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL) && !number(0, 2))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n starts picking up small pieces of lint and thread.", TRUE,
				    ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("$n looks at you and says, 'You could stand to loose a few pounds.'",
				    TRUE, ch, 0, 0, TO_ROOM);
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n looks around frustrated, as if he can't find what he wants to buy.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int rogue_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (IS_FIGHTING(ch))
		{
			...
			{
			case 5:
				act("$n says, 'You will DIE for intruding in my home!'", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			case 2:
	...
	if (cmd != 0)
	{
		...
		if (IS_AGG_CMD(cmd))
		{
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			act("$N screams in terror at you, 'Help, help! Someone get this creep off of me!'",
			    FALSE, pl, 0, ch, TO_CHAR);
			act("$N screams, 'Help, help! Someone get this creep off of me!'", TRUE, pl,
			    0, ch, TO_ROOM);
			if (ch->following)
				stop_follower(ch);
			return FALSE;
		...
		case CMD_OGLE:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N whispers to you, 'I'm yours for 5 gold, sweetie pie.'", FALSE, pl,
			    0, ch, TO_CHAR);
			act("$N whispers something to $n.", TRUE, pl, 0, ch, TO_ROOM);
			return TRUE;
			break;
		...
		case CMD_PINCH:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N whispers to you, 'I like that.' in a sexy voice.", FALSE, pl, 0, ch,
			    TO_CHAR);
		...
		case CMD_WAKE:
			...
			{
				act("Your gentle nudging awakens $N, who yawns and clears her throat.",
				    FALSE, pl, 0, ch, TO_CHAR);
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n looks across the dock for something or someone.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			case 4:
				act("$n looks as though he's been working hard all day.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			default:
				break;
...
int naval_three(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		...
		else if (IS_FIGHTING(ch))
		{
			...
			{
			case 1:
				act("$n glares at you, putting more weight into her swing.", TRUE,
				    ch, 0, 0, TO_ROOM);
				break;
			case 2:
			...
			case 4:
				...
				for (vict = world[ch->in_room].people; vict; vict = temp)
				{
					...
					{
						act("$n makes a powerful magical gesture.", TRUE,
						    ch, 0, 0, TO_ROOM);
...
int naval_four(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
			case 5:
				act("$n scans the horizon for sea vessels.", TRUE, ch, 0, 0,
				    TO_ROOM);
				break;
			case 2:
				mobsay(ch, "Howdy.");
			...
			case 3:
				...
				break;
			case 4:
				act("$n looks around the harbor, taking in everything.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			default:
				break;
...
int lighthouse_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((lighthouseone == 1) && (MIN_POS(ch, POS_STANDING + STAT_NORMAL)))
		...
		{
		case 0:
			act("$n stares long across ocean, taking a deep breath.", FALSE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 1:
			...
			break;
		case 2:
			act("$n says, 'Been workin' here 30 years and never got over how lovely it is.'",
			    FALSE, ch, 0, 0, TO_ROOM);
			break;
		case 3:
			...
			break;
		case 4:
			act("$n says, 'Only thing needs doin' now is to rebuild this old place.'",
			    FALSE, ch, 0, 0, TO_ROOM);
			break;
		case 5:
			act("$n says, 'Been here for a good hundred years, I reckin.'", FALSE, ch,
			    0, 0, TO_ROOM);
			break;
		case 6:
			act("$n says, 'Hehe.  Sometimes I'm surprised it's still standin.'", FALSE,
			    ch, 0, 0, TO_ROOM);
			act("$n giggles softly.", FALSE, ch, 0, 0, TO_ROOM);
			break;
		case 7:
			act("$n says, 'This one used to be just fine, till they dug it all out.'",
			    FALSE, ch, 0, 0, TO_ROOM);
			break;
		case 8:
			act("$n grumbles, 'God knows what they were doin' down there..'", FALSE, ch,
			    0, 0, TO_ROOM);
			act("$n chuckles.", FALSE, ch, 0, 0, TO_ROOM);
			break;
		case 9:
			act("$n says, 'Course the military never tells us common folk what's really goin' on.'",
			    FALSE, ch, 0, 0, TO_ROOM);
			break;
		case 10:
			act("$n says, 'Aahhhh well, 'tis just the same anyway.", FALSE, ch, 0, 0,
			    TO_ROOM);
			act("$n says, 'If they don't wanna tell us, it probably isn't important.",
			    FALSE, ch, 0, 0, TO_ROOM);
			break;
		case 11:
			...
			break;
		case 12:
			act("$n says, 'I heard it's gettin' worse, just what we need.'", FALSE, ch,
			    0, 0, TO_ROOM);
			break;
		case 13:
			...
			break;
		case 14:
			act("$n says, 'Ahh well, I'll shut up and enjoy the view.'", FALSE, ch, 0,
			    0, TO_ROOM);
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n pecks at something on the ground.", TRUE, ch, 0, 0,
				    TO_ROOM);
				break;
			case 4:
...
int commoner_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
			case 5:
				act("$n looks for a second, then looks away quickly.", TRUE, ch, 0,
				    0, TO_ROOM);
				break;
			case 2:
				mobsay(ch, "Hello.");
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n takes a deep breath as the breeze blows in, looking very relaxed.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("$n closes her eyes, and looks deep in thought.", TRUE, ch, 0,
				    0, TO_ROOM);
				break;
			default:
				break;
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("$n gasps as the struggle intensifies.", TRUE, ch, 0, 0,
				    TO_ROOM);
				break;
			default:
				break;
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 4:
				...
				break;
			case 5:
				act("$n looks at you for a moment, seeing if his audience is listening.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 6:
			...
			case 8:
				...
				break;
			case 9:
				act("$n says softly, 'Rumor has it Kalara and Lord Piergeiron are an item!'",
				    TRUE, ch, 0, 0, TO_ROOM);
			...
			case 10:
				...
				mobsay(ch,
				       "Mostly for when we get hit by goblin raids and whatnot.");
				break;
			case 11:
			...
			case 23:
				...
				break;
			case 24:
				act("$n says softly, 'I hear the old fisherman out on Deepwater Isle has discount tickets to the Moonshaes!'",
				    TRUE, ch, 0, 0, TO_ROOM);
			...
			case 35:
				mobsay(ch,
				       "I hear a brothel is opening in southern Verzanan soon.");
				act("$n winks knowingly.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 36:
				mobsay(ch,
	...
	if (!pl)
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL) && !number(0, 2))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("$n looks around, observing everything for trouble.", TRUE, ch,
				    0, 0, TO_ROOM);
				break;
			default:
				return FALSE;
	...
	if (!pl)
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL) && !number(0, 2))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n looks you up and down for a moment, then goes back to his duties.",
				    TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("$n scans the area for signs of trouble.", TRUE, ch, 0, 0,
...
int cell_drunk(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if (cmd != 0)
	{
		...
		case CMD_HI5:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N vomits all over $n, giving him something to frown about..", TRUE,
			    ch, 0, 0, TO_ROOM);
			act("$N vomits all over your clothing, giving you something to frown about.",
			    FALSE, pl, 0, ch, TO_CHAR);
			return TRUE;
			break;
		case CMD_INSULT:
		...
		case CMD_PUKE:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N sees $n puke, and begins vomiting uncontrollably.", TRUE, ch, 0, 0,
			    TO_ROOM);
			act("$N sees you puke, and begins vomiting uncontrollably.", FALSE, pl, 0,
			    ch, TO_CHAR);
			return TRUE;
			break;
		case CMD_CACKLE:
		...
		case CMD_SING:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N says, 'Will ya keep it down, $n? My head hurts!'.", TRUE, ch, 0, 0,
			    TO_ROOM);
			act("$N says, 'Will ya keep it down? My head hurts!'.", FALSE, pl, 0, ch,
			    TO_CHAR);
			return TRUE;
			break;
		case CMD_SNIFF:
		...
		case CMD_HISS:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N says, 'Quit cryin' about it, we're all in this together.'", TRUE,
			    ch, 0, 0, TO_ROOM);
			act("$N says, 'Quit cryin about it, we're all in this together.'", FALSE,
			    pl, 0, ch, TO_CHAR);
			return TRUE;
			break;
		case CMD_BURP:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N applauds and says with great cheer, 'Worthy! Good one!'.", TRUE, ch,
			    0, 0, TO_ROOM);
			act("$N applauds and says with great cheer, 'Worthy! Good one!", FALSE, pl,
			    0, ch, TO_CHAR);
			return TRUE;
			break;
		case CMD_FART:
			...
			act("$N says, 'Oh, god..  Will ya do that outside?!?'.", TRUE, ch, 0, 0,
			    TO_ROOM);
			act("$N says, 'Oh, god..  Will ya do that outside?!?'.", FALSE, pl, 0, ch,
			    TO_CHAR);
			return TRUE;
			break;
		case CMD_SLAP:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N slaps $n back with great cheer, thinking it all a game.", TRUE, ch,
			    0, 0, TO_ROOM);
			act("$N slaps you back with great cheer, thinking it all a game.", FALSE,
			    pl, 0, ch, TO_CHAR);
			return TRUE;
			break;
		case CMD_LAUGH:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N frowns and says, 'What are YOU laughin' about, $n?'.", TRUE, ch, 0,
			    0, TO_ROOM);
			act("$N frowns and says, 'What are YOU laughin' about, bub?'.", FALSE, pl,
			    0, ch, TO_CHAR);
			return TRUE;
			break;
		case CMD_HUG:
			...
			act("$N says, 'Don't hug me, $n, ya smell awful!'.", TRUE, ch, 0, 0,
			    TO_ROOM);
			act("$N says, 'Don't hug me jailbait, ya smell awful!'.", FALSE, pl, 0, ch,
			    TO_CHAR);
			return TRUE;
			break;
		case CMD_GRIN:
			...
			act("$n grins and whispers to you, 'Got a plan mate?'.", FALSE, pl, 0, ch,
			    TO_CHAR);
			return TRUE;
			break;
		case CMD_GASP:
		...
		case CMD_POUT:
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			do_action(pl, Gbuf1, cmd);
			act("$N says, 'Quit yer pouting, $n, and think of a way outta here'.", TRUE,
			    ch, 0, 0, TO_ROOM);
			act("$N says, 'Quit yer poutin' and think of a way outta here'.", FALSE, pl,
			    0, ch, TO_CHAR);
			return TRUE;
			break;
		default:
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			case 2:
				mobsay(ch,
				       "Jailer!! Lemme outta here, you worthless piece of shit!");
				break;
			case 3:
...
int varon(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (GET_LEVEL(ch) < 60)
	{
		act("The disruptor hums, then blows up in your face!", FALSE, ch, 0, victim,
		    TO_CHAR);
	...
	{
		act("The mighty weapon hums softly for an instant, then glows brightly!", FALSE, ch,
		    0, victim, TO_CHAR);
		act("A multi-colored beam of radiant energy bursts forth from its tip, ", FALSE, ch,
		    0, victim, TO_CHAR);
		act("striking $N dead in the chest!  They scream in agony as the energy", FALSE, ch,
		    0, victim, TO_CHAR);
		act("dissolves their body, ripping their life force away.", FALSE, ch, 0, victim,
		    TO_CHAR);
		act("In seconds, there is nothing left but a memory...", FALSE, ch, 0, victim,
		    TO_CHAR);

		act("$n points $s disruptor at you, and pulls the trigger with a grin.", FALSE, ch,
		    0, victim, TO_VICT);
		act("The weapon glows, firing a bright beam of radiant energy directly ", FALSE, ch,
		    0, victim, TO_VICT);
		act("at you! You try to dodge, but too late. The beam strikes you dead", FALSE, ch,
		    0, victim, TO_VICT);
		act("in the chest! AAAAAAAAAAHHHHHHHHH!! Pain rips through your body ", FALSE, ch,
		    0, victim, TO_VICT);
		act("in waves, more intense than you've ever felt. As you look down, ", FALSE, ch,
		    0, victim, TO_VICT);
		act("the most horrible realization possible overcomes you as your", FALSE, ch, 0,
		    victim, TO_VICT);
		act("entire body dissolves away, slowly enough for you to experience every", FALSE,
		    ch, 0, victim, TO_VICT);
		act("ounce of pure agony. Darkness washes over you, and with your last", FALSE, ch,
		    0, victim, TO_VICT);
		act("glimpse of thought, you realize that death has won this day... ", FALSE, ch, 0,
		    victim, TO_VICT);

		act("$n points his disruptor at $N, and pulls the trigger with a grin.", TRUE, ch,
		    0, victim, TO_NOTVICT);
		act("The disruptor glows and blasts forth a bright beam of radiant energy.", TRUE,
		    ch, 0, victim, TO_NOTVICT);
		act("The beam strikes $N in the chest, who screams in unbelievable ", TRUE, ch, 0,
		    victim, TO_NOTVICT);
		act("agony.  Their body begins to dissolve from the inside out, causing", TRUE, ch,
		    0, victim, TO_NOTVICT);
		act("the most exquisite pain imaginable. Within a few short seconds, ", TRUE, ch, 0,
		    victim, TO_NOTVICT);
		act("$N's body is gone, leaving only a horrible memory behind..", TRUE, ch, 0,
		    victim, TO_NOTVICT);
...
int guild_protection(P_char ch, P_char pl)
{
	...
	act("$n pressed a small metal pin on $s chest, which flares with brilliant blue light!",
	    FALSE, ch, 0, 0, TO_ROOM);
	send_to_char(
...
int guild_guard_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(5500)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The holy warrior steps in front of $n, blocking $s entry into the guild.",
			    FALSE, pl, 0, 0, TO_ROOM);
			act("The holy paladin says, 'I'm sorry, but only holy paladins may enter here.'",
			    FALSE, pl, 0, 0, TO_ROOM);
			send_to_char(
		...
		{
			act("The holy warrior bows reverently before $n as $s enters.", FALSE, pl,
			    0, 0, TO_ROOM);
			act("The holy paladin tells $n, 'You are welcome, brethren..'", FALSE, pl,
			    0, 0, TO_ROOM);
	...
	if (evil && (GET_ALIGNMENT(GET_OPPONENT(evil)) >= 0))
	{
		act("$n screams 'PROTECT THE INNOCENT! BANZAI! CHARGE! ARARAGGGHH!'", FALSE, ch, 0,
		    0, TO_ROOM);
		MobStartFight(ch, evil);
		return (TRUE);
	}
	...
	else if (IS_FIGHTING(ch))
	{
		...
		case 2:
			...
			break;
		case 3:
			act("$n performs a dazzling display of swordwork against you.", TRUE, ch, 0,
			    0, TO_ROOM);
			break;
		case 4:
			mobsay(ch,
...
int guild_guard_two(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(5510)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The minotaur slaps $n across the face as $s tries to enter the guild.",
			    FALSE, pl, 0, 0, TO_ROOM);
			act("The minotaur snarls, 'Get lost, idiot, or I'll have you for lunch!'",
			    FALSE, pl, 0, 0, TO_ROOM);
		...
		{
			act("The minotaur fixes $n with a stern eye as $s enters the guild.", FALSE,
			    pl, 0, 0, TO_ROOM);
			send_to_char(
...
int guild_guard_three(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(5520)) && (cmd == CMD_SOUTH))
	{
		...
		{
			act("The monk respectfully blocks $n's entry into the temple, and whisper something.",
			    FALSE, pl, 0, 0, TO_ROOM);
		...
		{
			act("The monk bows respectfully before $n as $s enters.", FALSE, pl, 0, 0,
			    TO_ROOM);
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
		case 1:
			act("$n does a flip to the side, and attack from a different angle.", TRUE,
			    ch, 0, 0, TO_ROOM);
			break;
		case 2:
...
int guild_guard_four(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(5534)) && (cmd == CMD_UP))
	{
		if (!GET_CLASS(pl, CLASS_BARD))
		{
			act("The bouncer roughly grabs $n and pushes $s back.", FALSE, pl, 0, 0,
			    TO_ROOM);
			act("The bouncer says, 'You don't have any business up there, pal, get lost.'",
			    FALSE, pl, 0, 0, TO_ROOM);
		...
		{
			act("The bouncer steps aside as $n climbs up the staircase.", FALSE, pl, 0,
			    0, TO_ROOM);
	...
	if (evil && (GET_ALIGNMENT(GET_OPPONENT(evil)) >= 0))
	{
		...
		act("$n yells, 'HEY! No fighting in here, dead beat!'", FALSE, ch, 0, evil,
		    TO_ROOM);
		act("The bouncer picks you and drags you out of the bar!", FALSE, evil, 0, ch,
		    TO_CHAR);
		...
		act("A bouncer storms into the room dragging $N by the collar", FALSE, ch, 0, evil,
		    TO_NOTVICT);
		act("$N kicks and screams, all to no avail, as the bouncer drags $S east.", FALSE,
		    ch, 0, evil, TO_NOTVICT);
		char_from_room(evil);
		...
		act("$N is thrown from the tavern and lands in a heap on the ground!", FALSE, ch, 0,
		    evil, TO_NOTVICT);
		act("The bouncer snarls at you, 'Next time I'll break your neck, punk!'", FALSE,
		    evil, 0, ch, TO_CHAR);
		act("The bouncer snarls at $N, 'Next time I'll break your neck, punk!'", FALSE, ch,
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		case 3:
			...
			break;
		case 4:
			act("$n stares you down, thinking himself to be hot shit.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 5:
...
int guild_guard_five(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(5540)) && (cmd == CMD_SOUTH))
	{
		...
		{
			act("The unicorn nudges $n affectionately as $s enters the guild.", FALSE,
			    pl, 0, 0, TO_ROOM);
			send_to_char(
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
		case 1:
			act("You hear a voice in your head, 'Stop this madness!'", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 2:
			act("$n forces a suggestion into your mind, 'FLEE! RUN AWAY!'", TRUE, ch, 0,
			    0, TO_ROOM);
			send_to_char("You flee in panic, running wildly under the unicorn's spell.",
				     pl);
			do_flee(pl, 0, 2);
			send_to_char("You flee in panic, running wildly under the unicorn's spell.",
				     pl);
...
int guild_guard_six(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(5560)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The massive treant blocks $n from going east with a wall of branches.",
			    FALSE, pl, 0, 0, TO_ROOM);
			send_to_char(
		...
		{
			act("The massive treant drops a leaf on $n as $s enters the grove.", FALSE,
			    pl, 0, 0, TO_ROOM);
			send_to_char(
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		case 2:
			...
			break;
		case 3:
			act("You feel as if the massive treant is watching you.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 4:
	...
	else if (IS_FIGHTING(ch))
	{
		...
		case 4:
			...
			act("The sound blasts your ears painfully, causing you to scream and flee!",
			    TRUE, ch, 0, 0, TO_ROOM);
			send_to_char("You flee in panic, running wildly under the treant's spell.",
				     ch);
			do_flee(pl, 0, 2);
			send_to_char("You flee in panic, running wildly under the treant's spell.",
				     ch);
...
int guild_guard_seven(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(5570)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The flesh golem roughly grabs $n and tosses $s backwards.", FALSE, pl,
			    0, 0, TO_ROOM);
			send_to_char(
...
int guild_guard_eight(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(5572)) && (cmd == CMD_EAST))
	{
		if (!GET_CLASS(pl, CLASS_NECROMANCER))
		{
			act("The guard steps in front of $n, preventing $s entrance into the tower.",
			    FALSE, pl, 0, 0, TO_ROOM);
			send_to_char(
		...
		{
			act("The guard bows deeply before $n, allowing $s entrance.", FALSE, pl, 0,
			    0, TO_ROOM);
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		case 4:
			...
			break;
		case 5:
			act("$n casts a minor cantrip, probably to amuse himself.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		default:
			break;
	...
	else if (IS_FIGHTING(ch))
	{
		...
		case 1:
			...
			act("You slowly fade out, and rematerialize on Deepwater Island!", TRUE, ch,
			    0, 0, TO_ROOM);
...
int guild_guard_nine(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(3038)) && (cmd == CMD_SOUTH))
	{
		if (GET_CLASS(pl, CLASS_SORCERER) || GET_CLASS(pl, CLASS_CONJURER) ||
		    GET_CLASS(pl, CLASS_SUMMONER))
		{
			act("The guard bows curtly before $n, and motions $s inside.", FALSE, pl, 0,
			    0, TO_ROOM);
			send_to_char(
		...
		{
			act("The guard steps in front of $n, barring $s from entering the tower.",
			    FALSE, pl, 0, 0, TO_ROOM);
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		case 4:
			...
			break;
		case 5:
			act("$n kicks a stone near the tower, looking very bored.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		default:
			break;
...
int guild_guard_ten(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(3067)) && (cmd == CMD_NORTH))
	{
		...
		{
			act("The guild guardian smiles at $n, and allows $s inside.", FALSE, pl, 0,
			    0, TO_ROOM);
		...
		{
			act("The guild guardian stops $n before $s can enter the temple.", FALSE,
			    pl, 0, 0, TO_ROOM);
			act("The guild guardian says, 'Only those of holy faith may enter.'", FALSE,
			    pl, 0, 0, TO_ROOM);
			send_to_char(
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
		case 2:
			act("$n smiles warmly at you, and appears to be nice fellow.", TRUE, ch, 0,
			    0, TO_ROOM);
			break;
		case 3:
	...
	else if (IS_FIGHTING(ch))
	{
		...
		case 2:
			...
			break;
		case 3:
			act("$n looks annoyed by your assault, but little more.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 4:
...
int guild_guard_eleven(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(3055)) && (cmd == CMD_SOUTH))
	{
		...
		{
			act("The guardian says to $n, 'Heya, mate, go right on in!", FALSE, pl, 0,
			    0, TO_ROOM);
		...
		else
		{
			act("The guardian steps in front of the door, barring $n's entrance.",
			    FALSE, pl, 0, 0, TO_ROOM);
			act("The guardian says, 'Only warriors can enter here! Now, get lost..'",
			    FALSE, pl, 0, 0, TO_ROOM);
			send_to_char(
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
		case 1:
			act("$n fights you in a perfect rhythm, dancing and dodging about.", TRUE,
			    ch, 0, 0, TO_ROOM);
			break;
		case 2:
			mobsay(ch,
			...
			break;
		case 3:
			act("$n slaps you as he dances by, making you look like an idiot.", TRUE,
			    ch, 0, 0, TO_ROOM);
			break;
		case 4:
...
int guild_guard_twelve(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(3283)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The rogue slaps $n across the face as $s tries to enter the guild.",
			    FALSE, pl, 0, 0, TO_ROOM);
...
int guild_guard_thirteen(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	if ((ch->in_room == real_room(2951)) && (cmd == CMD_NORTH))
	{
		...
		{
			act("The assassin bows, though never taking his eyes off $n as $s pass by him.",
			    FALSE, pl, 0, 0, TO_ROOM);
			send_to_char(
	...
	else if (IS_FIGHTING(ch))
	{
		...
		case 2:
			...
			break;
		case 3:
			act("$n moves with deadly grace, and is obviously very skilled.", TRUE, ch,
			    0, 0, TO_ROOM);
			break;
		case 4:
...
int young_paladin_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
		case 1:
			act("$n fights furiously, although his inexperience shows.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 2:
...
int wrestler_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
		case 2:
			act("$n circles around the arena, looking for a good attack opening.", TRUE,
			    ch, 0, 0, TO_ROOM);
			break;
		case 3:
...
int young_mercenary_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		case 4:
			...
			break;
		case 5:
			act("$n tries to look impressive by practicing his backstab.", TRUE, ch, 0,
			    0, TO_ROOM);
			break;
		default:
			break;
...
int young_monk_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else if (IS_FIGHTING(ch))
	{
		...
		case 2:
			...
			break;
		case 3:
			act("$n chops at you with her hands in a wicked hook pattern.", TRUE, ch, 0,
			    0, TO_ROOM);
			break;
		case 4:
...
int selune_one(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
		case 2:
			act("$n slips and falls off his barstool, burping loudly.", TRUE, ch, 0, 0,
			    TO_ROOM);
...
int selune_two(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		case 3:
			...
			break;
		case 4:
			act("$n smiles at you, then goes back to his conversation.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 5:
	...
	else if (IS_FIGHTING(ch))
	{
		...
		case 2:
			...
			break;
		case 3:
			act("$n swings, *SMACK* And hits you right across the jaw.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 4:
...
int selune_four(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		case 5:
			...
			break;
		case 6:
			act("$n tries to stumble into the other room, but fails.", TRUE, ch, 0, 0,
			    TO_ROOM);
			break;
		case 7:
...
int selune_five(P_char ch, P_char pl, int cmd, char * /*arg*/)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
		case 2:
			act("$n moves slowly through the room, dancing all the way.", TRUE, ch, 0,
			    0, TO_ROOM);
			break;
		case 3:
			act("$n does a little dance in front of one of the patrons.", TRUE, ch, 0,
			    0, TO_ROOM);
			break;
		case 4:
			...
			break;
		case 5:
			act("$n moves with the light-footed grace of a elven maiden.", TRUE, ch, 0,
			    0, TO_ROOM);
			break;
		default:
			break;
		       ...
		       (real_room(b_path[bp_var]) == ch->in_room))
		{
			...
			case 0:
				if (IS_FIGHTING(evil))
					stop_fighting(evil);
				StopAllAttackers(evil);
				act("$n yells, 'HEY! No fighting in here, dead beat!'", FALSE, ch,
				    0, evil, TO_ROOM);
			...
			case 1:
				act("A bouncer storms into the room, dragging $N by the collar!",
				    FALSE, ch, 0, evil, TO_NOTVICT);
				act("$N kicks and flails around as $s is dragged out.", FALSE, ch,
				    0, evil, TO_NOTVICT);
			...
			case 3:
				act("A bouncer throws $N from the tavern, who lands in a heap on the ground!",
				    FALSE, ch, 0, evil, TO_NOTVICT);
				act("The bouncer throws you onto the ground. *ouch* You land in a heap.",
				    FALSE, evil, 0, ch, TO_CHAR);
			...
			else
			{
				...
				act("A bouncer walks back in, smiling smugly.", TRUE, ch, 0, 0,
				    TO_ROOM);
				return (TRUE);
			}
		}
