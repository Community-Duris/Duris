...
, { MAKE_DAM_MOD_PRED(){ switch (damageType){
	case SPLDAM_GENERIC: if (has_innate(victim, MAGICAL_REDUCTION)){ dam_mod->mod += -0.2;
dam_mod->type = dam_mod_type::More;
}
break;
}
...
static dam_mod_predicate raw_damage_modifiers[] = { { MAKE_DAM_MOD_PRED(){ switch (
	GET_RACEWAR(caster)){ case RACEWAR_GOOD: dam_mod->mod += dam_factor[DF_GOOD_MODIFIER] - 1.0;
break;
case RACEWAR_EVIL:
