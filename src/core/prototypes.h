...
char *show_obj_to_char(P_obj, P_char, int, bool, const OutputContext &);
void show_char_to_char(P_char, P_char, int);
void show_char_to_char(P_char, P_char, int, const OutputContext &);
...
void list_char_to_char(P_char, P_char, int, const OutputContext &);
void list_obj_to_char(P_obj, P_char, int, bool);
void list_obj_to_char(P_obj, P_char, int, bool, const OutputContext &);
...
int rate_object(P_obj);
void rate_object_detailed(P_char, P_obj);
...
void insertFragEntry(char names[15][MAX_STRING_LENGTH], int frags[15], char *name, int newFrags,
		     int pos);
...
void deleteHallEntry(char names[15][MAX_STRING_LENGTH], int frags[15], int pos,
		     char killer[15][MAX_STRING_LENGTH]);
void insertHallEntry(char names[15][MAX_STRING_LENGTH], int frags[15], char *name, int newFrags,
		     int pos, char killer[15][MAX_STRING_LENGTH], char *killername);
...
signed char find_first_step(int src, int target, long hunt_flags, int is_ship, int wagon_type,
			    int *ttl_steps);
...
P_obj get_obj_in_list_vis(P_char ch, const char *name, P_obj list, bool no_tracks = TRUE);
...
int generic_find(const char *, int, P_char, P_char *, P_obj *);
int get_number(char **);
...
int shout_and_hunt(P_char ch, int max_distance, const char *shout_str,
		   int (*locator_proc)(P_char, P_char, int, char *), int vnums[], ulong act_mask,
		   ulong no_act_mask);
...
void spell_lesser_guardian_ward(int level, P_char ch, char *arg, int type, P_char victim,
				P_obj obj);
...
void save_dirty_shopkeepers(bool force = false);
...
bool cmd_allowed_while_casting(P_char, int);
...
bool input_allowed_while_currency_pending(const char *);
bool input_allowed_while_item_and_currency_pending(const char *);
int get_pending_transaction_cmd_from_q(struct txt_q *, char *, bool, bool);
...
/* DurisWeb hook toggles. Reads durisweb.hook.<id> from duris.properties.
