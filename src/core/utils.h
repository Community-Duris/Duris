...
#define CREATE(result, type, num, tag)                                                               \
	{                                                                                            \
		if (!((result) =                                                                     \
			      (type *)__malloc(sizeof(type) * (num), (tag), __FILE__, __LINE__)))    \
		{                                                                                    \
			panic_corruption_int(                                                        \
				"memory",                                                            \
				"CREATE: __malloc returned null for type=%s num=%d tag=%s at %s:%d", \
...
#define RECREATE(result, type, num)                                                              \
	{                                                                                        \
		if (!((result) = (type *)__realloc((void *)(result), sizeof(type) * (num),       \
						   __FILE__, __LINE__)))                         \
		{                                                                                \
			panic_corruption_int(                                                    \
				"memory",                                                        \
				"RECREATE: __realloc returned null for type=%s num=%d at %s:%d", \
...
#define CAN_CMD_PARALYSIS(cmd)                                                    \
	(((cmd) == CMD_LOOK) || ((cmd) == CMD_SCORE) || ((cmd) == CMD_QUI) ||     \
	 ((cmd) == CMD_TIME) || ((cmd) == CMD_IDEA) || ((cmd) == CMD_TYPO) ||     \
	 ((cmd) == CMD_BUG) || ((cmd) == CMD_CREDITS) || ((cmd) == CMD_GLANCE) || \
	 ((cmd) == CMD_RULES) || ((cmd) == CMD_WILL) || ((cmd) == CMD_PETITION))

#define IS_GLOBED(target) \
	(IS_AFFECTED2(target, AFF2_GLOBE) || IS_AFFECTED3(target, AFF3_GR_SPIRIT_WARD))
#define IS_MINGLOBED(ch) \
	(IS_AFFECTED3(target, AFF3_SPIRIT_WARD) || IS_AFFECTED(target, AFF_MINOR_GLOBE))

#define IS_ELITE(mob) (IS_SET(mob->specials.act, ACT_ELITE))
#define IS_IMMATERIAL(mob)                                                         \
	((GET_RACE(mob) == RACE_WRAITH) || (GET_RACE(mob) == RACE_GHOST) ||        \
	 (GET_RACE(mob) == RACE_SHADE) || (GET_RACE(mob) == RACE_A_ELEMENTAL) ||   \
	 (GET_RACE(mob) == RACE_F_ELEMENTAL) || (GET_RACE(mob) == RACE_PHANTOM) || \
	 (GET_RACE(mob) == RACE_WIGHT) || (GET_RACE(mob) == RACE_SHADOW) ||        \
	 (GET_RACE(mob) == RACE_DEVA) || (GET_RACE(mob) == RACE_SPECTRE) ||        \
	 (GET_RACE(mob) == RACE_V_ELEMENTAL) || (GET_RACE(mob) == RACE_BRALANI) || \
	 IS_AFFECTED4(mob, AFF4_PHANTASMAL_FORM))

#define LEGLESS(mob)                                                                \
	(IS_IMMATERIAL(mob) || IS_BEHOLDER(mob) || (GET_RACE(mob) == RACE_SLIME) || \
	 (GET_RACE(mob) == RACE_CONSTRUCT) || (GET_RACE(mob) == RACE_SNAKE) ||      \
	 (GET_RACE(mob) == RACE_INSECT) || (GET_RACE(mob) == RACE_PARASITE) ||      \
	 (GET_RACE(mob) == RACE_AQUATIC_ANIMAL) || (GET_RACE(mob) == RACE_PLANT) || \
	 (GET_RACE(mob) == RACE_EFREET) || (GET_RACE(mob) == RACE_PWORM))

#define IS_PC_PET(mob) \
	(IS_NPC(mob) && get_linked_char(mob, LNK_PET) && IS_PC(get_linked_char(mob, LNK_PET)))
...
#define IS_GREATER_DRACO(ch) \
	(IS_NPC(ch) &&       \
	 (GET_VNUM(ch) == 7 || GET_VNUM(ch) == 8 || GET_VNUM(ch) == 9 || GET_VNUM(ch) == 10))
#define IS_GREATER_AVATAR(ch) \
	(IS_NPC(ch) &&        \
	 (GET_VNUM(ch) == 82 || GET_VNUM(ch) == 83 || GET_VNUM(ch) == 84 || GET_VNUM(ch) == 85))

#define OUTDOOR_SNEAK(ch)                                                                   \
	(has_innate(ch, INNATE_OUTDOOR_SNEAK) && (world[(ch)->in_room].sector_type >= 2) && \
	 (world[(ch)->in_room].sector_type <= 4))
...
#define BAD_APPLYS(apply)                                                                   \
	(apply == APPLY_GOLD || apply == APPLY_CHAR_WEIGHT || apply == APPLY_CHAR_HEIGHT || \
...
#define IS_FULLY_RESTED(ch, skill_type, now) \
	(((now) - (ch)->specials.skill_usage[(skill_type)].time_of_first_use) >= SECS_PER_MUD_DAY)

#define SKILL_HOURS_TO_REST(ch, skill_type, now)                                           \
	(24 - (((now) - (ch)->specials.skill_usage[(skill_type)].time_of_first_use) * 24 / \
	       SECS_PER_MUD_DAY))
...
#define GET_CHAR_SKILL(ch, num)                                                             \
	(IS_AFFECTED5(ch, AFF5_MENTAL_ANGUISH) ? (MIN(GET_CHAR_SKILL_P((ch), (num)), 10)) : \
...
#define IS_FOREST_ROOM(r) \
	(world[r].sector_type == SECT_FOREST || world[r].sector_type == SECT_SNOWY_FOREST)

#define HAS_VEGETATION(sect)                                                                       \
	(sect == SECT_FIELD || sect == SECT_FOREST || sect == SECT_HILLS ||                        \
	 sect == SECT_MOUNTAIN || sect == SECT_UNDRWLD_WILD || sect == SECT_DESERT ||              \
	 sect == SECT_ARCTIC || sect == SECT_SWAMP || sect == SECT_UNDRWLD_MOUNTAIN ||             \
	 sect == SECT_UNDRWLD_SLIME || sect == SECT_UNDRWLD_MUSHROOM ||                            \
	 sect == SECT_SNOWY_FOREST || sect == SECT_UNDRWLD_WILD || sect == SECT_UNDRWLD_LOWCEIL || \
	 sect == SECT_CITY || sect == SECT_UNDRWLD_CITY)
#define IS_SWAMP_ROOM(r) (world[r].sector_type == SECT_SWAMP)
#define IS_CASTLE(r)                                                                             \
	(world[r].sector_type == SECT_CASTLE_WALL || world[r].sector_type == SECT_CASTLE_GATE || \
	 world[r].sector_type == SECT_CASTLE)

#define IS_UNDERWATER(c)                                          \
	((world[c->in_room].sector_type == SECT_UNDERWATER) ||    \
...
#define IS_WATER_ROOM(r)                                                                          \
	(world[r].sector_type == SECT_UNDRWLD_WATER ||                                            \
	 world[r].sector_type == SECT_UNDRWLD_NOSWIM || world[r].sector_type == SECT_OCEAN ||     \
	 world[r].sector_type == SECT_UNDERWATER || world[r].sector_type == SECT_UNDERWATER_GR || \
	 world[r].sector_type == SECT_WATER_NOSWIM || world[r].sector_type == SECT_WATER_SWIM ||  \
	 world[r].sector_type == SECT_WATER_PLANE)

#define IS_UNDERWORLD(r)                                  \
	(world[r].sector_type == SECT_UNDRWLD_CITY ||     \
...
#define NORMAL_PLANE(r)                                                                        \
	(world[r].sector_type != SECT_FIREPLANE && world[r].sector_type != SECT_WATER_PLANE && \
	 world[r].sector_type != SECT_EARTH_PLANE && world[r].sector_type != SECT_AIR_PLANE && \
	 world[r].sector_type != SECT_ETHEREAL && world[r].sector_type != SECT_ASTRAL &&       \
	 world[r].sector_type != SECT_NEG_PLANE)

#define IS_LIGHT(r)                                                              \
	((world[r].light > 0 && !IS_ROOM(r, ROOM_MAGIC_DARK)) || IS_SUNLIT(r) || \
	 IS_TWILIGHT_ROOM(r) || IS_ROOM(r, ROOM_MAGIC_LIGHT))

#define CAN_DAYPEOPLE_SEE(r) \
	(world[r].light > 0 || IS_SUNLIT(r) || IS_TWILIGHT_ROOM(r) || IS_ROOM(r, ROOM_MAGIC_LIGHT))

// This isn't simply !LIGHT, because twilight counts as both, and so does dark + lights in room.
#define CAN_NIGHTPEOPLE_SEE(r) \
	(IS_TWILIGHT_ROOM(r) || (!IS_SUNLIT(r) && !IS_ROOM(r, ROOM_MAGIC_LIGHT)))
...
#define IS_SUNLIT(r) \
	(IS_DAY && !IS_TWILIGHT && IS_OUTDOORS(r) && !IS_ROOM(r, ROOM_DARK | ROOM_MAGIC_DARK))
...
#define GET_PID(ch)                                                                          \
	(!IS_PC(ch) ? panic_corruption_int("utils", "GET_PID called on NPC '%s' at %s:%d",   \
					   GET_NAME(ch) ? GET_NAME(ch) : "(null)", __FILE__, \
					   __LINE__) :                                       \
		      (ch)->only.pc->pid)
#define GET_RNUM(ch)                                                                          \
	(!IS_NPC(ch) ? panic_corruption_int("utils", "GET_RNUM called on PC '%s' at %s:%d",   \
					    GET_NAME(ch) ? GET_NAME(ch) : "(null)", __FILE__, \
					    __LINE__) :                                       \
		       (ch)->only.npc->R_num)
#define GET_VNUM(ch)                                                                          \
	(!IS_NPC(ch) ? panic_corruption_int("utils", "GET_VNUM called on PC '%s' at %s:%d",   \
					    GET_NAME(ch) ? GET_NAME(ch) : "(null)", __FILE__, \
					    __LINE__) :                                       \
		       mob_index[(ch)->only.npc->R_num].virtual_number)
#define NPC_SPEC(ch, index)                                                                   \
	(!IS_NPC(ch) ? panic_corruption_int("utils", "NPC_SPEC called on PC '%s' at %s:%d",   \
					    GET_NAME(ch) ? GET_NAME(ch) : "(null)", __FILE__, \
					    __LINE__) :                                       \
		       (ch)->only.npc->spec[index])
#define GET_ID(ch)                                                                        \
	(IS_ALIVE(ch) ? (IS_NPC(ch) ? (mob_index[(ch)->only.npc->R_num].virtual_number) : \
				      ((ch)->only.pc->pid)) :                             \
			-2)
...
#define GET_MONEY(ch)                                                                    \
	((ch)->points.cash[0] + 10 * (ch)->points.cash[1] + 100 * (ch)->points.cash[2] + \
	 1000 * (ch)->points.cash[3])

#define GET_BALANCE(ch)                                                                     \
	((ch)->only.pc->spare1 + 10 * (ch)->only.pc->spare2 + 100 * (ch)->only.pc->spare3 + \
...
#define EXP_NOTCH(ch) \
	(GET_LEVEL(ch) <= MAXLVLMORTAL ? MAX(new_exp_table[GET_LEVEL(ch) + 1] / 10, 1) : 1)

#define GET_HEIGHT(ch)                                                             \
	((ush_int)((IS_AFFECTED3(ch, AFF3_ENLARGE) ? ((ch)->player.height * 1.5) : \
		    IS_AFFECTED3(ch, AFF3_REDUCE)  ? ((ch)->player.height / 1.5) : \
						     (ch)->player.height)))

#define GET_WEIGHT(ch)                                                \
	(IS_AFFECTED3(ch, AFF3_ENLARGE) ? ((ch)->player.weight * 2) : \
...
#define IS_SHOPKEEPER(a)                                                   \
	(IS_NPC(a) && ((mob_index[GET_RNUM(a)].qst_func == shop_keeper) || \
...
#define OBJ_VNUM(obj)                                                                             \
	(!obj ? panic_corruption_int("utils", "OBJ_VNUM called with NULL obj at %s:%d", __FILE__, \
				     __LINE__) :                                                  \
		((obj)->R_num >= 0 ? obj_index[(obj)->R_num].virtual_number : -1))
#define OBJ_SHORT(obj) ((obj)->short_description)
#define GET_OBJ_PROC(obj)                                                                   \
	(!obj ? panic_corruption_int("utils", "GET_OBJ_PROC called with NULL obj at %s:%d", \
				     __FILE__, __LINE__) :                                  \
		((obj)->R_num >= 0 ? obj_index[(obj)->R_num].func.obj : NULL))
...
#define CAN_CARRY_N(ch) \
	(IS_TRUSTED(ch) ? 3000 : (STAT_INDEX(GET_C_DEX(ch)) / 3) + (IS_NPC(ch) ? 12 : 6))
...
#define CAN_CARRY_OBJ(ch, obj)                                                    \
	(((total_carried_weight(ch) + GET_OBJ_WEIGHT(obj)) <= CAN_CARRY_W(ch)) && \
...
#define ROOM_VNUM(rroom_id) \
	((rroom_id >= 0 && rroom_id <= top_of_world) ? world[rroom_id].number : NOWHERE)
#define ROOM_VNUM0(rroom_id) \
	((rroom_id >= 0 && rroom_id <= top_of_world) ? world[rroom_id].number : 0)
...
#define CAN_GO(ch, door)                                                                       \
	(ch && EXIT(ch, door) && (EXIT(ch, door)->to_room != NOWHERE) &&                       \
	 (!IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED) || IS_AFFECTED2(ch, AFF2_PASSDOOR)) && \
...
#define VIRTUAL_CAN_GO(room, door)                                                     \
	(VIRTUAL_EXIT(room, door) && (VIRTUAL_EXIT(room, door)->to_room != NOWHERE) && \
...
#define IS_OPPOSING_ALIGN(ch, victim) \
...
#define MOB_IN_NO_MAGIC_ROOM(ch)                                                          \
	(IS_ROOM(ch->in_room, (ROOM_NO_MAGIC | ROOM_SILENT) || ch->in_room != NOWHERE) || \
...
#define LOOP_THRU_PEOPLE(IN_ROOM, PLAYER)                                    \
	for ((IN_ROOM) = world[(PLAYER)->in_room].people; (IN_ROOM) != NULL; \
	     (IN_ROOM) = (IN_ROOM)->next_in_room)

#define RANDOM_VNUM_BEGIN 100000
#define RANDOM_VNUM_END 200000
#define RANDOM_OBJ_VNUM 1250
#define IS_RANDOM_MOB(a) \
	(IS_NPC(a) && GET_VNUM(a) > RANDOM_VNUM_BEGIN && GET_VNUM(a) < RANDOM_VNUM_END)
...
#define IS_AGGRESSIVE(m)                                                            \
	(IS_NPC(m) && ((m)->only.npc->aggro_flags || (m)->only.npc->aggro2_flags || \
		       (m)->only.npc->aggro3_flags))

#define GET_LEVEL(character) ((character == NULL) ? -1 : (int)character->player.level)

// Most common fail: NPC, second: mortal, third: fog set.
#define IS_TRUSTED(ch) \
	(IS_PC(ch) && GET_LEVEL(ch) > MAXLVLMORTAL && !IS_SET(ch->specials.act, PLR_MORTAL))
...
#define OBJ_FALLING(o)                                                      \
	((o) && !IS_SET((o)->extra_flags, ITEM_LEVITATES) && OBJ_ROOM(o) && \
	 ((world[(o)->loc.room].sector_type == SECT_NO_GROUND) ||           \
	  ...
	  (world[(o)->loc.room].chance_fall >= number(1, 100)) || (o->z_cord > 0)))

#define IS_GUARD(ch)   \
	(IS_NPC(ch) && \
	 (IS_SET((ch)->only.npc->aggro_flags, AGGR_OUTCASTS) || isname("guard", GET_NAME(ch))))
...
#define CAN_BREATHE(ch)                                                                   \
	(IS_NPC(ch) &&                                                                    \
	 (IS_DRAGON(ch) || IS_AVATAR(ch) || IS_TITAN(ch) ||                               \
	  IS_ACT(ch, ACT_BREATHES_FIRE | ACT_BREATHES_LIGHTNING | ACT_BREATHES_FROST |    \
...
#define IS_GIANT(ch)                                                           \
	((GET_RACE(ch) == RACE_GIANT) || (GET_RACE(ch) == RACE_OGRE) ||        \
	 (GET_RACE(ch) == RACE_SGIANT) || (GET_RACE(ch) == RACE_MINOTAUR) ||   \
	 (GET_RACE(ch) == RACE_SNOW_OGRE) || (GET_RACE(ch) == RACE_FIRBOLG) || \
	 (GET_RACE(ch) == RACE_FIREGIANT) || (GET_RACE(ch) == RACE_FROSTGIANT))
...
#define IS_UNDEADRACE(ch)                                                                        \
	((GET_RACE(ch) == RACE_UNDEAD) || (GET_RACE(ch) == RACE_LICH) ||                         \
	 (GET_RACE(ch) == RACE_PVAMPIRE) || (GET_RACE(ch) == RACE_SHADE) ||                      \
	 (GET_RACE(ch) == RACE_REVENANT) || (GET_RACE(ch) == RACE_PSBEAST) ||                    \
	 (GET_RACE(ch) == RACE_WIGHT) || (GET_RACE(ch) == RACE_PHANTOM) || (IS_DRACOLICH(ch)) || \
	 (IS_UNDEAD(ch)))

#define IS_THEURPET_RACE(ch)                                                \
	((GET_RACE(ch) == RACE_ARCHON) || (GET_RACE(ch) == RACE_ASURA) ||   \
	 (GET_RACE(ch) == RACE_BRALANI) || (GET_RACE(ch) == RACE_GHAELE) || \
	 (GET_RACE(ch) == RACE_ELADRIN) || (GET_RACE(ch) == RACE_TITAN) ||  \
	 (GET_RACE(ch) == RACE_AVATAR))

#define IS_UNDEAD(ch)                                                                            \
	((GET_RACE(ch) == RACE_UNDEAD) || (GET_RACE(ch) == RACE_GHOST) ||                        \
	 (GET_RACE(ch) == RACE_VAMPIRE) || (GET_RACE(ch) == RACE_LICH) ||                        \
	 (GET_RACE(ch) == RACE_PDKNIGHT) || (GET_RACE(ch) == RACE_ZOMBIE) ||                     \
	 (GET_RACE(ch) == RACE_SPECTRE) || (GET_RACE(ch) == RACE_SKELETON) ||                    \
	 (GET_RACE(ch) == RACE_WRAITH) || (GET_RACE(ch) == RACE_SHADOW) || (IS_DRACOLICH(ch)) || \
...
#define IS_ELEMENTAL(ch)                                                             \
	((GET_RACE(ch) == RACE_F_ELEMENTAL) || (GET_RACE(ch) == RACE_A_ELEMENTAL) || \
	 (GET_RACE(ch) == RACE_W_ELEMENTAL) || (GET_RACE(ch) == RACE_V_ELEMENTAL) || \
	 (GET_RACE(ch) == RACE_I_ELEMENTAL) || (GET_RACE(ch) == RACE_E_ELEMENTAL))
...
#define IS_NOCORPSE(ch)                                                                     \
	((GET_RACE(ch) == RACE_UNDEAD) || (GET_RACE(ch) == RACE_GHOST) ||                   \
	 (GET_RACE(ch) == RACE_VAMPIRE) || (GET_RACE(ch) == RACE_LICH) ||                   \
	 (GET_RACE(ch) == RACE_ZOMBIE) || (GET_RACE(ch) == RACE_SPECTRE) ||                 \
	 (GET_RACE(ch) == RACE_SKELETON) || (GET_RACE(ch) == RACE_WRAITH) ||                \
	 (GET_RACE(ch) == RACE_DEVA) || (GET_RACE(ch) == RACE_SHADOW) ||                    \
	 (GET_RACE(ch) == RACE_W_ELEMENTAL) || (GET_RACE(ch) == RACE_A_ELEMENTAL) ||        \
	 (GET_RACE(ch) == RACE_F_ELEMENTAL) || (GET_RACE(ch) == RACE_E_ELEMENTAL) ||        \
	 (GET_RACE(ch) == RACE_V_ELEMENTAL) || (affected_by_spell(ch, TAG_CONJURED_PET)) || \
	 (GET_RACE(ch) == RACE_I_ELEMENTAL) || (IS_THEURPET_RACE(ch)) ||                    \
	 (GET_RACE(ch) == RACE_EFREET))

#define IS_ANIMAL(ch)                                                                \
	((GET_RACE(ch) == RACE_ANIMAL) || (GET_RACE(ch) == RACE_AQUATIC_ANIMAL) ||   \
	 (GET_RACE(ch) == RACE_FLYING_ANIMAL) || (GET_RACE(ch) == RACE_QUADRUPED) || \
	 (GET_RACE(ch) == RACE_PRIMATE) || (GET_RACE(ch) == RACE_HERBIVORE) ||       \
	 (GET_RACE(ch) == RACE_CARNIVORE) || (GET_RACE(ch) == RACE_PARASITE) ||      \
	 (GET_RACE(ch) == RACE_REPTILE) || (GET_RACE(ch) == RACE_DRAGONKIN) ||       \
...
#define IS_CONTROLLER_RACE(race)                                                     \
	((race >= RACE_HUMAN && race <= RACE_PLAYER_MAX && race != RACE_ILLITHID) || \
	 race == RACE_AGATHINON || race == RACE_HUMANOID || race == RACE_HALFORC ||  \
	 race == RACE_FAERIE || race == RACE_VAMPIRE || race == RACE_RAKSHASA ||     \
	 race == RACE_ANGEL || race == RACE_SNOW_OGRE || race == RACE_LYCANTH ||     \
	 race == RACE_ZOMBIE || race == RACE_PRIMATE || race == RACE_ELADRIN)
...
#define IS_MULTICLASS_PC(ch)                                \
	(IS_PC(ch) && ((ch)->player.secondary_class > 0) && \
	 ((ch)->player.secondary_class != BIT_32))
...
#define HAS_TAIL(ch) \
	(IS_CENTAUR(ch) || IS_MINOTAUR(ch) || IS_PSBEAST(ch) || IS_KOBOLD(ch) || IS_TIEFLING(ch))
...
#define IS_PURE_CASTER_CLASS(cls)                                                              \
	((cls) & (CLASS_SORCERER | CLASS_CONJURER | CLASS_ILLUSIONIST | CLASS_SUMMONER |       \
		  CLASS_NECROMANCER | CLASS_CLERIC | CLASS_SHAMAN | CLASS_BARD | CLASS_DRUID | \
		  CLASS_ETHERMANCER | CLASS_THEURGIST | CLASS_BLIGHTER))
#define IS_PARTIAL_CASTER_CLASS(cls) ((cls) & (CLASS_AVENGER))
#define IS_SEMI_CASTER_CLASS(cls)                                                    \
...
#define USES_COMMUNE(ch)                                                                         \
	((IS_NPC(ch) || GET_PRIME_CLASS(ch, CLASS_DRUID) || GET_PRIME_CLASS(ch, CLASS_RANGER) || \
	  GET_PRIME_CLASS(ch, CLASS_DRAGOON) ||                                                  \
	  (!(IS_CASTER_CLASS(ch->player.m_class) || USES_MANA(ch)) &&                            \
	   (GET_SECONDARY_CLASS(ch, CLASS_DRUID) || GET_SECONDARY_CLASS(ch, CLASS_RANGER)))))

#define USES_DEFOREST(ch)                                                          \
	(IS_PC(ch) && (GET_PRIME_CLASS(ch, CLASS_BLIGHTER) ||                      \
		       (!(IS_CASTER_CLASS(ch->player.m_class) || USES_MANA(ch)) && \
			GET_SECONDARY_CLASS(ch, CLASS_BLIGHTER))))

#define USES_MANA(ch)                                                                           \
	((GET_RACE(ch) == RACE_PILLITHID) || (GET_RACE(ch) == RACE_ILLITHID) ||                 \
	 (GET_PRIME_CLASS(ch, CLASS_PSIONICIST)) || (GET_PRIME_CLASS(ch, CLASS_MINDFLAYER)) ||  \
	 (!IS_CASTER_CLASS(ch->player.m_class) && (GET_SECONDARY_CLASS(ch, CLASS_PSIONICIST) || \
...
#define USES_SPELL_SLOTS(ch)                                                          \
	(USES_COMMUNE(ch) || USES_DEFOREST(ch) || IS_PUNDEAD(ch) || USES_TUPOR(ch) || \
...
#define CAN_SPEAK(ch)                                                                            \
	(IS_HUMANOID(ch) || IS_DRAGON(ch) || IS_DRAGONKIN(ch) || IS_DEMON(ch) || IS_GIANT(ch) || \
	 ((IS_PC_PET(ch) || IS_PC(ch)) && !IS_ANIMAL(ch)) || IS_UNDEAD(ch) || IS_EFREET(ch) ||   \
	 IS_TITAN(ch))
...
#define IS_AGG_CMD(cmd)                                                                            \
	((cmd == CMD_KILL) || (cmd == CMD_HIT) || (cmd == CMD_BACKSTAB) || (cmd == CMD_BASH) ||    \
	 (cmd == CMD_CAST) || (cmd == CMD_RESCUE) || (cmd == CMD_STEAL) || (cmd == CMD_KICK) ||    \
	 (cmd == CMD_USE) || (cmd == CMD_RECITE) || (cmd == CMD_CIRCLE) || (cmd == CMD_HITALL) ||  \
	 (cmd == CMD_MURDER) || (cmd == CMD_DISARM) || (cmd == CMD_ASSIST) ||                      \
	 (cmd == CMD_BODYSLAM) || (cmd == CMD_HEADBUTT) || (cmd == CMD_SPRINGLEAP) ||              \
	 (cmd == CMD_CHANT) || (cmd == CMD_DRAGONPUNCH) || (cmd == CMD_TRAP) ||                    \
	 (cmd == CMD_WILL) || (cmd == CMD_STAMPEDE) || (cmd == CMD_CHARGE) || (cmd == CMD_FIRE) || \
	 (cmd == CMD_BREATH) || (cmd == CMD_OGRE_ROAR) || (cmd == CMD_BEARHUG) ||                  \
	 (cmd == CMD_THROAT_CRUSH))

#define IS_AGG_SKILL(skl)                                                                         \
	((skl == SKILL_BACKSTAB) || (skl == SKILL_BASH) || (skl == SKILL_BUDDHA_PALM) ||          \
	 (skl == SKILL_CIRCLE) || (skl == SKILL_DISARM) || (skl == SKILL_DRAGON_PUNCH) ||         \
	 (skl == SKILL_HEADBUTT) || (skl == SKILL_HITALL) || (skl == SKILL_KICK) ||               \
	 (skl == SKILL_QUIVERING_PALM) || (skl == SKILL_RESCUE) || (skl == SKILL_SPRINGLEAP) ||   \
	 (skl == SKILL_STEAL) || (skl == SKILL_TRAP) || (skl == SONG_CHARMING) ||                 \
	 (skl == SONG_FORGETFULNESS) || (skl == SONG_HARMING) || (skl == SONG_SLEEP) ||           \
	 (skl == SKILL_THROAT_CRUSH) || (skl == SKILL_SHIELDPUNCH) || (skl == SKILL_ROUNDKICK) || \
	 (skl == SKILL_DIRT_TOSS) || (skl == SKILL_SWEEPING_THRUST) ||                            \
...
#define OLD_RACE_NEUTRAL(race) \
	((race == RACE_THRIKREEN) || (race == RACE_MINOTAUR) || (race == RACE_TIEFLING))

#define OLD_RACE_GOOD(race, align)                                                       \
	((race == RACE_HUMAN) || (race == RACE_GREY) || (race == RACE_MOUNTAIN) ||       \
	 (race == RACE_BARBARIAN) || (race == RACE_GNOME) || (race == RACE_HALFLING) ||  \
	 (race == RACE_HALFELF) || (race == RACE_CENTAUR) || (race == RACE_GITHZERAI) || \
	 (race == RACE_AGATHINON) || (race == RACE_WOODELF) || (race == RACE_FIRBOLG) || \
	 (race == RACE_ELADRIN) || (OLD_RACE_NEUTRAL(race) && (align >= 0)))

#define OLD_RACE_EVIL(race, align)                                                    \
	((race == RACE_DROW) || (race == RACE_DUERGAR) || (race == RACE_GITHYANKI) || \
	 (race == RACE_OGRE) || (race == RACE_GOBLIN) || (race == RACE_ORC) ||        \
	 (race == RACE_OROG) || (race == RACE_TROLL) || (race == RACE_KUOTOA) ||      \
	 (race == RACE_KOBOLD) || (race == RACE_DRIDER) ||                            \
	 (OLD_RACE_NEUTRAL(race) && (align < 0)))

#define OLD_RACE_PUNDEAD(race)                                                        \
	((race == RACE_LICH) || (race == RACE_PVAMPIRE) || (race == RACE_PDKNIGHT) || \
	 (race == RACE_SHADE) || (race == RACE_REVENANT) || (race == RACE_PSBEAST) || \
	 (race == RACE_WIGHT) || (race == RACE_GARGOYLE) || (race == RACE_PHANTOM))
...
#define HAS_FOOTING(ch)                                                 \
	((world[(ch)->in_room].sector_type != SECT_WATER_NOSWIM) &&     \
	 ...
	 (world[(ch)->in_room].sector_type != SECT_ASTRAL) && ((ch)->specials.z_cord == 0))

// Certain mobs are feared more than others. This define simply
// allows for quick use. -Lucrot Oct08

#define IS_GREATER_RACE(ch)                                                               \
	(IS_NPC(ch) && ((GET_RACE(ch) == RACE_DRAGON) || (GET_RACE(ch) == RACE_DEMON) ||  \
			(GET_RACE(ch) == RACE_DEVIL) || (GET_RACE(ch) == RACE_ANGEL) ||   \
			(GET_RACE(ch) == RACE_BEHOLDER) || (GET_RACE(ch) == RACE_LICH) || \
			(GET_RACE(ch) == RACE_AVATAR) || (GET_RACE(ch) == RACE_CONSTRUCT)))
...
#define IS_DAYBLIND(ch)                                                                          \
	(!has_innate(ch, INNATE_EYELESS) && !IS_TRUSTED(ch) && !IS_TWILIGHT_ROOM(ch->in_room) && \
	 !IS_AFFECTED4(ch, AFF4_GLOBE_OF_DARKNESS) && has_innate(ch, INNATE_DAYBLIND))
...
#define CAN_SING(ch)                                                     \
	(!IS_IMMOBILE(ch) && !is_silent(ch, FALSE) && !IS_STUNNED(ch) && \
	 GET_STAT(ch) >= STAT_RESTING)

#define IS_IMMOBILE(ch)                                                                      \
	(IS_AFFECTED2(ch, AFF2_MAJOR_PARALYSIS) || IS_AFFECTED2(ch, AFF2_MINOR_PARALYSIS) || \
	 IS_AFFECTED(ch, AFF_KNOCKED_OUT) || IS_AFFECTED(ch, AFF_BOUND) ||                   \
...
#define IS_COLD_VULN(ch)                                                       \
	(GET_RACE(ch) == RACE_THRIKREEN || GET_RACE(ch) == RACE_F_ELEMENTAL || \
	 GET_RACE(ch) == RACE_EFREET || FIRESHIELDED(ch) || IS_AFFECTED2(ch, AFF2_FIRE_AURA))

#define ENJOYS_FIRE_DAM(ch)                                                   \
	((GET_RACE(ch) == RACE_F_ELEMENTAL || GET_RACE(ch) == RACE_EFREET) || \
...
#define DO_SHAM_RESTORATION(victim)                                                              \
	(affected_by_spell(victim, SPELL_CURSE) || affected_by_spell(victim, SPELL_MALISON) ||   \
	 ...
	 affected_by_spell(victim, SPELL_FEEBLEMIND) || affected_by_spell(victim, SPELL_SLOW) || \
	 IS_AFFECTED(victim, AFF_BLIND) || IS_AFFECTED4(victim, AFF4_CARRY_PLAGUE) ||            \
	 affected_by_spell(victim, SPELL_DISEASE) || affected_by_spell(victim, SPELL_PLAGUE) ||  \
	 affected_by_spell(victim, TAG_ARMLOCK) || affected_by_spell(victim, TAG_LEGLOCK) ||     \
...
#define FLAME_REAVER_WEAPONS(wpn) \
	(IS_SWORD(wpn) || (wpn)->value[0] == WEAPON_FLAIL || (wpn)->value[0] == WEAPON_WHIP)

#define FROST_REAVER_WEAPONS(wpn) (IS_BLUDGEON(wpn))

// Can't seem to find this defined anywhere else.
#define IS_MELEE_CLASS(ch)                                                             \
	(GET_CLASS(ch, CLASS_WARRIOR | CLASS_PALADIN | CLASS_ROGUE | CLASS_MERCENARY | \
			       CLASS_DREADLORD | CLASS_REAVER | CLASS_AVENGER |        \
			       CLASS_ANTIPALADIN | CLASS_RANGER | CLASS_MONK | CLASS_DRAGOON))

#define SHAMAN_MOVE_SPELL(ch)                                                                 \
	(affected_by_spell(ch, SPELL_WOLFSPEED) || affected_by_spell(ch, SPELL_SNAILSPEED) || \
...
#define IS_BRAINLESS(victim)                                                         \
	((GET_RACE(victim) == RACE_PLANT) || (GET_RACE(victim) == RACE_GOLEM) ||     \
	 (GET_RACE(victim) == RACE_CONSTRUCT) || (GET_RACE(victim) == RACE_SLIME) || \
	 (GET_RACE(victim) == RACE_PWORM) || (GET_RACE(victim) == RACE_SKELETON) ||  \
	 (GET_RACE(victim) == RACE_ZOMBIE) || (IS_NPC(victim) && GET_VNUM(victim) == 250))

#define IS_SOULLESS(victim)                                                      \
	((GET_RACE(victim) == RACE_PLANT) || (GET_RACE(victim) == RACE_GOLEM) || \
	 IS_UNDEADRACE(victim) || (GET_RACE(victim) == RACE_CONSTRUCT) ||        \
...
#define IS_NECRO_GOLEM(ch)                                                                   \
	IS_NPC(ch) && (GET_RACE(ch) == RACE_GOLEM) &&                                        \
		(GET_VNUM(ch) == golem_data[0].vnum || GET_VNUM(ch) == golem_data[1].vnum || \
		 GET_VNUM(ch) == golem_data[2].vnum || GET_VNUM(ch) == golem_data[3].vnum)

#define IS_ANGEL(ch)                                                          \
	((GET_RACE(ch) == RACE_ANGEL) || (GET_RACE(ch) == RACE_ELADRIN) ||    \
	 (GET_RACE(ch) == RACE_AGATHINON) || (GET_RACE(ch) == RACE_ARCHON) || \
	 (GET_RACE(ch) == RACE_ASURA) || (GET_RACE(ch) == RACE_TITAN) ||      \
	 (GET_RACE(ch) == RACE_AVATAR) || (GET_RACE(ch) == RACE_GHAELE) ||    \
	 (GET_RACE(ch) == RACE_BRALANI) || (GET_RACE(ch) == RACE_DEVA) || IS_ANGELIC(ch))
...
#define INFRA_INVIS_RACE(race)                                                                     \
	(race == RACE_UNDEAD || race == RACE_GHOST || race == RACE_VAMPIRE || race == RACE_LICH || \
	 race == RACE_PDKNIGHT || race == RACE_ZOMBIE || race == RACE_SPECTRE ||                   \
	 race == RACE_SKELETON || race == RACE_WRAITH || race == RACE_SHADOW ||                    \
	 race == RACE_DRACOLICH || race == RACE_PVAMPIRE || race == RACE_SHADE ||                  \
	 race == RACE_REVENANT || race == RACE_PSBEAST || race == RACE_WIGHT ||                    \
	 race == RACE_GARGOYLE || race == RACE_PHANTOM || race == RACE_AQUATIC_ANIMAL ||           \
	 race == RACE_PARASITE || race == RACE_GOLEM || race == RACE_SNAKE ||                      \
	 race == RACE_ARACHNID || race == RACE_PLANT || race == RACE_SLIME ||                      \
	 race == RACE_A_ELEMENTAL || race == RACE_INSECT || race == RACE_W_ELEMENTAL ||            \
	 race == RACE_V_ELEMENTAL || race == RACE_I_ELEMENTAL || race == RACE_E_ELEMENTAL ||       \
	 (IS_AFFECTED4(obj, AFF4_VAMPIRE_FORM) && !GET_CLASS(obj, CLASS_THEURGIST)) ||             \
	 IS_RACEWAR_UNDEAD(obj))

// This is done in reverse 'cause it's faster that way.
#define HAS_LUNGS(race)                                                                  \
	(race != RACE_SHADE && race != RACE_REVENANT && race != RACE_LICH &&             \
	 race != RACE_PVAMPIRE && race != RACE_PDKNIGHT && race != RACE_PSBEAST &&       \
	 race != RACE_WIGHT && race != RACE_PHANTOM && race != RACE_GARGOYLE &&          \
	 race != RACE_F_ELEMENTAL && race != RACE_A_ELEMENTAL && race != RACE_UNDEAD &&  \
	 race != RACE_VAMPIRE && race != RACE_W_ELEMENTAL && race != RACE_E_ELEMENTAL && \
	 race != RACE_GHOST && race != RACE_GOLEM && race != RACE_PLANT &&               \
	 race != RACE_DRACOLICH && race != RACE_SLIME && race != RACE_CONSTRUCT &&       \
	 race != RACE_ZOMBIE && race != RACE_SPECTRE && race != RACE_SKELETON &&         \
	 race != RACE_WRAITH && race != RACE_V_ELEMENTAL && race != RACE_I_ELEMENTAL &&  \
...
#define COMBAT_PULSE(ch)                                                                        \
	(ch->specials.base_combat_round +                                                       \
	 ch->points.combat_pulse *                                                              \
		 ((.04 / 3) * ch->specials.base_combat_round * ch->specials.base_combat_round - \
		  (1.10 / 3) * ch->specials.base_combat_round + 2.73))

// New effects of attributes:
// Vamp multiplier for ch: value between 1.1 and 2.2.
#define VAMPPERCENT(ch)                                                  \
	(BOUNDEDF(1.10, (GET_C_POW(ch) / 90.0), 2.50) +                  \
	 (IS_PC(ch) && GET_PRIME_CLASS(ch, CLASS_ANTIPALADIN) ?          \
		  get_property("vamping.maxHP.bonus.antipaladin", 1.0) : \
		  get_property("vamping.maxHP.bonus.all", 0.0)))
...
#define MAGICDAMBONUS(ch)              \
	((GET_C_STR(ch) < 121) ? 100 : \
	 (GET_C_STR(ch) < 141) ? 110 : \
	 (GET_C_STR(ch) < 181) ? 120 : \
				 130)
