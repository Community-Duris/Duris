...
#define FIND_MORT_ZCOORD \
	BIT_8 // mort zcoord == can get target if no more than 2 z diff, or any targ in room if hawkvision
...
#define AFFTYPE_NOAPPLY \
	BIT_9 // do not apply anything from this affect on char, it's used to store data only
...
struct scribing_data_type
{
	...
	void (*done_func)(
		P_char); // optional function to call when done scribing.  used by 'scribe all'
};
...
#define PLR3_ABORT_CASTING BIT_16 /* allow this player to abort spells */
