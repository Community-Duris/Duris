...
void event_tree_growth(P_char ch, P_char victim, P_obj xobj, void *data)
{
	...
	for (obj = object_list; obj; obj = obj->next)
	{
		...
		if (obj && ((GET_OBJ_VNUM(obj) == RES_COPSE_VNUM) || (GET_OBJ_VNUM(obj) == RES_TSTAND_VNUM)))
		{
			if (obj->loc.room)
				room = obj->loc.room;
			else
...
void event_harvest_tree(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	{
		if (((GET_OBJ_VNUM(tobj) == RES_COPSE_VNUM) || (GET_OBJ_VNUM(tobj) == RES_TSTAND_VNUM)) && (tobj->value[TREE_BUSY] == GET_PID(ch)))
			tree = tobj;
		break;
	}
