...
- [x] Audited boon, reward, and zone hot paths perform no synchronous database, Redis,
      or filesystem I/O on the simulation thread.
- [x] One eligibility outcome or claim has one stable operation ID and cannot award or
      advance progress twice under duplicate or restart replay.
- [x] Epic, currency, and item rewards commit through their authoritative ledgers and
      current rows rather than cached absolute saves.
- [x] Group fan-out is bounded, immutable after capture, and applied set-wise with
      deterministic participant identities.
- [x] Repeat, cooldown, completion, and account-bound claim guards remain correct under
      concurrent execution and ambiguous commits.
- [x] Zone-touch records contain typed bounded fields and no raw SQL durable payload.
