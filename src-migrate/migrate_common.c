...
void free_mig_obj(struct mig_obj *obj)
{
	...
	if (obj->action_descr)
		free(obj->action_descr);
	if (obj->spellbook_bits)
		free(obj->spellbook_bits);
...
void free_mig_affect(struct mig_affect *af)
{
	while (af)
	{
		...
		if (af->wear_off_char)
			free(af->wear_off_char);
		if (af->wear_off_room)
			free(af->wear_off_room);
		free(af);
		af = next;
	}
}
...
void free_mig_player(struct mig_player *p)
{
	...
	if (p->poof_out)
		free(p->poof_out);
	if (p->poof_in_sound)
		free(p->poof_in_sound);
	if (p->poof_out_sound)
		free(p->poof_out_sound);
	if (p->granted_cmds)
		free(p->granted_cmds);
...
int save_item_to_db(struct mig_obj *obj, const char *table, const char *owner_col, int owner_id,
		    int container_id, int equip_slot)
{
	...
	if (obj->weight_set)
		snprintf(weight_str, sizeof(weight_str), "%d", obj->weight);
	else
		strcpy(weight_str, "NULL");
	if (obj->cost_set)
		snprintf(cost_str, sizeof(cost_str), "%d", obj->cost);
	else
		strcpy(cost_str, "NULL");
	if (obj->timer_set)
		snprintf(timer_str, sizeof(timer_str), "%ld", obj->timer);
	else
		strcpy(timer_str, "NULL");
	...
	if (obj->bitvector_set & 1)
		snprintf(bv1, sizeof(bv1), "%lu", obj->bitvector1);
	else
		strcpy(bv1, "NULL");
	if (obj->bitvector_set & 2)
		snprintf(bv2, sizeof(bv2), "%lu", obj->bitvector2);
	else
		strcpy(bv2, "NULL");
	if (obj->bitvector_set & 4)
		snprintf(bv3, sizeof(bv3), "%lu", obj->bitvector3);
	else
		strcpy(bv3, "NULL");
	if (obj->bitvector_set & 8)
		snprintf(bv4, sizeof(bv4), "%lu", obj->bitvector4);
	else
		strcpy(bv4, "NULL");
	if (obj->bitvector_set & 16)
		snprintf(bv5, sizeof(bv5), "%lu", obj->bitvector5);
	else
		strcpy(bv5, "NULL");
	...
	if (strcmp(table, "player_items") == 0)
	{
		...
		snprintf(
			...
			"%s, %s, %s, %s, %s, %llu)",
			table, owner_col, owner_id, obj->vnum, equip_slot, container_str,
			weight_str, cost_str, timer_str, extra_str, wear_str, type_str, v0, v1, v2,
			v3, v4, v5, v6, v7, name_str, short_str, desc_str, action_str, bv1, bv2,
			bv3, bv4, bv5, obj_uid);
	}
	else if (equip_slot >= 0)
	{
		...
		snprintf(query, sizeof(query),
			 ...
			 table, owner_col, owner_id, obj->vnum, equip_slot, container_str,
			 weight_str, cost_str, timer_str, extra_str, wear_str, type_str, v0, v1, v2,
			 v3, v4, v5, v6, v7, name_str, short_str, desc_str, action_str, obj_uid);
	}
	else if (strcmp(table, "locker_items") == 0)
	{
		// locker_items - no equip_slot, but include bitvectors for encrusted items
		snprintf(
			query, sizeof(query),
			...
			"%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, "
			"%s, %s, %s, %s, %s, %llu)",
			table, owner_col, owner_id, obj->vnum, container_str, weight_str, cost_str,
			timer_str, extra_str, wear_str, type_str, v0, v1, v2, v3, v4, v5, v6, v7,
			name_str, short_str, desc_str, action_str, bv1, bv2, bv3, bv4, bv5,
			obj_uid);
	}
	else
	{
		// corpse_items, saved_items - no equip_slot, no bitvectors
		snprintf(query, sizeof(query),
			 "INSERT INTO %s (%s, vnum, container_id, quantity, "
			 "weight, cost, timer, extra_flags, wear_flags, item_type, "
			 "value0, value1, value2, value3, value4, value5, value6, value7, "
	...
	if (obj->spellbook_bits && item_id > 0 && strcmp(table, "player_items") == 0)
	{
		...
		if (json)
		{
			...
			item_qry(
				...
				"VALUES (%d, 'SPELLBOOK', '%s')",
				item_id, esc_json ? esc_json : json);
			if (esc_json)
				free(esc_json);
			free(json);
		}
			...
			for (int i = 0; i < MAX_OBJ_AFFECT; i++)
			{
				    ...
				    obj->affected[i].modifier != 0)
				{
					...
					item_qry("INSERT INTO %s (item_id, location, modifier) "
						 "VALUES (%d, %d, %d)",
						 affects_table, item_id, obj->affected[i].location,
						 obj->affected[i].modifier);
				}
			}
		}
...
int count_player_dir_files(const char *base_path, const char *filter_ext,
			   const char *filter_exclude)
{
	...
	for (char letter = 'a'; letter <= 'z'; letter++)
	{
		...
		while ((entry = readdir(dir)) != NULL)
		{
			...
			if (filter_ext)
			{
				if (!strstr(entry->d_name, filter_ext))
					continue;
			}
