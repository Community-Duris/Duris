...
int migrate_ships_from_files(void)
{
	...
	while (*ret != '$')
	{
		...
		if (ship->mainsail < 0)
			ship->mainsail = 0;
		if (ship->mainsail > maxsail)
			ship->mainsail = maxsail;
		...
		fscanf(f2, "%d %d %d %d %d %d %d %d %d %d\n", &(ship->crew.sail_chief),
		       &(ship->crew.guns_chief), &(ship->crew.rpar_chief), &dummy, &dummy, &dummy,
		...
		for (int i = 0; i < MAXSLOTS; i++)
		{
			if (fscanf(f2, "%d %d\n", &(ship->slot[i].type), &(ship->slot[i].index)) !=
			    2)
			{
				break;
			}
			fscanf(f2, "%d %d\n", &(ship->slot[i].position), &(ship->slot[i].timer));
			fscanf(f2, "%d %d %d %d %d\n", &(ship->slot[i].val0), &(ship->slot[i].val1),
			...
			else if (ship->slot[i].type == SLOT_CARGO ||
				 ship->slot[i].type == SLOT_CONTRABAND)
			{
				ship->slot[i].val2 = -1;
				ship->slot[i].val3 = -1;
				ship->slot[i].val4 = -1;
