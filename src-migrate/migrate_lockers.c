...
static int migrate_locker_file(const char *filepath, const char *locker_name)
{
	...
	if (stat_vers > 36)
		mig_getInt(&bufptr); // secondary_class
	if (stat_vers > 38)
		MIG_GET_BYTE(bufptr); // spec
	...
	else
	{
		...
		if (esc)
		{
			MYSQL_RES *res = db_query(
				"SELECT pid FROM player_data WHERE LOWER(name) = LOWER('%s')", esc);
			if (res)
			{
