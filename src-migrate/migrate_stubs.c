...
Guild::Guild(char *_name, unsigned int _racewar, unsigned int _id_number,
    unsigned long _prestige, unsigned long _construction,
    unsigned long _money, unsigned int _bits) {
    // stub
}

void Guild::save() { }
unsigned int Guild::get_max_members() { return 100; }
unsigned int Guild::max_size() { return 100; }
...
bool Guild::add_member(P_char ch, int rank) { return false; }
int Guild::update() { return 0; }
void Guild::update_member(P_char ch) { }
void Guild::update_bits(P_char ch) { }
void Guild::secede(P_char ch) { }
bool Guild::is_allied_with(P_Guild ally) { return false; }
bool Guild::is_enemy(P_char enemy) { return false; }
P_Alliance Guild::get_alliance() { return NULL; }
void Guild::display(P_char ch) { }
void Guild::ledger(P_char ch, char *args) { }
bool Guild::is_clan() { return false; }
bool Guild::is_guild() { return true; }
bool Guild::is_kingdom() { return false; }
void Guild::kick(P_char victim) { }
void Guild::kick(P_char kicker, char *char_name) { }
bool Guild::sub_money(int p, int g, int s, int c) { return false; }
...
int Guild::max_assoc_size() { return 100; }
...
int wear(P_char ch, P_obj obj_object, int keyword, bool showit) { return 0; }
void writeSavedItem(void *obj) { }
void all_affects(void *ch, int i) { }
P_obj read_object(int number, int type) { return NULL; }
void obj_to_obj(void *o1, void *o2) { }
void obj_to_char(P_obj obj, P_char ch) { }
void str_free(char *source) { if (source) free(source); }
int GET_LEVEL(P_char ch) { return 0; }
int vitality_limit(P_char ch) { return 0; }
int calculate_hitpoints(P_char ch) { return 1; }
int calculate_mana(P_char ch) { return 1; }
void extract_obj(void *obj, int i) { free(obj); }
int flag2idx(int flag) { return 0; }
void update_skills(P_char ch) { }
struct affected_type *get_spell_from_char(P_char ch, int spell, void *context) { return NULL; }
...
static bool sql_save_ship_armor(int ship_id, P_ship ship) {
    if (!DB || !ship || ship_id <= 0) return false;
    for (int i = 0; i < 4; i++) {
        char query[256];
        ...
        if (!sql_run_query(query)) return false;
    }
    return true;
}
...
bool sql_save_ship(P_ship ship) {
    if (!DB || !ship || !ship->ownername) return false;

    char *esc_owner = sql_escape_string(ship->ownername);
    if (!esc_owner) return false;
...
bool sql_save_account(struct acct_entry *acc) {
    ...
    snprintf(del_query, sizeof(del_query),
        "delete from account_ips where account_name='%s'", esc_name);
    ...
    for (struct acct_ip *ip = acc->acct_unique_ips; ip; ip = ip->next) {
        ...
        if (esc_hostname && esc_ip) {
            char ip_query[512];
            snprintf(ip_query, sizeof(ip_query),
        ...
        if (esc_hostname) free(esc_hostname);
