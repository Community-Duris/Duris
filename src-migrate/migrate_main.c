...
static void remap_artifact_locations(void)
{
	...
	if (res)
	{
		...
		while ((row = mysql_fetch_row(res)))
		{
			qry("UPDATE artifacts_mortal SET location = %s WHERE vnum = %s", row[2],
			    row[0]);
			updated++;
		}
		mysql_free_result(res);
	...
	updated = 0;
	res = db_query("SELECT ab.vnum, old.name, new.pid "
	...
	if (res)
	{
		...
		while ((row = mysql_fetch_row(res)))
		{
			qry("UPDATE artifact_bind SET owner_pid = %s WHERE vnum = %s", row[2],
			    row[0]);
			updated++;
		}
		mysql_free_result(res);
	...
	updated = 0;
	res = db_query("SELECT a.vnum, ab.owner_pid "
		       "FROM artifacts a "
	...
	if (res)
	{
		...
		while ((row = mysql_fetch_row(res)))
		{
			qry("UPDATE artifacts_mortal SET location = %s WHERE vnum = %s", row[1],
			    row[0]);
			updated++;
		}
		mysql_free_result(res);
...
static void remap_ip_info(void)
{
	...
	int updated = 0;
	MYSQL_RES *res = db_query("SELECT ii.pid, old.name, new.pid as new_pid "
				  "FROM ip_info ii "
				  "JOIN _old_pid_map old ON ii.pid = old.pid "
				  "JOIN player_data new ON LOWER(old.name) = LOWER(new.name) ");
	if (res)
...
static void remap_epic_tables(void)
{
	...
	// remap epic_bonus
	int updated = 0;
	MYSQL_RES *res = db_query("SELECT eb.pid, old.name, new.pid as new_pid "
				  "FROM epic_bonus eb "
	...
	// remap epic_gain
	updated = 0;
	res = db_query("SELECT DISTINCT eg.pid, new.pid as new_pid "
		       "FROM epic_gain eg "
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++)
	{
		if (strcmp(argv[i], "--all") == 0)
			...
			do_players = 1;
		else if (strcmp(argv[i], "--lockers") == 0)
			do_lockers = 1;
		else if (strcmp(argv[i], "--ships") == 0)
			do_ships = 1;
		else if (strcmp(argv[i], "--guilds") == 0)
			do_guilds = 1;
		else if (strcmp(argv[i], "--recipes") == 0)
			do_recipes = 1;
		else if (strcmp(argv[i], "--spellbooks") == 0)
			do_spellbooks = 1;
		else if (strcmp(argv[i], "--shopkeepers") == 0)
			do_shopkeepers = 1;
